    ...
    case 4: { /* Probably :-) 32-bpp */
      ...
      if (alpha == 255) {
        *pixel = color;
        ...
        R = ((dR + ((((color & Rmask) >> Rshift) - dR) * alpha >> 8))
             << Rshift) &
            Rmask;
        G = ((dG + ((((color & Gmask) >> Gshift) - dG) * alpha >> 8))
             << Gshift) &
            Gmask;
        B = ((dB + ((((color & Bmask) >> Bshift) - dB) * alpha >> 8))
             << Bshift) &
            Bmask;
        *pixel = R | G | B;
        if (Amask != 0) {
...
#else
          A = ((dA + ((((color & Amask) >> Ashift) - dA) * alpha >> 8))
               << Ashift) &
              Amask;
#endif
          *pixel |= A;
      ...
      if (alpha == 255) {
        *((Uint32 *)dst->pixels + y * dst->pitch / 4 + x) = color;
        ...
        if (A = 255 - ((aTmp * (255 - surfaceAlpha)) >> 8)) {
          aTmp *= surfaceAlpha;
          R = (preMultR + ((aTmp * ((dc & Rmask) >> Rshift)) >> 8)) / A
                  << Rshift &
              Rmask;
          G = (preMultG + ((aTmp * ((dc & Gmask) >> Gshift)) >> 8)) / A
                  << Gshift &
              Gmask;
          B = (preMultB + ((aTmp * ((dc & Bmask) >> Bshift)) >> 8)) / A
                  << Bshift &
              Bmask;
        }
...
\returns Returns 0 on success, -1 on failure.
*/
int _HLineAlpha(SDL_Surface *dst, Sint16 x1, Sint16 x2, Sint16 y,
                Uint32 color) {
...
\returns Returns 0 on success, -1 on failure.
*/
int _VLineAlpha(SDL_Surface *dst, Sint16 x, Sint16 y1, Sint16 y2,
                Uint32 color) {
...
\returns Returns 0 on success, -1 on failure.
*/
int circleColor(SDL_Surface *dst, Sint16 x, Sint16 y, Sint16 rad,
                Uint32 color) {
  Sint16 left, right, top, bottom;
  int result;
  Sint16 x1, y1, x2, y2;
    ...
    if (oct == endoct) {
      ...
      if (startoct == endoct) {
        ...
        if (start > end) {
          ...
          // the whole circle, we need to set the rest to true, because the
          // while loop will end at the bottom.
          drawoct = 255;
        } else {
          drawoct &= 255 - (1 << oct);
        }
  ...
  if ((color & 255) == 255) {
    ...
    do {
      ...
      if (cx > 0) {
        xpcx = x + cx;
        xmcx = x - cx;
        // always check if we're drawing a certain octant before adding a pixel
        // to that octant.
        if (drawoct & 4)
        ...
        if (drawoct & 6)
          result |= fastPixelColorNolock(
              dst, x, ypcy, color); // 4 + 2; drawoct[2] || drawoct[1]
        if (drawoct & 96)
       ...
       */
      if (stopval_start == cx) {
        // works like an on-off switch because start & end may be in the same
        // octant.
...
int characterColor(SDL_Surface *dst, Sint16 x, Sint16 y, char c, Uint32 color) {
  ...
  if (gfxPrimitivesFont[ci] == NULL) {
    gfxPrimitivesFont[ci] = SDL_CreateRGBSurface(
        SDL_SWSURFACE | SDL_HWSURFACE | SDL_SRCALPHA, charWidth, charHeight, 32,
  ...
  if ((gfxPrimitivesFontColor[ci] != color) || (forced_redraw)) {
    ...
    if (charRotation > 0) {
      rotatedCharacter =
          rotateSurface90Degrees(gfxPrimitivesFont[ci], charRotation);
...
void _murphyWideline(SDL_gfxMurphyIterator *m, Sint16 x1, Sint16 y1, Sint16 x2,
  ...
  ang = atan(
      (double)m->v /
      (double)m->u); /* calc new initial point - offset both sides of ideal */
  sang = sin(ang);
  cang = cos(ang);
  ...
  for (q = 0; dd <= tk; q++) { /* outer loop, stepping perpendicular to line */
    ...
    if (d0 < m->kt) { /* square move */
      ...
      if (d1 < m->kt) { /* normal diagonal */
        ...
        d1 += m->kd;
        if (dd > tk) {
          _murphyIteration(m, miter, ml1bx, ml1by, ml2bx, ml2by, ml1x, ml1y,
                           ml2x, ml2y);
