...
#define GLOBALS // define las variables externas en este modulo.
#include "div.h"
...
void ord_paleta(void) {
  ...
  n = 0;
  do {
    pal[n * 4 + 3] = (pal[n * 4] = palette[n * 3]) +
                     (pal[n * 4 + 1] = palette[n * 3 + 1]) +
                     (pal[n * 4 + 2] = palette[n * 3 + 2]);
  } while (++n < 256);
...
char find_ord(char *dac) {
  ...
  do {
    if (*pal != 255) {
      dif = (r - (int)*pal) * (r - (int)*pal);
      pal++;
      dif += (g - (int)*pal) * (g - (int)*pal);
      pal++;
      dif += (b - (int)*pal) * (b - (int)*pal);
      pal += 2;
      if (dif < dmin) {
        dmin = dif;
        color = pal - 4;
