...
gcc zx.c sdlgfx/SDL_rotozoom.c spectrum/colourspace.c spectrum/libscrplus.c
spectrum/bits.c -o zx.so -fPIC -funsigned-char -shared -O3 `sdl-config --cflags`
`sdl-config --libs`
...
void buffer_to_video() {
  ...
  for (x = 0; x < wide; x++) {
    for (y = 0; y < height; y++) {
      c = buffer[x + wide * y];
      put_pixel32(image, x, y,
                  SDL_MapRGB(image->format, palette[c * 3] * 4,
                             palette[c * 3 + 1] * 4, palette[c * 3 + 2] * 4));
      // 32bitpal[c]);
    }
  }
  ...
  for (y = 0; y < h; y++) {
    for (x = 0; x < w; x++) {
      ...
      put_pixel32(image2, x, y,
                  SDL_MapRGB(image2->format, rgb[0], rgb[1], rgb[2]));
    }
  }
