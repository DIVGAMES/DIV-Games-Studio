...
int main(int argc, char *argv[]) {
...
#ifdef WINDOWS
  const char *infile = "input", *outfile = "output";
  stdmsg = stdout;
#else  // !WINDOWS
  const char *infile = "/dev/stdin", *outfile = "/dev/stdout";
  stdmsg = stderr;
#endif // WINDOWS
  int arg;
  for (arg = 1; arg < argc; arg++) {
      ...
      if ((sscanf(argv[arg] + 3, "%01x", &clm) == 1) && (clm != 0)) {
        ...
        if (comma) {
          if ((sscanf(comma + 1, "%01x", &clm) == 1) && (clm != 0)) {
            ...
            fprintf(stdmsg,
                    "Bad USEmask %s (ignoring) - must be single nonzero hex "
                    "digit\n",
                    comma + 1);
          }
        }
        ...
        fprintf(
            stdmsg,
            "Bad CLUTmask %s (ignoring) - must be single nonzero hex digit\n",
            argv[arg] + 3);
      }
      continue;
    }
    ...
    if (strncmp(argv[arg], "-w=", 3) == 0) {
      ...
      if (sscanf(argv[arg] + 3, "%u", &nw) == 1) {
        if (nw > 256) {
          fprintf(stdmsg,
                  "Bad width value %u; forcing 256 (which is maximum)\n", nw);
          nw = 256;
        }
        w = nw;
      }
    ...
    if (strncmp(argv[arg], "-h=", 3) == 0) {
      ...
      if (sscanf(argv[arg] + 3, "%u", &nh) == 1) {
        if (nh > 192) {
          fprintf(stdmsg,
                  "Bad height value %hhu; forcing 192 (which is maximum)\n",
                  nh);
          nh = 192;
        }
        h = nh;
  ...
  if (importpal) {
    if (!IS_RENDER_ULAPLUS(render)) {
      fprintf(stdmsg,
              "-p (palette) may only be used in ULAplus mode (+U); ignoring\n");
      importpal = false;
    } else
      importpal = import_palette(palfile, palent);
  }
  if (!importpal) // note: not an 'else' because importpal might have been set
                  // to false in the above clause
  ...
  if (infile) {
    ...
    fprintf(stdmsg, "No infile given\n"); // should be impossible, as we load
                                          // from /dev/stdin by default
    return (EXIT_FAILURE);
  }
  if (!image) {
