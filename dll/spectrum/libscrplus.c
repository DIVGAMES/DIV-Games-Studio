...
int import_palette(FILE *f, palette palent) {
  ...
  case 0xB0: // Palette file (TAP)
    ...
    if (s != buf)
      return (2); // "fgets: %s\n", strerror(errno)
    if (strcmp(buf, "\024\00164colour") != 0)
      return (3); // "Palette file header incorrect\n"
    fseek(f, 0x6D, SEEK_SET);
    if (fgetc(f) != 0xC9)
      return (4); // "Palette file corrupt (no 0xC9 at 0x6D)\n"
    for (uint i = 0; i < 64; i++)
      palent[i] = fgetc(f);
    return (0);
  case 0x40: // Raw palette file
    ...
    return (0);
  default:
    return (-1); // "Palette file is bad length!\nDetails: %zuB (should be 176B
                 // for a TAP, 64B for raw)\n", (size_t)size
  }
}
...
void pget(const SDL_Surface *image, int ix, int iy, uchar *r, uchar *g,
          uchar *b) {
...
int resample(const SDL_Surface *image, uint w, uint h, sizer crop,
             rgb_t pix[w][h]) {
  if (!image)
    return (1);
  if (w < 1)
...
int apply_tweak(uint w, uint h, uchar render, rgb_t pix[w][h],
                uchar tweaks[3][2], yuv_t post[w][h]) {
  for (uint x = 0; x < w; x++)
    for (uint y = 0; y < h; y++) {
      hsv_t hsv, hpost;
...
int pickattrs(uint w, uint h, uchar render, yuv_t yuv[w][h],
              uchar pal[w / 8][h][2]) {
  for (uint x = 0; x < w / 8; x++) {
    for (uint y = 0; y < (IS_RENDER_TIMEX(render) ? h : h / 8); y++) {
      if (IS_RENDER_BW(render))
        ...
        double mad;
        for (uchar a = 1; a < 128;
             a++) // 1, not 0, because a=0 => i==p => continue
        {
          int i = a & 7;
          int p = (a >> 3) & 7;
          if (i <= p)
          ...
          for (uint dx = 0; dx < 8; dx++) {
            for (uint dy = 0; dy < (IS_RENDER_TIMEX(render) ? 1 : 8); dy++) {
              const uchar *xyuv =
                  yuv[(x * 8) + dx][IS_RENDER_TIMEX(render) ? y : (y * 8) + dy];
...
int dither(uint w, uint h, uchar render, yuv_t yuv[w][h],
           uchar pal[w / 8][h][2], const palette palent, bool dithflow,
           bool npx[w][h]) {
  yuv_t nyuv[w][h];
  ...
  for (uint x = 0; x < w; x++) {
    for (uint y = 0; y < h; y++) {
      int bx = floor(x / 8),
          by = IS_RENDER_TIMEX(render) ? y : (int)floor(y / 8);
      const uchar *a = pal[bx][by], *ax = NULL, *ay = NULL;
      if (bx + 1 < (int)w / 8)
        ax = pal[bx + 1][by];
      if (by + 1 < IS_RENDER_TIMEX(render) ? h : h / 8)
        ay = pal[bx][by + 1];
      ...
      npx[x][y] = (di < dp);
      bool flowx = (x + 1 < w) && (dithflow || ((x + 1) % 8) ||
                                   (ax && (a[0] == ax[0]) && (a[1] == ax[1]))),
           flowy =
               (y + 1 < h) &&
               (dithflow || (IS_RENDER_TIMEX(render) ? true : ((y + 1) % 8)) ||
                (ay && (a[0] == ay[0]) && (a[1] == ay[1])));
      for (uint j = 0; j < 3; j++) {
        int dy = (npx[x][y] ? iyuv : pyuv)[j] - nyuv[x][y][j];
        if (flowx) {
          nyuv[x + 1][y][j] = min(
              max(floor((y == 0 ? yuv : nyuv)[x + 1][y][j] - dy * 7.0 / 16.0),
                  0),
              255);
          if (flowy)
            nyuv[x + 1][y + 1][j] =
                min(max(floor(yuv[x + 1][y + 1][j] - dy / 16.0), 0), 255);
        }
        if (flowy) {
          if (x > 0)
            nyuv[x - 1][y + 1][j] = min(
                max(floor(nyuv[x - 1][y + 1][j] - dy * 3.0 / 16.0), 0), 255);
          nyuv[x][y + 1][j] =
              min(max(floor(nyuv[x][y + 1][j] - dy * 5.0 / 16.0), 0), 255);
        }
      }
    }
  }
...
int optima(uint w, uint h, uchar render, yuv_t yuv[w][h],
           uchar opt[w / 8][h][2][4]) {
  for (uint x = 0; x < w / 8; x++) {
    for (uint y = 0; y < (IS_RENDER_TIMEX(render) ? h : h / 8); y++) {
      int mt = 0;
      ...
      for (uint t = 0; t < 3; t++) {
        if (t == 1)
          msv /= 2; // considerable preference for Y distinction
        int ydist[256];
        ...
        for (uint dx = 0; dx < 8; dx++) {
          for (uint dy = 0; dy < (IS_RENDER_TIMEX(render) ? 1 : 8); dy++) {
            int py = yuv[(x * 8) + dx]
                        [IS_RENDER_TIMEX(render) ? y : (y * 8) + dy][t];
            ydist[py]++;
            ny = min(ny, py);
            my = max(my, py);
          }
        ...
        if (my < ny + 2) {
          ...
          if ((t == 0) || (((nl > 1) ? ((sl * sl - sll) / (nl - 1)) : 0) +
                               ((nh > 1) ? ((sh * sh - shh) / (nh - 1)) : 0) <
                           msv)) {
            msy = ny;
          ...
          for (sy = ny + 1; sy < my; sy++) {
            ...
            double v = ((nl > 1) ? ((sl * sl - sll) / (nl - 1)) : 0) +
                       ((nh > 1) ? ((sh * sh - shh) / (nh - 1)) : 0);
            if (v < msv) {
              msy = sy;
              msv = v;
...
int optipal(uint w, uint h, uchar render, uchar opt[w / 8][h][2][4],
            uchar palopt[w / 8][h][2][2]) {
  for (uint x = 0; x < w / 8; x++)
    for (uint y = 0; y < (IS_RENDER_TIMEX(render) ? h : h / 8); y++)
      for (uint t = 0; t < 2; t++) {
        int mp;
...
int countopt(uint w, uint h, uchar render, uchar palopt[w / 8][h][2][2],
             uint distopt[256]) {
...
int pickpal(const uint distopt[256], palette palent, int depress,
            uchar clutmask, uchar usemask) {
  bool chosen[256];
  double ddistopt[256];
  ...
  int q = 0;
  unsigned int fnm = 0;
  while ((fnm < 4) && !(clutmask & usemask & (1 << fnm)))
    fnm++;
  for (uint p = 0; p < 64; p++) {
    if (!(clutmask & usemask & (1 << (p >> 4)))) {
      // use existing palette entry
    } else if ((p >= 8 + (fnm * 16)) &&
               (distopt[palent[(p % 8) + (fnm * 16)]] > 5000) &&
               (clutmask & usemask & (1 << fnm))) {
...
int ula_pickattrs(uint w, uint h, uchar render, uchar opt[w / 8][h][2][4],
                  const palette palent, uchar pal[w / 8][h][2], uchar usemask) {
  for (uint x = 0; x < w / 8; x++)
    for (uint y = 0; y < (IS_RENDER_TIMEX(render) ? h : h / 8); y++) {
      double mv;
      bool set = false;
        ...
        for (uint p = 0; p < 64; p++)
          if (usemask & (1 << (p >> 4))) {
            ...
            for (q = ql; q < ql + 8; q++) {
              ...
              double u =
                  hypot(v * v * opt[x][y][t][3], w * w * opt[x][y][1 - t][3]);
              if ((u < mv) || (!set)) {
                mv = u;
                pal[x][y][0] = p;
                pal[x][y][1] = q;
...
int writebmp(FILE *fp, uint w, uint h, uchar render, uchar pal[w / 8][h][2],
             const palette palent, bool npx[w][h]) {
  ...
  for (int sy = h - 1; sy >= 0; sy--) {
    for (uint sx = 0; sx < w; sx++) {
      uint bx = floor(sx / 8),
           by = IS_RENDER_TIMEX(render) ? sy : floor(sy / 8);
      rgb_t rgb;
      if (IS_RENDER_ULAPLUS(render)) {
...
int writescr(FILE *fp, uint w, uint h, uchar render, uchar pal[w / 8][h][2],
             const palette palent, bool npx[w][h]) {
  if (w > 256)
    return (1);
  if (h > 192)
  ...
  for (uint i = 0; i < 0x1800; i++) {
    uint sx = (i & 0x1f) << 3,
         sy = ((i & 0x1800) >> 5) | ((i & 0xe0) >> 2) | ((i & 0x700) >> 8);
  ...
  for (uint i = 0; i < (IS_RENDER_TIMEX(render) ? 0x1800 : 0x300); i++) {
    ...
    if ((bx < w / 8) && (by < (IS_RENDER_TIMEX(render) ? h : h / 8)))
      palp = pal[bx][by];
    if (IS_RENDER_ULAPLUS(render)) {
      uchar b = (palp[0] & 0x7) | ((palp[1] & 0x7) << 3) |
                ((palp[0] & 0x30) << 2); // construct attribute byte
      fputc(b, fp);
    } else {
      fputc(palp[0], fp);
    }
...
int writezxp(FILE *fp, uint w, uint h, uchar render, uchar pal[w / 8][h][2],
             const palette palent, bool npx[w][h]) {
  ...
  for (unsigned int y = 0; y < (IS_RENDER_TIMEX(render) ? h : h / 8); y++) {
    for (unsigned int x = 0; x < w / 8; x++) {
      ...
      uchar b;
      if (IS_RENDER_ULAPLUS(render))
        b = (pal[x][y][0] & 0x7) | ((pal[x][y][1] & 0x7) << 3) |
            ((pal[x][y][0] & 0x30) << 2); // construct attribute byte
      else
        b = pal[x][y][0];
      fprintf(fp, "%02x", b);
    }
...
void zxtorgb(uchar zx, rgb_t rgb) {
  ...
  int g = zx & 4;
  int b = zx & 1;
  int br = (zx & 8) ? 240 : 200; // figures based on
                                 // http://www.worldofspectrum.org/forums/showpost.php?p=453665&postcount=2
                                 // (na_th_an)
  rgb[0] = r ? br : 0;
  rgb[1] = g ? br : 0;
  rgb[2] = b ? br : 0;
  if ((zx % 8) == 1)
    rgb[2] += 15; // make blue and bright blue slightly brighter.  Based on
                  // http://www.worldofspectrum.org/forums/showpost.php?p=461355&postcount=27
                  // (aowen)
}

void ulatorgb(uchar ula, rgb_t rgb) {
  // 144, 74, 37 for 3-bit (r, g) - based on
  // http://scratchpad.wikia.com/wiki/ZX_Spectrum_64_Colour_Mode#Emulation 170,
  // 85 for 2-bit (b) bit pattern: gggrrrbb
