...
struct {
  ...
  unsigned char *pal;
  bool *npx;
  bool *c_palyuv, *c_pix, *c_post, *c_opt, *c_palopt, *c_distopt, *c_palent,
      *c_pal, *c_npx, *unsaved;
...
static gboolean delete (__attribute__((unused)) GtkWidget *widget,
                        __attribute__((unused)) GdkEvent *event,
                        __attribute__((unused)) gpointer data) {
  return (false);
}

static gboolean maindelete(__attribute__((unused)) GtkWidget *widget,
                           __attribute__((unused)) GdkEvent *event,
                           __attribute__((unused)) gpointer data) {
  if (*state.unsaved) {
    GtkWidget *dialog = gtk_dialog_new_with_buttons(
        "Unsaved!", NULL, GTK_DIALOG_DESTROY_WITH_PARENT, GTK_STOCK_OK,
        GTK_RESPONSE_ACCEPT, GTK_STOCK_CANCEL, GTK_RESPONSE_REJECT, NULL);
    GtkWidget *box = gtk_dialog_get_content_area(GTK_DIALOG(dialog));
    GtkWidget *text =
        gtk_label_new("The image has not been saved.  Really exit?");
...
static void clicked_kill(__attribute__((unused)) GtkWidget *widget,
                         __attribute__((unused)) gpointer data) {
...
static void sh_destroy(__attribute__((unused)) GtkWidget *widget,
                       gpointer data) {
...
static void tweaker(GtkRange *range, gpointer data) {
  ...
  if (i < 6) {
    ((unsigned char(*)[2])state.tweaks)[i >> 1][i & 1] =
...
static void clicked(GtkWidget *widget, gpointer data) {
  ...
  if (strcmp(what, "Refresh") == 0) {
    ...
    apply_tweak(
        *state.w, *state.h, *state.render, (rgb_t(*)[*state.h])state.pix,
        (unsigned char(*)[2])state.tweaks, (yuv_t(*)[*state.h])state.post);
    show_post();
    *state.c_post = false;
    *state.c_pal = true;
    *state.c_npx = true;
    *state.c_opt = true;
    ...
    gtk_file_chooser_set_do_overwrite_confirmation(GTK_FILE_CHOOSER(loader),
                                                   true);
    if (gtk_dialog_run(GTK_DIALOG(loader)) == GTK_RESPONSE_ACCEPT) {
      char *filename = gtk_file_chooser_get_filename(GTK_FILE_CHOOSER(loader));
      if (filename) {
        ...
        if (f) {
          writebmp(f, *state.w, *state.h, *state.render,
                   (unsigned char(*)[*state.h][2])state.pal, state.palent,
                   (bool(*)[*state.h])state.npx);
          fclose(f);
          *state.unsaved = false;
        } else {
          GtkWidget *dialog = gtk_message_dialog_new(
              GTK_WINDOW(state.main), GTK_DIALOG_DESTROY_WITH_PARENT,
              GTK_MESSAGE_ERROR, GTK_BUTTONS_CLOSE,
              "Failed to save to '%s': %s", filename, g_strerror(errno));
    ...
    if (gtk_dialog_run(GTK_DIALOG(loader)) == GTK_RESPONSE_ACCEPT) {
      ...
      if (filename) {
        FILE *f = fopen(filename, "wb");
        if (f) {
          writescr(f, *state.w, *state.h, *state.render,
                   (unsigned char(*)[*state.h][2])state.pal, state.palent,
                   (bool(*)[*state.h])state.npx);
          fclose(f);
          *state.unsaved = false;
    ...
    if (gtk_dialog_run(GTK_DIALOG(loader)) == GTK_RESPONSE_ACCEPT) {
      ...
      if (filename) {
        FILE *f = fopen(filename, "wb");
        if (f) {
          writezxp(f, *state.w, *state.h, *state.render,
                   (unsigned char(*)[*state.h][2])state.pal, state.palent,
                   (bool(*)[*state.h])state.npx);
          fclose(f);
          *state.unsaved = false;
    ...
    if (!state.sh_pix) {
      ...
      state.pb_pix =
          gdk_pixbuf_new(GDK_COLORSPACE_RGB, false, 8, *state.w, *state.h);
    ...
    if (!state.sh_post) {
      ...
      g_signal_connect(state.sh_post, "destroy", G_CALLBACK(sh_destroy),
                       shs + 1);
      ...
      for (unsigned int i = 0; i < 3; i++) {
        ...
        gtk_range_set_value(GTK_RANGE(slider),
                            ((unsigned char(*)[2])state.tweaks)[i][0]);
        g_signal_connect(slider, "value-changed", G_CALLBACK(tweaker),
                         tws + (i * 2));
        GtkWidget *label =
            gtk_label_new((const char *[3]){"+V", "+S", "+H"}[i]);
        ...
        if (i != 2) {
          ...
          gtk_scale_add_mark(GTK_SCALE(slider), 128, GTK_POS_RIGHT, NULL);
          gtk_range_set_value(GTK_RANGE(slider),
                              ((unsigned char(*)[2])state.tweaks)[i][1]);
          g_signal_connect(slider, "value-changed", G_CALLBACK(tweaker),
                           tws + (i * 2) + 1);
          gtk_box_pack_start(GTK_BOX(box2), slider, true, true, 0);
          GtkWidget *label =
      ...
      g_signal_connect(preview, "clicked", G_CALLBACK(clicked),
                       (gpointer) "Preview tweaks");
      gtk_box_pack_start(GTK_BOX(boxtweaks), preview, false, true, 0);
      GtkWidget *refresh = gtk_button_new_with_label("Apply");
      g_signal_connect(refresh, "clicked", G_CALLBACK(clicked),
                       (gpointer) "Refresh");
    ...
    if (!state.sh_pal) {
      ...
      g_signal_connect(state.sh_pal, "destroy", G_CALLBACK(sh_destroy),
                       shs + 2);
      ...
      for (unsigned int i = 0; i < 4; i++) {
        ...
        gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(state.palmasks[0][i]),
                                     *state.clutmask & (1 << i));
        g_signal_connect(state.palmasks[0][i], "toggled",
                         G_CALLBACK(setclutmask), cms + i);
        ...
        gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(state.palmasks[1][i]),
                                     *state.usemask & (1 << i));
        g_signal_connect(state.palmasks[1][i], "toggled",
                         G_CALLBACK(setusemask), cms + i);
      ...
      g_signal_connect(ipbtn, "clicked", G_CALLBACK(clicked),
                       (gpointer) "Import Palette");
            ...
            *state.clutmask = 0;
            for (unsigned int i = 0; i < 4; i++)
              gtk_toggle_button_set_active(
                  GTK_TOGGLE_BUTTON(state.palmasks[0][i]), false);
          }
          fclose(fp);
          *state.c_palent = true;
        }
    ...
    if (!state.sh_out) {
      ...
      g_signal_connect(state.sh_out, "destroy", G_CALLBACK(sh_destroy),
                       shs + 3);
    ...
    if (!state.sh_palopt) {
      ...
      g_signal_connect(state.sh_palopt, "delete-event", G_CALLBACK(delete),
                       NULL);
      g_signal_connect(state.sh_palopt, "destroy", G_CALLBACK(sh_destroy),
                       shs + 4);
    ...
    if (!state.sh_pref) {
      ...
      g_signal_connect(state.sh_pref, "destroy", G_CALLBACK(sh_destroy),
                       shs + 5);
      ...
      gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(render_ulaplus),
                                   IS_RENDER_ULAPLUS(*state.render));
      g_signal_connect(render_ulaplus, "toggled", G_CALLBACK(clicked),
                       (gpointer) "render_ulaplus");
      gtk_box_pack_start(GTK_BOX(box1), render_ulaplus, true, true, 0);

      GtkWidget *render_bw = gtk_toggle_button_new_with_label("B&W mode");
      gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(render_bw),
                                   IS_RENDER_BW(*state.render));
      g_signal_connect(render_bw, "toggled", G_CALLBACK(clicked),
                       (gpointer) "render_bw");
      gtk_box_pack_start(GTK_BOX(box1), render_bw, true, true, 0);

      GtkWidget *render_timex =
          gtk_toggle_button_new_with_label("Timex 8x1 mode");
      gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(render_timex),
                                   IS_RENDER_TIMEX(*state.render));
      g_signal_connect(render_timex, "toggled", G_CALLBACK(clicked),
                       (gpointer) "render_timex");
      gtk_box_pack_start(GTK_BOX(box1), render_timex, true, true, 0);

      GtkWidget *dither_fix = gtk_toggle_button_new_with_label("Dither fix");
      gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(dither_fix),
                                   !*state.dithflow);
      g_signal_connect(dither_fix, "toggled", G_CALLBACK(clicked),
                       (gpointer) "dither_fix");
      gtk_box_pack_start(GTK_BOX(box1), dither_fix, true, true, 0);

      gtk_box_pack_start(GTK_BOX(box1), gtk_label_new("Aspect:"), false, true,
                         0);
      GtkWidget *sizer_scale = gtk_radio_button_new_with_label(NULL, "Scale");
      gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(sizer_scale),
                                   *state.crop == SIZER_SCALE);
      g_signal_connect(sizer_scale, "toggled", G_CALLBACK(clicked),
                       (gpointer) "sizer_scale");
      gtk_box_pack_start(GTK_BOX(box1), sizer_scale, true, true, 0);
      GtkWidget *sizer_crop = gtk_radio_button_new_with_label_from_widget(
          GTK_RADIO_BUTTON(sizer_scale), "Crop");
      gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(sizer_crop),
                                   *state.crop == SIZER_CROP);
      g_signal_connect(sizer_crop, "toggled", G_CALLBACK(clicked),
                       (gpointer) "sizer_crop");
      gtk_box_pack_start(GTK_BOX(box1), sizer_crop, true, true, 0);
      GtkWidget *sizer_zoom = gtk_radio_button_new_with_label_from_widget(
          GTK_RADIO_BUTTON(sizer_scale), "Zoom");
      gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(sizer_zoom),
                                   *state.crop == SIZER_ZOOM);
      g_signal_connect(sizer_zoom, "toggled", G_CALLBACK(clicked),
                       (gpointer) "sizer_zoom");
      gtk_box_pack_start(GTK_BOX(box1), sizer_zoom, true, true, 0);

      gtk_box_pack_start(GTK_BOX(box1), gtk_label_new("depress:"), false, true,
                         0);
...
int main(int argc, char *argv[]) {
  ...
  unsigned char clutmask = 0xF,
                usemask =
                    0xF; // clutmask is which CLUTs may be written to (pickpal);
                         // usemask is which CLUTs may be used (ula_pickattrs)
  bool dithflow = false;
  ...
  state.w = &w;
  state.h = &h;
  state.clutmask = &clutmask;
  state.usemask = &usemask;
  ...
  state.npx = (bool *)npx;
  state.c_palyuv = &c_palyuv;
  state.c_pix = &c_pix;
  state.c_post = &c_post;
  state.c_opt = &c_opt;
  state.c_palopt = &c_palopt;
  state.c_distopt = &c_distopt;
  state.c_palent = &c_palent;
  state.c_pal = &c_pal;
  state.c_npx = &c_npx;
  ...
  g_signal_connect(sh_post, "clicked", G_CALLBACK(clicked),
                   (gpointer) "sh_post");
  ...
  g_signal_connect(sh_palopt, "clicked", G_CALLBACK(clicked),
                   (gpointer) "sh_palopt");
  ...
  g_signal_connect(sh_pref, "clicked", G_CALLBACK(clicked),
                   (gpointer) "sh_pref");
  ...
  g_signal_connect(saveb, "clicked", G_CALLBACK(clicked),
                   (gpointer) "Save BMP");
  ...
  g_signal_connect(saves, "clicked", G_CALLBACK(clicked),
                   (gpointer) "Save SCR");
  ...
  g_signal_connect(savez, "clicked", G_CALLBACK(clicked),
                   (gpointer) "Save ZXP");
...
void do_pipeline(void) {
  ...
  if (*state.c_pix) {
    resample(*state.image, *state.w, *state.h, *state.crop,
             (rgb_t(*)[*state.h])state.pix);
    show_pix();
    *state.c_pix = false;
    *state.c_post = true;
  }
  ...
  if (IS_RENDER_ULAPLUS(*state.render)) {
    if (*state.c_opt) {
      optima(*state.w, *state.h, *state.render, (yuv_t(*)[*state.h])state.post,
             (unsigned char(*)[*state.h][2][4])state.opt);
      *state.c_opt = false;
      *state.c_palopt = true;
      *state.c_pal = true;
    }
    if (*state.c_palopt) {
      optipal(*state.w, *state.h, *state.render,
              (unsigned char(*)[*state.h][2][4])state.opt,
              (unsigned char(*)[*state.h][2][2])state.palopt);
      show_palopt();
      *state.c_palopt = false;
      *state.c_distopt = true;
    }
    if (*state.c_distopt) {
      countopt(*state.w, *state.h, *state.render,
               (unsigned char(*)[*state.h][2][2])state.palopt, state.distopt);
      *state.c_distopt = false;
      *state.c_palent = true;
    }
    if (*state.c_palent) {
      pickpal(state.distopt, state.palent, *state.depress, *state.clutmask,
              *state.usemask);
      show_pal();
    ...
    if (*state.c_pal) {
      ula_pickattrs(*state.w, *state.h, *state.render,
                    (unsigned char(*)[*state.h][2][4])state.opt, state.palent,
                    (unsigned char(*)[*state.h][2])state.pal, *state.usemask);
      *state.c_pal = false;
      *state.c_npx = true;
    }
  } else {
    ...
    show_pal();
    if (*state.c_pal) {
      pickattrs(*state.w, *state.h, *state.render,
                (yuv_t(*)[*state.h])state.post,
                (unsigned char(*)[*state.h][2])state.pal);
      *state.c_pal = false;
      *state.c_npx = true;
    }
  }
  if (*state.c_npx) {
    dither(*state.w, *state.h, *state.render, (yuv_t(*)[*state.h])state.post,
           (unsigned char(*)[*state.h][2])state.pal, state.palent,
           *state.dithflow, (bool(*)[*state.h])state.npx);
    show_out();
    *state.c_npx = false;
    *state.unsaved = true;
  }
...
void show_pal(void) {
  for (unsigned int i = 0; i < 4; i++) {
    if (state.pb_pal[i]) {
      ...
      g_assert(gdk_pixbuf_get_colorspace(state.pb_pal[i]) ==
               GDK_COLORSPACE_RGB);
  ...
  if (state.pb_out) {
    ...
    for (unsigned int y = 0; y < height; y++) {
      for (unsigned int x = 0; x < width; x++) {
        rgb_t rgb = {0, 0, 0};
        if ((y < *state.h) && (x < *state.w)) {
          unsigned int bx = floor(x / 8),
                       by = IS_RENDER_TIMEX(*state.render) ? y : floor(y / 8);
          if (IS_RENDER_ULAPLUS(*state.render)) {
            const unsigned char *a =
                ((unsigned char(*)[*state.h][2])state.pal)[bx][by];
            int ula =
                state
                    .palent[((bool(*)[*state.h])state.npx)[x][y] ? a[0] : a[1]];
            ulatorgb(ula, rgb);
          } else {
      ...
      for (unsigned int x = 0; x < width; x++) {
        ...
        if ((y < *state.h) && (x < *state.w)) {
          unsigned int bx = floor(x / 8),
                       by = IS_RENDER_TIMEX(*state.render) ? y : floor(y / 8);
          if (IS_RENDER_ULAPLUS(*state.render)) {
            int ula = ((unsigned char(*)[*state.h][2][2])
                           state.palopt)[bx][by][(x & 4) ? 1 : 0][0];
            ulatorgb(ula, rgb);
          } else {
            memcpy(rgb, (rgb_t){127, 127, 127}, 3);
          }
