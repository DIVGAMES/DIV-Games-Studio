...
int vtweak(int value, int shift, int scale) {
  return ((int)min(
      max((value + (shift << 1) - 255 - 127.5) * scale / (double)(256 - scale) +
              127.5,
          0),
      255));
}
...
int ctweak(int value, int shift, int scale) {
  return ((int)min(
      max((value + (shift << 1) - 255) * scale / (double)(256 - scale), 0),
      255));
}

int htweak(int value, int shift) { return ((value + shift + 128) % 256); }
