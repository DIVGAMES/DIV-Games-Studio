...
#define end_struct                                                             \
  long_header + 14 + 10 * 10 + 10 * 7 + 8 + 11 + 9 + 10 * 4 + 1026 + 146 +     \
      32 * 3

#define COMMON_PARAMS                                                          \
  void *(*DIV_import)(char *name), void (*DIV_export)(char *name, void *obj)
...
// You must define these functions on the DLL if you want DIV32RUN to call them
// up.
// 컴컴컴컴컴컴컴컴컴컴컴컴컴컴컴컴컴컴컴컴컴컴컴컴컴컴컴컴컴컴컴컴컴컴컴컴컴컴
...
void put_sprite(unsigned char *si, int32_t x, // Put one sprite
                int32_t y, int32_t an, int32_t al, int32_t xg, int32_t yg,
                int32_t ang, int32_t size, int32_t flags);
...
#define process_size                                                           \
  (*_process_size) // Size of every process (on mem[], number of ints)
...
#define set_palette                                                            \
  (*_set_palette) // Set to 1 to activate a new palette,
                  // set to 2 to activate it twice, etc.

#define buffer ((char *)*((unsigned long *)_buffer)) // Pointer to video buffer
#define background                                                             \
  ((char *)*((unsigned long *)_background)) // Pointer to video background
#define ghost                                                                  \
  ((char *)*((unsigned long *)_ghost)) // Pointer to ghost-layering table
...
EXTERN int32_t (*div_rand)(int32_t rang_low,
                           int32_t rang_hi); // Random between two numbers
EXTERN void (*div_text_out)(char *texto, int32_t x,
                            int32_t y); // Screen print function
...
// 컴컴컴컴컴컴컴컴컴컴컴컴컴컴컴컴컴컴컴컴컴컴컴컴컴컴컴컴컴컴컴컴컴컴컴컴컴컴
// Games exported structures (see documentation - Language - Global -
// Structures)
// 컴컴컴컴컴컴컴컴컴컴컴컴컴컴컴컴컴컴컴컴컴컴컴컴컴컴컴컴컴컴컴컴컴컴컴컴컴컴
...
#define DIRINFO                                                                \
  ((_dirinfo *)&mem[long_header + 14 + 10 * 10 + 10 * 7 + 8 + 11 + 9 + 10 * 4])
#define FILEINFO                                                               \
  ((_fileinfo *)&mem[long_header + 14 + 10 * 10 + 10 * 7 + 8 + 11 + 9 +        \
                     10 * 4 + 1026])
#define VIDEO_MODES                                                            \
  ((_video_modes *)&mem[long_header + 14 + 10 * 10 + 10 * 7 + 8 + 11 + 9 +     \
                        10 * 4 + 1026 + 146])
...
#define GLOBAL_IMPORT()                                                        \
  ...
  div_text_out = (void (*)(char *, int32_t, int32_t))DIV_import("div_text_"    \
                                                                "out");
