...
void ascii_version() { retval(VERSION_ASCII); }
...
void ascii_paste() {
  ...
  if (posiciondeltexto < 0)
    retval(-1);
  if (posiciondeltexto > strlen(texto1))
    retval(-1);
  if (strlen(texto1) < strlen(texto2))
    retval(-1);
  if (strlen(texto2) + posiciondeltexto > strlen(texto1))
    retval(-1);
  int contador;
...
void ascii_copy() {
  ...
  if (iniciotexto < 0)
    retval(-1);
  if (iniciotexto > strlen(texto1))
    retval(-1);
  if (longitexto > strlen(texto1))
    retval(-1);
  if (iniciotexto + longitexto > strlen(texto1))
    retval(-1);
...
void input_version() { retval(VERSION_INPUT); }
