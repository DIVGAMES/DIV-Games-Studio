...
int32_t render_dim[10]= {320,3000,1,100,200,0,0,0,-900,0}; // stores render data
...
uint32_t textures_dim[max_textures][3]; // textures width and transparency (ON/OFF)
...
// if off-screen, quits
	if ( tmpvar[0]>=wide || tmpvar[1]<0 || tmpvar[2]>height || tmpvar[3]<0 ) {
		retval ( 1 );
		return;
	}
...
void init_object_log() {
	...
	oblog= ( uint32_t* ) div_malloc ( wide*height*sizeof ( uint32_t ) ); //alocates mem
...
void gen_ztab ( int32_t cord1,int32_t cord2 ) {
	int32_t sline,curs,olds,ptpwidth,halfway,movez,incz,val,start,cont=0;
...
/***************************** sorry no comments....didn't fancy tying.********************/
void tri_gen_ztab ( int32_t cord1, int32_t cord2,char top,char low ) {
	int32_t sline,curs,olds,ptpwidth,halfway,movez,incz,val,start,cont=0,ccord,fcord;
	ptpwidth=abs ( cord1-cord2 );
...
// creates reference whether to draw from left to right ot right to left
	if ( x[top]+ ( ( x[low]-x[top] ) * ( y[mid]-y[top] ) ) / ( y[low]-y[top] ) <=x[mid] ) {
		leftl=3;
		rightl=4;
	}
	else {
