...
comments: I've commented the code as much as i can without making it impossibly
long.
...
int32_t wall_dim[10]; // stores wall/floor data
int32_t render_dim[10] = {320, 3000, 1, 100,  200,
                          0,   0,    0, -900, 0}; // stores render data
uint32_t tex_clip[3];      // stores temporary texture clipped co-ordinates
int32_t *ztab = NULL;      // temporary wall/floor lookup
signed short *zbuf = NULL; // Zbuffer
signed short *tbuf[10] = {NULL, NULL, NULL, NULL, NULL,
                          NULL, NULL, NULL, NULL, NULL}; // TBUFFFERS
char *textures[max_textures];                            // texture pointers
uint32_t textures_dim[max_textures]
                     [3];   // textures width and transparency (ON/OFF)
uint32_t *widthlook = NULL; // quick lookup table for screen
uint32_t *oblog = NULL;     // object logging buffer

// DECLERATIONS

int32_t check_tri_length(void);
int32_t check_length(void);
void col_draw_wall(void);
void tex_draw_wall(void);
void col_draw_tri_floor(void);
void clip_wall(void);
void clip_floor(void);
void tri_gen_ztab(int32_t cord1, int32_t cord2, char top, char low);
void gen_ztab(int32_t cord1, int32_t cord2);
...
void draw_sprite() {
  ...
  nz = wall_dim[1];
  wall_dim[0] = nz * sin((float)render_dim[8] / RADS_TO_DEGS) +
                nx * cos((float)render_dim[8] / RADS_TO_DEGS);
  wall_dim[1] = nz * cos((float)render_dim[8] / RADS_TO_DEGS) -
                nx * sin((float)render_dim[8] / RADS_TO_DEGS);

  if (wall_dim[1] < 0) {
    retval(1); // if Z value is behind camera, quits
    return;
  }
  ...
  wall_dim[0] -= (((textures_dim[wall_dim[7]][0] >> 9) * wall_dim[8]) / 100);
  wall_dim[4] =
      wall_dim[6] + ((textures_dim[wall_dim[7]][1] * wall_dim[8]) / 100);

  // 3D to 2D translation
  tmpvar[0] = (wide >> 1) +
              (wall_dim[0] * render_dim[4]) / (wall_dim[1] + render_dim[4]);
  tmpvar[1] = (wide >> 1) +
              (wall_dim[2] * render_dim[4]) / (wall_dim[1] + render_dim[4]);
  tmpvar[3] = render_dim[3] -
              (wall_dim[6] * render_dim[4]) / (wall_dim[1] + render_dim[4]);
  tmpvar[2] = render_dim[3] -
              (wall_dim[4] * render_dim[4]) / (wall_dim[1] + render_dim[4]);

  // if off-screen, quits
  if (tmpvar[0] >= wide || tmpvar[1] < 0 || tmpvar[2] > height ||
      tmpvar[3] < 0) {
    retval(1);
    return;
  }
  ...
  if (tmpvar[2] < -1) {
    tmpvar[8] =
        tmpvar[6] * ((0 - tmpvar[2]) - 1); // vertical clipping (top of sprite)
    tmpvar[2] = -1;
  } else
    tmpvar[8] = 0;

  if (tmpvar[3] >= height)
    tmpvar[3] = height - 1; // vertical clipping (base of sprite)

  for (nx = tmpvar[0]; nx < tmpvar[1]; nx++) { // draws from left to right
    if (nx >= 0 && nx < wide) {                // if onscreen

      // gets starting X texture co-ordintate
      tmpvar[4] = ((nx - tmpvar[0]) * (textures_dim[wall_dim[7]][0] >> 8)) /
                  (tmpvar[1] - tmpvar[0]); // gets starting
        ...
        if (textures_dim[wall_dim[7]][2] == 0) { // if no transparent areas
          for (nz = tmpvar[2] + 1; nz <= tmpvar[3]; nz++) {
            ...
            if (wall_dim[1] <= zbuf[nx + widthlook[nz]]) {
              background[nx + widthlook[nz]] =
                  thetex[tmpvar[4] + (tmpvar[5] >> 16) *
                                         (textures_dim[wall_dim[7]][0] >> 8)];
          ...
          for (nz = tmpvar[2] + 1; nz <= tmpvar[3]; nz++) {
            tmpvar[7] = thetex[tmpvar[4] + (tmpvar[5] >> 16) *
                                               (textures_dim[wall_dim[7]][0] >>
                                                8)]; // gets pixel colour
            ...
            if (wall_dim[1] <= zbuf[nx + widthlook[nz]]) {
              background[nx + widthlook[nz]] =
                  thetex[tmpvar[4] + (tmpvar[5] >> 16) *
                                         (textures_dim[wall_dim[7]][0] >> 8)];
              zbuf[nx + widthlook[nz]] = tmpvar[1];
              oblog[nx + widthlook[nz]] = (tmpvar[4] << 21) +
                                          ((tmpvar[5] >> 16) << 11) +
                                          wall_dim[7] + 1;
            }
...
void alloc_tbuffer() {
  int32_t no = getparm(); // gets TBUFFER id
  ...
  tbuf[no] = (signed short *)div_malloc(wide * height *
                                        sizeof(signed short)); // allocates mem
...
void copy_background_to_tbuffer() {
  ...
  tmpbuf = tbuf[no];
  memcpy((char *)tmpbuf, background,
         wide * height); // copies background to Tbuffer
  retval(1);
}
...
void copy_tbuffer_to_background() {
  ...
  tmpbuf = tbuf[no];
  memcpy(background, (char *)tmpbuf,
         wide * height); // copies Tbuffer to background
  retval(1);
}
...
void copy_zbuffer_to_tbuffer() {
  ...
  tmpbuf = tbuf[no];
  memcpy((char *)tmpbuf, (char *)zbuf,
         (wide * height) << 1); // copies ZBUFFER to TBUFFER
  retval(1);
}
...
void copy_tbuffer_to_zbuffer() {
  ...
  tmpbuf = tbuf[no];
  memcpy((char *)zbuf, (char *)tmpbuf,
         (wide * height) << 1); // Copies Tbuffer to Zbuffer
  retval(1);
}

/************************RETURNS 1 IF TEXTURE EXISTS ***********************/
void texture_exists() {
  int32_t texid = getparm(); // gets texture id
...
void copy_to_texture() {
  int32_t y = getparm();   // y
  int32_t x = getparm();   // x
  int32_t cpy = getparm(); // source texture
  int32_t dst = getparm(); // destination texture
  int32_t c, d;
  char *dsttex;
  char *cpytex;

  if (dst < 0 || dst >= max_textures || cpy < 0 ||
      cpy >= max_textures) { // if invalid texture id, quits
    retval(-2);
    return;
  }

  if (textures[dst] == NULL ||
      textures[cpy] == NULL) { // if any texture doesn't exist, quits
    retval(-1);
    return;
  }
  ...
  for (c = 0; c<textures_dim[cpy][0]>> 8; c++) { // works left to right
    for (d = 0; d < textures_dim[cpy][1]; d++) { // works Top to bottom
      ...
      if (c + x >= 0 && c + x < (textures_dim[dst][0] >> 8) && d + y >= 0 &&
          d + y < textures_dim[dst][1]) {
        // if colour is not 0, copies to texture
        if (cpytex[c + d * (textures_dim[cpy][0] >> 8)] != 0)
          dsttex[(x + c) + (y + d) * (textures_dim[dst][0] >> 8)] =
              cpytex[c + d * (textures_dim[cpy][0] >> 8)];
      }
    }
  }
...
void texture_put_pixel() {
  ...
  int32_t col = getparm(); // gets colour
  int32_t y = getparm();   // gets y
  int32_t x = getparm();   // gets x
  int32_t dst = getparm(); // gets texture
  char *dsttex;
  ...
  if (x >= 0 && x < (textures_dim[dst][0] >> 8) && y >= 0 &&
      y < textures_dim[dst][1] && col >= 0 && col < 256)
    dsttex[x + y * (textures_dim[dst][0] >> 8)] = col;
...
void texture_get_pixel() {
  ...
  if (x >= 0 && x < (textures_dim[dst][0] >> 8) && y >= 0 &&
      y < textures_dim[dst][1])
    retval(dsttex[x + y * (textures_dim[dst][0] >> 8)]);
  else
    retval(0);
}

/********************************* RETURNS TEXTURE ID FROM OBJECT
 * LOG**************/
...
void init_object_log() {
  ...
  if (oblog != NULL)
    div_free(oblog); // if used free's mem

  oblog =
      (uint32_t *)div_malloc(wide * height * sizeof(uint32_t)); // alocates mem
...
void rotate_pointx() {
  int32_t angle = getparm() / 100; // gets angle
  int32_t y = getparm();           // gets y
  int32_t x = getparm();           // gets x

  int32_t nx = x; // maths stuff, & returns new X
  int32_t ny = y;
  x = ny * sin((float)angle / RADS_TO_DEGS) +
      nx * cos((float)angle / RADS_TO_DEGS);
  retval(x);
}
...
void rotate_pointy() {
  ...
  int32_t nx = x; // maths stuff & returns new y
  int32_t ny = y;
  y = ny * cos((float)angle / RADS_TO_DEGS) -
      nx * sin((float)angle / RADS_TO_DEGS);
  retval(y);
}
...
void stencil_zbuffer() {
  char *thetex;
  int32_t texid = getparm(); // gets stencil texture id
  int32_t c, upto;
  ...
  if ((textures_dim[texid][0] >> 8) != wide ||
      textures_dim[texid][1] != height) {
    retval(-3);
    return;
  }
  ...
  for (c = 0; c < upto; c++) {
    if (thetex[c] == 0)
      zbuf[c] = 32767; // iftransparent pixel, clear part of Zbuffer
    else
      zbuf[c] = -1; // else make it un-usable
  }
...
int32_t check_length() {
  int32_t x = abs(wall_dim[0] - wall_dim[2]);
  int32_t y = abs(wall_dim[1] - wall_dim[3]);
  int32_t _x = x - ((x >> 6) << 6);
  int32_t _y = y - ((y >> 6) << 6);
  x = x >> 6;
  y = y >> 6;

  if (sqrt((_x * _x) + (_y * _y)) + (sqrt((x * x) + (y * y)) * 64) <=
      render_dim[1])
    return (1);
 ...
 * ***********/
void Max_Textures() { retval(max_textures); }
...
/*****************************SETS TEXTURE TO BE DRAWN AS IF
 * TRANSPARENT************/
...
void grab_texture() {
  ...
  if (tmpvar[4] >= max_textures ||
      tmpvar[4] < 0) { //   if invalid texture,quits
    retval(-2);
    return;
  }

  if (textures[tmpvar[4]] != NULL)
    div_free(textures[tmpvar[4]]); // if previously used, free's mem

  textures[tmpvar[4]] = (char *)div_malloc((1 + tmpvar[0]) * tmpvar[1] *
                                           sizeof(char)); // allocates mem
  ...
  for (tmpvar[2] = 0; tmpvar[2] < tmpvar[0];
       tmpvar[2]++) { // reads screen left to right....
    for (tmpvar[3] = 0; tmpvar[3] < tmpvar[1]; tmpvar[3]++) { // top to bottom

      if (background[tmpvar[2] + tmpvar[3] * wide] == 0)
        textures_dim[tmpvar[4]][2] = 1; // if col0, turns transparmency ON

      thetex[tmpvar[2] + tmpvar[3] * tmpvar[0]] =
          background[tmpvar[2] + tmpvar[3] * wide]; // stores texture
    }
  }
...
void init_modeA() {
  ...
  if (ztab != NULL)
    div_free(ztab); // if previously used, frees memory
  ...
  ztab = (int32_t *)div_malloc(wide * sizeof(int32_t));
  zbuf = (signed short *)div_malloc(wide * height *
                                    sizeof(signed short)); // allocates memory
  widthlook = (uint32_t *)div_malloc(height * sizeof(uint32_t));

  if (ztab == NULL || zbuf == NULL ||
      widthlook == NULL) { // if could not allocate any portion of memory, free
                           // rest & quit
    if (ztab != NULL)
      div_free(ztab);
  ...
  render_dim[4] = ((render_dim[4] * wide) /
                   render_dim[0]); // fixes render perspective for resolution
...
void set_render_perspective() {
  render_dim[4] = (getparm() * wide) /
                  320; // gets render perspective and adjusts for resolution
  render_dim[0] = wide;
...
void get_render_perspective() { retval((render_dim[4] * 320) / wide); }
...
void clear_zbuffer() {
  ...
  memset((char *)zbuf, 127,
         (wide * height)
             << 1); // clears ZBuffer with highest possible value using MEMSET
  retval(1);
}

/******************************* GENERATES WALL Z LOOKUP TABLE
 * *******************/
/***************************** sorry no comments....didn't fancy
 * tying.********************/
void gen_ztab(int32_t cord1, int32_t cord2) {
  int32_t sline, curs, olds, ptpwidth, halfway, movez, incz, val, start,
      cont = 0;
  ...
  if (cord1 < 0) {
    for (olds = 0; olds < 16; olds++) {
      ...
      halfway =
          (wide >> 1) +
          ((wall_dim[0] + (((wall_dim[2] - wall_dim[0]) * start) >> 14)) *
           render_dim[4]) /
              ((wall_dim[1] + (((wall_dim[3] - wall_dim[1]) * start) >> 14)) +
               render_dim[4]);
  ...
  for (olds = 0; olds < 16; olds++) {
    ...
    halfway =
        (wide >> 1) +
        ((wall_dim[0] + (((wall_dim[2] - wall_dim[0]) * movez) >> 14)) *
         render_dim[4]) /
            ((wall_dim[1] + (((wall_dim[3] - wall_dim[1]) * movez) >> 14)) +
             render_dim[4]);

    if (halfway == (cord1 + (cord2 + (((cord1 - cord2) * 2) / 4))) >> 1)
      break;

    if (abs(halfway - cord1) <=
        abs(halfway - (cord2 + (((cord1 - cord2) * 2) / 4))))
      movez += incz;
    else
      movez -= incz;
  }
  ...
  if (halfway > 0) {
    for (sline = ((halfway * start) / movez); sline < halfway; sline++) {
      ...
      curs = (wide >> 1) +
             ((wall_dim[0] + (((wall_dim[2] - wall_dim[0]) * val) >> 14)) *
              render_dim[4]) /
                 ((wall_dim[1] + (((wall_dim[3] - wall_dim[1]) * val) >> 14)) +
                  render_dim[4]);
  ...
  if (ptpwidth - halfway > 0) {
    for (sline = (((start - movez) * (ptpwidth - halfway)) / (16383 - movez))
                 << 1;
         sline <= (ptpwidth - halfway) << 1; sline++) {
      if (cont == -1)
        return;
...
void tri_gen_ztab(int32_t cord1, int32_t cord2, char top, char low) {
  ...
  if (cord1 < 0) {
    for (olds = 0; olds < 16; olds++) {
      incz = (incz >> 1);
      halfway = render_dim[3] -
                (wall_dim[6] * render_dim[4]) /
                    ((wall_dim[top] +
                      (((wall_dim[low] - wall_dim[top]) * start) >> 14)) +
                     render_dim[4]);
  ...
  for (olds = 0; olds < 16; olds++) {
    ...
    halfway = render_dim[3] -
              (wall_dim[6] * render_dim[4]) /
                  ((wall_dim[top] +
                    (((wall_dim[low] - wall_dim[top]) * movez) >> 14)) +
                   render_dim[4]);

    if (halfway == ccord + (((fcord - ccord) * 2) / 4))
      break;

    if (abs(halfway - (ccord + (((fcord - ccord) * 2) / 4))) >=
        abs(halfway - fcord))
      movez += incz;
    else
      movez -= incz;
  ...
  olds = render_dim[3] -
         (wall_dim[6] * render_dim[4]) / (wall_dim[top] + render_dim[4]);
  ...
  if (halfway > 0) {
    for (sline = ((halfway * start) / movez); sline < halfway; sline++) {
      ...
      curs = render_dim[3] -
             (wall_dim[6] * render_dim[4]) /
                 ((wall_dim[top] +
                   (((wall_dim[low] - wall_dim[top]) * val) >> 14)) +
                  render_dim[4]);

      if (curs >= 0 && curs < height) {
        ztab[curs] = val;
        cont = 1;
      } else {
        if (cont == 1)
...
void clip_wall() {
  ...
  if (wall_dim[3] < 0) { // if part is behind camera
    wall_dim[2] += (((wall_dim[0] - wall_dim[2]) * -wall_dim[3]) /
                    (wall_dim[1] - wall_dim[3])); // X value
    wall_dim[8] += (((wall_dim[5] - wall_dim[8]) * -wall_dim[3]) /
                    (wall_dim[1] - wall_dim[3])); // wall height value

    if (render_dim[2] == 1) { // if  textured
      tex_clip[0] = 0;
      // texture X ending co-ordinate
      tex_clip[1] = textures_dim[wall_dim[6]][0] -
                    ((textures_dim[wall_dim[6]][0] * -wall_dim[3]) /
                     (wall_dim[1] - wall_dim[3]));
    }
...
/*************************** CLIPS FLOOR ***********************************/
/*
Exact same as Clip_wall() except it goes through all combinations of points
being offscreen assuming at least 1 point is onscreen. I can't really comment
more than that.
*/
...
void clip_floor() {
  ...
  if (wall_dim[1] < 0 && wall_dim[3] >= 0 && wall_dim[5] >= 0) {
    oldx = wall_dim[0];
    oldy = wall_dim[1];
    wall_dim[0] += ((wall_dim[2] - wall_dim[0]) * -wall_dim[1]) /
                   (wall_dim[3] - wall_dim[1]);
    wall_dim[1] = 0;
    col_draw_tri_floor();
  ...
  if (wall_dim[5] < 0 && wall_dim[3] >= 0 && wall_dim[1] >= 0) {
    oldx = wall_dim[4];
    oldy = wall_dim[5];
    wall_dim[4] += ((wall_dim[0] - wall_dim[4]) * -wall_dim[5]) /
                   (wall_dim[1] - wall_dim[5]);
    wall_dim[5] = 0;
    col_draw_tri_floor();
  ...
  if (wall_dim[1] < 0 && wall_dim[3] < 0 && wall_dim[5] >= 0) {
    wall_dim[0] += ((wall_dim[4] - wall_dim[0]) * -wall_dim[1]) /
                   (wall_dim[5] - wall_dim[1]);
    wall_dim[1] = 0;
    wall_dim[2] += ((wall_dim[4] - wall_dim[2]) * -wall_dim[3]) /
                   (wall_dim[5] - wall_dim[3]);
    wall_dim[3] = 0;
    col_draw_tri_floor();
  }
  ...
  if (wall_dim[1] < 0 && wall_dim[3] >= 0 && wall_dim[5] < 0) {
    wall_dim[0] += ((wall_dim[2] - wall_dim[0]) * -wall_dim[1]) /
                   (wall_dim[3] - wall_dim[1]);
    wall_dim[1] = 0;
    wall_dim[4] += ((wall_dim[2] - wall_dim[4]) * -wall_dim[5]) /
                   (wall_dim[3] - wall_dim[5]);
    wall_dim[5] = 0;
    col_draw_tri_floor();
  }
...
void draw_tri_floor() {
  ...
  if (check_tri_length() != 1) {
    retval(-1); // if too bug, quit
    return;
  }
  ...
  nz = wall_dim[3];
  wall_dim[2] = nz * sin((float)render_dim[8] / RADS_TO_DEGS) +
                nx * cos((float)render_dim[8] / RADS_TO_DEGS);
  wall_dim[3] = nz * cos((float)render_dim[8] / RADS_TO_DEGS) -
                nx * sin((float)render_dim[8] / RADS_TO_DEGS);

  nx = wall_dim[4]; // Maths rotations on point 3
  nz = wall_dim[5];
  wall_dim[4] = nz * sin((float)render_dim[8] / RADS_TO_DEGS) +
                nx * cos((float)render_dim[8] / RADS_TO_DEGS);
  wall_dim[5] = nz * cos((float)render_dim[8] / RADS_TO_DEGS) -
                nx * sin((float)render_dim[8] / RADS_TO_DEGS);

  if (wall_dim[1] < 0 && wall_dim[3] < 0 &&
      wall_dim[5] < 0) { // if completely behind camera, quit
    retval(1);
    return;
  }
...
void col_draw_tri_floor() {
  ...
  int32_t x[5], y[3], start, fini1, fini2, curz;

  x[0] = (wide >> 1) + (wall_dim[0] * render_dim[4]) /
                           (wall_dim[1] + render_dim[4]); // 3D->2D
  y[0] = render_dim[3] -
         (wall_dim[6] * render_dim[4]) / (wall_dim[1] + render_dim[4]);
  x[1] = (wide >> 1) +
         (wall_dim[2] * render_dim[4]) / (wall_dim[3] + render_dim[4]);
  y[1] = render_dim[3] -
         (wall_dim[6] * render_dim[4]) / (wall_dim[3] + render_dim[4]);
  x[2] = (wide >> 1) +
         (wall_dim[4] * render_dim[4]) / (wall_dim[5] + render_dim[4]);
  y[2] = render_dim[3] -
         (wall_dim[6] * render_dim[4]) / (wall_dim[5] + render_dim[4]);
  ...
  // The next section of code just creates a reference to the order the points
  // are in (top first)
  ...
  if (y[low] > y[top])
    tri_gen_ztab(y[top], y[low], (top * 2) + 1,
                 (low * 2) + 1); // generates Z lookup table

  else
    return;

  if (y[top] < 0)
    start = (0 - y[top]); // offscreen clipping
  else
    start = 0;

  if (y[mid] < height)
    fini1 = y[mid] - y[top]; // where to end drawing from point 1 to 2
  else
    fini1 = (height - y[top]) - 1;

  if (y[low] < height)
    fini2 = y[low] - y[top]; // where to end drawing from point 2 to 3
  else
    fini2 = (height - y[top]) - 1;

  // creates reference whether to draw from left to right ot right to left
  if (x[top] + ((x[low] - x[top]) * (y[mid] - y[top])) / (y[low] - y[top]) <=
      x[mid]) {
    leftl = 3;
    rightl = 4;
  } else {
  ...
  while (1) {
    x[leftl] = x[top] + ((x[low] - x[top]) * start) /
                            (y[low] - y[top]); // gets 1st co-ordinate

    // gets 2nd co-ordinate when drawing from point 1 to point 2
    if (start <= fini1) {
      if (y[mid] > y[top])
        x[rightl] = x[top] + ((x[mid] - x[top]) * start) / (y[mid] - y[top]);
      else
        x[rightl] = x[mid];
    } else { // gets 2nd co-ordinate when drawing from point 2 to point 3
      if (y[low] > y[mid])
        x[rightl] = x[mid] + ((x[low] - x[mid]) * (start - (y[mid] - y[top]))) /
                                 (y[low] - y[mid]);
      else
        x[rightl] = x[mid];
    }
    ...
    if (x[3] != -1) {
      curz = wall_dim[(top * 2) + 1] +
             ((ztab[start + y[top]] *
               (wall_dim[(low * 2) + 1] - wall_dim[(top * 2) + 1])) /
              16383); // gets Z value

      if (render_dim[9] == 0) { // if Object Logging is OFF
        for (; x[3] <= x[4]; x[3]++) {
          if (curz <
              zbuf[x[3] + widthlook[start + y[top]]]) { // if closer than area
                                                        // of Zbuffer...
            background[x[3] + widthlook[start + y[top]]] =
                wall_dim[7];                               // draws pixel
        ...
        for (; x[3] <= x[4]; x[3]++) {
          if (curz <
            ...
            oblog[x[3] + widthlook[start + y[top]]] =
                0; // draws info to object log
          }
        }
      }
    }
...
void draw_wall() {
  ...
  if (check_length() != 1) {
    retval(-1); // if illegal lenght, quits
    return;
  }
  ...
  if (render_dim[2] == 0) { // if coloured mode
    if (wall_dim[6] < 256 && wall_dim[6] >= 0 && wall_dim[1] >= 0) {
      clip_wall(); // if legal colour, clips & draws
      col_draw_wall();
    }
  } else { // if textured mode
    if (wall_dim[6] < max_textures && wall_dim[6] >= 0 &&
        wall_dim[1] >= 0) { // if legal texture, clips & draws
 ...
 * WALL**********************************/
void col_draw_wall() {
  int32_t scord[6], tmpvar[7];

  scord[0] = (wide >> 1) + (wall_dim[0] * render_dim[4]) /
                               (wall_dim[1] + render_dim[4]); // 3D->2D
  scord[1] = render_dim[3] -
             (wall_dim[4] * render_dim[4]) / (wall_dim[1] + render_dim[4]);
  scord[2] = (wide >> 1) +
             (wall_dim[2] * render_dim[4]) / (wall_dim[3] + render_dim[4]);
  scord[3] = render_dim[3] -
             (wall_dim[7] * render_dim[4]) / (wall_dim[3] + render_dim[4]);
  scord[4] = render_dim[3] -
             (((wall_dim[4] + wall_dim[5]) + render_dim[6]) * render_dim[4]) /
                 (wall_dim[1] + render_dim[4]);
  scord[5] = render_dim[3] -
             (((wall_dim[7] + wall_dim[8]) + render_dim[6]) * render_dim[4]) /
                 (wall_dim[3] + render_dim[4]);

  // if offscreen, quits
  if ((scord[0] <= 0 && scord[2] <= 0) ||
      (scord[0] >= wide && scord[2] >= wide) ||
      (scord[0] <= scord[2] + 1 && scord[0] >= scord[2] - 1))
    return; // if offscreen quit

  if ((scord[1] >= height && scord[3] >= height && scord[4] >= height &&
       scord[5] >= height) ||
      (scord[1] < 0 && scord[3] < 0 && scord[4] < 0 && scord[5] < 0))
    return;

  gen_ztab(scord[0], scord[2]); // generates z lookup

  if (scord[0] < 0)
    tmpvar[3] = 0 - scord[0]; // X clipping
  else
    tmpvar[3] = 0;

  if (scord[0] >= wide)
    tmpvar[3] = (scord[0] - wide) + 1;

  for (tmpvar[3] = tmpvar[3]; tmpvar[3] <= abs(scord[0] - scord[2]);
       tmpvar[3]++) {

    tmpvar[0] = scord[0] + (((scord[2] - scord[0]) * tmpvar[3]) /
                            abs(scord[0] - scord[2])); // gets x co

    if (tmpvar[0] < 0 || tmpvar[0] >= wide)
      return;

    tmpvar[1] = scord[1] + (((scord[3] - scord[1]) * tmpvar[3]) /
                            abs(scord[0] - scord[2])); // gets bottom y co
    tmpvar[2] = scord[4] + (((scord[5] - scord[4]) * tmpvar[3]) /
                            abs(scord[0] - scord[2])); // gets top y co

    if (tmpvar[1] >= 0 && tmpvar[2] < height && tmpvar[0] >= 0 &&
        tmpvar[0] < wide) { // if oncreen

      tmpvar[4] =
          wall_dim[1] + (((wall_dim[3] - wall_dim[1]) * ztab[tmpvar[0]]) >>
                         14); // gets Z value

      if (tmpvar[1] >= height)
        tmpvar[5] = height - 1; // vertical clipping
      else
        tmpvar[5] = tmpvar[1];
      ...
      if (tmpvar[1] > tmpvar[2]) { // if two points are not the same
        ...
        if (render_dim[9] == 0) { // if Object Logging is OFF
          for (tmpvar[1] = tmpvar[5]; tmpvar[1] > tmpvar[6]; tmpvar[1]--) {
            if (tmpvar[4] <=
                zbuf[tmpvar[0] +
                     widthlook[tmpvar[1]]]) { // if closer than Zbuffer
              background[tmpvar[0] + widthlook[tmpvar[1]]] =
                  wall_dim[6]; // draw pixel
              zbuf[tmpvar[0] + widthlook[tmpvar[1]]] =
                  tmpvar[4]; // updates Zbuffer
            }
          }
          ...
          for (tmpvar[1] = tmpvar[5]; tmpvar[1] > tmpvar[6]; tmpvar[1]--) {
            if (tmpvar[4] <=
                zbuf[tmpvar[0] +
                     widthlook[tmpvar[1]]]) { // if closer than zbuffer
              background[tmpvar[0] + widthlook[tmpvar[1]]] =
                  wall_dim[6]; // draws pixel
              zbuf[tmpvar[0] + widthlook[tmpvar[1]]] =
                  tmpvar[4]; // updates object logging
              oblog[tmpvar[0] + widthlook[tmpvar[1]]] =
                  0; // writess object logging information
            }
          }
        }
      }
...
void tex_draw_wall() {
  ...
  if (scord[0] < 0)
    tmpvar[3] = 0 - scord[0]; // horizontal clipping
  else
    tmpvar[3] = 0;
  ...
  for (tmpvar[3] = tmpvar[3]; tmpvar[3] <= abs(scord[0] - scord[2]);
    ...
    if (tmpvar[0] < 0 || tmpvar[0] >= wide)
      return; // if offscreen at any time, quits
    ...
    if (tmpvar[1] >= 0 && tmpvar[2] < height) { // if oncreen
      ...
      tmpvar[4] =
          wall_dim[1] + (((wall_dim[3] - wall_dim[1]) * ztab[tmpvar[0]]) >>
                         14); // gets Z value
      texx = ((((ztab[tmpvar[0]] >> 7) * (tex_clip[1] - tex_clip[0])) >> 7)) >>
             8; // gets Texture X value
      texx +=
          (tex_clip[0] + (((ztab[tmpvar[0]] - ((ztab[tmpvar[0]] >> 7) << 7)) *
                           (tex_clip[1] - tex_clip[0])) >>
                          14)) >>
          8;

      if (tex_clip[2] == 1)
        texx = (textures_dim[wall_dim[6]][0] >> 8) - texx -
               1; // if wall has been reversed, reverses texture
      ...
      if (tmpvar[1] > tmpvar[2]) { // if 2 points are not the same
        ...
        tmpvar[10] =
            ((textures_dim[wall_dim[6]][1] - 1) << 16) /
            (tmpvar[1] -
             tmpvar[2]); // gets vetical texture increment per pixel x65356
        tmpvar[9] = ((textures_dim[wall_dim[6]][1] - 1) << 16) -
                    (tmpvar[10] *
                     (tmpvar[1] - tmpvar[5])); // gets starting texture y x65536

        if (render_dim[9] == 0) { // if Object Logging is OFF
          if (textures_dim[wall_dim[6]][2] ==
              0) { // if texture has no transparencies
            for (tmpvar[8] = tmpvar[5]; tmpvar[8] > tmpvar[6]; tmpvar[8]--) {
              if (tmpvar[4] <=
                  zbuf[tmpvar[0] +
                       widthlook[tmpvar[8]]]) { // if closer than zbuffer
                background[tmpvar[0] + widthlook[tmpvar[8]]] =
                    curtex[texx + (tmpvar[9] >> 16) *
                                      (textures_dim[wall_dim[6]][0] >> 8)];
                zbuf[tmpvar[0] + widthlook[tmpvar[8]]] =
                    tmpvar[4]; // updates zbuffer
              }
            ...
            for (tmpvar[8] = tmpvar[5]; tmpvar[8] > tmpvar[6]; tmpvar[8]--) {
              if (tmpvar[4] <=
                  zbuf[tmpvar[0] +
                       widthlook[tmpvar[8]]]) { // if closer than zbuffer
                tmpvar[1] = curtex[texx + (tmpvar[9] >> 16) *
                                              (textures_dim[wall_dim[6]][0] >>
                                               8)]; // gets pixel colour
        ...
        } else { // if object logging is ON (same comments as above but with
                 // Oblog[]
          if (textures_dim[wall_dim[6]][2] == 0) {
            for (tmpvar[8] = tmpvar[5]; tmpvar[8] > tmpvar[6]; tmpvar[8]--) {
              if (tmpvar[4] <=
                  zbuf[tmpvar[0] + widthlook[tmpvar[8]]]) { // if closer
                background[tmpvar[0] + widthlook[tmpvar[8]]] =
                    curtex[texx + (tmpvar[9] >> 16) *
                                      (textures_dim[wall_dim[6]][0] >> 8)];
                zbuf[tmpvar[0] + widthlook[tmpvar[8]]] = tmpvar[4];
                oblog[tmpvar[0] + widthlook[tmpvar[8]]] =
                    (texx << 21) + ((tmpvar[9] >> 16) << 11) + wall_dim[6] + 1;
              }
            ...
            for (tmpvar[8] = tmpvar[5]; tmpvar[8] > tmpvar[6]; tmpvar[8]--) {
              if (tmpvar[4] <=
                ...
                if (tmpvar[1] != 0) {
                  ...
                  zbuf[tmpvar[0] + widthlook[tmpvar[8]]] = tmpvar[4];
                  oblog[tmpvar[0] + widthlook[tmpvar[8]]] =
                      (texx << 21) + ((tmpvar[9] >> 16) << 11) + wall_dim[6] +
                      1;
                }
              }
...
void get_pixels_depth() {
  int32_t y = getparm(); // gets y val
  int32_t x = getparm(); // gets x val
...
void get_camera_x() { retval(render_dim[5]); }

void get_camera_y() { retval(render_dim[6]); }
void get_camera_z() { retval(render_dim[7]); }

void get_camera_angle() { retval(render_dim[8]); }

void get_camera_horizon() { retval(render_dim[3]); }
