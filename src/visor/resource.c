...
void resource_create(Resource *resource_struct, char *name)
{
	...
	if (resource_struct->Packed == NULL) {
		ERROR = (char *)ERR4;
		printf("Nombre del recurso: %s\n", name);
		exit(1);
	};
	fread(Clave, 4, 1, resource_struct->Packed);
	Clave[4] = 0;
	if (strcmp(Clave, "PACK")) {
		ERROR = (char *)ERR5;
		exit(1);
	};
	fread(&resource_struct->Num_Pack, 4, 1, resource_struct->Packed);
	if ((resource_struct->MyPack = (Pack *)lf_malloc(
		 sizeof(Pack) * resource_struct->Num_Pack)) == NULL)
		return;
	fread(resource_struct->MyPack, sizeof(Pack), resource_struct->Num_Pack,
	      resource_struct->Packed);
}
...
FILE *resource_GetFilePtrPacked(Resource *resource_struct, char *Name)
{
	...
	for (x = 0; x < resource_struct->Num_Pack; x++)
		if (!strcmp(Name, resource_struct->MyPack[x].Name)) {
			fseek(resource_struct->Packed,
			      resource_struct->MyPack[x].Offsets, SEEK_SET);
...
char *resource_GetPacked(Resource *resource_struct, char *Name)
{
	...
	fseek(resource_struct->Packed,resource_struct->MyPack[x].Offsets
      ,SEEK_SET);
	...
	lf_free(Buffer);
      ...
      {
*/
			if ((Buffer = (unsigned char *)lf_malloc(
				 resource_struct->MyPack[x].RealLength)) ==
			    NULL)
				return (NULL);
			fseek(resource_struct->Packed,
			      resource_struct->MyPack[x].Offsets, SEEK_SET);
			fread(Buffer, resource_struct->MyPack[x].RealLength, 1,
			      resource_struct->Packed);
			return ((char *)Buffer);
			//      }
		}
