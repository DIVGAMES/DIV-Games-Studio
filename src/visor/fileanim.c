...
void fileanim_create(fileanim *fileanim_struct, char *name)
{
	...
	if (strcmp(fileanim_struct->header.Chunk, "A3D")) {
		ERROR = (char *)ERR15;
		exit(1);
	};
	if ((fileanim_struct->anims = (a3d_anim *)lf_malloc(
		 sizeof(a3d_anim) * fileanim_struct->header.nAnims)) == NULL)
		return;
	fread(fileanim_struct->anims, sizeof(a3d_anim),
	      fileanim_struct->header.nAnims, file);
	for (x = 0; x < fileanim_struct->header.nAnims; x++) {
		fileanim_struct->anims[x].offset = nmatrix;
		nmatrix += fileanim_struct->header.nObjects *
			   fileanim_struct->anims[x].nFrames;
	}
	if ((fileanim_struct->buffer =
		 (lptmatrix)lf_malloc(sizeof(tmatrix) * nmatrix)) == NULL)
		return;
