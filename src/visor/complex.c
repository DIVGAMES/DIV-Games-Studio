...
void complex_create(complex *complex_struct, hlrender *hlrnd, char *name,
		    Resource *res)
{
	FILE *file;
	char cControl[20];
	...
	complex_struct->angx = 0;
	complex_struct->angy = 0;
	complex_struct->angz = 0;
	complex_struct->x = 0;
	complex_struct->y = 0;
	complex_struct->z = 0;
	...
	if (strcmp(cControl, "O3D")) {
		ERROR = (char *)ERR9;
		exit(1);
	};
	fread(&sWork, 1, 2, file);		      // Versión
	fread(&complex_struct->nObjects, 1, 2, file); // Numero de Objetos
	if (complex_struct->nObjects > MAXOBJECTS) {
		ERROR = (char *)ERR10;
		exit(1);
	};
	for (i = 0; i < complex_struct->nObjects; i++)
		if ((complex_struct->Objects[i] =
			 (lptobject)lf_malloc(sizeof(tobject))) == NULL)
			return;
	fread(&complex_struct->nMaterials, 1, 2, file); // Numero de materiales
	if ((complex_struct->Materials = (lptmaterial)lf_malloc(
		 sizeof(tmaterial) * complex_struct->nMaterials)) == NULL)
		return;
	fread(&complex_struct->nDummies, 1, 2, file); // Numero de Dummies
	if (complex_struct->nDummies) {
		if ((complex_struct->Dummies = (lptbbox)lf_malloc(
			 sizeof(tbbox) * complex_struct->nDummies)) == NULL)
			return;
	} else {
	...
	if (complex_struct->nTapes) {
		if ((complex_struct->Tapes = (lpttape)lf_malloc(
			 sizeof(ttape) * complex_struct->nTapes)) == NULL)
			return;
	} else {
	...
	if ((complex_struct->Anims = (lptmatrix)lf_malloc(
		 sizeof(tmatrix) * complex_struct->nObjects *
		 complex_struct->nFrames)) == NULL)
		return;
	...
	for (i = 0; i < complex_struct->nObjects; i++) {
		...
		fread(&complex_struct->Objects[i]->nVertex, 1, 2,
		      file); // Numero de vertices
		if ((complex_struct->Objects[i]->Vertices =
			 (lptvertex)lf_malloc(
			     sizeof(tvertex) *
			     complex_struct->Objects[i]->nVertex)) == NULL)
			return;

		fread(&complex_struct->Objects[i]->nFaces, 1, 2,
		      file); // Numero de caras
		if (complex_struct->Objects[i]->nFaces)
			if ((complex_struct->Objects[i]->Faces =
				 (lptface)lf_malloc(
				     sizeof(tface) *
				     complex_struct->Objects[i]->nFaces)) ==
			    NULL)
				return;

		fread(&matindex, 1, 2, file); // Material usado.
		fread(complex_struct->Objects[i]->name, 1, 20,
		      file); // Nombre del objeto

		printf("name: %s\n", complex_struct->Objects[i]->name);

		fread(&complex_struct->Objects[i]->Bbox,
		      sizeof(complex_struct->Objects[i]->Bbox), 1, file);

		complex_struct->Objects[i]->Centro.rx =
		    (complex_struct->Objects[i]->Bbox.x1 +
		     complex_struct->Objects[i]->Bbox.x2) /
		    2;
		complex_struct->Objects[i]->Centro.ry =
		    (complex_struct->Objects[i]->Bbox.y1 +
		     complex_struct->Objects[i]->Bbox.y2) /
		    2;
		complex_struct->Objects[i]->Centro.rz =
		    (complex_struct->Objects[i]->Bbox.z1 +
		     complex_struct->Objects[i]->Bbox.z2) /
		    2;
		...
		for (j = 0; j < complex_struct->Objects[i]->nFaces; j++) {
			...
			complex_struct->Objects[i]->Faces[j].v1 =
			    &complex_struct->Objects[i]->Vertices[wfac.a];
			complex_struct->Objects[i]->Faces[j].v2 =
			    &complex_struct->Objects[i]->Vertices[wfac.b];
			complex_struct->Objects[i]->Faces[j].v3 =
			    &complex_struct->Objects[i]->Vertices[wfac.c];
			complex_struct->Objects[i]->Faces[j].material =
			    &complex_struct->Materials[wfac.matidx];
		}

		printf("name: %s\n", complex_struct->Objects[i]->name);

		if ((complex_struct->Objects[i]->matrix =
			 (lptmatrix)lf_malloc(sizeof(tmatrix))) == NULL)
			return;

		printf("name: %s\n", complex_struct->Objects[i]->name);

		hlrender_addobject(complex_struct->hl_render,
				   complex_struct->Objects[i]);
	}
	...
	for (i = 0; i < complex_struct->nMaterials; i++) {
		...
		if (strcmp(wmat.textura, "NULL")) {
			...
			complex_struct->Materials[i].textura =
			    sprite3d_CreateTexture(
				complex_struct->hl_render->ccamera, res,
				wmat.textura);
			if (wmat.transp > 0)
				complex_struct->Materials[i].tipo_map =
				    TRANS_PER;
			else if (wmat.map_type == TMETAL)
				complex_struct->Materials[i].tipo_map =
				    TEXT_PER;
			else
				complex_struct->Materials[i].tipo_map = TEXTURA;
		} else {
			complex_struct->Materials[i].textura->buffer = NULL;
			if (wmat.map_type == TFLAT)
				complex_struct->Materials[i].tipo_map = FLAT;
			else if (wmat.map_type == TPHONG)
				complex_struct->Materials[i].tipo_map = GOURAUD;
			else if (wmat.map_type == TMETAL)
				complex_struct->Materials[i].tipo_map =
				    FLAT_TRANS;
			else
	...
	for (i = 0; i < complex_struct->nObjects; i++)
		for (j = 0; j < complex_struct->Objects[i]->nFaces; j++)
			complex_struct->Objects[i]->Faces[j].tipo_map =
			    complex_struct->Objects[i]
				->Faces[j]
				.material->tipo_map;
	fread(complex_struct->Dummies, sizeof(tbbox), complex_struct->nDummies,
	      file);
	fread(complex_struct->Tapes, sizeof(ttape), complex_struct->nTapes,
	      file);

#ifdef TAPES_ACTIVAS
	for (i = 0; i < complex_struct->nTapes; i++) {
		for (j = 0;
		     j < complex_struct->Objects[complex_struct->Tapes[i].Obj1]
			     ->nFaces;
		     j++) {
			if (complex_struct
				->Objects[complex_struct->Tapes[i].Obj1]
				->Faces[j]
				.v1 ==
			    &complex_struct
				 ->Objects[complex_struct->Tapes[i].Obj1]
				 ->Vertices[complex_struct->Tapes[i].Vert1])
				complex_struct
				    ->Objects[complex_struct->Tapes[i].Obj1]
				    ->Faces[j]
				    .v1 =
				    &complex_struct
					 ->Objects[complex_struct->Tapes[i]
						       .Obj2]
					 ->Vertices[complex_struct->Tapes[i]
							.Vert2];
	...
	fread(complex_struct->Anims, sizeof(tmatrix),
	      complex_struct->nFrames * complex_struct->nObjects, file);
	fclose(file);
...
void complex_destroy(complex *complex_struct)
{
	...
	if (complex_struct->Clon) {
		for (i = 0; i < complex_struct->nObjects; i++) {
			...
			hlrender_deleteobject(complex_struct->hl_render,
					      complex_struct->Objects[i]);
		}
		return;
	} else {
		...
		for (i = 0; i < complex_struct->nMaterials; i++)
			sprite3d_DestroyTexture(
			    complex_struct->hl_render->ccamera,
			    complex_struct->Materials[i].textura);
...
void complex_update(complex *complex_struct)
{
	...
	if (complex_struct->curframe >= complex_struct->nFrames)
		complex_struct->curframe = 1;

	for (i = 0; i < complex_struct->nObjects; i++) {
		MatMult(complex_struct->Objects[i]->matrix,
			&complex_struct->Anims[complex_struct->curframe *
						   complex_struct->nObjects +
					       i],
			&complex_struct->CompMat);
	}
	//  if (++complex_struct->curframe==complex_struct->nFrames)
	//  complex_struct->curframe=0;
	memcpy(complex_struct->wobjects, complex_struct->Objects,
	       sizeof(complex_struct->Objects));
}
...
void complex_sort(complex *complex_struct)
{
	...
	for (j = 0; j < complex_struct->nObjects; j++, iWork--) {
		for (i = 0; i < iWork; i++)
			if (complex_struct->wobjects[i]->z <
			    complex_struct->wobjects[i + 1]->z) {
				wobj = complex_struct->wobjects[i];
				complex_struct->wobjects[i] =
				    complex_struct->wobjects[i + 1];
...
void complex_draw(complex *complex_struct)
{
	...
	complex_x_ini = complex_struct->hl_render->ccamera->ancho;
	complex_x_fin = 0;
	complex_y_ini = complex_struct->hl_render->ccamera->alto;
	complex_y_fin = 0;
	for (i = 0; i < complex_struct->nObjects; i++)
		hlrender_drawobject(complex_struct->hl_render,
				    complex_struct->wobjects[i]);

	complex_struct->bbox_x_ini = complex_x_ini;
	complex_struct->bbox_x_fin = complex_x_fin;
	complex_struct->bbox_y_ini = complex_y_ini;
	complex_struct->bbox_y_fin = complex_y_fin;
}
...
void complex_setrotation(complex *complex_struct, float cangx, float cangy,
			 float cangz)
{
...
void complex_transform(complex *complex_struct)
{
	...
	MatMult(&temp, &mwork,
		&complex_struct
		     ->CompMat); // OJO esta al reves por los sprites
				 //////////////////////////////////////////////
	mwork = Identity;
	mwork._00 = cz;
	mwork._01 = sz;
...
void complex_makefacenormals(complex *complex_struct)
{
	...
	for (i = 0; i < complex_struct->nObjects; i++)
		for (j = 0; j < complex_struct->Objects[i]->nFaces; j++) {
			ax = (complex_struct->Objects[i]->Faces[j].v2->rx -
			      complex_struct->Objects[i]->Faces[j].v1->rx);
			ay = (complex_struct->Objects[i]->Faces[j].v2->ry -
			      complex_struct->Objects[i]->Faces[j].v1->ry);
			az = (complex_struct->Objects[i]->Faces[j].v2->rz -
			      complex_struct->Objects[i]->Faces[j].v1->rz);

			bx = (complex_struct->Objects[i]->Faces[j].v3->rx -
			      complex_struct->Objects[i]->Faces[j].v1->rx);
			by = (complex_struct->Objects[i]->Faces[j].v3->ry -
			      complex_struct->Objects[i]->Faces[j].v1->ry);
			bz = (complex_struct->Objects[i]->Faces[j].v3->rz -
			      complex_struct->Objects[i]->Faces[j].v1->rz);

			complex_struct->Objects[i]->Faces[j].rnx =
			    ay * bz - az * by;
			complex_struct->Objects[i]->Faces[j].rny =
			    az * bx - ax * bz;
			complex_struct->Objects[i]->Faces[j].rnz =
			    ax * by - ay * bx;

			normalize(&complex_struct->Objects[i]->Faces[j].rnx,
				  &complex_struct->Objects[i]->Faces[j].rny,
				  &complex_struct->Objects[i]->Faces[j].rnz);
		}
}
...
{
	float d, dx = complex_struct->x - nx, dy = complex_struct->y - ny,
		 dz = complex_struct->z - nz;
