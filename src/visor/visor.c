...
void lfset(void *buf, long c, long d) {
	printf("Should fill buffer [%x] with c [%d] for d [%d]
bytes\n",buf,c,d); long f=0;
...
buffer_sprite3d *visor_init(lptvisor datos_render)
{
	...
	sprite3d_create(&camara, my_buffer->buffer, my_buffer->ancho,
			my_buffer->alto);
	if (num_mallocs == 0)
		return (NULL);
	...
	animated_create(&animado, &fichero_animaciones, &world_camara,
			datos_render->nombre, &resource_camara);
...
void visor_loop(lptvisor datos_render)
{
	...
	complex_setrotation(&animado.complex_struct, datos_render->beta,
			    datos_render->alfa, 0);
	...
	for (i = 0; i < animado.complex_struct.nObjects; i++) {
		...
		if (!stricmp(animado.complex_struct.Objects[i]->name,
			     "control") ||
		    i == animado.complex_struct.nObjects - 1) {
			sbuffer->punto_x =
			    (int)animado.complex_struct.Objects[i]->Centro.fpx -
			    animado.complex_struct.bbox_x_ini;
			sbuffer->punto_y =
			    (int)animado.complex_struct.Objects[i]->Centro.fpy -
			    animado.complex_struct.bbox_y_ini;
			break;
		}
	}
...
{
	int i, j;
	int ancho = (animado.complex_struct.bbox_x_fin -
		     animado.complex_struct.bbox_x_ini + 1) *
		    2;
	int alto = animado.complex_struct.bbox_y_fin -
		   animado.complex_struct.bbox_y_ini + 1;
	...
	j = 0;
	for (i = animado.complex_struct.bbox_y_ini;
	     i <= animado.complex_struct.bbox_y_fin; i++) {
		memcpy(sbuffer->buffer + j * ancho,
		       wbuffer->buffer + i * wbuffer->ancho * 2 +
			   animado.complex_struct.bbox_x_ini * 2,
		       ancho);
		j++;
	}
