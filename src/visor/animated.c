...
void animated_create(animated *animated_struct, fileanim *f_anm,
		     hlrender *hlrnd, char *name, Resource *res)
{
	complex_create(&animated_struct->complex_struct, hlrnd, name, res);

	printf("name: %s\n", animated_struct->complex_struct.Objects[0]->name);

	if (animated_struct->complex_struct.nObjects !=
	    f_anm->header.nObjects) {
		ERROR = (char *)ERR16;
		exit(1);
	};
...
{
	animated_struct->complex_struct.Anims =
	    fileanim_getanim(animated_struct->fanm, order,
			     &animated_struct->complex_struct.nFrames);
