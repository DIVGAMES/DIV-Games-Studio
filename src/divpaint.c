...
void bezier(int x0, int y0, int x1, int y1, int _x0, int _y0, int _x1, int _y1,
	    int inc0);
...
void vuelca_barra_oscurecida(byte *p, int real_an, int x, int y, int an,
			     int al);
...
byte pincel_por_defecto[16] = {16, 16, 16, 16, 16, 16, 16, 16,
...
void test_mouse_box(int a, int b, int c, int d)
{
	if (mouse_in(a, b, a + c * big2 - 1, b + d * big2 - 1)) {
		...
		if (mouse_in(a + 2, b + 2, a + c - 10, b + 9))
			...
			mouse_graf = 1;
	} else if (mouse_in(barra_x, barra_y, barra_x + barra_an * big2 - 1,
			    barra_y + 19 * big2 - 1)) {
		if (big && !mouse_shift) {
			mouse_shift = 1;
		...
		if (mouse_in(barra_x, barra_y, barra_x + 9, barra_y + 9))
			mouse_graf = 2;
		else if (mouse_in(barra_x, barra_y + 10, barra_x + 9,
				  barra_y + 18))
			mouse_graf = 5;
		else
			mouse_graf = 1;
...
void test_mouse_box2(int a, int b, int c, int d)
{
	if (mouse_in(a, b, a + c * big2 - 1, b + d * big2 - 1)) {
		...
		if (mouse_in(barra_x, barra_y, barra_x + 9, barra_y + 9))
			...
			mouse_graf = 1;
	} else if (mouse_in(zx, zy, zx + zan - 1, zy + zal - 1))
		mouse_graf = actual_mouse - zoom;
	else
		mouse_graf = 1;
}
...
{
	if (mouse_in(barra_x, barra_y, barra_x + barra_an * big2 - 1,
		     barra_y + 19 * big2 - 1)) {
		if (big && !mouse_shift) {
			mouse_shift = 1;
			mouse_x = barra_x + (mouse_shift_x - barra_x) / 2;
...
void edit_scr(void)
{
	...
	do {
		...
		volcado_edicion();
		 ...
		 !(mouse_b &&
		   mouse_in(barra_x, barra_y + 10, barra_x + 9, barra_y + 18)));
}
	...
	do {
		...
		if (((mouse_b & 1) && !(old_mouse_b & 1)) && mouse_graf >= 10) {
			if (!mask[*(map + coord_x + coord_y * map_an)])
				if (*(map + coord_x + coord_y * map_an) !=
				    color) {
					save_undo(coord_x, coord_y, 1, 1);
					*(map + coord_x + coord_y * map_an) =
					    color;
				} else {
					save_undo(coord_x, coord_y, 1, 1);
...
void edit_mode_1(void)
{
	...
	do {
		...
		a = -1; // Ultima acción realizada (en esta iteración) - sin
			// definir
		...
		if ((mouse_b & 1) && mouse_graf >= 10) {
			...
			if (_b == 0 || coord_x != _x || coord_y != _y) {
				...
				if (_a == 1)
					...
					line(coord_x, coord_y, coord_x, coord_y,
					     0);
			}
...
void edit_mode_2(void)
{
	...
	do {
		...
		case 1:
			if (mouse_graf >= 10) {
				...
				if (undo_error)
					...
					an = (x0 > coord_x) ? x0 - coord_x
							    : coord_x - x0;
					al = (y0 > coord_y) ? y0 - coord_y
							    : coord_y - y0;
					if (mouse_b & 1) {
						s = 0;
						do {
		...
		if (back) {
			...
			if (hacer_zoom == 1)
				zoom_map();
			else if (hacer_zoom == -1 && hacer_zoom_an > 0 &&
				 hacer_zoom_al > 0)
				fondo_edicion(hacer_zoom_x, hacer_zoom_y,
					      hacer_zoom_an, hacer_zoom_al);
			hacer_zoom_an = 0;
			hacer_zoom_al = 0;
			hacer_zoom = 0;
		}
...
void edit_mode_3(void)
{
	...
	int s; // Estado 0-espera 1er click, 1-espera 2do click, 2-espera 3er
	       // click
	int x0, y0; // Coordenadas iniciales
	int an, al;
	...
	do {
		...
		case 2:
			if (mouse_graf >= 10) {
				line_fx = efecto4;
				undo_error = 0;
				bezier(x0, y0, x1, y1, coord_x, coord_y,
				       coord_x, coord_y, 1);
				if (undo_error)
					s = 0;
				else {
		...
		case 3:
			if (mouse_graf >= 10) {
				line_fx = efecto4;
				undo_error = 0;
				bezier(x0, y0, x1, y1, _x0, _y0, coord_x,
				       coord_y, 1);
				if (undo_error)
					s = 0;
				else {
...
void edit_mode_5(void)
{
	...
	do {
		...
		case 2:
			if (mouse_graf >= 10) {
				y0 =
				    sqrt(abs(coord_x - x1) * abs(coord_x - x1) +
					 abs(coord_y - y1) * abs(coord_y - y1));

				// OJO, esto se puede poner a gusto del
				// consumidor
				...
				line_fx = efecto5;
				undo_error = 0;
				bezier(x1, y1, coord_x, coord_y, x0, y0,
				       coord_x, coord_y, 0);
				if (undo_error)
					s = 0;
				else {
...
void select_caja(int n)
{
	...
	if (r >= 0) {
		modo_caja = r;
		bar[13] = iconos_caja[2 + modo_caja];
		dibuja_barra(22);
		dibuja_regla();
	}
}
		...
		case 1:
			if (mouse_graf >= 10) {
				...
				if (key(_L_CTRL) || key(_R_CTRL)) {
					...
					undo_error = 0;
					caja(x0, y0, x0 + an, y0 + al);
					if (undo_error)
						s = 0;
				} else {
					undo_error = 0;
					caja(x0, y0, coord_x, coord_y);
					if (undo_error)
						s = 0;
				}
				if (mouse_b & 1) {
...
void select_circulo(int n)
{
	...
	if (r >= 0) {
		modo_circulo = r;
		bar[13] = iconos_circulo[2 + modo_circulo];
		dibuja_barra(22);
		dibuja_regla();
	}
}
...
void zoom_porcion(int x, int y, int an, int al)
{
	...
	if (x < zoom_x)
		hacer_zoom_x = zx - ((zoom_x - x) << zoom);
	else
		hacer_zoom_x = zx + ((x - zoom_x) << zoom);
	if (y < zoom_y)
		hacer_zoom_y = zy + ((y - zoom_y) << zoom);
	else
		hacer_zoom_y = zy + ((y - zoom_y) << zoom);
	hacer_zoom_an = an << zoom;
	hacer_zoom_al = al << zoom;

	if (hacer_zoom_x + hacer_zoom_an <= zx ||
	    hacer_zoom_y + hacer_zoom_al <= zy || hacer_zoom_x >= zx + zan ||
	    hacer_zoom_y >= zy + zal) {
		hacer_zoom_an = 0;
		hacer_zoom_al = 0;
	} else {
		if (hacer_zoom_x < zx) {
			hacer_zoom_an -= zx - hacer_zoom_x;
			hacer_zoom_x = zx;
		}
		if (hacer_zoom_y < zy) {
			hacer_zoom_al -= zy - hacer_zoom_y;
			hacer_zoom_y = zy;
		}
		if (hacer_zoom_x + hacer_zoom_an > zx + zan)
			hacer_zoom_an = zx + zan - hacer_zoom_x;
		if (hacer_zoom_y + hacer_zoom_al > zy + zal)
			hacer_zoom_al = zy + zal - hacer_zoom_y;
		if (!hacer_zoom)
			hacer_zoom = -1;
	}
}
	...
	do {
		...
		if ((mouse_b & 1) && mouse_graf >= 10) {
			...
			if (clock_spray != -1) {
				...
				if (u == undo_spray ||
				    abs(coord_x - ux) > undo_box ||
				    abs(coord_y - uy) > undo_box) {
					u = 0;
					ux = coord_x;
					uy = coord_y;
					save_undo(
					    coord_x - pincel_an / 2 - undo_box,
					    coord_y - pincel_al / 2 - undo_box,
					    pincel_an + undo_box * 2 + 1,
					    pincel_al + undo_box * 2 + 1);
				} else {
					zoom_porcion(coord_x - pincel_an / 2,
						     coord_y - pincel_al / 2,
						     pincel_an * 2 + 1,
						     pincel_al * 2 + 1);
					u++;
...
void edit_mode_9(void)
{
	...
	for (a = 2; a < 11; a++)
		bar[a] = 1;
	bar[11] = 166;
	bar[12] = iconos_fill[2 + modo_fill];
	bar[13] = 0;
	dibuja_barra(0);
	dibuja_regla();
	...
	do {
		...
		if ((mouse_b & 1) && !(old_mouse_b & 1) && mouse_graf >= 10) {
			if (!mask[*(map + coord_y * map_an + coord_x)] &&
			    (*(map + coord_y * map_an + coord_x) != color ||
			     modo_fill == 3 || textura_color != NULL)) {
...
void barra_bloque(int n)
{
	...
	case 0:
		...
		bar[14] = 0;
		if (modo_seleccion < 4)
			bar[12] = 135 + modo_seleccion;
		else
			bar[12] = modo_seleccion + 169 - 4;
		dibuja_barra(22);
		dibuja_regla();
		break;
	case 1:
...
void edit_mode_10(void)
{
	...
	do {
		...
		case 3:
			...
			//        if ((mouse_b&1) && selected_icon==12) { // ***
			//        Delete, borra selección
			//          box2mab(); mab_delete(); do {read_mouse();}
			//          while(mouse_b&1);
			//        }

			if ((mouse_b & 1) &&
			    selected_icon == 12) { // *** Mover, Copiar, ...
				do {
					read_mouse();
				} while (mouse_b & 1);
				sel_status = 0;
			...
			if (((mouse_b & 1) && selected_icon == 14) ||
			    key(_K)) { // *** Cortar ventana
				s = 0;
				sel_status = 0;
				barra_bloque(0);
				hacer_zoom = 1;
		...
		if ((key(_L_CTRL) || key(_R_CTRL)) && (mouse_b & 1) &&
		    mouse_graf >= 10 && sel_status && s >= 2) {
			if (modo_seleccion == 0 || modo_seleccion == 4 ||
			    (modo_seleccion == 3 && s < 2)) {
				...
				if (y < 0)
					y = 0;
				if (x + sel_x1 - sel_x0 >= map_an)
					x = map_an - (sel_x1 - sel_x0) - 1;
				if (y + sel_y1 - sel_y0 >= map_al)
					y = map_al - (sel_y1 - sel_y0) - 1;
				sel_x1 -= sel_x0;
				sel_x0 = x;
				sel_x1 += x;
				sel_y1 -= sel_y0;
				...
				if (y < 0)
					y = 0;
				if (x + mab_x1 - mab_x0 >= map_an)
					x = map_an - (mab_x1 - mab_x0) - 1;
				if (y + mab_y1 - mab_y0 >= map_al)
					y = map_al - (mab_y1 - mab_y0) - 1;
				if ((m = (int *)malloc(
					 ((map_an * map_al + 31) / 32) * 4)) !=
				    NULL) {
					memcpy(m, mab,
					       ((map_an * map_al + 31) / 32) *
						   4);
		...
		volcado_edicion();
	} while (!(mouse_b & 2) && !(key(_ESC) && s != 1 && s != 2) &&
		 modo < 100 &&
		 !(mouse_b &&
		   mouse_in(barra_x, barra_y + 10, barra_x + 9, barra_y + 18)));
...
void test_sel(void)
{
	if (hacer_zoom_x > sel_x0)
		hacer_zoom_x = sel_x0;
	if (hacer_zoom_x > sel_x1)
		hacer_zoom_x = sel_x1;
	if (hacer_zoom_y > sel_y0)
		hacer_zoom_y = sel_y0;
	if (hacer_zoom_y > sel_y1)
		hacer_zoom_y = sel_y1;
	if (hacer_zoom_an < sel_x0)
		hacer_zoom_an = sel_x0;
	if (hacer_zoom_an < sel_x1)
		hacer_zoom_an = sel_x1;
	if (hacer_zoom_al < sel_y0)
		hacer_zoom_al = sel_y0;
	if (hacer_zoom_al < sel_y1)
		hacer_zoom_al = sel_y1;
}

void test_mab(void)
{
	if (hacer_zoom_x > mab_x0)
		hacer_zoom_x = mab_x0;
	if (hacer_zoom_x > mab_x1)
		hacer_zoom_x = mab_x1;
	if (hacer_zoom_y > mab_y0)
		hacer_zoom_y = mab_y0;
	if (hacer_zoom_y > mab_y1)
		hacer_zoom_y = mab_y1;
	if (hacer_zoom_an < mab_x0)
		hacer_zoom_an = mab_x0;
	if (hacer_zoom_an < mab_x1)
		hacer_zoom_an = mab_x1;
	if (hacer_zoom_al < mab_y0)
		hacer_zoom_al = mab_y0;
	if (hacer_zoom_al < mab_y1)
		hacer_zoom_al = mab_y1;
}
...
void corta_mapa(void)
{
	...
	if ((v_mapa = (struct tmapa *)malloc(sizeof(struct tmapa))) != NULL) {
		...
		if ((v_mapa->map = (byte *)malloc(an * al)) != NULL) {
			...
			ltoa(siguiente_codigo++,
			     v_mapa->filename + strlen(v_mapa->filename), 10);
			*v_mapa->path = '\0';
			v_mapa->map_an = an;
			v_mapa->map_al = al;
			for (x = 0; x < 512; x += 2) {
				if (v.mapa->puntos[x] >= mab_x0 &&
				    v.mapa->puntos[x] <= mab_x1 &&
				    v.mapa->puntos[x + 1] >= mab_y0 &&
				    v.mapa->puntos[x + 1] <= mab_y1) {
					v_mapa->puntos[x] =
					    v.mapa->puntos[x] - mab_x0;
					v_mapa->puntos[x + 1] =
					    v.mapa->puntos[x + 1] - mab_y0;
				} else {
					v_mapa->puntos[x] = -1;
			...
			v_mapa->TengoNombre =
			    0; // No tiene descripcion por defecto
			v_mapa->Codigo = 0;
...
void mab_delete(void)
{
	...
	if (save_undo(mab_x0, mab_y0, mab_x1 - mab_x0 + 1,
		      mab_y1 - mab_y0 + 1)) {
		if (textura_color == NULL) {
			for (y = mab_y0; y <= mab_y1; y++)
				for (x = mab_x0; x <= mab_x1; x++)
					if (is_mab(x, y))
						*(map + x + y * map_an) = color;
		} else {
			for (y = mab_y0; y <= mab_y1; y++)
				for (x = mab_x0; x <= mab_x1; x++)
					if (is_mab(x, y))
...
void mab_invertir(void)
{
	...
	if (save_undo(mab_x0, mab_y0, mab_x1 - mab_x0 + 1,
		...
		create_dac4();
		for (x = 0; x < 256; x++) {
			p[x] = find_color(63 - dac[x * 3], 63 - dac[x * 3 + 1],
					  63 - dac[x * 3 + 2]);
		}
		for (y = mab_y0; y <= mab_y1; y++)
			for (x = mab_x0; x <= mab_x1; x++)
	...
	case 2:
		if ((mouse_b & 1) && mouse_graf >= 10) {
			...
			if (*(map + coord_y * map_an + coord_x)) {
				...
				if (s == 0 ||
				    (coord_x < sel_x0 || coord_x > sel_x1 ||
				     coord_y < sel_y0 || coord_y > sel_y1)) {
					x0 = coord_x;
					x1 = coord_x;
					y0 = coord_y;
					y1 = coord_y;
...
void find_first_x_from(int xi, int y)
{
	...
	for (n = 0; n < n_puntos - 1; n++) {
		...
		if (y0 <= y && y1 >= y && y0 != y1) {
			x = (float)x0 +
			    (float)((x1 - x0) * (y - y0)) / (float)(y1 - y0);
			if (x > xi && x < xmin)
				xmin = x;
		}
	}

	// Se ha obtenido xmin, ahora se ha de contabilizar k1 y k2 actualizando
	// t
...
int edit_mode_6_lines(int s)
{
	...
	case 0:
		if ((mouse_b & 1) && mouse_graf >= 10) {
			...
			mab_x0 = mab_x1 = coord_x;
			mab_y0 = mab_y1 = coord_y;
	...
	case 1:
		...
		if ((mouse_b & 1) && mouse_graf >= 10 && n_puntos < 1024 &&
			...
			if (coord_x < mab_x0)
				mab_x0 = coord_x;
			else if (coord_x > mab_x1)
				mab_x1 = coord_x;
			if (coord_y < mab_y0)
				mab_y0 = coord_y;
			else if (coord_y > mab_y1)
				mab_y1 = coord_y;
			mab_line(oldx, oldy, coord_x, coord_y);
			oldx = coord_x;
			oldy = coord_y;
			if (n_puntos > 3 && oldx >= poligono[0] - 1 &&
			    oldx <= poligono[0] + 1 &&
			    oldy >= poligono[1] - 1 &&
			    oldy <= poligono[1] + 1) {
				if (oldx != poligono[0] ||
	...
	case 0:
		if ((mouse_b & 1) && mouse_graf >= 10) {
			if (modo_seleccion == 1) {
				...
				fill_dac[*(map + coord_y * map_an + coord_x)] =
				    1;
			} else {
...
int ShowCharBuffer(int WhatChar, int cx, int cy, char *ptr, int an,
		   char *buffer);
...
void edit_mode_12(void)
{
	...
	if (x = determina_fnt()) {
		...
		font = NULL; // Pintará con el font del editor
	...
	do {
		...
		if (font == NULL) {
			...
			if (mouse_graf >= 10 && (mouse_b & 1) && !key(_SPC)) {
				sel_x0 = coord_x - font_an / 2;
				sel_y0 = coord_y - font_al / 2;
				sel_x1 = sel_x0 + font_an - 1;
				sel_y1 = sel_y0 + font_al - 1;
				sel_status = 1;
				hotkey = 0;
				tx = sel_x0;
				ty = sel_y0;
			...
			if (sel_status == 1) {
				...
				} else if (ascii && scan_code != 15 &&
					   scan_code != 1 && ilon < 256) {
					if (save_undo(sel_x0, sel_y0, font_an,
						      font_al)) {
						test_anterior(); // Para
								 // contrarrestar
								 // el efecto
			...
			if (sel_status == 1) {
				if (ascii == 13) {
					...
					if (sel_x0 != tx) {
					   ...
					   scan_code != 1 && ilon < 256) {
					GetCharSizeBuffer(ascii, &fan, &fal,
							  (char *)font);
					if (fan == 1) {
						if (save_undo(sel_x0, sel_y1, 1,
							      1)) {
							test_anterior();
							sel_x0 += spacelen;
							sel_x1 += spacelen;
		...
		if (((mouse_b & 1) && selected_icon == 1) ||
		    ((scan_code == 14 && hotkey) && !key(_L_SHIFT) &&
		     !key(_R_SHIFT))) {
			if (sel_x0 != tx) {
				if (undo_back()) {
					if (font == NULL) {
...
void edit_mode_13(void)
{
	...
	do {
		...
		if (v.mapa->puntos[num_punto * 2] != -1)
			anal_barra(v.mapa->puntos[num_punto * 2],
				   v.mapa->puntos[num_punto * 2 + 1]);
		else {
			wbox(barra, vga_an / big2, vga_al, c2, barra_an - 23, 2,
			     21, 15);
			p = copia;
			copia = barra;
			text_color = c4;
		...
		wwrite(barra, vga_an / big2, 19, 56 + 2 * 16, 6, 1, (byte *)num,
		       c1);
		wwrite(barra, vga_an / big2, 19, 55 + 2 * 16, 6, 1, (byte *)num,
		       c4);
	    ...
	    int inc0)
{
	...
	if (save_undo(x - pincel_an / 2, y - pincel_al / 2,
		      _x - x + 1 + pincel_an, _y - y + 1 + pincel_al)) {
		if (inc0)
			line_pixel(p[0], p[1]);
		n = 1;
		do {
...
void line0(int x0, int y0, int x1, int y1, int inc0)
{
	...
	if (inc0 == -1)
		...
		unded =
		    (int64_t)save_undo(x - pincel_an / 2, y - pincel_al / 2,
				       dx + 1 + pincel_an, dy + 1 + pincel_al);
...
{ // Brush
	...
	if (textura_color == NULL) {
		if (tipo_pincel) {
			if (x >= 0 && y >= 0 && x + pincel_an <= map_an &&
			    y + pincel_al <= map_al && !difuminar) {
				q = map + x + y * map_an;
				p = pincel;
				if (mask_on) {
					do {
...
void _line_pixel(int x, int y)
{ // Un simple pixel (para line_pixel,spray y writes)
	...
	if (x >= 0 && y >= 0 && x < map_an && y < map_al) {
		if (difuminar) {
			if (x < map_an - 1 && x > 0)
				c1 = *(ghost +
				       *(map + x + y * map_an - 1) * 256 +
				       *(map + x + y * map_an + 1));
			else
				c1 = *(map + x + y * map_an);
			if (y < map_al - 1 && y > 0)
				c2 = *(ghost +
				       *(map + x + (y - 1) * map_an) * 256 +
				       *(map + x + (y + 1) * map_an));
			else
				c2 = *(map + x + y * map_an);
			_color = color;
			color = *(ghost + (memptrsize)c1 * 256 + c2);
			pixel(map + x + y * map_an);
			color = _color;
		} else
			pixel(map + x + y * map_an);
	}
...
void caja(int x0, int y0, int x1, int y1)
{
	...
	if (save_undo(x0 - pincel_an / 2, y0 - pincel_al / 2, an + pincel_an,
		      al + pincel_al)) {
		if (modo_caja) {
			do {
...
void regla_edicion(void)
{
	...
	select_zoom();
	test_mouse();
	if ((key(_L_SHIFT) || key(_R_SHIFT)) && (mouse_b & 1) &&
	    mouse_graf >= 10) {
	...
	if ((mouse_b & 1) && mouse_in(barra_x + _ir, barra_y,
				      barra_x + _ir + 127, barra_y + 18)) {
		if (editable(&n))
			reglas[regla].col[n] = color;
		else
			color = reglas[regla].col[n];
		quitar_textura();
		dibuja_regla();
	}
...
void mover(byte *sp, int an, int al)
{
	...
	int s = 0; // Status 0-Normal, 1-Rotando_a, 2-Rotando_b, 3-Escalando_a,
		   // 4-...
	...
	do {
		...
		if (s == 1 || s == 2) {
			...
			if (xg < 0)
				xg += 360;
			wbox(barra, vga_an / big2, 19, c2, barra_an - 23, 2, 21,
			     15);
			p = copia;
			copia = barra;
			text_color = c3;
		...
		if (s == 3 || s == 4) {
			...
			num[1] = (xg / 10) % 10 + 48;
			num[0] = (xg / 100) % 10 + 48;
			writetxt(barra_an - 22, 6, 0, num);
			copia = p;
		}

		if (mouse_in(barra_x + 48, barra_y + 2, barra_x + barra_an - 2,
			     barra_y + 17)) {
			selected_icon = (mouse_x - barra_x - 48) / 16;
			if (bar[selected_icon] > 1) {
				wresalta_box(barra, vga_an / big2, 19,
					     48 + selected_icon * 16, 2, 15,
					     15);
			} else
				selected_icon = -1;
		} else
		...
		if ((_mouse_graf = mouse_graf) >= 10) {
			if (_coord_x >= 0 && _coord_y >= 0 &&
			    _coord_x < map_an && _coord_y < map_al) {
				xg = an / 2;
				yg = al / 2;
				if (size == 1 && ang == 0) {
					if (save_undo(_coord_x - xg,
						      _coord_y - yg, an, al))
						sp_normal(sp, _coord_x,
							  _coord_y, an, al, xg,
							  yg,
							  block * 2 + ghost);
					else
					...
					_x = _coord_x;
					_y = _coord_y;
					sp_size(&_x, &_y, &_an, &_al, xg, yg,
						ang, size);
					if (save_undo(_x, _y, _an, _al))
						sp_rotado(sp, _coord_x,
							  _coord_y, an, al, xg,
							  yg, block * 2 + ghost,
							  ang, size);
					else
		...
		zoom_map();
		volcar_barras(0);
		salvaguarda(fondo_raton, mouse_shift_x, mouse_shift_y,
			    _mouse_graf, 0);
		...
		if (selected_icon >= 0)
			put_barra(48 + selected_icon * 16, 2,
				  bar[selected_icon]);
	...
	if (key(_ESC) || (mouse_b && mouse_in(barra_x, barra_y + 10,
					      barra_x + 9, barra_y + 18))) {
		put_barra(2, 10, 45);
		volcar_barras(0);
...
void cuentagotas(void)
{
	...
	if ((_barra = (byte *)malloc(vga_an * 19 * big2)) != NULL) {
		memcpy(_barra, barra, vga_an * 19 * big2);
		...
		do {
			...
			if (mouse_graf >= 10) {
				...
				_copia = copia;
				copia = barra;
				wbox(barra, vga_an / big2, 19, col, 64, 2, 15,
				     15);
				wbox(barra, vga_an / big2, 19, c2, 80, 2, 31,
				     15);
				text_color = c4;
				num[2] = col % 10 + 48;
				num[1] = (col / 10) % 10 + 48;
				num[0] = (col / 100) % 10 + 48;
				strcpy((char *)&num[3], "Dec");
				writetxt(109, 3, 2, num);
				num[1] = (col % 16 > 9) ? col % 16 + 55
							: col % 16 + 48;
				num[0] = (col / 16 > 9) ? col / 16 + 55
							: col / 16 + 48;
				strcpy((char *)&num[2], "Hex");
				writetxt(109, 10, 2, num);
				copia = _copia;
			} else {
				_copia = copia;
				copia = barra;
				wbox(barra, vga_an / big2, 19, c1, 64, 2, 15,
				     15);
				wbox(barra, vga_an / big2, 19, c2, 80, 2, 31,
				     15);
				copia = _copia;
			...
			volcado_edicion();
			 ...
			 !(mouse_b && mouse_in(barra_x, barra_y + 10,
					       barra_x + 9, barra_y + 18)));

		if (key(_ESC) ||
		    (mouse_b && mouse_in(barra_x, barra_y + 10, barra_x + 9,
					 barra_y + 18))) {
			put_barra(2, 10, 45);
			volcar_barras(0);
			put(mouse_x, mouse_y, mouse_graf);
		...
		memcpy(barra, _barra, vga_an * 19 * big2);
...
int editable_seleccion(int *n, int x, int y)
{
	...
	*n = (mouse_x - x) / r + 1;
	...
	if (!reglas[regla].fijo) {
		if (mouse_y >= y + 9 && mouse_y < y + 16 && mouse_x >= x &&
		    mouse_x < x + 128) {
			switch (reglas[regla].tipo) {
			case 0:
				if (*n == 1)
	...
	for (a = _ir; a < _ir + 128; a += s0) {
		...
		if (!reglas[regla].fijo)
			if (reglas[regla].tipo) {
				if (((a - _ir) / s0 + 1) % reglas[regla].tipo ==
				    0) {
					wbox(barra, vga_an / big2, 19, c0, a,
					     10, s0 - 1, 1);
					wbox(barra, vga_an / big2, 19, c2, a,
					     11, s0 - 1, 6);
					wput(barra, vga_an / big2, 19, a + s1,
					     11, 32);
					wbox(barra, vga_an / big2, 19, c0,
					     a - 1, 11, 1, 6);
					wbox(barra, vga_an / big2, 19, c0,
					     a + s0 - 1, 11, 1, 6);
				}
			} else if (a == _ir) {
				wbox(barra, vga_an / big2, 19, c0, a, 10,
				     s0 - 1, 1);
				wbox(barra, vga_an / big2, 19, c2, a, 11,
				     s0 - 1, 6);
				wput(barra, vga_an / big2, 19, a + s1, 11, 32);
				wbox(barra, vga_an / big2, 19, c0, a - 1, 11, 1,
				     6);
				wbox(barra, vga_an / big2, 19, c0, a + s0 - 1,
				     11, 1, 6);
			}
	}

	wbox(barra, vga_an / big2, 19, color, 80, 2, 7,
	     8); // Caja del color, invoca a select_color()
...
void dibuja_regla_seleccion(byte *p, int c, int d, int x, int y)
{
	...
	switch (reglas[regla].numcol) {
	case 8:
		wwrite(p, c, d, x + 128 + 23, y, 1, texto[310], c3);
		break;
	case 16:
		wwrite(p, c, d, x + 128 + 23, y, 1, texto[311], c3);
		break;
	case 32:
		wwrite(p, c, d, x + 128 + 23, y, 1, texto[312], c3);
		break;
	}
	...
	case 0:
		wwrite(p, c, d, x + 128 + 71, y, 1, texto[313], c3);
		break;
	case 1:
		wwrite(p, c, d, x + 128 + 71, y, 1, texto[314], c3);
		break;
	}
	...
	switch (reglas[regla].tipo) {
	case 0:
		wwrite(p, c, d, x + 128 + 48, y + 8, 1, texto[315], c3);
		break;
	case 1:
		wwrite(p, c, d, x + 128 + 48, y + 8, 1, texto[316], c3);
		break;
	case 2:
		wwrite(p, c, d, x + 128 + 48, y + 8, 1, texto[317], c3);
		break;
	case 4:
		wwrite(p, c, d, x + 128 + 48, y + 8, 1, texto[318], c3);
		break;
	case 8:
		wwrite(p, c, d, x + 128 + 48, y + 8, 1, texto[319], c3);
		break;
	}
		...
		if ((m = determina_help()) > 0) {
			...
			if (v.click_handler == help2 && old_prg != NULL) {
				for (m = 1; m < max_windows; m++) {
					if (ventana[m].click_handler ==
					    programa2) {
						if (ventana[m].prg == old_prg &&
						    ventana[m].primer_plano <
							2) {
							ventana[m].estado = 1;
							wgra(ventana[m].ptr,
							     ventana[m].an /
...
void volcado_edicion(void)
{
	...
	if (hacer_zoom == 1)
		...
		fondo_edicion(hacer_zoom_x, hacer_zoom_y, hacer_zoom_an,
			      hacer_zoom_al);
	hacer_zoom_an = 0;
	hacer_zoom_al = 0;
	hacer_zoom = 0;
	...
	if (selected_icon >= 0)
		put_barra(48 + selected_icon * 16, 2, bar[selected_icon]);
...
int iconos_mode[] = {14,  2,   177, 160, 161, 178, 162, 179,
		     163, 164, 180, 165, 131, 100, 191, 190};
...
void select_mode(void)
{
	...
	if (modo_seleccion == -1)
		iconos_mode[12] = 131;
	else if (modo_seleccion < 4)
		iconos_mode[12] = 131 + modo_seleccion;
	else
		iconos_mode[12] = modo_seleccion + 167 - 4;
...
void select_fx(int n, int *efecto)
{
	...
	if (mouse_in(icono_x, barra_y + 2, icono_x + 15, barra_y + 17) &&
	    (mouse_b & 1)) {
		...
		if (barra_y > vga_al / 2 - 9 * big2)
			b = barra_y - d - 1;
		else
			b = barra_y + 19 * big2 + 1;
		if (a < barra_x)
			a = barra_x;
		if (a + c > barra_x + barra_an * big2)
			a = barra_x + barra_an * big2 - c;
		ajusta_caja(&a, &b, &c, &d);
		c /= big2;
		d /= big2;
		do {
		...
		do {
			...
			if (mouse_in(a + 2, b + 18, a + 9, b + 25)) {
				...
				while (mouse_b & 1) {
					salvaguarda(fondo_raton, mouse_shift_x,
						    mouse_shift_y, mouse_graf,
						    0);
					put(mouse_shift_x, mouse_shift_y,
					    mouse_graf);
					volcado(copia);
					salvaguarda(fondo_raton, mouse_shift_x,
						    mouse_shift_y, mouse_graf,
					...
					a = mouse_shift_x - ix;
					b = mouse_shift_y - iy;
					wvolcado(copia, vga_an, vga_al, p, a, b,
						 c * big2, d * big2, 0);
					volcado_parcial(a, b, c * big2,
							d * big2);
				}
				wrectangulo(p, c, d, c2, 0, 0, c, d);
			}
			...
			wvolcado(copia, vga_an, vga_al, p, a, b, c * big2,
				 d * big2, 0);
		...
		if (!mouse_in(barra_x, barra_y, barra_x + barra_an - 1,
			      barra_y + 18) ||
		    !(mouse_b & 1) ||
		    mouse_in(icono_x, barra_y + 2, icono_x + 15,
			     barra_y + 17)) {
			wput(p, c, d, c - 9, 2, -45);
			wvolcado(copia, vga_an, vga_al, p, a, b, c * big2,
				 d * big2, 0);
...
void select_fill(int n)
{
	...
	if (r >= 0) {
		modo_fill = r;
		bar[n] = iconos_fill[2 + modo_fill];
		put_barra(48 + n * 16, 2, bar[n]);
	}
}
...
int select_icon(int icono_x, int *iconos)
{
	...
	if (mouse_in(icono_x, barra_y + 2, icono_x + 15, barra_y + 17) &&
		...
		a = barra_x + (icono_x - barra_x) * big2 -
		    (col - 1) * 8 * big2 - 2 * big2;
		if (barra_y > vga_al / 2 - 9 * big2)
			b = barra_y - d - 1;
		else
...
void volcado_raton_b(void)
{
	...
	if (mouse_graf < 10)
		...
		salvaguarda(fondo_raton, moux, mouy, mouse_graf, 1);
}
...
void select_mascara(int n)
{
	...
	if ((key(_M) && hotkey) ||
	    (mouse_in(barra_x + 48 + n * 16, barra_y + 2, barra_x + 55 + n * 16,
		      barra_y + 17) &&
	     (mouse_b & 2))) {
		c = (128 + 3) * big2;
		d = (128 + 11 + 8) * big2;
		a = barra_x;
		if (barra_y > vga_al / 2 - 8)
			b = barra_y - d - 1;
		else
			b = barra_y + 19 * big2 + 1;
		if ((p = (byte *)malloc(c * d)) == NULL) {
		...
		do {
			...
			if (mouse_in(a + 2, b + 2, a + c - 10, b + 9)) {
				...
				if (mouse_b & 1) {
					...
					while (mouse_b & 1) {
						...
						a = mouse_shift_x - ix;
						b = mouse_shift_y - iy;
						wvolcado(copia, vga_an, vga_al,
							 p, a, b, c * big2,
							 d * big2, 0);
						volcado_parcial(a, b, c * big2,
								d * big2);
						salvaguarda(fondo_raton,
							    mouse_shift_x,
							    mouse_shift_y,
							    mouse_graf, 0);
						put(mouse_shift_x,
			...
			if ((mouse_b & 1) &&
			    mouse_in(a + 2, b + d - 9, a + c - 2, b + d - 2)) {
				if (mouse_x < a + c / 2) {
					for (i = 0; i < 256; i++)
						mask[i] ^= 1;
				} else {
			...
			if ((mouse_b & 1) &&
			    !mouse_in(a, b, a + c - 1, b + d - 1))
				if (mouse_graf >= 10) {
					mask[*(map + coord_y * map_an +
					       coord_x)] = 1;
					dibuja_regla();
				} else
					salir = 1;
			...
			volcado_parcial(a + x * big2, b + y * big2, 9 * big2,
					9 * big2);

		} while (!(mouse_b & 2) && !key(_ESC) && !salir && !key(_M) &&
			 !((mouse_b & 1) &&
			   mouse_in(a + c - 9, b + 2, a + c - 2, b + 9)));

		if (!mouse_in(barra_x, barra_y, barra_x + barra_an - 1,
			      barra_y + 18) ||
		    !(mouse_b & 1) || key(_M) ||
		    mouse_in(barra_x + 48 + n * 16, barra_y + 2,
			     barra_x + 57 + n * 16, barra_y + 17)) {
			volcado_raton_a();
			wput(p, c, d, c - 9, 2, -45);
			x = 1 + (col % 16) * 8;
			y = 9 + (col / 16) * 8;
...
void pinta_ventana_mascara(byte *p, int c, int d)
{
	...
	for (n = 0; n < 256; n++)
		if (mask[n]) {
			...
			if (y >= x)
				wbox(p, c, d, c0, 4 + (n % 16) * 8,
				     12 + (n / 16) * 8, 3, 3);
			else
				wbox(p, c, d, c4, 4 + (n % 16) * 8,
				     12 + (n / 16) * 8, 3, 3);
		}
...
int crear_mapbr_thumbs(struct t_listboxbr *l)
{
	...
	for (con = 0; con < l->maximo; con++) {
		...
		if (man <= 32 * big2 &&
		    mal <= 32 * big2) // El grafico se deja tal cual
		{
			if ((thumb_map[con].ptr = (char *)malloc(man * mal)) ==
			    NULL) {
		...
		} else // Crea el thumbnail
		{
			...
			if (coefredx * (float)(thumb_map[con].an - 1) >=
			    (float)man)
				coefredx = (float)(man - 1) /
					   (float)(thumb_map[con].an - 1);
			if (coefredy * (float)(thumb_map[con].al - 1) >=
			    (float)mal)
				coefredy = (float)(mal - 1) /
					   (float)(thumb_map[con].al - 1);

			if ((temp2 = (byte *)malloc(thumb_map[con].an *
						    thumb_map[con].al)) ==
			    NULL) {
				thumb_map[con].ptr = NULL;
				thumb_map[con].status = 0;
			...
			for (y = 0; y < thumb_map[con].al; y++) {
				...
				for (x = 0; x < thumb_map[con].an; x++) {
					temp2[y * thumb_map[con].an + x] =
					    temp[((memptrsize)a) * man +
						 (memptrsize)b];
					b += coefredx;
				}
				a += coefredy;
			...
			if ((thumb_map[con].ptr = (char *)malloc(
				 (thumb_map[con].an * thumb_map[con].al) /
				 4)) == NULL) {
				free(temp2);
				thumb_map[con].ptr = NULL;
				thumb_map[con].status = 0;
...
void select_color(int n)
{ // Número de "icono" como parámetro
	int x, y, col, oldcol, ix, iy;
	int a, b, c, d;
	int salir, volcar;
	int _regla, boton;
	...
	if ((TipoTex & 4) && ((key(_T) && hotkey) ||
			      (mouse_in(barra_x + 56 + n * 16, barra_y + 11,
					barra_x + 62 + n * 16, barra_y + 17) &&
			       (mouse_b & 1)))) {
		TipoBrowser = BRUSH;
		dialogo(MapperBrowseFPG0);

		num_tex = ltexturasbr.inicial + ltexturasbr.zona -
			  10; // Posicion en browser
		tex_cod = atoi(m3d_fpgcodesbr +
			       num_tex * an_textura); // Codigo de esa posicion

		if (thumb_tex[num_tex].Code == 0 || !v_terminado)
			return;
		...
		if ((temp = (byte *)malloc(man * mal)) == NULL) {
			...
			fseek(FilePaintFPG, thumb_tex[num_tex].FilePos,
			      SEEK_SET);

			if (fread(temp, 1, man * mal, FilePaintFPG) !=
			    man * mal) {
				free(temp);
	...
	if ((TipoTex & 8) &&
	    ((key(_U) && hotkey) ||
	     (mouse_in(barra_x + 56 - 8 + n * 16, barra_y + 11,
		       barra_x + 62 - 8 + n * 16, barra_y + 17) &&
	      (mouse_b & 1)))) {
		TipoBrowser = MAPBR;
		dialogo(MapperBrowseFPG0);
		if (v_terminado) {
			num_tex = thumb_map[ltexturasbr.inicial +
					    ltexturasbr.zona - 10]
				      .Code;
	...
	if ((key(_C) && hotkey) ||
		...
		do {
			...
			if ((mouse_b & 1) &&
			    mouse_in(a + 170, b + 10, a + 233, b + 128 + 9)) {
				regla = (mouse_y - b - 10) / 8;
				dibuja_regla();
				volcar = 1;
			}

			if ((mouse_b & 1) &&
			    mouse_in(a + 138, b + 140, a + 138 + 95,
				     b + 140 + 15)) {
				...
				if (boton == 2) {
					...
					switch (reglas[regla].numcol) {
					case 8:
						reglas[regla].numcol = 16;
						break;
					case 16:
						reglas[regla].numcol = 32;
						break;
					case 32:
						reglas[regla].numcol = 8;
						break;
					}
					reglas[regla].fijo = 0;
				} else
				...
				do
					read_mouse();
				while (mouse_b & 1);
			} else if (!mouse_in(a + 138, b + 140, a + 138 + 95,
					     b + 140 + 15))
				boton = -1;

			if (editable_seleccion(&x, a + 10, b + 128 + 12))
				mouse_graf = 2;
			else if (!mouse_in(a, b, a + c - 1, b + d - 1) &&
				 mouse_in(barra_x, barra_y,
					  barra_x + barra_an - 1,
					  barra_y + 18)) {
				if (mouse_in(barra_x, barra_y + 10, barra_x + 9,
					     barra_y + 18))
					mouse_graf = 5;
				else if (mouse_in(barra_x, barra_y, barra_x + 9,
						  barra_y + 9) ||
					 editable(&x))
					mouse_graf = 2;
			}

			// Volcar: 0-Nada (ratón), 1-Información color,
			// 2-Ventana entera.
			...
			if (volcar == 1) {
				...
				wvolcado(copia, vga_an, vga_al, p, a, b,
					 c * big2, d * big2, 0);
				volcado_parcial(a + x * big2, b + y * big2,
						9 * big2, 9 * big2);
				volcado_parcial(a + 138 * big2, b + 10 * big2,
						31 * big2, 128 * big2);
			}
			...
			if ((mouse_b & 1) && mouse_in(a + 10, b + 132 + 8,
						      a + 137, b + 147 + 8)) {
				if (editable_seleccion(&x, a + 10, b + 132 + 8))
					reglas[regla].col[x] = color;
				else {
			...
			if ((mouse_b & 1) &&
			    !mouse_in(a, b, a + c - 1, b + d - 1))
				if (mouse_in(barra_x, barra_y,
					     barra_x + barra_an - 1,
					     barra_y + 18)) {
					if (mouse_in(barra_x + _ir, barra_y,
						     barra_x + _ir + 127,
						     barra_y + 18)) {
						if (editable(&x))
							reglas[regla].col[x] =
						...
						dibuja_regla();
						volcar = 1;
					} else if (mouse_in(barra_x + _ir - 8,
							    barra_y,
							    barra_x + _ir - 1,
							    barra_y + 18)) {
						if (textura_color != NULL) {
		...
		volcado_raton();
		free(p);
	} else if ((ascii == '0' && hotkey) ||
		   (mouse_in(barra_x + 56 + n * 16, barra_y + 2,
			     barra_x + 62 + n * 16, barra_y + 10) &&
		    (mouse_b & 1))) {
		if (textura_color != NULL) {
			quitar_textura();
			if (color != 0) {
	...
	if (adelante) {
		for (n = 1; n < max_windows; n++) {
			if (ventana[n].tipo == 100 &&
			    ventana[n].mapa->map_an == map_an &&
			    ventana[n].mapa->map_al == map_al &&
			    ventana[n].primer_plano != 2) {
				copy(old, n);
				old = n;
			}
		}
...
void color_up(void)
{
	...
	for (n = 1; n <= reglas[regla].numcol; n++)
		if (reglas[regla].col[n] == color)
			c = n;
	if (c) {
		if (c < reglas[regla].numcol)
			color = reglas[regla].col[c + 1];
	} else
		color = reglas[regla].col[1];
	quitar_textura();
}
void color_down(void)
{
	...
	if (c) {
		...
		color = reglas[regla].col[reglas[regla].numcol];
	quitar_textura();
}
...
void pinta_ventana_colores(byte *p, int c, int d)
{
	...
	if (y >= x)
		wbox(p, c, d, c0, 12 + (color % 16) * 8, 12 + (color / 16) * 8,
		     3, 3);
	else
		wbox(p, c, d, c4, 12 + (color % 16) * 8, 12 + (color / 16) * 8,
		     3, 3);
	...
	for (y = 0; y < 16; y++) {
		...
		case 32:
			for (x = 0; x < 32; x++)
				wbox(p, c, d, reglas[y].col[x + 1], 175 + x,
				     10 + y * 8, 1, 7);
			break;
		case 16:
			for (x = 0; x < 16; x++)
				wbox(p, c, d, reglas[y].col[x + 1], 175 + x * 2,
				     10 + y * 8, 2, 7);
			break;
		case 8:
			for (x = 0; x < 8; x++)
				wbox(p, c, d, reglas[y].col[x + 1], 175 + x * 4,
				     10 + y * 8, 4, 7);
			break;
		}
...
void mover_barra(void)
{
	...
	if ((mouse_b & 1) &&
	    mouse_in(barra_x, barra_y, barra_x + 9, barra_y + 9)) {

		wrectangulo(barra, vga_an / big2, vga_al, c4, 0, 0, barra_an,
			    19);
		barx = barra_x - mouse_shift_x;
		bary = barra_y - mouse_shift_y;
		do {
			read_mouse();
			fondo_edicion(barra_x - 4, barra_y - big2,
				      barra_an * big2 + 4, 20 * big2);
			barra_x = mouse_shift_x + barx;
			barra_y = mouse_shift_y + bary;
			coord_barra();
			volcar_barras(0);
		...
		wrectangulo(barra, vga_an / big2, vga_al, c2, 0, 0, barra_an,
			    19);
		fondo_edicion(barra_x - 4, barra_y - big2, barra_an * big2 + 4,
			      20 * big2);
		volcar_barras(0);
	}
}
...
void select_zoom(void)
{
	...
	if (z = (key(_Z) && hotkey) || (mouse_b & 4 && zoom < 3) ||
		(mouse_b & 8 && zoom > 0)) {
		if (mouse_b & 8) {
			zoom = (zoom - 1) % 4;
		} else {
		...
		r = select_icon(barra_x + 48, iconos_zoom);
		if (r >= 0)
			zoom = r;
	}

	if (mouse_x >= zx + zan || mouse_x < zx || mouse_y >= zy + zal ||
	    mouse_y < zy)
		z = 0;
	...
	if (r >= 0) {
		...
		bar[0] = 101 + zoom;
		put_barra(48, 2, bar[0]);
		zoom_background = 0;

		if ((map_an << zoom) <
		    vga_an) { // Copia de este trozo en zoom_map
			zan = map_an << zoom;
			zx = (vga_an - zan) / 2;
		} else {
			zx = 0;
			zan = (vga_an >> zoom) << zoom;
		}
...
void mover_zoom(void)
{
	...
	if ((mouse_b & 1) &&
	    mouse_in(barra_x + 26, barra_y + 2, barra_x + 47, barra_y + 17)) {
		if (zoom_move == c3)
			zoom_move = c1;
		else
			zoom_move = c3;
	...
	if (zoom_move == c3) {
		...
		if (zx == 0 && zan >= vga_an)
			if (real_mouse_x < 0) {
				...
				if (zoom_x > map_an - vga_an / (1 << zoom))
					zoom_x = map_an - vga_an / (1 << zoom);
				if (n != zoom_x)
					m |= 1;
			}
		n = zoom_y;
		if (zy == 0 && zal >= vga_al)
			if (real_mouse_y < 0) {
				...
				if (zoom_y > map_al - vga_al / (1 << zoom))
					zoom_y = map_al - vga_al / (1 << zoom);
				if (n != zoom_y)
					m |= 2;
			}
		if (m & 1)
			zoom_cx = zoom_x + (vga_an / 2) / (1 << zoom);
		if (m & 2)
			zoom_cy = zoom_y + (vga_al / 2) / (1 << zoom);
		if (m)
			hacer_zoom = 1;
	}
}
...
void dibuja_barra(int _an)
{
	...
	//  for (a=1;a<18;a++) { barra[a*vga_an]=c2;
	//  barra[a*vga_an+barra_an-1]=c2; }
	...
	switch (modo) {
	case 0:
		put_barra_inv(10, 2, 177);
		break;
	case 1:
		put_barra_inv(10, 2, 160);
		break;
	case 2:
		put_barra_inv(10, 2, 161);
		break;
	case 3:
		put_barra_inv(10, 2, 178);
		break;
	case 4:
		put_barra_inv(10, 2, 162);
		break;
	case 5:
		put_barra_inv(10, 2, 179);
		break;
	case 6:
		put_barra_inv(10, 2, 163);
		break;
	case 7:
		put_barra_inv(10, 2, 164);
		break;
	case 8:
		put_barra_inv(10, 2, 180);
		break;
	case 9:
		put_barra_inv(10, 2, 165);
		break;
	case 10:
		if (modo_seleccion < 4)
			put_barra_inv(10, 2, 131 + modo_seleccion);
		else
			put_barra_inv(10, 2, modo_seleccion + 167 - 4);
		break;
	case 11:
		put_barra_inv(10, 2, 100);
		break;
	case 12:
		put_barra_inv(10, 2, 191);
		break;
	case 13:
		put_barra_inv(10, 2, 190);
		break;
	}
}
...
void anal_barra(int an, int al)
{
	...
	num[1] = (an / 100) % 10 + 48;
	num[0] = (an / 1000) % 10 + 48;
	writetxt(barra_an - 22, 3, 0, num);
	num[3] = al % 10 + 48;
	num[2] = (al / 10) % 10 + 48;
	num[1] = (al / 100) % 10 + 48;
	num[0] = (al / 1000) % 10 + 48;
	writetxt(barra_an - 22, 10, 0, num);
	copia = p;
}
...
int nueva_barra(int an, int al)
{
	...
	barras[n].x = barra_x;
	if (barra_y > vga_al / 2 - 9 * big2)
		barras[n].y = barra_y - al - 1;
	else
		barras[n].y = barra_y + 19 * big2 + 1;
...
void volcar_barras(int oscurecidas)
{
	...
	if (!oscurecidas)
		vuelca_barra(barra, vga_an, barra_x, barra_y, barra_an, 19);
	else
		vuelca_barra_oscurecida(barra, vga_an, barra_x, barra_y,
					barra_an, 19);

	for (n = 0; n < 10; n++)
		if (barras[n].on) {
			if (!oscurecidas) {
				vuelca_barra(barras[n].ptr, barras[n].an,
					     barras[n].x, barras[n].y,
					     barras[n].an, barras[n].al);
			} else {
				vuelca_barra_oscurecida(
				    barras[n].ptr, barras[n].an, barras[n].x,
				    barras[n].y, barras[n].an, barras[n].al);
			}
		}
}
...
void put_barra_inv(int x, int y, int n)
{
	...
	wput_in_box(barra, vga_an / big2, vga_an / big2, 19, x, y,
		    -n); //*** OJO *** esto no vale
	wresalta_box(barra, vga_an / big2, 19, x, y, *((word *)graf[n]) / big2,
		     *((word *)(graf[n] + 2)) / big2);
