...
unsigned short s3mperiodtable[] = {1712, 1616, 1524, 1440, 1356, 1280,
				   1208, 1140, 1076, 1016, 960,  907};

/* S3M Set Finetune values */
unsigned short s3mfinetunetable[] = {7895, 7941, 7985, 8046, 8107, 8169,
				     8232, 8280, 8363, 8413, 8463, 8529,
				     8581, 8651, 8723, 8757};
...
void judas_forwards3m(void) { s3m_wind = 1; }

void judas_rewinds3m(void) { s3m_wind = -1; }
...
int judas_loads3m(char *name)
{
	...
	if ((header.byte1a != 0x1a && header.byte1a != 0) ||
	    (header.filetype != 0x10) || (memcmp("SCRM", header.id, 4))) {
		judas_close(handle);
		return 0;
	}
	...
	for (count = 0; count < S3M_MAXCHANNELS; count++) {
		...
		if (header.chsettings[count] < 16) {
			...
			if (header.mastermult & 128) {
				if (header.chsettings[count] & 8)
					s3m_chpanning[s3m_channels] = S3M_RIGHT;
				else
					s3m_chpanning[s3m_channels] = S3M_LEFT;
			} else
				s3m_chpanning[s3m_channels] = MIDDLE;
			s3m_channels++;
		}
	}
	/*
	...
	judas_error = JUDAS_READ_ERROR;
	if (judas_read(handle, &s3m_order, header.ordernum) !=
	    header.ordernum) {
		judas_close(handle);
		return 0;
	}
	 ...
	 */
	judas_error = JUDAS_READ_ERROR;
	if (judas_read(handle, &instparapointers,
		       header.instnum * sizeof(short)) !=
	    header.instnum * sizeof(short)) {
		judas_close(handle);
		return 0;
	}
	if (judas_read(handle, &pattparapointers,
		       header.pattnum * sizeof(short)) !=
	    header.pattnum * sizeof(short)) {
		judas_close(handle);
		return 0;
	}
	...
	/*
	 * Read in and use default channel pan positions (panning magic) if
	 * enabled
	 */
	if (header.panningmagic == 252) {
		if (judas_read(handle, &defaultchannelpanpos,
			       s3m_channels * sizeof(char)) !=
		    s3m_channels * sizeof(char)) {
			judas_close(handle);
			return 0;
		}
		for (count = 0; count < s3m_channels; count++) {
			if (defaultchannelpanpos[count] & 32) {
				...
				s3m_chpanning[count] =
				    ((defaultchannelpanpos[count] & 15) << 4) |
				    (defaultchannelpanpos[count] & 15);
			} else {
				/* Panning pos for this channel not defined. Use
				 * default */
				switch (s3m_chpanning[count]) {
				case MIDDLE:
					s3m_chpanning[count] =
					    0x77; /* 128 would be faster though
						   */
					break;
				case S3M_LEFT:
	...
	for (count = 0; count < header.instnum; count++) {
		 ...
		 */
		judas_error = JUDAS_READ_ERROR;
		if (judas_seek(handle, instparapointers[count] << 4,
			       SEEK_SET) == -1) {
			free(instbuffer);
			judas_close(handle);
			return 0;
		 ...
		 * Read the instrument
		 */
		if (judas_read(handle, s3mip, sizeof(S3M_INSTRUMENT)) !=
		    sizeof(S3M_INSTRUMENT)) {
			free(instbuffer);
			judas_close(handle);
			return 0;
		}
	 ...
	 */
	{
		...
		for (count = header.pattnum * s3m_channels * 64; count;
		     count--) {
			noteptr->note = 0xff;
	...
	for (count = 0; count < header.pattnum; count++) {
		...
		if (pattparapointers[count]) {
			 ...
			 */
			judas_error = JUDAS_READ_ERROR;
			if (judas_seek(handle, pattparapointers[count] << 4,
				       SEEK_SET) == -1) {
				free(instbuffer);
				judas_frees3m();
				judas_close(handle);
			...
			if (judas_read(handle, &loadpattlen,
				       sizeof loadpattlen) !=
			    sizeof loadpattlen) {
				free(instbuffer);
				judas_frees3m();
			...
			if (loadpattlen > 2) {
				/*
				 * Weird, packed data length has the length
				 * indicator word included (I really checked
				 * this out!)
				 */
				loadpattlen -= 2;
				judas_error = JUDAS_OUT_OF_MEMORY;
				...
				judas_error = JUDAS_READ_ERROR;
				if (judas_read(handle, packbuffer,
					       loadpattlen) != loadpattlen) {
					free(instbuffer);
					free(packbuffer);
					judas_frees3m();
					judas_close(handle);
	...
	for (count = 0; count < header.instnum; count++) {
		...
		if ((s3mip->type == 1) && (s3mip->length)) {
			 ...
			 */
			judas_seek(handle,
				   (s3mip->paraptr | (s3mip->paraptrhigh << 16))
				       << 4,
				   SEEK_SET);
			 ...
			 */
			if ((s3mip->flags & 1) &&
			    (s3mip->loopstart != s3mip->loopend)) {
				ip->sp->voicemode = VM_LOOP | VM_ON;
				ip->sp->repeat =
				    ip->sp->start + s3mip->loopstart;
...
unsigned char judas_gets3mpos(void) { return s3m_pos; }

unsigned char judas_gets3mline(void) { return s3m_line; }

unsigned char judas_gets3mtick(void) { return s3m_tickcount; }

unsigned char judas_gets3mchannels(void) { return s3m_channels; }
...
static int init_s3mplayer(void)
{
	...
	if (!judas_locktables())
		return 0;
	if (!judas_memlock(&s3mplayer_code_lock_start,
			   (int)&s3mplayer_code_lock_end -
			       (int)&s3mplayer_code_lock_start))
		return 0;
	if (!judas_memlock(&s3mperiodtable, sizeof s3mperiodtable))
		return 0;
	if (!judas_memlock(&s3mfinetunetable, sizeof s3mfinetunetable))
		return 0;
	if (!judas_memlock(&header, sizeof header))
		return 0;
	if (!judas_memlock(&s3m_loaded, sizeof s3m_loaded))
		return 0;
	if (!judas_memlock(&s3m_channels, sizeof s3m_channels))
		return 0;
	if (!judas_memlock(&s3m_chpanning, sizeof s3m_chpanning))
		return 0;
	if (!judas_memlock(&s3m_poshasbeenplayed, sizeof s3m_poshasbeenplayed))
		return 0;
	if (!judas_memlock(&s3m_order, sizeof s3m_order))
		return 0;
	if (!judas_memlock(&s3m_instrument, sizeof s3m_instrument))
		return 0;
	if (!judas_memlock(&s3m_track, sizeof s3m_track))
		return 0;
	if (!judas_memlock(&s3m_pattlength, sizeof s3m_pattlength))
		return 0;
	if (!judas_memlock(&s3m_pattptr, sizeof s3m_pattptr))
		return 0;
	if (!judas_memlock(&s3m_oldpos, sizeof s3m_oldpos))
		return 0;
	if (!judas_memlock(&s3m_pos, sizeof s3m_pos))
		return 0;
	if (!judas_memlock(&s3m_line, sizeof s3m_line))
		return 0;
	if (!judas_memlock(&s3m_ticktempo, sizeof s3m_ticktempo))
		return 0;
	if (!judas_memlock(&s3m_tickcount, sizeof s3m_tickcount))
		return 0;
	if (!judas_memlock(&s3m_globalvol, sizeof s3m_globalvol))
		return 0;
	if (!judas_memlock(&s3m_patternbreak, sizeof s3m_patternbreak))
		return 0;
	if (!judas_memlock(&s3m_patterndelay, sizeof s3m_patterndelay))
		return 0;
	if (!judas_memlock(&s3m_patternloopline, sizeof s3m_patternloopline))
		return 0;
	if (!judas_memlock(&s3m_patternloopcount, sizeof s3m_patternloopcount))
		return 0;
	if (!judas_memlock(&s3m_roundcounter, sizeof s3m_roundcounter))
		return 0;
	if (!judas_memlock(&s3m_wind, sizeof s3m_wind))
		return 0;
	if (!judas_memlock(&s3m_windingpause, sizeof s3m_windingpause))
		return 0;
...
void s3mplayer(void)
{
	...
	if (s3m_wind > 0) {
		...
		if (s3m_pos < header.ordernum - 1) {
			int count;
			for (count = s3m_pos + 1; count < header.ordernum;
			     count++) {
				if (s3m_order[count] != 254 &&
				    s3m_order[count] != 255)
					break;
			...
			if (count != header.ordernum) {
				...
				s3m_tickcount = 0;
				s3m_patterndelay = 0;
				for (count = 0; count < header.ordernum;
				     count++) {
					s3m_poshasbeenplayed[count] = 0;
				}
	...
	if ((!s3m_tickcount) && (!s3m_patterndelay)) {
				...
				if (s3m_pos >= header.ordernum) {
					...
					if (s3m_roundcounter != 1) {
						...
						for (count = 0;
						     ...
						     count++) {
							s3m_poshasbeenplayed
							    [count] = 0;
						}
						s3m_poshasbeenplayed[s3m_pos] =
						    1;
					} else {
						judas_stops3m();
						return;
		...
		noteptr =
		    (NOTE
			 *)&s3m_pattptr[s3m_pattlength * s3m_order[s3m_pos] +
					sizeof(NOTE) * s3m_line * s3m_channels];
		...
		for (count = s3m_channels; count > 0; count--) {
			 ...
			 * isn't notedelay
			 */
			if ((tptr->effect != 19) || (tptr->nybble1 != 0xd) ||
			    (tptr->nybble2 == 0)) {
				if (tptr->newnote)
				...
				if (tptr->newinstrument) {
					...
					tptr->notevol =
					    s3m_instrument[tptr->instrument]
						.vol;
			...
			case 6:
				if (tptr->effectdata)
					tptr->portaspeed = tptr->effectdata;
	...
	if (s3m_tickcount) {
		...
		for (count = s3m_channels; count > 0; count--) {
			...
			case 10: {
				...
				case 1:
					arpnote = tptr->note +
						  (tptr->arpeggiointerval >> 4);
					if (arpnote > 95)
						arpnote = 95;
					tptr->period =
					    ((8363 * 16 *
					      s3mperiodtable[arpnote % 12]) >>
					     (arpnote / 12)) /
					    tptr->c2rate;
					break;
...
static void startnewnote(TRACK *tptr, CHANNEL *chptr)
{
	...
	if (tptr->sp) {
		 ...
		 */
		if (((tptr->effect == 7) || (tptr->effect == 12)) &&
		    (chptr->voicemode != VM_OFF)) {
			/*
			 * Toneportamento
			 */
			tptr->targetperiod =
			    ((8363 * 16 * s3mperiodtable[tptr->note % 12]) >>
			     (tptr->note / 12)) /
			    tptr->c2rate;
			tptr->tp = 1;
		} else {
	...
	case 0xd:
		...
		if (s3m_tickcount == tptr->nybble2) {
			...
			if (tptr->newinstrument) {
				...
				tptr->instrument = tptr->newinstrument - 1;
				tptr->notevol =
				    s3m_instrument[tptr->instrument].vol;
				tptr->vol = tptr->notevol;
			}
			if (tptr->voleffect != 255) {
				tptr->notevol = tptr->voleffect;
