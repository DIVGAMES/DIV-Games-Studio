...
static void gus_setupchannel(unsigned char channel, unsigned start,
			     unsigned length);
...
#pragma aux judas_get_ds = "mov ax, ds" modify[ax] value[ax];
 ...
 * JUDAS device names
 */
char *judas_devname[] = {"No Sound",	  "Sound Blaster",
			 "Sound Blaster Pro", "Sound Blaster 16",
			 "UltraSound",	"File Writer"};
 ...
 * Mixmode names
 */
char *judas_mixmodename[] = {"8-bit mono", "8-bit stereo", "16-bit mono",
			     "16-bit stereo"};
 ...
 * JUDAS error texts
 */
char *judas_errortext[] = {"Everything OK",	   "Couldn't open file",
			   "Couldn't read file",      "Incorrect file format",
			   "Out of memory",	   "Hardware init failure",
			   "Configuration incorrect", "Out of channels"};

static unsigned char gus_dmalatch[] = {0x40, 0x41, 0x40, 0x42,
				       0x40, 0x43, 0x44, 0x45};

static unsigned char gus_irqlatch[] = {0x40, 0x40, 0x40, 0x43, 0x40, 0x42,
				       0x40, 0x44, 0x40, 0x41, 0x40, 0x45,
				       0x46, 0x40, 0x40, 0x47};
...
void judas_config(void)
{
	...
	if (envstr) {
		...
		while (*envstr) {
			unsigned sb_type = 0;
			if ((*envstr == 'A') || (*envstr == 'a'))
				sscanf(envstr + 1, "%x", &judascfg_port);
			if ((*envstr == 'I') || (*envstr == 'i'))
				sscanf(envstr + 1, "%d", &judascfg_irq);
			if ((*envstr == 'D') || (*envstr == 'd')) {
				sscanf(envstr + 1, "%d", &judascfg_dma1);
			}
			if ((*envstr == 'T') || (*envstr == 't')) {
				sscanf(envstr + 1, "%d", &sb_type);
				if ((sb_type == 2) || (sb_type == 4))
	       ...
	       int interpolation)
{
	...
	if (judascfg_device > DEV_FILE)
		return 0;
	if (judascfg_device != DEV_NOSOUND && judascfg_device != DEV_FILE) {
		if (judascfg_port < 0x200)
			return 0;
		if (judascfg_port > 0x2ff)
			return 0;
		if (judascfg_irq < 2)
			return 0;
		if (judascfg_irq > 15)
	...
	if (mixrate > 44100)
		mixrate = 44100;
	if ((mixer != FASTMIXER) && (mixer != QUALITYMIXER))
		return 0;
	/*
	 * If user wants to re-initialize, shutdown first
	 */
	if (judas_initialized)
		judas_uninit();
	...
	if (mixer == QUALITYMIXER) {
		...
		if (interpolation)
			judas_mixroutine = &qmix_cubic;
		else
			judas_mixroutine = &qmix_linear;
		...
		if (interpolation)
			judas_mixroutine = &ipmix;
		else
			judas_mixroutine = &normalmix;
	}
	...
	if (judas_bufferlength < 512)
		judas_bufferlength = 512;
	if (judas_bufferlength > (DMA_MAXSIZE - 64))
		judas_bufferlength = DMA_MAXSIZE - 64;
	...
	if (judas_device != DEV_FILE) {
		if (!dma_reserve(44100 / PER_SECOND * 4 + 64))
			return 0;
	}
	if (!initmixer())
		return 0;
	...
	if (judas_device != DEV_FILE) {
		...
		if (judas_irq < 8) {
			...
			outp(PICMASK2,
			     judas_oldpicmask2 & ~(1 << (judas_irq & 3)));
	...
	case DEV_SB16:
		...
		if (judas_mixmode & SIXTEENBIT)
			memset((char *)dma_address, 0, judas_bufferlength);
		else
			memset((char *)dma_address, 0x80, judas_bufferlength);
		dma_program(DMA_WRITE_LOOP, 0, judas_bufferlength);
		if (judas_mixmode & SIXTEENBIT) {
	...
	case DEV_GUS:
		if (judas_mixmode & SIXTEENBIT)
			memset((char *)dma_address, 0, judas_bufferlength + 64);
		else
			memset((char *)dma_address, 0x80,
			       judas_bufferlength + 64);
...
static void gus_setupchannels(void)
{
	if (judas_mixmode & SIXTEENBIT) {
		if (judas_mixmode & STEREO) {
			...
			gus_setupchannel(1, (judas_bufferlength >> 2) + 16,
					 judas_bufferlength >> 2);
		} else {
		...
		if (judas_mixmode & STEREO) {
			gus_setupchannel(0, 0, judas_bufferlength >> 1);
			gus_setupchannel(1, (judas_bufferlength >> 1) + 32,
					 judas_bufferlength >> 1);
		} else {
			     ...
			     unsigned length)
{
	...
	outpw(judas_port + GF1_DATA_LOW,
	      (((judas_mixrate << 9) + 22050) / 44100) << 1);
	outp(judas_port + GF1_REG_SELECT, SET_VOLUME);
...
int initmixer(void)
{
	...
	if (mixer_firsttime) {
		...
		judas_volumetable =
		    locked_malloc(256 * 256 * sizeof(int) + 1024);
		 ...
		 */
		judas_volumetable =
		    (int *)((((unsigned)judas_volumetable) + 1023) &
			    0xfffffc00);
...
static int judas_lock(void)
{
	if (judas_locked)
		return 1;
	if (!judas_memlock(&judas_code_lock_start,
			   (int)&judas_code_lock_end -
			       (int)&judas_code_lock_start))
		return 0;
	if (!judas_memlock(&judas_device, sizeof judas_device))
		return 0;
	if (!judas_memlock(&judas_mixroutine, sizeof judas_mixroutine))
		return 0;
	if (!judas_memlock(&judas_mixersys, sizeof judas_mixersys))
		return 0;
	if (!judas_memlock(&judas_initialized, sizeof judas_initialized))
		return 0;
	if (!judas_memlock(&judas_ds, sizeof judas_ds))
		return 0;
	if (!judas_memlock(&judas_irq, sizeof judas_irq))
		return 0;
	if (!judas_memlock(&judas_int, sizeof judas_int))
		return 0;
	if (!judas_memlock(&judas_dma, sizeof judas_dma))
		return 0;
	if (!judas_memlock(&judas_port, sizeof judas_port))
		return 0;
	if (!judas_memlock(&judas_irqcount, sizeof judas_irqcount))
		return 0;
	if (!judas_memlock(&dsp_version, sizeof dsp_version))
		return 0;
	if (!judas_memlock(&judas_mixrate, sizeof judas_mixrate))
		return 0;
	if (!judas_memlock(&judas_mixmode, sizeof judas_mixmode))
		return 0;
	if (!judas_memlock(&judas_bufferlength, sizeof judas_bufferlength))
		return 0;
	if (!judas_memlock(&judas_buffermask, sizeof judas_buffermask))
		return 0;
	if (!judas_memlock(&judas_clipbuffer, sizeof judas_clipbuffer))
		return 0;
	if (!judas_memlock(&judas_zladdbuffer, sizeof judas_zladdbuffer))
		return 0;
	if (!judas_memlock(&judas_cliptable, sizeof judas_cliptable))
		return 0;
	if (!judas_memlock(&judas_volumetable, sizeof judas_volumetable))
		return 0;
	if (!judas_memlock(&judas_mixpos, sizeof judas_mixpos))
		return 0;
	if (!judas_memlock(&judas_samplesize, sizeof judas_samplesize))
		return 0;
	if (!judas_memlock(&judas_player, sizeof judas_player))
		return 0;
	if (!judas_memlock(&judas_bpmcount, sizeof judas_bpmcount))
		return 0;
	if (!judas_memlock(&judas_bpmtempo, sizeof judas_bpmtempo))
		return 0;
	if (!judas_memlock(&judas_channel, sizeof judas_channel * CHANNELS))
		return 0;
	if (!judas_memlock(&judas_zerolevell, sizeof judas_zerolevell))
		return 0;
	if (!judas_memlock(&judas_zerolevelr, sizeof judas_zerolevelr))
		return 0;
	if (!judas_memlock(&judas_clipped, sizeof judas_clipped))
		return 0;
	if (!judas_memlock(&filewriterbuffer, sizeof filewriterbuffer))
		return 0;
	if (!judas_memlock(&filewriterbuffersize, sizeof filewriterbuffersize))
		return 0;
	judas_locked = 1;
	return 1;
}
...
static int judas_unlock(void)
{
	if (!judas_locked)
		return 1;
	if (!judas_memunlock(&judas_code_lock_start,
			     (int)&judas_code_lock_end -
				 (int)&judas_code_lock_start))
		return 0;
	if (!judas_memunlock(&judas_device, sizeof judas_device))
		return 0;
	if (!judas_memunlock(&judas_mixroutine, sizeof judas_mixroutine))
		return 0;
	if (!judas_memunlock(&judas_mixersys, sizeof judas_mixersys))
		return 0;
	if (!judas_memunlock(&judas_initialized, sizeof judas_initialized))
		return 0;
	if (!judas_memunlock(&judas_ds, sizeof judas_ds))
		return 0;
	if (!judas_memunlock(&judas_irq, sizeof judas_irq))
		return 0;
	if (!judas_memunlock(&judas_int, sizeof judas_int))
		return 0;
	if (!judas_memunlock(&judas_dma, sizeof judas_dma))
		return 0;
	if (!judas_memunlock(&judas_port, sizeof judas_port))
		return 0;
	if (!judas_memunlock(&judas_irqcount, sizeof judas_irqcount))
		return 0;
	if (!judas_memunlock(&dsp_version, sizeof dsp_version))
		return 0;
	if (!judas_memunlock(&judas_mixrate, sizeof judas_mixrate))
		return 0;
	if (!judas_memunlock(&judas_mixmode, sizeof judas_mixmode))
		return 0;
	if (!judas_memunlock(&judas_bufferlength, sizeof judas_bufferlength))
		return 0;
	if (!judas_memunlock(&judas_buffermask, sizeof judas_buffermask))
		return 0;
	if (!judas_memunlock(&judas_clipbuffer, sizeof judas_clipbuffer))
		return 0;
	if (!judas_memunlock(&judas_zladdbuffer, sizeof judas_zladdbuffer))
		return 0;
	if (!judas_memunlock(&judas_cliptable, sizeof judas_cliptable))
		return 0;
	if (!judas_memunlock(&judas_volumetable, sizeof judas_volumetable))
		return 0;
	if (!judas_memunlock(&judas_mixpos, sizeof judas_mixpos))
		return 0;
	if (!judas_memunlock(&judas_samplesize, sizeof judas_samplesize))
		return 0;
	if (!judas_memunlock(&judas_player, sizeof judas_player))
		return 0;
	if (!judas_memunlock(&judas_bpmcount, sizeof judas_bpmcount))
		return 0;
	if (!judas_memunlock(&judas_bpmtempo, sizeof judas_bpmtempo))
		return 0;
	if (!judas_memunlock(&judas_channel, sizeof judas_channel * CHANNELS))
		return 0;
	if (!judas_memunlock(&judas_zerolevell, sizeof judas_zerolevell))
		return 0;
	if (!judas_memunlock(&judas_zerolevelr, sizeof judas_zerolevelr))
		return 0;
	if (!judas_memunlock(&judas_clipped, sizeof judas_clipped))
		return 0;
	if (!judas_memunlock(&filewriterbuffer, sizeof filewriterbuffer))
		return 0;
	if (!judas_memunlock(&filewriterbuffersize,
			     sizeof filewriterbuffersize))
		return 0;
	judas_locked = 0;
	return 1;
}
