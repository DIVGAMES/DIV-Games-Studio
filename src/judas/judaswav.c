...
int judas_wavwriter_open(char *name)
{
	...
	handle = fopen(name, "rb"); // O_WRONLY | O_CREAT | O_TRUNC | O_BINARY,
				    // S_IREAD | S_IWRITE);
	if (handle == -1) {
		return handle;
	}
	...
	if (filewriterbuffer == NULL) {
		if (!(filewriterbuffer = locked_malloc(filewriterbuffersize)))
			return -1;
	}
...
int judas_wavwriter_writesome(int handle)
{
	...
	if (write(handle, filewriterbuffer, filewriterbuffersize) <
	    filewriterbuffersize) {
		close(handle);
		return -1;
	}
	return handle;
...
*/
	...
	for (;;) {
		...
		if (!memcmp("fmt ", &header.formattext, 4))
			break;
	...
	for (;;) {
		...
		if (!memcmp("data", &header.datatext, 4))
			break;
	...
	if (header.channels == 2) // Stereo
	{
		if (header.bits == 16) {
			...
			judas_error = JUDAS_READ_ERROR;
			if (length < 500000) {
				if (judas_read(handle, buffer, length) !=
				    length) {
					free(buffer);
					judas_freesample(smp);
					judas_close(handle);
				...
				if (!JudasProgressRead(handle, buffer,
						       length)) {
					free(buffer);
					judas_freesample(smp);
					judas_close(handle);
	...
	} else // Mono
	{
		if (header.bits == 16) {
			judas_error = JUDAS_READ_ERROR;
			if (length < 500000) {
				if (judas_read(handle, smp->start, length) !=
				    length) {
					judas_freesample(smp);
					judas_close(handle);
					return (SI);
				...
				if (!JudasProgressRead(handle, smp->start,
						       length)) {
					judas_freesample(smp);
					judas_close(handle);
					return (SI);
...
*/
	 ...
	 */
	judas_error = JUDAS_READ_ERROR;
	memcpy(&header, FileBuffer + FilePos, 12);
	FilePos += 12;
	judas_error = JUDAS_WRONG_FORMAT;
	if (memcmp("RIFF", header.rifftext, 4)) {
		return (SI);
	...
	for (;;) {
		judas_error = JUDAS_READ_ERROR;
		memcpy(header.formattext, FileBuffer + FilePos, 8);
		FilePos += 8;
		if (!memcmp("fmt ", &header.formattext, 4))
			break;
	 ...
	 * Read in the FORMAT chunk
	 */
	memcpy(&header.format, FileBuffer + FilePos, 16);
	FilePos += 16;
	/*
	 * Skip data if the format chunk was bigger than what we use
	 */
	...
	for (;;) {
		judas_error = JUDAS_READ_ERROR;
		memcpy(header.datatext, FileBuffer + FilePos, 8);
		FilePos += 8;
		if (!memcmp("data", &header.datatext, 4))
			break;
	...
	if (header.channels == 2) {
		if (header.bits == 16) {
			...
			judas_error = JUDAS_READ_ERROR;
			memcpy(buffer, FileBuffer + FilePos, length);
			FilePos += length;
			src = buffer;
			dest = (short *)smp->start;
			while (count--) {
		...
		if (header.bits == 16) {
			judas_error = JUDAS_READ_ERROR;
			memcpy(smp->start, FileBuffer + FilePos, length);
			FilePos += length;
			smp->repeat = smp->start;
			smp->end = smp->start + reallength;
			smp->voicemode = VM_ON | VM_16BIT;
