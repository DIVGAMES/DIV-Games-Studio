...
#pragma aux kbd_get_ds = "mov ax, ds" modify[ax] value[ax];
...
{
#ifdef NOTYET
	if (!judas_memlock(&kbd_code_lock_start,
			   (int)&kbd_code_lock_end - (int)&kbd_code_lock_start))
		return 0;
#endif
	return 1;
}
...
{
#ifdef NOTYET
	judas_memunlock(&kbd_code_lock_start,
			(int)&kbd_code_lock_end - (int)&kbd_code_lock_start);
#endif
}
