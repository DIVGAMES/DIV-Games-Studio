...
typedef struct _HeaderSetup {
	char
	    name[_MAX_FNAME]; // Nombre del archivo (en asciiz, sin path) FIX:
			      // increase buffer size to fit large file names on
			      // modern platforms and avoiding loading errors
...
struct _dirhead {
	char pack[_MAX_FNAME]; // Nombre del propio packfile (no va en el
			       // archivo) FIX: increase buffer size to fit
			       // large file names on modern platforms and
	...
	if (imagen_install != NULL) {
		for (y = 0; y < 87 * big2; y++)
			for (x = 0; x < 142 * big2; x++) {
				ptr[141 * big2 + x + (30 * big2 + y) * v.an] =
				    imagen_install[x + y * 142 * big2];
			}
	} else {
...
void Setup0()
{
	...
	_get(226, 6 * 16 + 8, y + 19 * 0, (v.an - 4) - (6 * 16 + 8),
	     (byte *)Copy_Right, 127, 0, 0);

	_get(227, 4, y + 19 * 1, 6 * 16, (byte *)Unid, 127, 0, 0);
	_get(228, 6 * 16 + 8, y + 19 * 1, (v.an - 4) - (6 * 16 + 8),
	     (byte *)DefDir, 127, 0, 0);
}
...
*/
			...
			while (p < fpg + file_len && *(int32_t *)p < 1000 &&
			       *(int32_t *)p > 0) {
			...
			if (!lst[1])
				sprintf(cwork, (char *)texto[540], 1);
			if ((ptr = (int32_t *)lst[2])) {
				if (ptr[13] != 640 || ptr[14] != 480)
					sprintf(cwork, (char *)texto[541], 2);
			}
			if (!(ptr = (int32_t *)lst[3]))
				sprintf(cwork, (char *)texto[540], 3);
			if (ptr[13] != 640 || ptr[14] != 480)
				sprintf(cwork, (char *)texto[541], 3);
			if (!is_point(ptr, 1))
				sprintf(cwork, (char *)texto[542], 1, 3);
			if (!lst[4] && !lst[5])
				sprintf(cwork, (char *)texto[540], 4);
			if (!lst[13])
				sprintf(cwork, (char *)texto[540], 13);
			if (!lst[14])
				sprintf(cwork, (char *)texto[540], 14);
			if (!lst[15])
				sprintf(cwork, (char *)texto[540], 15);
			if (!lst[16])
				sprintf(cwork, (char *)texto[540], 16);
			if (!lst[17])
				sprintf(cwork, (char *)texto[540], 17);
...
int px(int *ptr, int n) { return (*((word *)ptr + 32 + n * 2)); }
...
void Setupm1()
{
	...
	int an = v.an / big2, al = v.al / big2;
	_show_items();
	wwrite(ptr, an, al, v.an / 2 - 16, 12 + 11 + 19 + 9, 0, (byte *)"#",
	       c3);
}
...
void Setupm0()
{
	...
	_get(520, 4, y + 19 * 1, (v.an - 16) / 2 - 16, (byte *)TDisk1, 127, 0,
	     0);
	_get(414, v.an / 2 - 8, y + 19 * 1, v.an / 2 + 4, (byte *)TDisk2, 127,
	     0, 0);
...
void create_zip(char *dWork)
{
	...
	struct zip_t *zip =
	    zip_open("foo.zip", ZIP_DEFAULT_COMPRESSION_LEVEL, 0);
	...
	// Hack: Windows MSYS2 needs libSDL_mixer-1-2-0.dll instead normal
	// SDL_mixer.dll, probably a MSYS bug to fix
	...
	// FIX: Windows needs libmad-0.dll and zlib1.dll to run properly the
	// executable
...
void crear_instalacion(void)
{
	...
	// *** Tratamiento de la unidad de Disco destino (permite
	// "a:","dir","\dir\new","d:\tmp",...)

	if (strlen(Unid) == 1) { // Se supone una letra entre 'a' y 'z' como una
				 // unidad y no directorio
		strupr(Unid);
	...
	if (_drive <= 2) {
		strcpy(dir, "/");
		is_disk = _drive;
	} // En un disquete no creará directorios

	for (x = 1; x < strlen(dir); x++)
		if (dir[x] == '/') { // Crea directorios ...
			//    strcpy(cWork,drive);
			//    strcat(cWork,dir);
			//    cWork[x+2]=0;
			strcpy(cWork, full);
	...
	if (!include_setup)
		x = 0;
	if ((__ins = _ins = ins = (char *)malloc(n + x + (_MAX_FNAME))) ==
	    NULL) {
		v_texto = (char *)texto[357];
		dialogo(err0);
		fclose(fin);
		return;
	}
	...
	if ((MiHeaderSetup =
		 (HeaderSetup *)malloc(nfiles * sizeof(HeaderSetup))) == NULL) {
		v_texto = (char *)texto[357];
		dialogo(err0);
		free(_ins);
		return;
	...
	for (x = 0; x < nfiles; x++) {
		if (x == 0)
			...
			chr = (byte *)ins;
			if (*ins == '+') { // When the file can not be included
					   // in the PACKFILE
				ins++;
				topack = 0;
			} else
			...
			for (n = 0; n < x; n++) {
				if (!strcmp(MiHeaderSetup[n].name,
					    MiHeaderSetup[x].name))
					break;
			}
			if (n < x ||
			    !strcmp(MiHeaderSetup[n].name, "SOUND.CFG")) {
				fprintf(stdout,
					"[IDE - CREAR_INSTALACION] found "
					"duplicated file [%s] %d of %d file(s) "
					"(__ins [%X], _ins [%X], "
					"ins [%X])\n",
					MiHeaderSetup[x].name, x + 1, nfiles,
					__ins, _ins, ins);
				fflush(stdout);

				// Si el fichero llevaba '+', pone un '+' a su
				// aparición anterior
		...
		fprintf(stdout,
			"[IDE - CREAR_INSTALACION] packing file [%s] %d of %d "
			"file(s) (__ins [%X], _ins [%X], ins [%X])\n",
			MiHeaderSetup[x].name, x + 1, nfiles, __ins, _ins, ins);
		fflush(stdout);
	}
  ...
  if (!empaquetar) dirhead.nfiles=0; // Cuando no deba generarse el PACKFILE
	...
	// *** OJO *** Cuando el usuario seleccione la opción "Instalación
	// protegida" nfiles es el número de ficheros totales (EXE + OVL [+
	// SETUP] + char * ins) Crea el directorio en
	// hdir[].name/offset/len1/len2 y su cabecera en
	// dirhead.packname/head/crc1/crc2/crc3/nfiles
	...
	if (dirhead.nfiles) {
		...
		// 1º Obtiene los CRC del system\exec.exe y install\setup.ovl
		// (si include_setup)
		...
		if ((hdir = (struct tdir *)malloc(
			 dirhead.nfiles * sizeof(struct tdir))) == NULL) {
			v_texto = (char *)texto[357];
			dialogo(err0);
			fclose(fout);
			free(_ins);
		...
		// 6º Graba todos los archivos a continuacion (rellenando
		// hdir[])
		...
		for (n = 0; n < dirhead.nfiles; n++) {
			...
			// Carga fin, lo comprime, y lo graba en fout (-1 si
			// error)

			hdir[n].len1 = comprimir_fichero(
			    fin, fout, (unsigned long)hdir[n].len2);
		...
		// 7º Actualiza la lista de ficheros a incluir (_ins), con el
		// PACKFILE
		...
		strcpy(__ins, "install/PACKFILE.DAT"); // añade el PACKFILE como
						       // el último fichero
...
//    else         fin=fopen("install/div32run.386","rb");
  ...
  DaniDel(cWork);              // Borra el INSTALL.DIV
	...
	v_titulo =
	    (char *)texto[359]; // Diálogo indicando el final de la instalación
	...
	while (len) {
		...
		if (NewVolume) {
			...
			if (curvol != 0 && NewVolume < 2) {
				...
				v_texto = (char *)
				    texto[232]; // Please, insert a new disk.
				dialogo(aceptar0);
				if (!v_aceptar) {
					retval = 0;
					break;
			...
			if (vols) {
				if (fwrite("stp\x1a\x0d\x0a\x00", 1, 8, fout) !=
				    8) { // Any other error
					retval = 0;
					break;
				}
		...
		if (totfree >
		    16384) { // Enough disk space to record 16384 bytes
			if (len > 16384) { // Enough to save complete 16384
					   // bytes block
			...
			if (len <= totfree) { // Da igual, coge de todas formas
				...
				if (fwrite(buffer, 1, totfree, fout) !=
				    totfree) {
					retval = 0;
					break;
				}
