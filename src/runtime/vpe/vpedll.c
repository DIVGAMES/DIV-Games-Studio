...
void load_wld(void)
{
	...
	wldfuera:
		if ((fichero = div_open_file(
			 (char *)&mem[text_offset + nombre])) == NULL) {
			e(159);
			vpe_inicializada = 0;
			return;
		} else {
...
void start_mode8(void)
{
	...
	my_region =
	    (t_region *)((memptrsize)region + sizeof(t_region) * num_region);
...
void set_sector_height(void)
{
	...
	if (suelo != -1)
		new_region->FloorH = INT_FIX(suelo);
	if (techo != -1)
		new_region->CeilH = INT_FIX(techo);
}
...
void loop_mode8(void)
{
	...
	for (i = 0; i < 10; i++) {
		if (mode8_list[i] != -1) {
			...
			my_region =
			    (t_region *)((memptrsize)region +
					 sizeof(t_region) * mode8_list[i]);
			ancho = my_region->x1 - my_region->x0;
			alto = my_region->y1 - my_region->y0;
			InitGraph((char *)(copia + my_region->y0 * vga_an +
					   my_region->x0),
				  ancho, alto); // Init gfx system
			SetActiveView(m8[i].height,
				      mem[m8[i].camera + _M8_Object], ancho,
				      alto,
				      (char *)(copia + my_region->y0 * vga_an +
					       my_region->x0),
				      vga_an);
			if (m8[i].angle > 128)
				m8[i].angle = 128;
...
void _object_data_input(int ide)
{
	...
	while (mem[ide + _Angle] < 0)
		mem[ide + _Angle] += 360000;
	while (mem[ide + _Angle] >= 360000)
		mem[ide + _Angle] -= 360000;
	...
	if (mem[ide + _Resolution] < 2) {
		...
		if (FIX_INT(po->pp->x) != mem[ide + _X] ||
			...
			new_region = FindRegion(po->pp->x, po->pp->y, po->H,
						po->Type & O_STRUCT);
			if (new_region != NULL) {
				ClearObjRegion(po);
		...
		if ((int)po->pp->x !=
			((mem[ide + _X] * 256) / mem[ide + _Resolution]) *
			    256 ||
		    (int)po->pp->y !=
			(((FIN_GRID * mem[ide + _Resolution] - mem[ide + _Y]) *
			  256) /
			...
			po->pp->x =
			    ((mem[ide + _X] * 256) / mem[ide + _Resolution]) *
			    256;
			po->pp->y = (((FIN_GRID * mem[ide + _Resolution] -
				       mem[ide + _Y]) *
				      256) /
				     mem[ide + _Resolution]) *
				    256;
	...
	if ((p = mem[ide + _XGraph]) > 0) {
		if (mem[ide + _Status] == 2 || mem[ide + _Status] == 4) {
			for (i = 0; i < 10; i++) {
				if ((mode8_list[i] != -1) &&
				    (mem[ide + _Cnumber] == 0 ||
				     (mem[ide + _Cnumber] & (1 << i)))) {

					if (mem[m8[i].camera + _Resolution] >
					    1) {
		...
		if (mem[ide + _Graph] &&
		    (mem[ide + _Status] == 2 || mem[ide + _Status] == 4)) {
...
void _object_data_output(int ide)
{
	...
	if (mem[ide + _Resolution] < 2) {
		...
		if (po->H != INT_FIX(mem[ide + _Z]) / mem[ide + _Resolution]) {
			mem[ide + _Z] =
			    ((int)po->H * (int)mem[ide + _Resolution]) / 65536;
		}
		if ((int)po->pp->x !=
			((mem[ide + _X] * 256) / mem[ide + _Resolution]) *
			    256 ||
			...
			(((FIN_GRID * mem[ide + _Resolution] - mem[ide + _Y]) *
			 ...
			 mem[ide + _Resolution]) *
			    256) {
			mem[ide + _X] =
			    (((int)po->pp->x / 256) * mem[ide + _Resolution]) /
			    256;
			mem[ide + _Y] =
			    ((FIN_GRID * 256 - (int)po->pp->y / 256) *
			     mem[ide + _Resolution]) /
			    256;
		}
	}
...
void go_to_flag(void)
{
	...
	if (mem[id + _Resolution] < 2) {
		...
		mem[id + _X] =
		    ((int)flags[aux]->x / 65536) * mem[id + _Resolution];
		mem[id + _Y] =
		    ((int)flags[aux]->y / 65536) * mem[id + _Resolution];
	}
...
void set_sector_texture(void)
{
	...
	if (suelo != -1)
		TexAlloc(&new_region->FloorTC, suelo, num_fpg_aux);
	if (techo != -1)
		TexAlloc(&new_region->CeilTC, techo, num_fpg_aux);
}
