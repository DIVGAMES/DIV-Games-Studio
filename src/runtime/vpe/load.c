...
struct PicInfo *GetPic2(int piccode, int num_fpg) {
	...
	//---------------------------------------------------------------------------
		...
		// Preparo el fondo porque no se ha definido
		if(piccode == -1) {
			pic         = (struct PicInfo *)AddEntry(&Pics);
			pic->code   = piccode;
			pic->fpg    = num_fpg;
