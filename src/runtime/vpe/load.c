...
void LoadZone(char *Buffer)
{
	...
	// Prepare WallPtrs
	WallPtrs =
	    (struct Wall **)CacheAlloc(Walls.Number * 2 * sizeof(memptrsize));
...
struct PicInfo *GetPic(int piccode, int num_fpg)
{
	...
	//---------------------------------------------------------------------------
	else if (piccode < 1000 && piccode > 0) {
		sprintf(combo_error, "%s %d %s %d", text[177], piccode,
			text[178], num_fpg);
...
void LoadPic(struct PicInfo *pic)
{
	...
	if (pic->Raw == NULL) {
		...
		while (i < 1000) {
			if (fpg_grf[i] != NULL) {
				n = *(fpg_grf[i] +
				      15); // Numero de puntos de control
				if (pic->code ==
				    *fpg_grf[i]) { // Si lo encuentra termina
					correcto = 1;
					break;
				}
			}
		...
		if (correcto) {
			...
			case 2048:
				...
				break;
				//-----------------------------------------------------------
				// Si no es cuadrada y potencia de 2 genera un
				// cuadrado de 2 x 2 de color negro
				//-----------------------------------------------------------
			default:
				e(158);
			...
			if (*(fpg_grf[i] + 1) != -1) {
				//---------------------------------------------------------------------
				// Roto la textura 90 grados para uso interno de
				// las VPE
				//---------------------------------------------------------------------
				ancho = pic->Width;
				while (ancho) {
...
struct PicInfo *GetPic2(int piccode, int num_fpg)
{
	...
	//---------------------------------------------------------------------------
		...
		if (piccode ==
		    -1) { // Preparo el fondo porque no se ha definido
			pic = (struct PicInfo *)AddEntry(&Pics);
			pic->code = piccode;
			pic->fpg = num_fpg;
			pic->Width = pic->Height = 2;
...
void LoadPic2(struct PicInfo *pic)
{
	...
	if (pic->Raw == NULL) {
		...
		while (i < 1000) {
			if (fpg_aux[i] != NULL) {
				n = *(fpg_aux[i] +
				      15); // Numero de puntos de control
				if (pic->code ==
				    *fpg_aux[i]) { // Si lo encuentra termina
					correcto = 1;
					break;
				}
			}
		...
		if (correcto) {
			...
			//-----------------------------------------------------------------------
			ptr = pic->Raw =
			    (char *)MemRealloc(tex_pointer, &tex_size, size);
			ancho = pic->Width;
			alto = pic->Height;
			for (i = 0; i < ancho; i++) {
				for (j = 0; j < alto; j++) {
		...
		//-------------------------------------------------------------------------
		// Si no encuentra la textura genera un cuadrado de 2 x 2 de
		// color negro
		//-------------------------------------------------------------------------
		else {
			pic->Width = pic->Height = 2;
...
void set_fog_table(int intensidad, int r, int g, int b)
{
	...
	for (i = 0; i < 256; i++) {
		...
		tabla_color[i + 4 * 256] =
		    *(ghost + i * 256 + tabla_color[i + 8 * 256]);
		tabla_color[i + 2 * 256] =
		    *(ghost + i * 256 + tabla_color[i + 4 * 256]);
		tabla_color[i + 1 * 256] =
		    *(ghost + i * 256 + tabla_color[i + 2 * 256]);
		tabla_color[i + 3 * 256] =
		    *(ghost + tabla_color[i + 4 * 256] * 256 +
		      tabla_color[i + 2 * 256]);
		tabla_color[i + 6 * 256] =
		    *(ghost + tabla_color[i + 4 * 256] * 256 +
		      tabla_color[i + 8 * 256]);
		tabla_color[i + 5 * 256] =
		    *(ghost + tabla_color[i + 4 * 256] * 256 +
		      tabla_color[i + 6 * 256]);
		tabla_color[i + 7 * 256] =
		    *(ghost + tabla_color[i + 6 * 256] * 256 +
		      tabla_color[i + 8 * 256]);
		tabla_color[i + 12 * 256] =
		    *(ghost + find_col * 256 + tabla_color[i + 8 * 256]);
		tabla_color[i + 14 * 256] =
		    *(ghost + find_col * 256 + tabla_color[i + 12 * 256]);
		tabla_color[i + 13 * 256] =
		    *(ghost + tabla_color[i + 12 * 256] * 256 +
		      tabla_color[i + 14 * 256]);
		tabla_color[i + 10 * 256] =
		    *(ghost + tabla_color[i + 8 * 256] * 256 +
		      tabla_color[i + 12 * 256]);
		tabla_color[i + 11 * 256] =
		    *(ghost + tabla_color[i + 10 * 256] * 256 +
		      tabla_color[i + 12 * 256]);
		tabla_color[i + 9 * 256] =
		    *(ghost + tabla_color[i + 8 * 256] * 256 +
		      tabla_color[i + 10 * 256]);
...
void LoadMath(void)
{
	...
	for (i = 0; i < DEG360; i++) {
		SinTable[i] = (VPEFixed)(
		    65536.0 * sin((2.0 * PI * (float)i) / (float)DEG360));
		CosTable[i] = (VPEFixed)(
		    65536.0 * cos((2.0 * PI * (float)i) / (float)DEG360));
		// printf("sini %05lu.%05lu cosi: %lu\n",((SinTable[i]<<16) &
		// 0xFFFF0000), SinTable[i]&0xFFFF,CosTable[i]);
	}
	//  printf("tantable: %d\n",sizeof(itangente));
