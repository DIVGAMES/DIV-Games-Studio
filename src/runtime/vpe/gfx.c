...
*****************************************************************************/
void draw_buffer(VPEDword src, VPEDword dest, VPEDword w, VPEDword h,
		 VPEDword delta);
...
void DrawBuffer(struct View *v)
{
	...
	draw_buffer((VPEDword)v->Buffer, Dest, (VPEDword)v->Width >> 2,
		    (VPEDword)v->Height, (VPEDword)(ScrWidth - v->Width));
}

/****************************************************************************
 Internal function for Blitter
*****************************************************************************/
#pragma aux draw_buffer = "cld"                                                \
			  "scan_loop:"                                         \
			  "mov ecx, edx"                                       \
			  "rep movsd"                                          \
			  "add edi, eax"                                       \
			  "dec ebx"                                            \
			  "jnz scan_loop" parm[esi][edi][edx][ebx][eax];
...
void ScrClear(void) { memset((VPEByte *)ScrBase, 0, ScrWidth * ScrHeight); }
