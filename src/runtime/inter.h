...
void put_sprite(int file, int graph, int x, int y, int size, int angle,
		int flags, int reg, byte *, int, int);
...
struct _mouse { // x1
	int x, y, z, file, graph, angle, size, flags, region, left, middle,
	    right, cursor, speed;
};
...
#define end_struct                                                             \
	long_header + 14 + 10 * 10 + 10 * 7 + 8 + 11 + 9 + 10 * 4 + 1026 +     \
	    146 + 32 * 3
...
#define _FCount                                                                \
	18 // Cuenta de llamadas a funcion (para saltarse retornos en frame)
...
#define _M8_NextSector                                                         \
	42	  // Sector que esta detras de la pared con la que colisiona
...
GLOBAL int pila[long_pila + max_exp +
...
GLOBAL int
    inicio_privadas; // Inicio de variables privadas (proceso en ejecución)
...
GLOBAL int dacout_r, dacout_g, dacout_b,
    dacout_speed; // Fade, que restar y a que veloc.

GLOBAL int now_dacout_r, now_dacout_g,
    now_dacout_b; // Situación actual de dac[]
...
// GLOBAL int scr_x,scr_y;   // Posición en copia de la ventana de scroll
// (región) GLOBAL int scr_an,scr_al; // Dimensiones de la ventana de scroll
...
// GLOBAL int map1_x,map1_y,map2_x,map2_y; // Posición del scroll (esq. sup.
// izda.) GLOBAL int block1,block2; // Nº de scanes de la primera porción de los
// buffers
...
GLOBAL void (*putsprite)(byte *si, int x, int y, int an, int al, int xg, int yg,
