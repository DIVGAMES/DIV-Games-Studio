...
//	memcpy(&msg[1],partida_red.datos,
//	        partida_red.longitud_datos);

	//construct the packet
	if(partida_red.servidor == 1 && new_tcpsock) {
		result = SDLNet_TCP_Send(new_tcpsock,
		                         partida_red.datos,
		                         partida_red.longitud_datos);
	}
	else {
		result = SDLNet_TCP_Send(tcpsock,
		                         partida_red.datos,
		                         partida_red.longitud_datos);
	}
	if(result<len) {
//fprintf(stdout,"SDLNet_TCP_Send: %s\n", SDLNet_GetError());
	...
	if(partida_red.dispositivo == IPX) {
		SendPacketToAll(partida_red.player_id,
		               (void *)((int)partida_red.datos + partida_red.player_id * partida_red.longitud_datos),
		                partida_red.longitud_datos);
	}
	else {
		COM_Send(partida_red.servidor,
		         0,
		         partida_red.datos,
		         partida_red.longitud_datos);
	}
...
//	longitud /= net->num_players;
	...
	//---------------------------------------------------------------------------
	if(num_games = net_get_games(game_id)) {
...
//	memset(partida_red.datos,
//	       1,
//	       result);

//	memcpy(partida_red.datos,
//	       msg,
//	       result);
}
...
//	partida_red.player_id   = player_id;
	...
	c = net_join_game(game_id,
	                 (char *)&mem[text_offset + nombre],
	                 (byte *)&mem[datos],
	                  longitud);
