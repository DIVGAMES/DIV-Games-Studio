...
int net_init_ipx(int game_id)
{
	...
	//---------------------------------------------------------------------------
	// Compruebo si se inició la comunicacion, en caso afirmativo se
	// finaliza
	//---------------------------------------------------------------------------
...
*/
	...
	partida_red.servidor =
	    dial; // El que marca es uno y el que recibe la llamada 0.
...
void net_send()
{
  ...
  if (partida_red.servidor)
    printf("Jugadores: %d, NumPlayer: %d, Dir: %s, Soy el
  Servidor\n",partida_red.num_players,partida_red.player_id,ShowInternalAddress(&IPX_MyAddress));
  else
    printf("Jugadores: %d, NumPlayer: %d, Dir: %s, Soy un
  Nodo\n",partida_red.num_players,partida_red.player_id,ShowInternalAddress(&IPX_MyAddress));
*/
	if (!partida_red.activa ||
	    (partida_red.num_players < 2 && partida_red.dispositivo == IPX))
		return;
	...
	if (partida_red.servidor == 1 && new_tcpsock)
		result = SDLNet_TCP_Send(new_tcpsock, partida_red.datos,
					 partida_red.longitud_datos);
	else
		result = SDLNet_TCP_Send(tcpsock, partida_red.datos,
					 partida_red.longitud_datos);

	if (result < len) {
		// fprintf(stdout,"SDLNet_TCP_Send: %s\n", SDLNet_GetError());
		// It may be good to disconnect sock because it is likely
		// invalid now.
	}
#ifdef NOTYET
	if (partida_red.dispositivo == IPX) {
		SendPacketToAll(partida_red.player_id,
				(void *)((int)partida_red.datos +
					 partida_red.player_id *
					     partida_red.longitud_datos),
				partida_red.longitud_datos);
	} else {
		COM_Send(partida_red.servidor, 0, partida_red.datos,
			 partida_red.longitud_datos);
	}
#endif
}
...
int net_create_game(int game_id, char *nombre, int num_jugadores)
{
	...
	if (num_jugadores < 0 || num_jugadores >= MAX_USERS)
		...
		//---------------------------------------------------------------------------
		// Compruebo si el número de jugadores es correcto, en caso
		// contrario retorna
		//---------------------------------------------------------------------------
#ifdef NOTYET
	if (partida_red.dispositivo == IPX) {
...
void net_join_game(void)
{
	...
	c = _net_join_game(game_id, (char *)&mem[text_offset + nombre],
			   (byte *)&mem[datos], longitud);
	pila[sp] = c;
}
...
int join_game(int num_games) { return 1; }
...
#define MAXLEN partida_red.longitud_datos
	...
	if (partida_red.servidor) {
		...
		if (!new_tcpsock) {
			...
			if (result != MAXLEN) {
				// An error may have occured, but sometimes you
				// can just ignore it It may be good to
				// disconnect sock because it is likely invalid
				// now.
				return;
			}
		}
		...
		if (result != MAXLEN) {
			// An error may have occured, but sometimes you can just
			// ignore it It may be good to disconnect sock because
