...
fprintf(stdout, "[RUNTIME LIBRARY - READ_PACKFILE] trying to read %s from %d files\n", file, npackfiles);
		...
		if((packptr = (byte *)malloc(len_desc)) != NULL) {
			if((ptr = (char *)malloc(packdir[n].len)) != NULL) {
				...
				if(f != NULL) {
					...
					{
fprintf(stdout,
        "[RUNTIME LIBRARY - READ_PACKFILE] looking for %s against %s - data:\n\t-> packdir.offset: %d\n\t-> packdir.len: %d\n\t-> packdir.len_desc: %d\n\n",
        full,
        packdir[n].filename,
        packdir[n].offset,
        packdir[n].len,
...
//fprintf(stdout, "[RUNTIME - LIBRARY] unload_map ERROR: BAD ID\n");
//fflush(stdout);
		return;
	}

	if(g[0].grf[pila[sp]]) {
		free((void *)(((byte *)g[0].grf[pila[sp]]) - 1330));
//printf("Freeing %lx\n ", (((byte *)g[0].grf[pila[sp]]) - 1330));
//fprintf(stdout, "[RUNTIME - LIBRARY] unload_map ID: %d (%X)\n", pila[sp], (((byte *)g[0].grf[pila[sp]]) - 1330));
//fflush(stdout);

		g[0].grf[pila[sp]] = NULL;
//fprintf(stdout, "[RUNTIME - LIBRARY] unload_map g[0].grf[%d]: %X\n", pila[sp], (((byte *)g[0].grf[pila[sp]]) - 1330));
//fflush(stdout);
	}
}
...
void DIV_stop_scroll(void) {
	...
	iscroll[snum].on    = 0;
	iscroll[snum].fast  = NULL; // Fix: make sure all pointers are resetted so "finalizacion" function don't crash attempting to free invalid memory.
	iscroll[snum]._scr_1 = NULL;
	iscroll[snum]._scr_2 = NULL;
...
void DIV_write_in_map(void) {
	...
	if((ptr = (byte *)malloc(1330 + 64 + 4 + an * al)) != NULL) {
		...
		*((word *)ptr + 33) = cy;
		...
		pila[sp] = next_map_code;
//fprintf(stdout, "[RUNTIME - LIBRARY] write_in_map ID: %d (%X = %X)\n", next_map_code, g[0].grf[next_map_code], ptr);
//fflush(stdout);
	}
	else {
		e(100);
