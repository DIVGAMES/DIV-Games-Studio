...
FILE *fpopen(byte *file, char *mode) { return __fpopen(file, mode); }
		...
		if ((packptr = (byte *)malloc(len_desc)) != NULL) {
			if ((ptr = (char *)malloc(packdir[n].len)) != NULL) {
				...
				if (f != NULL) {
					...
					fclose(f);
#ifdef ZLIB
					if (!uncompress(packptr, &len_desc,
							(byte *)ptr,
							packdir[n].len))
#else
					if (false)
...
void load_pal(void)
{
	...
	if (strcmp((char *)pal, "pal\x1a\x0d\x0a"))	 // not a pal file
		if (strcmp((char *)pal, "fpg\x1a\x0d\x0a")) // not an fpg
			if (strcmp((char *)pal,
				   "fnt\x1a\x0d\x0a")) // not a fnt file

				if (strcmp(
					(char *)pal,
					"map\x1a\x0d\x0a")) { // not a map file

					if (es_PCX(
						(byte *)pal)) { // Take the PCX
								// palette
...
void new_map(void)
{
	...
	if ((ptr = (byte *)malloc(1330 + 64 + 4 + ancho * alto)) != NULL) {
		...
		*((int *)ptr + 14) = alto;
		*((int *)ptr + 15) =
		    1; // Se define un punto de control (el centro)
		*((word *)ptr + 32) = cx;
		*((word *)ptr + 33) = cy;
...
void load_fpg(void)
{
	...
	if (m != palcrc) {
		xlat[0] = 0;
		for (n = 1; n < 256; n++) {
			find_color(ptr[n * 3 + 8], ptr[n * 3 + 9],
				   ptr[n * 3 + 10]);
			xlat[n] = find_col;
		}
	}
	...
	while (ptr <= (ptr2 + file_len) && *(int *)ptr3 < 1000 &&
	       *(int *)ptr3 > 0) {
...
void start_scroll(void)
{
	...
	if (ptr2 == NULL)
		...
		s = 2; // Tipo de scroll, normal(1) o parallax(2)
	...
	if ((iscroll[snum]._sscr1 = (byte *)malloc(
		 iscroll[snum].an * (iscroll[snum].al + 1))) == NULL) {
		e(100);
		return;
	}
	if ((iscroll[snum].fast =
		 (tfast *)malloc(iscroll[snum].al * sizeof(tfast))) == NULL) {
		e(100);
		return;
	}
	...
	if (s == 2) {
		...
		if ((iscroll[snum]._sscr2 = (byte *)malloc(
			 iscroll[snum].an * (iscroll[snum].al + 1))) == NULL) {
	...
	iscroll[snum].on =
	    s; // Al final si no ha habido errores, fija la variable scroll
...
void get_id(void)
{
	...
	do {
		if (i != id && mem[i + _Bloque] == bloque &&
		    (mem[i + _Status] == 2 || mem[i + _Status] == 4)) {
...
void fade(void)
{
	...
	if (now_dacout_r != dacout_r || now_dacout_g != dacout_g ||
...
void load_fnt(void)
{
	...
	if (adaptar_paleta) {
		adaptar(fonts[ifonts] + 1356 + sizeof(TABLAFNT) * 256,
			f_i[ifonts].len - 1356 - sizeof(TABLAFNT) * 256,
			fonts[ifonts] + 8, NULL);
...
void checkpal_font(int ifonts)
{
	...
	if (f_i[ifonts].syspal != palcrc) {
		...
		if (f_i[ifonts].syspal !=
		    f_i[ifonts].fonpal) { // Debe "recargarlo"

			if (npackfiles) {
				file_len =
				    read_packfile((byte *)&mem[pila[sp]]);
				if (file_len == -1)
					goto fntfuera;
				if (file_len == -2)
					return;
...
void adaptar(byte *ptr, int len, byte *pal, byte *xlat)
{
	...
	if (adaptar_paleta) {
		...
		if (xlat == NULL) {
			...
			for (n = 1; n < 256; n++) {
				find_color(pal[n * 3 + 0], pal[n * 3 + 1],
					   pal[n * 3 + 2]);
				xlat[n] = find_col;
			}
		}
...
void __write(void)
{
	...
	while (texto[x].font != NULL) {
		...
		if (pila[sp - 1] == texto[x].centro &&
		    pila[sp - 2] == texto[x].y && pila[sp - 3] == texto[x].x)
			break;
	}
...
void _xput(void)
{
	...
	put_sprite(file, graf, x, y, angle, size, flags, reg, copia2, vga_an,
		   vga_al);
}
...
void map_xput(void)
{
	...
	if ((ptr = g[file].grf[graf1]) != NULL) {
		put_sprite(file, graf2, x, y, angle, size, flags, -1,
			   (byte *)ptr + 64 + ptr[15] * 4, ptr[13], ptr[14]);
	} else
		e(121);
}
...
void map_put(void)
{
	...
	if ((ptr = g[file].grf[graf1]) != NULL) {
		put_sprite(file, graf2, x, y, 0, 100, 0, -1,
			   (byte *)ptr + 64 + ptr[15] * 4, ptr[13], ptr[14]);
	} else
		e(121);
}
...
void map_block_copy(void)
{
	...
	if ((ptrd = g[file].grf[grafd]) != NULL) {
		if ((ptr = g[file].grf[graf]) != NULL) {
			...
			if (clipx0 >= vga_an || clipx1 <= 0)
				goto no;
			if (clipy0 >= vga_al || clipy1 <= 0)
				goto no;
			...
			if (x >= clipx0 && x + an <= clipx1 && y >= clipy0 &&
			    y + al <= clipy1) // Pinta sprite sin cortar
				sp_normal(si, x, y, an, al, 0);
			else if (x < clipx1 && y < clipy1 && x + an > clipx0 &&
				 y + al > clipy0) // Pinta sprite cortado
...
void load(void)
{
	...
	if (fbytes != lon) {
		// fprintf(stdout,"Bytes read: %d bytes wanted: %d len: %d
...
#ifdef DEBUG
	if (v.tipo)
		new_mode = 1; // Avisa al debugger de un cambio de modo de v�deo
#endif
...
void unload_pcm(void) { UnloadSound(pila[sp]); }
...
void unload_song(void) { UnloadSong(pila[sp]); }

//����������������������������������������������������������������������������
//      song(id_cancion)
//����������������������������������������������������������������������������

void song(void) { PlaySong(pila[sp]); }
...
void set_song_pos(void) { SetSongPos(pila[sp]); }

//����������������������������������������������������������������������������
//      get_song_pos()
//����������������������������������������������������������������������������

void get_song_pos(void) { pila[++sp] = GetSongPos(); }

//����������������������������������������������������������������������������
//      get_song_line()
//����������������������������������������������������������������������������

void get_song_line(void) { pila[++sp] = GetSongLine(); }

//����������������������������������������������������������������������������
//      is_playing_sound(id_canal)
//����������������������������������������������������������������������������

void is_playing_sound(void) { pila[sp] = IsPlayingSound(pila[sp] - 1); }

//����������������������������������������������������������������������������
//      is_playing_song()
//����������������������������������������������������������������������������

void is_playing_song(void) { pila[++sp] = IsPlayingSong(); }
...
void frame_fli(void) { pila[++sp] = Nextframe(); }
...
void _abs(void) { pila[sp] = abs(pila[sp]); }
...
void _exit_dos(void)
{
	...
	if ((f = fopen("system/exec.err", "wb")) != NULL) {
		...
		fwrite(&mem[pila[sp - 1]], 1,
		       strlen((char *)(&mem[pila[sp - 1]])) + 1, f);
		fclose(f);
	}
#else
	printf("%s\n", (char *)&mem[pila[sp - 1]]);
...
int joy_position(int eje)
{
	...
	if (ej[eje] == -1)
		...
		if (result > 0 && result < 25600) {
			if (joy_filter < 0)
				joy_filter = 0;
			else if (joy_filter > 100)
				joy_filter = 100;
			ej[eje] = (result * (100 - joy_filter) +
				   ej[eje] * joy_filter) /
				  100;
		}
		result = ej[eje];
...
void read_joy(void)
{
	...
	if (init_joy <= 10) {
		...
		if (x == 0 || y == 0)
			return; // Los timeout no los tiene en cuenta
...
void convert_palette(void)
{
	...
	if (!capar(pal_ofs) || !capar(pal_ofs + 256)) {
		e(136);
		return;
	}
	if (file < 0 || file > max_fpgs) {
...
void _strcpy(void)
{
	...
	if ((unsigned)pila[sp] > 255)
		if ((mem[pila[sp - 1] - 1] & 0xFFFFF) + 1 <
		    strlen((char *)&mem[pila[sp]])) {
			sp--;
			e(140);
			return;
		}
...
void _strcat(void)
{
	...
	if ((unsigned)pila[sp] > 255)
		...
		n = 1;
	if ((mem[pila[sp - 1] - 1] & 0xFFFFF) + 1 <
	    strlen((char *)&mem[pila[sp - 1]]) + n) {
		sp--;
		e(140);
		return;
	}
	if ((unsigned)pila[sp] > 255)
		strcat((char *)&mem[pila[sp - 1]], (char *)&mem[pila[sp]]);
	else
		sprintf((char *)&mem[pila[sp - 1]], "%s%c%c",
			(char *)&mem[pila[sp - 1]], pila[sp], '\0');
	sp--;
}
...
void _strcmp(void)
{
	if ((unsigned)pila[sp - 1] > 255) {
		...
		if ((unsigned)pila[sp] > 255) {
			...
			pila[sp - 1] =
			    strcmp((char *)&pila[sp - 1], (char *)&pila[sp]);
		}
	}
	sp--;
}
...
{ // (cadena,n,m) borra <n> char del inicio y <m> del final
	...
	if ((mem[pila[sp] - 1] & 0xFFFFF) + 1 <
	    strlen((char *)&mem[pila[sp]]) - n - m) {
		e(140);
		return;
	}
...
{
	return (
	    strcmp((char *)a + offset_clave * 4, (char *)b + offset_clave * 4));
}

int sort3(const void *a, const void *b)
{
	return (-strcmp((char *)a + offset_clave * 4,
			(char *)b + offset_clave * 4));
}
...
{
	return ((int)(xlat_rnd[*(a + offset_clave * 4)] ^
		      (xlat_rnd[*(a + 1 + offset_clave * 4)] / 2) ^
		      (xlat_rnd[*(a + 2 + offset_clave * 4)] / 4) ^
		      (xlat_rnd[*(a + 3 + offset_clave * 4)] / 8)) -
		(int)(xlat_rnd[*(b + offset_clave * 4)] ^
		      (xlat_rnd[*(b + 1 + offset_clave * 4)] / 2) ^
...
/*
function 122 int fopen(0,0)         // Casi como en C (filename,"rwa+") (siempre
modo binario) function 123 int fclose(0)          // Casi como en C (handle)
pero si handle es 0 hace un fcloseall function 124 int fread(0,0,0)       //
Casi como en C (&buffer,longitud(en ints),handle) pero con s�lo una longitud de
ints function 125 int fwrite(0,0,0)      // Inversa a fread
(&buffer,longitud,handle) function 126 int fseek(0,0,0)       // Id�ntica a C
(handle,posicion,modo) (modo seek_set/cur/end) function 127 int ftell(0) //
Id�ntica a C (handle) function 128 int filelength(0)      // Longitud de un
fichero abierto (handle) function 129 int flush()            // Vacia buffers de
escritura y devuelve n� de ficheros abiertos

function 130 int get_dirinfo(0,0)   // Lee un directorio
("dir\*.pr?",_hidden+_system+_subdir) en la estructura dirinfo(files,name[]) y
devuelve "files" (n�mero) function 131 string get_fileinfo(0) // Rellena la
estructura fileinfo (fullpath (o cwd), drive, dir, name, ext, size, date, time,
attrib) y devuelve puntero a "fullpath" o "cwd"
...
function 135 int mkdir(0)           // Crea un directorio nuevo (a partir del
cwd) function 136 int remove(0)          // Borra archivos o directorios (se
aceptan comodines) (auto rmdir) function 137 int disk_free(0)       // Espacio
disponible en una unidad (en Kbs) function 138 int memory_free()      // Memoria
disponible (en Kbs) function 139 int ignore_error(0)    // Ignorar un error de
ejecuci�n (n�mero)
*/
...
{ // Busca el archivo, ya que puede haber sido incluido en la instalaci�n
	...
	if (f == NULL) {
		if ((f = fopen(full, modo)) == NULL) { // "paz\fixero.est"
			if (_fullpath(full, (char *)&mem[pila[sp]],
				      _MAX_PATH) == NULL) {
				pila[sp] = 0;
				return;
			}
			...
			if ((f = fopen(full, modo)) ==
			    NULL) { // "est\paz\fixero.est"
				strcpy(full, fname);
				strcat(full, ext);
				if ((f = fopen(full, modo)) ==
				    NULL) { // "fixero.est"
...
void _ftell(void)
{
	...
	pila[sp] =
	    (int)(ftell((FILE *)tabfiles[pila[sp] / 2]) + unit_size - 1) /
	    unit_size;
}
...
void __filelength(void)
{
	...
	len =
	    (ftell((FILE *)tabfiles[pila[sp] / 2]) + unit_size - 1) / unit_size;
...
void get_fileinfo(void)
{
	...
	rc = _dos_findfirst(filename,
			    _A_NORMAL | _A_SYSTEM | _A_HIDDEN | _A_SUBDIR, &ft);
	if (rc)
		return;
...
void remove_file(void)
{
	...
	rc = _dos_findfirst((char *)&mem[pila[sp]],
			    _A_NORMAL | _A_SYSTEM | _A_HIDDEN, &ft);
	while (!rc) {
...
/*
function 137 int disk_free(0)                      // Espacio disponible en una
(unidad), en bytes function 138 int memory_free()                     // Memoria
disponible (en Kbs) function 139 int ignore_error(0)                   //
Ignorar un error de ejecuci�n (n�mero)
*/
...
void disk_free(void)
{
	...
	if (!(regs.w.cflag & INTR_CF)) {       // si no hubo error
		if (!(regs.w.dx & (1 << 9))) { // i/o no permitida
			...
			MBfree = structdiskfree.sectors_per_cluster *
				 structdiskfree.bytes_per_sector;
...
void delete_draw(void)
{
	...
	if ((draw_id = pila[sp]) == -1) {
		...
		if ((draw_id & 1) == 0 || draw_id < 1 ||
		    draw_id > max_drawings * 2 - 1) {
			pila[sp] = 0;
			e(175);
			return;
		}
...
void move_draw(void)
{
	...
	if (drawing[draw_id].porcentaje < 0 ||
	    drawing[draw_id].porcentaje > 15) {
		drawing[draw_id].tipo = 0;
		e(174);
	}
}
...
void write_in_map(void)
{
	...
	while (*ptr2 && cx + fnt[*ptr2].ancho <= an) {
		if (fnt[*ptr2].ancho == 0) {
			...
			texn2(ptr + 68, an, fonts[fuente] + fnt[*ptr2].offset,
			      cx, fnt[*ptr2].incY, fnt[*ptr2].ancho,
			      fnt[*ptr2].alto);
...
double evaluar(void)
{
	...
	do {
		...
		case p_or:
			pila[sp - 1] =
			    (double)((int)pila[sp - 1] | (int)pila[sp]);
			sp--;
			break;
		case p_xor:
			pila[sp - 1] =
	...
	reget_token:
		...
		default:
			if (tolower(*(expresion - 1)) >= 'a' &&
				...
				while (n < 31 && tolower(*expresion) >= 'a' &&
				       tolower(*expresion) <= 'z') {
...
double get_num(void)
{ // Lee el n�mero que hay en *expresion (double en hex o dec)
	double x = 0;
	double dec = 10;

	if (*expresion == '0' &&
	    tolower(*(expresion + 1)) == 'x') { // Numeros en hex
...
int capar(int dir)
{
	...
	if (dir > 0 && dir <= imem_max)
		...
		for (n = 0; n < 256; n++) {
			if (dir >= divmalloc[n].imem1 &&
			    dir <= divmalloc[n].imem2)
				return (dir);
		}
		return (0);
	}
...
char *fname[] = {
    "signal", "key", "load_pal", "load_fpg", "start_scroll", "stop_scroll",
    "out_region", "graphic_info", "collision", "get_id", "get_distx",
    "get_disty", "get_angle", "get_dist", "fade", "load_fnt", "write",
    "write_int", "delete_text", "move_text", "unload_fpg", "rand",
    "define_region", "xput", "put", "put_screen", "map_xput", "map_put",
    "put_pixel", "get_pixel", "map_put_pixel", "map_get_pixel", "get_point",
    "clear_screen", "save", "load", "set_mode", "load_pcm/wav",
    "unload_pcm/wav", "sound", "stop_sound", "change_sound", "set_fps",
    "start_fli", "frame_fli", "end_fli", "reset_fli", "system",
    "refresh_scroll", "fget_dist", "fget_angle", "play_cd", "stop_cd",
    "is_playing_cd", "start_mode7", "stop_mode7", "advance", "abs", "fade_on",
    "fade_off", "rand_seed", "sqrt", "pow", "map_block_copy", "move_scroll",
    "near_angle", "let_me_alone", "exit", "roll_palette", "get_real_point",
    "get_joy_button", "get_joy_position", "convert_palette", "load_map/pcx",
    "reset_sound", "unload_map/pcx", "unload_fnt", "set_volume",

    // Nuevas funciones a�adidas para DIV 2.0

    "set_color", "net_join_game", "net_get_games", "stop_mode8", "x_advance",
    "char", "path_find", "path_line", "path_free", "new_map", "load_wld",
    "start_mode8", "go_to_flag", "set_sector_height", "get_sector_height",
    "set_point_m8", "get_point_m8", "set_fog", "set_sector_texture",
    "get_sector_texture", "set_wall_texture", "get_wall_texture",
    "set_env_color", "strcpy", "strcat", "strlen", "strcmp", "strchr", "strstr",
    "strset", "strupr", "strlwr", "strdel", "screen_copy", "sort", "load_song",
    "unload_song", "song", "stop_song", "set_song_pos", "get_song_pos",
    "get_song_line", "is_playing_sound", "is_playing_song", "fopen", "fclose",
    "fread", "fwrite", "fseek", "ftell", "filelength", "flush", "get_dirinfo",
    ...
    "save_map", "write_in_map", "calculate", "itoa", "change_channel", "malloc",
    "free", "encode", "encode_file", "decode_file", "compress_file",
    "uncompress_file", "find_color", "load_screen", "force_pal"};
