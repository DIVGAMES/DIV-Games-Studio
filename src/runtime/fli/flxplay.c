...
{ /* Initialize SDL
   */
	 ...
	 */
	if ((flc.mainscreen =
		 SDL_SetVideoMode(flc.screen_w, flc.screen_h, flc.screen_depth,
				  (SDL_HWPALETTE | SDL_FULLSCREEN))) == NULL) {
		fprintf(stderr, "SDL: Couldn't set video mode %dx%dx%d: %s\n",
			flc.screen_w, flc.screen_h, flc.screen_depth,
			SDL_GetError());
		exit(3);
	}
	...
	while (Lines--) {
		...
		while (Count & 0xc000) {
			...
			if ((Count & 0xc000) ==
				...
				pDst +=
				    (0x10000 - Count) * flc.mainscreen->pitch;
			}
