...
{ // Detecta los modos de vídeo disponibles
		...
		if (vbeGetModeInfo(*modelist) == 0) {
			if (ModeInfoBlock.BitsPerPixel == 8 &&
				...
				if (n == num_video_modes) {
					video_modes[num_video_modes].ancho =
					    ModeInfoBlock.XResolution;
					video_modes[num_video_modes].alto =
					    ModeInfoBlock.YResolution;
					video_modes[num_video_modes].modo =
					    *modelist;
					num_video_modes++;
				} else {
	...
	qsort((void *)&(video_modes[0].ancho), num_video_modes,
	      sizeof(struct _video_modes), compare_mode);
}
