...
void dirty(int did)
{
	...
	if (pf != NULL) {
		...
		//		printf("Re-indexing for (oldid) %d id %d %d
...
void inicializacion(void)
{
	...
	m8 = (struct _m8
		  *)&mem[long_header + 14 + 10 * 10 + 10 * 7 + 8 + 11 + 9];
	dirinfo = (struct _dirinfo *)&mem[long_header + 14 + 10 * 10 + 10 * 7 +
					  8 + 11 + 9 + 10 * 4];
	fileinfo =
	    (struct _fileinfo *)&mem[long_header + 14 + 10 * 10 + 10 * 7 + 8 +
				     11 + 9 + 10 * 4 + 1026];
	video_modes =
	    (struct _video_modes *)&mem[long_header + 14 + 10 * 10 + 10 * 7 +
					8 + 11 + 9 + 10 * 4 + 1026 + 146];
	/*
...
*/
	iloc = mem[2]; // Start of local variables | Inicio de la imagen de las
		       // variables locales
	iloc_len =
	    mem[6] + mem[5]; // Length of local ( public and private ) |
			     // Longitud de las locales (p�blicas y privadas)
	iloc_pub_len = mem[6]; // Length of local public variables | Longitud de
			       // las variables locales p�blicas
	inicio_privadas = iloc_pub_len;
	imem = mem[8]; // Final code , local and texts ( length cmp ) | Final de
		       // c�digo, locales y textos (longitud del cmp)
	...
	//  if((texto=(struct t_texto *)malloc(sizeof(struct
	//  t_texto)*max_textos))==NULL) exer(1);
...
// setup linked list
	...
	for (n = 0; n < num_video_modes; n++) {
		if (video_modes[n].ancho == vga_an &&
		    video_modes[n].alto == vga_al) {
			break;
		}
	}
	...
	for (n = 0; n < 10;
	     n++) { // En un principio no hay sistema de scroll, ni de m7
		(scroll + n)->z = 512;
		(scroll + n)->ratio = 200;
		(scroll + n)->region1 = -1;
		(scroll + n)->region2 = -1;
	...
	// DLL_3 Exportaci�n de funciones y variables (para utilizarlas en las
	// DLL)
#ifdef DIVDLL
	// Exportadas desde 'C'
	// files
...
//      actualiza_pila(id,valor);       // Pone un valor al final de la pila
//      (rtf)
...
#ifdef NETPLAY
			_net_loop(); // Process netplay routine before calling
				     // process
#endif
			id = ide;
			ip = mem[id + _IP];
...
void frame_start(void)
{
	...
	if (ss_status && ss_frame != NULL) {
		...
		if (get_reloj() > ss_time_counter) {
			...
			do {
				...
				mou_check = mouse->x + mouse->y + mouse->left +
	...
	if (get_reloj() > ultimo_reloj) {
		ffps = (ffps * 9.0f +
			1000.0f / (float)(get_reloj() - ultimo_reloj)) /
		       10.0f;
		fps = (int)(ffps + 0.5f);
	}
	...
	if (overall_reloj) {
		...
		if (ffps2 > 0)
			ffps2 = (ffps2 * 3.0f +
				 (double)4600.0 / (game_ticks / game_frames)) /
				4.0f;
		else
	...
	if (get_reloj() >
	    (freloj +
	     ireloj / 3)) { // Permite comerse hasta un tercio del sgte frame
...
//			usleep(((int)freloj-old_reloj)-1);
	...
	if (joy_status == 1 && joy_timeout >= 6) {
		...
		read_joy();
		joy_check = joy->button1 + joy->button2 + joy->left +
			    joy->right + joy->up + joy->down;
	...
	mou_check =
	    mouse->x + mouse->y + mouse->left + mouse->right + mouse->middle;
...
void frame_end(void)
{
...
#ifdef __EMSCRIPTEN__
	sprintf(buf, "$('#fps').text(\"FPS: %d/%d (max frameskip: %d)\");", fps,
		dfps, max_saltos);
	...
	if (!dll_loaded) {
		...
		process_palette =
		    (void (*)())DIV_import("process_palette"); // ok
		process_active_palette =
		    (void (*)())DIV_import("process_active_palette"); // ok

		process_sound =
		    (void (*)(char *, int))DIV_import("process_sound"); // ok

		process_fpg =
		    (void (*)(char *, int))DIV_import("process_fpg"); // ok
		process_map =
		    (void (*)(char *, int))DIV_import("process_map"); // ok
		process_fnt =
		    (void (*)(char *, int))DIV_import("process_fnt"); // ok

		background_to_buffer =
		    (void (*)())DIV_import("background_to_buffer"); // ok
		buffer_to_video =
		    (void (*)())DIV_import("buffer_to_video"); // ok

		post_process_scroll =
		    (void (*)())DIV_import("post_process_scroll"); // ok
		post_process_m7 =
		    (void (*)())DIV_import("post_process_m7"); // ok
		post_process_buffer =
		    (void (*)())DIV_import("post_process_buffer"); // ok
		pre_process_buffer =
		    (void (*)())DIV_import("pre_process_buffer");      // ok
		post_process = (void (*)())DIV_import("post_process"); // ok

		putsprite = (void (*)(byte *, int, int, int, int, int, int, int,
				      int, int))DIV_import("put_sprite"); // ok
	...
	// Si el usuario modific� mouse.x o mouse.y, posiciona el rat�n
	// debidamente
#ifndef SDL2
	...
	if (!saltar_volcado) {
		...
		// *** OJO *** Restaura las zonas de copia fuera del scroll y
		// del modo 7
		...
		if (restore_type == 0 || restore_type == 1) {
			if (!iscroll[0].on || iscroll[0].x || iscroll[0].y ||
			    iscroll[0].an != vga_an ||
			    iscroll[0].al != vga_al) {
		...
		do {
			...
			for (n = 0; n < 10; n++)
				if (iscroll[n].on && (scroll + n)->z >= max &&
				    !iscroll[n].painted) {
					scrollide = n + 1;
					max = (scroll + n)->z;
				}
			...
			if (otheride) {
				if (otheride == 1) {
					...
					x1s = -1;
					v_function =
					    -1; // No errors (don't show?)
					put_sprite(mouse->file, mouse->graph,
						   mouse->x, mouse->y,
						   mouse->angle, mouse->size,
						   mouse->flags, mouse->region,
						   copia, vga_an, vga_al);
					mouse_x0 = x0s;
					mouse_x1 = x1s;
					mouse_y0 = y0s;
		...
		if(ide || m7ide || scrollide || otheride)
			printf("ide: %d, m7ide: %d, scrollide: %d, otheride:
		%s\n", ide,m7ide,scrollide,
...
*/
		...
		for (id = id_start; id <= id_end; id += iloc_len)
			if ((mem[id + _Status] == 2 ||
			     mem[id + _Status] == 4) &&
			    mem[id + _Ctype] == 0 && !mem[id + _Executed]) {
		...
		if (n < max_textos) {
			memb[nullstring[0] * 4] =
			    0; // El texto "en el aire" no se muestra nunca
			memb[nullstring[1] * 4] = 0;
			memb[nullstring[2] * 4] = 0;
			memb[nullstring[3] * 4] = 0;
			pinta_textos(0);
		...
		if (debugger_step) {
			...
			if (buffer_to_video != NULL) {
				...
				if (old_dump_type) {
					...
					// A�ade los volcados de este frame a
					// los restore del anterior

					for (n = id_start; n <= id_end;
					     n += iloc_len)
						if (mem[n + _x1] != -1)
							volcado_parcial(
							    mem[n + _x0],
		...
		do {
			...
			if (mem[id2 + _BigBro] == 0) {
				if (mem[id_init + _Son] != 0) {
					mem[id2 + _BigBro] =
					    mem[id_init + _Son];
					mem[mem[id_init + _Son] + _SmallBro] =
					    id2;
				}
				id2 = 0;
			} else
...
void exer(int e)
{
	...
	// m�ximo de procesos = %u", procesos,(id_end-id_start)/iloc_len+1);
#ifdef NETPLAY
	if (inicializacion_red)
		net_end();
#endif
...
//	freopen( "CON", "w", stderr );
...
#ifndef DROID
	if (argc < 2 && exesize == 0) {
		printf("DIV2015 Run time library - version 2.02a - "
	...
	if (argc > 1 && exesize == 0) {
		if ((f = fopen(argv[1], "rb")) == NULL) {
			...
			_dos_setdrive((int)toupper(*divpath) - 'A' + 1,
				      &divnum);
			chdir(divpath);
...
//#endif
	...
	if (DIV_VER == 'D') {
		...
		if (mimem[3] > 0) {
			imem_max =
			    mimem[8] + mimem[3] * (iloc_len) + iloc_len + 2;
		} else {
		...
		// printf("ptrsize %d\nmem: %x iloc: %d iloc_len: %d iloc_max
		// %d\nvars:
...
#define MEM_MULTI 2
		...
		if ((mem = (int *)malloc(4 * imem_max + 1032 * 5 + 16 * 1025 +
					 3)) != NULL) {
			...
			if ((ptr = (byte *)malloc(len)) != NULL) {
				...
				len_descomp = mem[9];
#ifdef ZLIB
				if (!uncompress((unsigned char *)&mem[9],
						&len_descomp, ptr, len))
#else
				if (false)
#endif
				{
					...
					if (m) {
						//	fwrite((uint32_t
						//*)&mem[9],1,len_descomp,m);
						fwrite((uint32_t *)mem, 1,
						       len_descomp + 80, m);
						fclose(m);
					}
#endif
					...
					if ((_argc = argc - 1) > 10)
						_argc = 10;
					for (n = 0; n < _argc && n < argc - 1;
					     n++) {
						memcpy(&mem[i], argv[n + 1],
						       strlen(argv[n + 1]) + 1);
						_argv(n) = i;
						i += (strlen(argv[n + 1]) + 4) /
						     4;
					...
					if (OSDEP_NumJoysticks() > 0) {
						...
						printf("NumAxes: %d: Numhats: "
						       "%d : NumButtons: %d\n",
						       OSDEP_JoystickNumAxes(
							   divjoy),
						       OSDEP_JoystickNumHats(
							   divjoy),
						       OSDEP_JoystickNumButtons(
							   divjoy));
#endif
						if (OSDEP_JoystickNumHats(
							divjoy) == 0) {
							OSDEP_JoystickClose(
							    divjoy);
							divjoy = NULL;
					...
					interprete();
#ifndef __EMSCRIPTEN__
					_dos_setdrive((int)toupper(*divpath) -
							  'A' + 1,
						      &divnum);
					chdir(divpath);

					exit(
					    26); // Exit without clearing screen
#endif
				} else {
					free(ptr);
					exer(1);
...
int is_pak(FILE *f, char *name)
{
	...
	if (!strcmp(head, "dat\x1a\x0d\x0a") && nfiles > 0) {
		if (prg_id == id[0] || prg_id == id[1] || prg_id == id[2]) {
			packdir = (struct _packdir *)malloc(
			    nfiles * sizeof(struct _packdir));
			// printf("packdir: %x %d\n",packdir,nfiles);
			if (packdir != NULL) {
				if (fread(packdir, sizeof(struct _packdir),
					  nfiles, f) == nfiles) {
					for (n = 0; n < nfiles; n++) {
						strcpy(buf,
						       packdir[n].filename);
		...
		if (unzGoToFirstFile(zip) == UNZ_OK) {
			do {
				if (unzGetCurrentFileInfo(
					zip, &fileInfo, szCurrentFileName,
