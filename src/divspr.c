...
struct t_listbox lanim_spr_hombre = {3,  95 + 20, (char *)anim_spr_hombre,
				     20, 7,       93};
struct t_listbox lanim_spr_hombre_a = {3,  95 + 20, (char *)anim_spr_hombre_a,
				       20, 7,       93};
struct t_listbox lanim_spr_mujer = {3,  95 + 20, (char *)anim_spr_mujer,
				    20, 7,       93};
struct t_listbox lanim_spr_mujer_a = {3,  95 + 20, (char *)anim_spr_mujer_a,
				      20, 7,       93};
struct t_listbox lanim_spr_enano = {3,  95 + 20, (char *)anim_spr_enano,
				    20, 7,       93};
struct t_listbox lanim_spr_enano_a = {3,  95 + 20, (char *)anim_spr_enano_a,
				      20, 7,       93};
int anim_actual;
...
int ParseAnimFile(void)
{
	...
	for (con2 = 0; con2 < 6; con2++) {
		...
		switch (con2) {
		case HOMBRE:
			strcat(AnimFile, "GENSPR/HOMBRE/ANIM.HPP");
			break;
		case HOMBRE_A:
			strcat(AnimFile, "GENSPR/HOMBRE_A/ANIM.HPP");
			break;
		case MUJER:
			strcat(AnimFile, "GENSPR/MUJER/ANIM.HPP");
			break;
		case MUJER_A:
			strcat(AnimFile, "GENSPR/MUJER_A/ANIM.HPP");
			break;
		case ENANO:
			strcat(AnimFile, "GENSPR/ENANO/ANIM.HPP");
			break;
		case ENANO_A:
			strcat(AnimFile, "GENSPR/ENANO_A/ANIM.HPP");
			break;
		}

		if ((f = fopen(AnimFile, "rb")) == NULL)
			return (0);
		...
		switch (con2) {
		case HOMBRE:
			memcpy(anim_spr_hombre, anim_aux, 2000);
			anim_total_hombre = anim_total;
			break;
		case HOMBRE_A:
			memcpy(anim_spr_hombre_a, anim_aux, 2000);
			anim_total_hombre_a = anim_total;
			break;
		case MUJER:
			memcpy(anim_spr_mujer, anim_aux, 2000);
			anim_total_mujer = anim_total;
			break;
		case MUJER_A:
			memcpy(anim_spr_mujer_a, anim_aux, 2000);
			anim_total_mujer_a = anim_total;
			break;
		case ENANO:
			memcpy(anim_spr_enano, anim_aux, 2000);
			anim_total_enano = anim_total;
			break;
		case ENANO_A:
			memcpy(anim_spr_enano_a, anim_aux, 2000);
...
void GenSpr0(void)
{
	...
	// Entrada de tamaño
	_get(426, 3 + 1, 11 + 65 + 2, text_len(texto[426]) + 16,
	     (byte *)TamText, 5, 1, 1000);

	// Entrada de imagenes
	_get(429, 3 + 1 + text_len(texto[426]) + 22, 11 + 65 + 2, 62,
	     (byte *)NumFrames, 4, 0, 0);

	// Botones frame anterior/siguiente (<< y >>)
	_button(435, v.an - 30, 10 + 70 + 5, 0);
	_button(427, v.an - 14, 10 + 70 + 5, 0);

	// Flags Hombre/Mujer/Niño - armado
	_flag(423, 3, 95 + 2, &TipoModelo[0]);
	_flag(424, 3 + 11 + text_len(texto[423]), 95 + 2, &TipoModelo[1]);
	_flag(425, 3 + 22 + text_len(texto[423]) + text_len(texto[424]), 95 + 2,
	      &TipoModelo[2]);
	_flag(550,
	      3 + 33 + text_len(texto[423]) + text_len(texto[424]) +
		  text_len(texto[425]),
	      95 + 2, &Armado);
	/*
  // Botones Imagenes (- +)
...
*/
	...
	if (ThumbSprite != NULL) {
		...
		for (x = 0; x < 96 * big2; x++)
			v.ptr[(py + cy) * v.an + ((4 + 64 + 3) * big2 + x)] =
			    c12;
		for (y = 0; y < 64 * big2; y++)
			v.ptr[((12 * big2) + y) * v.an + (px + cx)] = c12;
...
void GenSpr2(void)
{
	...
	case 4: // Frame anterior
		IniCfg();
		FloatFrame -=
		    (float)InfoCfg.num_frames / (float)atoi(NumFrames);
	...
	if ((mouse_x - v.x) / big2 > 3 && (mouse_x - v.x) / big2 < 3 + 66 &&
		...
		mouse_graf = 2;
	} else if ((mouse_x - v.x) / big2 >= 3 + 66 + 2 &&
		   (mouse_x - v.x) / big2 < 3 + 66 + 1 + 96 &&
		   (mouse_y - v.y) / big2 >= 12 &&
		   (mouse_y - v.y) / big2 < 12 + 64) {
		mouse_graf = 2;
		if (mouse_b) {
			IniCfg();
			if (mouse_b & 2) {
				//      read_mouse();
				FloatFrame += (float)InfoCfg.num_frames /
					      (float)atoi(NumFrames);
			...
			leer_mouse = 1; // rotate=0;
	} else
		leer_mouse = 1; // rotate=0;

	if (mouse_b & 1) {
		if ((mouse_x - v.x) / big2 > 3 &&
		    (mouse_x - v.x) / big2 < 3 + 66 &&
		    (mouse_y - v.y) / big2 > 11 &&
		    (mouse_y - v.y) / big2 < 11 + 66) {
			v_modo = 0;
			v_texto = (char *)texto[433];
...
void GenSpr3(void) { leer_mouse = 1; }
...
void CreaSpriteFPG(void)
{
	...
	while (1) {
		...
		visor_loop((lptvisor)&InfoCfg);
		if (InfoSpr->buffer == NULL) {
			Progress((char *)texto[514], atoi(NumFrames),
				 atoi(NumFrames));
			v_texto = (char *)texto[45];
			dialogo((voidReturnType)err0);
			fin_dialogo = 1;
		...
		InfoCfg.frame = (int)(FloatFrame + 0.5);
		if (InfoCfg.frame >= InfoCfg.num_frames ||
		    con_frame > atoi(NumFrames))
			break;
	}
	if (con_frame <= atoi(NumFrames))
...
void DumpSpriteFPG(byte *mapa, int ancho, int alto, int cod)
{
	...
	if (atoi(NumFrames) > 1)
		Progress((char *)texto[514], con_frame, atoi(NumFrames));
	...
	sprintf(str_desc, "%s %d/%s. %s,%s", anim_spr[anim_actual], con_frame++,
		NumFrames, AngText, PerText);
	Anadir_FPG(MiFPG, cod, str_desc, str_file, ancho, alto, 1,
		   (char *)puntos, (char *)mapa, 0, 0);
}
...
void CargarTextura(char *NombreTextura)
{
	...
	if (tipomapa) {
		if ((temp = (byte *)malloc(man * mal + man)) != NULL) {
			...
			switch (tipomapa) {
			case 1:
				descomprime_MAP(mapa, temp, 0);
				break;
			case 2:
				descomprime_PCX(mapa, temp, 0);
				break;
			case 3:
				descomprime_BMP(mapa, temp, 0);
				break;
			case 4:
				n = descomprime_JPG(mapa, temp, 0, x);
				break;
			}
			swap(man, map_an);
			swap(mal, map_al);
			...
			if ((temp2 = (byte *)malloc(an_final * al_final)) !=
			    NULL) {
...
void CargarSprite(void)
{
		...
		for (n = 0, blue = 0; blue < 32; blue++) {
			for (green = 0; green < 64; green++) {
				for (red = 0; red < 32; red++) {
					x = fast_find_color(2 * blue, green,
							    2 * red);
					if (x)
						t64[n++] = x;
					else
	...
	for (y = 0; y < mal; y++) {
		for (x = 0; x < man; x++) {
			...
			// printf("col x[%d], y[%d], col[%d]
			// n:[%d]\n",x,y,col1,n); if(col1==240) col1=0;
