...
int sbinit(void)
{
	...
	for (x = 0; x < 100; x++) {
		if (inp(judascfg_port + 0x0E) & 0x80) {
			if (inp(judascfg_port + 0x0A) == 0xAA)
				break;
		}
	}
...
void sbmalloc(void)
{
	...
	for (con = 0; con < 2; con++) {
		memset(&r, 0, sizeof(r));
		r.w.ax = 0x0100;	  // Función Reservar Bloque Mem. DOS
		...
		aligned[con] =
		    (unsigned char *)(((unsigned int)aligned_physical[con]
				       << 16) &
				      0xF0000);
	}
}
...
//------------------------------------------------------------------------------
void spkon(void) { dspwrite(0xD1); }
//------------------------------------------------------------------------------
void spkoff(void) { dspwrite(0xD3); }
//------------------------------------------------------------------------------
...
{
	if (DmaNum < 4)
		return (inp(0x08) & (1 << DmaCha));
	else
		return (inp(0xD0) & (1 << DmaCha));
}
//------------------------------------------------------------------------------
// Copyright (c), David Welch, 1993
//------------------------------------------------------------------------------
