...
#define lfrf                                                                   \
	55      // num_par      Pseudo-Frame (frame a un porcentaje,
		// frame(100)==frame)
...
void compilar(void)
{
	...
	for (n = 0; n < 256; n++)
		if (lower[n])
			if (n >= '0' && n <= '9')
				lex_case[n] = (void *)l_num;
			else
				lex_case[n] = (void *)l_id;
		else
			lex_case[n] = (void *)l_err;
...
#ifdef listados
	if ((lst = fopen("div.lst", "wt")) == NULL)
		c_error(0, 0);
#endif

#ifdef formato
	if ((ls = fopen("div.frm", "wt")) == NULL)
		c_error(0, 0);
#endif
	...
	imem = 0;
	if ((mem_ory = mem = (int *)malloc(imem_max * sizeof(int))) == NULL)
		c_error(0, 0);
	...
	iloc = 0;
	iloc_len = 0;
	if ((loc = (int *)malloc(iloc_max * sizeof(int))) == NULL)
		c_error(0, 0);
	memset(loc, 0, iloc_max * sizeof(int));

	ifrm_max = default_buffer / 2;
	if ((frm = (int *)malloc(ifrm_max * sizeof(int))) == NULL)
		c_error(0, 0);
...
void test_buffer(int **buffer, int *maximo, int n)
{
	...
	if (n > *maximo - security_distance) {
		...
		*maximo = n + buffer_grow;
		...
		memset((byte *)*buffer + (*maximo - max) * sizeof(int), 0,
		       max * sizeof(int));
	}
}
...
lex_scan:
	...
	case l_lit:
		...
		if ((f = div_open_file(ivnom.b)) != NULL) {
			...
			if (!strcmp(ext, ".PCM"))
				...
				if (!strcmp(cwork, "pal\x1a\x0d\x0a"))
					empaquetable = 1;
				if (!strcmp(cwork, "map\x1a\x0d\x0a"))
					empaquetable = 1;
				if (!strcmp(cwork, "fpg\x1a\x0d\x0a"))
					empaquetable = 1;
				if (!strcmp(cwork, "fnt\x1a\x0d\x0a"))
					empaquetable = 1;
				if (!strcmp(cwork, "wld\x1a\x0d\x0a"))
					empaquetable = 1;
				if (!strcmp(cwork, "wld\x1a\x0d\x0a\x01"))
					empaquetable = 1;
			}
#endif
			fclose(f);
	...
	default: // puntero a un lex_ele
		...
		//      if (pieza==p_and || pieza==p_or || pieza==p_xor ||
		//      pieza==p_xorptr || pieza==p_andofs)
		//        tipo_t=1;
...
void emitir_comentario(void)
{
	...
	if (convert) {
		...
		while (n--)
			fwrite("    ", 1, 4, ls);
		fwrite(com + 1, 1, strlen(com) - 1, ls);
		com[0] = 0;
		fwrite("\n", 1, 1, ls);
		n_cr = 1;
		old_tipo_t = 0;
	...
	while (pieza != p_end) {
		if (pieza == p_struct) { // Miembro struct ───
			...
			if (pieza == p_corab) {
				...
				member = NULL;
				lexico();
				if (((*ob).sglo.items1 = constante()) < 0)
					c_error(4, 123);
				if (pieza == p_coma) {
					lexico();
					if (((*ob).sglo.items2 = constante()) <
					    0)
						c_error(4, 123);
					if (pieza == p_coma) {
						lexico();
			...
			if (dup > 1) {
				...
				test_buffer(&frm, &ifrm_max,
					    i + (*ob).sglo.len_item *
						    (*ob).sglo.totalitems);
				while (--dup) {
					memcpy(&mem[i], &mem[offstruct + len],
					       (*ob).sglo.len_item << 2);
					memcpy(&frm[i], &frm[offstruct + len],
					       (*ob).sglo.len_item << 2);
					i += (*ob).sglo.len_item;
				}
			}
			...
			if (pieza == p_asig) {
				...
				strcpy((char *)&mem[offstruct + len + 1],
				       (char *)&mem[pieza_num]);
				len += 1 + ((*ob).cglo.totalen + 5) / 4;
				itxt = _itxt; // Saca la cadena del segmento de
					      // textos
				lexico();
			} else {
				test_buffer(&mem, &imem_max, offstruct + len);
	...
	while (pieza != p_end) {
		if (pieza == p_struct) { // Miembro struct ───
			...
			if (pieza == p_corab) {
				...
				member = NULL;
				lexico();
				if (((*ob).sloc.items1 = constante()) < 0)
					c_error(4, 123);
				if (pieza == p_coma) {
					lexico();
					if (((*ob).sloc.items2 = constante()) <
					    0)
						c_error(4, 123);
					if (pieza == p_coma) {
						lexico();
			...
			if (((*ob).sloc.len_item =
				 analiza_struct_local(offstruct + len)) == 0)
				c_error(0, 57);
			i = offstruct + len;
			...
			if (dup > 1) {
				...
				test_buffer(&frm, &ifrm_max,
					    i + (*ob).sloc.len_item *
						    (*ob).sloc.totalitems);
				while (--dup) {
					memcpy(&loc[i], &loc[offstruct + len],
					       (*ob).sloc.len_item << 2);
					memcpy(&frm[i], &frm[offstruct + len],
					       (*ob).sloc.len_item << 2);
					i += (*ob).sloc.len_item;
			...
			if (pieza == p_asig) {
				...
				strcpy((char *)&loc[offstruct + len + 1],
				       (char *)&mem[pieza_num]);
...
{ // tras " struct id [ <const> ] " // id­member
	...
	while (pieza != p_end) {
		if (pieza == p_struct) { // Miembro struct ───
			...
			if (((*ob).sloc.len_item =
				 analiza_struct_private(offstruct + len)) == 0)
				c_error(0, 57);
			i = offstruct + len;
			dup = (*ob).sloc.totalitems + 1;
			if (dup > 1) {
...
void UnimportDll() { DIV_UnImportDll(pe); }
...
void sintactico(void)
{
	...
	if (pieza == p_setup_program) {
		program_type = 1;
		strcpy(cWork, "install\\setup.ins");
	} else
		strcpy(cWork, "system\\EXEC.INS");
	if ((lins = fopen(cWork, "wb")) == NULL)
		c_error(0, 0);
	...
	while (pieza == p_import) {
		...
		if (nombre_dll == NULL)
			c_error(0, 63);
		if ((num_extern = ImportDll(nombre_dll)) == 0)
			c_error(0, 63);
		if (num_extern > 0) {
			save_error(0);
	...
	if (pieza == p_global) {
		...
		tabu++;
#endif
		while (pieza == p_id || pieza == p_struct ||
		       pieza == p_string || pieza == p_byte ||
		       pieza == p_word) {
				...
				if (dup > 1) {
					...
					test_buffer(
					    ...
					    imem + (*ob).sglo.len_item *
						       (*ob).sglo.totalitems);
					while (--dup) {
						memcpy(&mem[imem], &mem[_imem],
						       (*ob).sglo.len_item
							   << 2);
						memcpy(&frm[imem], &frm[_imem],
						       (*ob).sglo.len_item
							   << 2);
						imem += (*ob).sglo.len_item;
					}
				...
				if (dup > 1) {
					...
					test_buffer(
					    ...
					    imem + (*ob).sloc.len_item *
						       (*ob).sloc.totalitems);
					while (--dup) {
						memcpy(&loc[iloc], &loc[_imem],
						       (*ob).sloc.len_item
							   << 2);
						memcpy(&frm[iloc], &frm[_imem],
						       (*ob).sloc.len_item
							   << 2);
						iloc += (*ob).sloc.len_item;
					}
	...
	if (pieza == p_private) {
		...
		while (pieza == p_id || pieza == p_struct ||
			...
			if (pieza == p_struct) {
				...
				if (((*ob).sloc.len_item =
					 analiza_struct_private(_imem)) == 0)
					c_error(0, 57);
				...
				ob = o;
				if ((*ob).tipo != tnone) { // Mira si se repite
							   // un parámetro ...
					if (parametros == -1 &&
				...
				// Si el objeto no es tnone, se repite un
				// parámetro o bien es un error
...
void tglo_init2(int tipo)
{
	...
	while (1) {
		...
		if (pieza == p_cerrar || pieza == p_ptocoma) {
			if (*(imemptr - (int)mem + (int)frm) == 1 ||
				...
				if (*(imemptr - (int)mem + (int)frm) != 1 &&
				    tipo != 1)
					imemptr = (byte *)&mem[imem];
			} else if (*(imemptr - (int)mem + (int)frm) == 2 ||
				   tipo == 2) {
				imemptr++;
				imem = ((int)imemptr - (int)mem + 3) / 4;
		...
		if (pieza == p_abrir) { // Dup x 1
			...
			if (*(imemptr - (int)mem + (int)frm) == 1 ||
				...
				*((word *)imemptr) = valor;
				...
				oimemptr = imemptr;
				while (*(oimemptr - (int)mem + (int)frm) == 2)
					oimemptr++;
				if (tipo == 0)
					if (strlen((char *)&mem_ory[valor]) >
					    (int)(oimemptr - imemptr))
						c_error(2, 33);
		...
		if (dup > 1) {
			...
			test_buffer(
			    &mem, &imem_max,
			    (((int)oimemptr - (int)mem) + len * dup + 3) / 4);
			while (--dup) {
				for (n = 0; n < len; n++) {
					...
					if (*(imemptr - (int)mem + (int)frm) ==
					    1) {
						if (*(oimemptr + n - (int)mem +
						      (int)frm) != 1)
							c_error(3, 136);
					} else if (*(oimemptr + n - (int)mem +
						     (int)frm) == 1)
						c_error(3, 136);
...
void factor(void)
{
	...
	case p_id:
		...
		case tfunc:
			...
			case 101:
			case 102:
			case 107: // strcpy/cat/set/upr/lwr/del
			case 108:
			case 109:
			case 110:
				tipo_factor = 2;
...
void l_ensamblador(void)
{
	...
	while (i < imem) {
		...
		case lcal:
			...
			break;
		case lret:
			fprintf(sta, "%5u\tret %u", i, mem[i + 1]);
			i++;
			break;
		case lasp:
			...
			break;
		case lfrm:
			fprintf(sta, "%5u\tfrm %u", i, mem[i + 1]);
			i++;
			break;
		case lcbp:
		...
		case lpar:
			...
			break;
		case lrtf:
			fprintf(sta, "%5u\trtf %u", i, mem[i + 1]);
			i++;
			break;
		case lclo:
			...
			break;
		case lfrf:
			fprintf(sta, "%5u\tfrf %u", i, mem[i + 1]);
			i++;
			break;
		case limp:
...
void linea_ensamblador(void)
{
	...
	switch (mem[ilst]) {
	case lnop:
		fprintf(lst, "%5u\tnop", ilst);
		break;
	case lcar:
		fprintf(lst, "%5u\tcar %u", ilst, mem[ilst + 1]);
		ilst++;
		break;
	case lasi:
		fprintf(lst, "%5u\tasi", ilst);
		break;
	case lori:
		fprintf(lst, "%5u\tori", ilst);
		break;
	case lxor:
		fprintf(lst, "%5u\txor", ilst);
		break;
	case land:
		fprintf(lst, "%5u\tand", ilst);
		break;
	case ligu:
		fprintf(lst, "%5u\tigu", ilst);
		break;
	case ldis:
		fprintf(lst, "%5u\tdis", ilst);
		break;
	case lmay:
		fprintf(lst, "%5u\tmay", ilst);
		break;
	case lmen:
		fprintf(lst, "%5u\tmen", ilst);
		break;
	case lmei:
		fprintf(lst, "%5u\tmei", ilst);
		break;
	case lmai:
		fprintf(lst, "%5u\tmai", ilst);
		break;
	case ladd:
		fprintf(lst, "%5u\tadd", ilst);
		break;
	case lsub:
		fprintf(lst, "%5u\tsub", ilst);
		break;
	case lmul:
		fprintf(lst, "%5u\tmul", ilst);
		break;
	case ldiv:
		fprintf(lst, "%5u\tdiv", ilst);
		break;
	case lmod:
		fprintf(lst, "%5u\tmod", ilst);
		break;
	case lneg:
		fprintf(lst, "%5u\tneg", ilst);
		break;
	case lptr:
		fprintf(lst, "%5u\tptr", ilst);
		break;
	case lnot:
		fprintf(lst, "%5u\tnot", ilst);
		break;
	case laid:
		fprintf(lst, "%5u\taid", ilst);
		break;
	case lcid:
		fprintf(lst, "%5u\tcid", ilst);
		break;
	case lrng:
		fprintf(lst, "%5u\trng %u", ilst, mem[ilst + 1]);
		ilst++;
		break;
	case ljmp:
		fprintf(lst, "%5u\tjmp %u", ilst, mem[ilst + 1]);
		ilst++;
		break;
	case ljpf:
		fprintf(lst, "%5u\tjpf %u", ilst, mem[ilst + 1]);
		ilst++;
		break;
	case lfun:
		fprintf(lst, "%5u\tfun %u", ilst, mem[ilst + 1]);
		ilst++;
		break;
	case lcal:
		fprintf(lst, "%5u\tcal %u", ilst, mem[ilst + 1]);
		ilst++;
		break;
	case lret:
		fprintf(lst, "%5u\tret %u", ilst, mem[ilst + 1]);
		ilst++;
		break;
	case lasp:
		fprintf(lst, "%5u\tasp", ilst);
		break;
	case lfrm:
		fprintf(lst, "%5u\tfrm %u", ilst, mem[ilst + 1]);
		ilst++;
		break;
	case lcbp:
		fprintf(lst, "%5u\tcbp %u", ilst, mem[ilst + 1]);
		ilst++;
		break;
	case lcpa:
		fprintf(lst, "%5u\tcpa", ilst);
		break;
	case ltyp:
		fprintf(lst, "%5u\ttyp %u", ilst, mem[ilst + 1]);
		ilst++;
		break;
	case lpri:
		fprintf(lst, "%5u\tpri %u", ilst, mem[ilst + 1]);
		ilst++;
		break;
	case lcse:
		fprintf(lst, "%5u\tcse %u", ilst, mem[ilst + 1]);
		ilst++;
		break;
	case lcsr:
		fprintf(lst, "%5u\tcsr %u", ilst, mem[ilst + 1]);
		ilst++;
		break;
	case lshr:
		fprintf(lst, "%5u\tshr", ilst);
		break;
	case lshl:
		fprintf(lst, "%5u\tshl", ilst);
		break;
	case lipt:
		fprintf(lst, "%5u\tipt", ilst);
		break;
	case lpti:
		fprintf(lst, "%5u\tpti", ilst);
		break;
	case ldpt:
		fprintf(lst, "%5u\tdpt", ilst);
		break;
	case lptd:
		fprintf(lst, "%5u\tptd", ilst);
		break;
	case lada:
		fprintf(lst, "%5u\tada", ilst);
		break;
	case lsua:
		fprintf(lst, "%5u\tsua", ilst);
		break;
	case lmua:
		fprintf(lst, "%5u\tmua", ilst);
		break;
	case ldia:
		fprintf(lst, "%5u\tdia", ilst);
		break;
	case lmoa:
		fprintf(lst, "%5u\tmoa", ilst);
		break;
	case lana:
		fprintf(lst, "%5u\tana", ilst);
		break;
	case lora:
		fprintf(lst, "%5u\tora", ilst);
		break;
	case lxoa:
		fprintf(lst, "%5u\txoa", ilst);
		break;
	case lsra:
		fprintf(lst, "%5u\tsra", ilst);
		break;
	case lsla:
		fprintf(lst, "%5u\tsla", ilst);
		break;
	case lpar:
		fprintf(lst, "%5u\tpar %u", ilst, mem[ilst + 1]);
		ilst++;
		break;
	case lrtf:
		fprintf(lst, "%5u\trtf %u", ilst, mem[ilst + 1]);
		ilst++;
		break;
	case lclo:
		fprintf(lst, "%5u\tclo %u", ilst, mem[ilst + 1]);
		ilst++;
		break;
	case lfrf:
		fprintf(lst, "%5u\tfrf %u", ilst, mem[ilst + 1]);
		ilst++;
		break;
	case limp:
		fprintf(lst, "%5u\timp %u", ilst, mem[ilst + 1]);
		ilst++;
		break;
	case lext:
		fprintf(lst, "%5u\text %u", ilst, mem[ilst + 1]);
		ilst++;
		break;
	case lchk:
		fprintf(lst, "%5u\tchk", ilst);
		break;
	case ldbg:
		fprintf(lst, "%5u\tdbg", ilst);
		break;

	case lcar2:
		fprintf(lst, "%5u\tcar2 %u %u", ilst, mem[ilst + 1],
			mem[ilst + 2]);
		ilst += 2;
		break;
	case lcar3:
		fprintf(lst, "%5u\tcar3 %u %u %u", ilst, mem[ilst + 1],
			mem[ilst + 2], mem[ilst + 3]);
		ilst += 3;
		break;
	case lcar4:
		fprintf(lst, "%5u\tcar4 %u %u %u %u", ilst, mem[ilst + 1],
			mem[ilst + 2], mem[ilst + 3], mem[ilst + 4]);
		ilst += 4;
		break;
	case lasiasp:
		fprintf(lst, "%5u\tasiasp", ilst);
		break;
	case lcaraid:
		fprintf(lst, "%5u\tcaraid %u", ilst, mem[ilst + 1]);
		ilst++;
		break;
	case lcarptr:
		fprintf(lst, "%5u\tcarptr %u", ilst, mem[ilst + 1]);
		ilst++;
		break;
	case laidptr:
		fprintf(lst, "%5u\taidptr", ilst);
		break;
	case lcaraidptr:
		fprintf(lst, "%5u\tcaraidptr %u", ilst, mem[ilst + 1]);
		ilst++;
		break;
	case lcaraidcpa:
		fprintf(lst, "%5u\tcaraidcpa %u", ilst, mem[ilst + 1]);
		ilst++;
		break;
	case laddptr:
		fprintf(lst, "%5u\taddptr", ilst);
		break;
	case lfunasp:
		fprintf(lst, "%5u\tfunasp %u", ilst, mem[ilst + 1]);
		ilst++;
		break;
	case lcaradd:
		fprintf(lst, "%5u\tcaradd %u", ilst, mem[ilst + 1]);
		ilst++;
		break;
	case lcaraddptr:
		fprintf(lst, "%5u\tcaraddptr %u", ilst, mem[ilst + 1]);
		ilst++;
		break;
	case lcarmul:
		fprintf(lst, "%5u\tcarmul %u", ilst, mem[ilst + 1]);
		ilst++;
		break;
	case lcarmuladd:
		fprintf(lst, "%5u\tcarmuladd %u", ilst, mem[ilst + 1]);
		ilst++;
		break;
	case lcarasiasp:
		fprintf(lst, "%5u\tcarasiasp %u", ilst, mem[ilst + 1]);
		ilst++;
		break;
	case lcarsub:
		fprintf(lst, "%5u\tcarsub %u", ilst, mem[ilst + 1]);
		ilst++;
		break;
	case lcardiv:
		fprintf(lst, "%5u\tcardiv %u", ilst, mem[ilst + 1]);
		ilst++;
		break;

	case lptrwor:
		fprintf(lst, "%5u\tptrwor", ilst);
		break;
	case lasiwor:
		fprintf(lst, "%5u\tasiwor", ilst);
		break;
	case liptwor:
		fprintf(lst, "%5u\tiptwor", ilst);
		break;
	case lptiwor:
		fprintf(lst, "%5u\tptiwor", ilst);
		break;
	case ldptwor:
		fprintf(lst, "%5u\tdptwor", ilst);
		break;
	case lptdwor:
		fprintf(lst, "%5u\tptdwor", ilst);
		break;
	case ladawor:
		fprintf(lst, "%5u\tadawor", ilst);
		break;
	case lsuawor:
		fprintf(lst, "%5u\tsuawor", ilst);
		break;
	case lmuawor:
		fprintf(lst, "%5u\tmuawor", ilst);
		break;
	case ldiawor:
		fprintf(lst, "%5u\tdiawor", ilst);
		break;
	case lmoawor:
		fprintf(lst, "%5u\tmoawor", ilst);
		break;
	case lanawor:
		fprintf(lst, "%5u\tanawor", ilst);
		break;
	case lorawor:
		fprintf(lst, "%5u\torawor", ilst);
		break;
	case lxoawor:
		fprintf(lst, "%5u\txoawor", ilst);
		break;
	case lsrawor:
		fprintf(lst, "%5u\tsrawor", ilst);
		break;
	case lslawor:
		fprintf(lst, "%5u\tslawor", ilst);
		break;
	case lcpawor:
		fprintf(lst, "%5u\tcpawor", ilst);
		break;

	case lptrchr:
		fprintf(lst, "%5u\tptrchr", ilst);
		break;
	case lasichr:
		fprintf(lst, "%5u\tasichr", ilst);
		break;
	case liptchr:
		fprintf(lst, "%5u\tiptchr", ilst);
		break;
	case lptichr:
		fprintf(lst, "%5u\tptichr", ilst);
		break;
	case ldptchr:
		fprintf(lst, "%5u\tdptchr", ilst);
		break;
	case lptdchr:
		fprintf(lst, "%5u\tptdchr", ilst);
		break;
	case ladachr:
		fprintf(lst, "%5u\tadachr", ilst);
		break;
	case lsuachr:
		fprintf(lst, "%5u\tsuachr", ilst);
		break;
	case lmuachr:
		fprintf(lst, "%5u\tmuachr", ilst);
		break;
	case ldiachr:
		fprintf(lst, "%5u\tdiachr", ilst);
		break;
	case lmoachr:
		fprintf(lst, "%5u\tmoachr", ilst);
		break;
	case lanachr:
		fprintf(lst, "%5u\tanachr", ilst);
		break;
	case lorachr:
		fprintf(lst, "%5u\torachr", ilst);
		break;
	case lxoachr:
		fprintf(lst, "%5u\txoachr", ilst);
		break;
	case lsrachr:
		fprintf(lst, "%5u\tsrachr", ilst);
		break;
	case lslachr:
		fprintf(lst, "%5u\tslachr", ilst);
		break;
	case lcpachr:
		fprintf(lst, "%5u\tcpachr", ilst);
		break;

	case lstrcpy:
		fprintf(lst, "%5u\tstrcpy", ilst);
		break;
	case lstrfix:
		fprintf(lst, "%5u\tstrfix", ilst);
		break;
	case lstrcat:
		fprintf(lst, "%5u\tstrcat", ilst);
		break;
	case lstradd:
		fprintf(lst, "%5u\tstradd", ilst);
		break;
	case lstrdec:
		fprintf(lst, "%5u\tstrdec", ilst);
		break;
	case lstrsub:
		fprintf(lst, "%5u\tstrsub", ilst);
		break;
	case lstrlen:
		fprintf(lst, "%5u\tstrlen", ilst);
		break;
	case lstrigu:
		fprintf(lst, "%5u\tstrigu", ilst);
		break;
	case lstrdis:
		fprintf(lst, "%5u\tstrdis", ilst);
		break;
	case lstrmay:
		fprintf(lst, "%5u\tstrmay", ilst);
		break;
	case lstrmen:
		fprintf(lst, "%5u\tstrmen", ilst);
		break;
	case lstrmei:
		fprintf(lst, "%5u\tstrmei", ilst);
		break;
	case lstrmai:
		fprintf(lst, "%5u\tstrmai", ilst);
		break;
	case lcpastr:
		fprintf(lst, "%5u\tcpastr", ilst);
		break;
...
void g1(int op) { gen(0, op, 0); }

void g2(int op, int pa) { gen(1, op, pa); }
