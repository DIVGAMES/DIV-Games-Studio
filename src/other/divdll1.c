...
/*
	...
	Input:
		...
		char *name      :       The ASCIIZ name of the object to be
   exported, by which it will be registered in the exports-pool.

		void *obj       :       The address of the object to be
   exported.
...
/*
   ...
   imported.
	...
	Returns:
		...
		The address of the object or NULL when the object couldn't be
   found.
*/
{
	EXPORTENTRY *e;
	...
	if ((e = findexportentry(name)) == NULL) {
		sprintf(dll_error = msgbuf,
			"Couldn't resolve external reference: '%s'", name);
		return NULL;
	}
...
void DIV_UnImportDll(PE *pefile) { PE_Free(pefile); }
