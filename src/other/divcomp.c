...
// 12 - Poder inicializar los registros en las estructuras globalmente (struct
// pepe[9] x=10; end) 13 - Sentencia compiler_options
// (_max_process,_extended_conditions,_simple_conditions,_case_sensitive,_ignore_errors,_free_sintax)
// 14 - Auto "\0" y ZERO2SPACE al poner un char en un string (strfix)
// 15 - Nuevo sistema de tipos (int/char/string) en expresiones y condiciones
// (tipo_expresion) 16 - Auto conversion de "A" a char("A") en asignaciones a
// char (literal/constante literal/string -> char) 17 - Sobrecarga de = += + en
// operaciones de cadenas (strcpy,strcat,stradd) 18 - A┬ñadir y quitar
// caracteres con string++/--, string-=n, string-n (strdec,strsub,strlen) 19 -
// Poder definir literales en varias lineas del fuente (varios consecutivos se
// convierten en uno) 20 - Corregido fallo del coloreador de listados (al llegar
// un eof con /* sin cerrar) 21 - Comparaciones entre cadenas
// (strigu/dis/may/men/mai/mei) 22 - Nuevo sistema con 4 strings "en el aire"
// para permitir mas operaciones con cadenas (+/-) 23 - Descartada la idea de
// pasar los offset a bytes (se pretendia hacer para &cadena[n],
//      bytes y words: pero en [ptr+n] no se puede saber por cuanto multiplicar
//      n)
// 24 - Solucionados algunos fallos relacionados con los accesos remotos
// 25 - Unidas las reservadas del modo7 con las del modo 8
// 26 - A┬ñadidas la opciones _no_check (no comprobar rangos ni ides) y
// _no_strfix 27 - Funciones ascii:
// char,strcpy,strcat,strlen,strcmp,strchr,strstr,strset,upper,lower y strdel 28
// - Habilitado un 'parche' para indicar al compilador que algunas funciones
// devuelven un string 29 - Poder recibir un parametro en un string/char global
// o local (cpachr, cpastr) 30 - Habilitado un sistema multi-par/pri y
// arreglados algunos temas de las llamadas a procesos 31 - Habilitada la idea
// de recibir un par┬ámetro en un string private, incluso indicando su longitud
// (ej, proc(string s[2])) 32 - Permitir la redeclaracion del par┬ámetro string
// dentro de la seccion private (con la misma longitud) 33 - Procesos tipo
// "function" (no retornan al llamador hasta haber terminado) (FCount,CallerId)
// 34 - Arreglado el tema de los retornos de function (al caller del ┬último
// proceso) 35 - Poder hacer STEP sobre funciones (se hace un go here! a la
// siguiente sentencia) 36 - A┬ñadir bytes globales y un nuevo, nuevamente,
// sistema de inicializaci┬ón de datos
//      (ver p_byte, bglo y p_pointerbyte)
// 37 - Preparar el debugger para el soporte de bytes y words
// 38 - Ultimos cambios en el Debugger (en Ver datos), y conversi┬ón al
// Castellano. 39 - A┬ñadir bytes locales y privados (bloc) 40 - Bytes miembros
// de estructuras (globales/locales y privados) 41 - Inicializaci┬ón de
// estructuras contemplando los bytes miembros (frm,ifrm_max) 42 - A┬ñadidas al
// int┬érprete instrucciones para el manejo de words (???wor) 43 - Words al
// igual que bytes (wglo wloc *word ???wor) 44 - Poder recibir un byte o word
// como par┬ámetro (y redeclararlo como private) 45 - Fallo del coloreador
// (debajo de process ... poner ?????????) 46 - Que en las listas de procesos
// (F5) aparezcan tambi┬én las funciones (editor y debugger) 47 - Fallo de que
// desaparezcan (completamente) los sprites en los modos 8 48 - Pasar los IMPORT
// al comienzo del programa y emitir un error adecuado para los IMPORT antiguos
// (y adaptar help) 49 - Quitado el formateador de listados (dejado todo en
// divfrm.cpp por si en el futuro se quiere hacer, de forma separada a divc.cpp)
// 50 - Que las funciones puedan devolver un valor con return despues de haber
// hecho FRAME (sistema multipila) 51 - A┬ñadido el tipo INT opcional para los
// datos enteros convencionales en DIV 1 52 - Declaraci┬ón de punteros a INT
// globales/locales/privados y miembros de estructuras (tpigl,tpilo) de hasta 3
// dimensiones 53 - Utilizaci┬ón de punteros INT, generaci┬ón de c┬ódigo para
// los mismos (y pruebas) 54 - Soporte en el debugger para punteros INT globales
// y locales (p, p[0], p[1], ...) 55 - Punteros a int como par┬ámetros y
// redeclaraci┬ón en secci┬ón PRIVATE 56 - Prohibir la redeclaraci┬ón de
// variables private (solo si es 1 vez como parametro y otra como private) 57 -
// Punteros a byte/word, tanto global como local y private, member o no,
// declaraci┬ón y c┬ódigo 58 - Soporte en el debugger de punteros de tipo byte y
// word (globales y locales) 59 - Eliminada la posibilidad de utilizar * para
// definir un puntero (y en el debugger se usa POINTER)
//      (esto es importante para no animar a utilizar *puntero despu┬és ...)
// 60 - Probar punteros int/word/byte (glo/loc/pri/mem/par)
// 61 - Implementados "string pointer" exactamente igual que los "byte pointer"
// (s┬ólo varia la visualizaci┬ón en el debug) 62 - Punteros a estructuras
// STRUCT POINTER NOMBRESTRUCT tal; (por defecto a 0) 63 - Resuelto el problema
// del acceso a los miembros de la estructura original 64 - Soporte en el
// debugger para punteros a estructuras (!!! mas chungo de lo que yo pensaba) 65
// - Funcion de "capar()" direcciones en DEBUG para evitar en lo posible los
// page fault 66 - Resueltos "n-mil" problemas con los putos punteros, el
// debugger y la madre que lo pari┬ó ... 67 - Hacer que int/byte/word/string
// pointer p1,p2; defina dos punteros 68 - Funcion screen_copy que puede pasar
// una region de pantalla a una region de un gr┬áfico 69 - Argc/argv al igual
// que en C, con los par┬ámetros del programa 70 - Nuevo sistema de carga de
// funciones, definiendo el retorno y el tipo de los par┬ámetros
//      (y eliminadas las excepciones antes contempladas con las funciones de
//      cadenas)
...
// 74 - Opciones _no_id_check _no_range_check _no_null_check (englobadas en
// _no_check) 75 - Opcion de compilacion _no_optimize (para poder deshabiliatar
// la optimizacion)

//    fopen, fclose, fread, fwrite, fseek, ftell, filelen (ver flushall,
//    fcloseall) findfisrt, findnext, fullpath, splitpath, set/getcwd,
//    set/getdrive, fprintf remove (danidel), chdir, mkdir
...
// C - Multiasignaciones, utilizando variables privadas ocultas
// (x=3,45,y*10,2..4,-8;) D - pensar algun truquillo tipo "macros" facil de
// implementar y que quede bien E - Finalizar el autoformateador de listados F -
// Generar un NFO informativo del programa compilado (TAB+LST+EML)
...
//    - Tambi┬én se podr┬ían permitir las operaciones, aunque los resultados
//    fueran err┬óneos

//    - Aun as┬í, ┬¿como explicar a la gente que ptrint++ funciona, pero que
//    ptrbyte++ no?
...
//    - Hombre, tambi┬én servir┬ía para pasar tablas y estructuras por
//    par┬ámetros.
//    - Quiz┬á se deber┬ía prohibir utilizar "p_mul" (*) para diferenciarlos de
//      los punteros en C.

//    - El problema es: ┬¿Se prohibe operar (sumas y dem┬ás) con ptrbyte/word?
//    -> NO

//    - ┬¿Poder utilizar *ptrbyte como ptrbyte[0] ? -> NO, ya que ello
//    inducir┬ía
...
//    - ┬¿Nuevo dise┬ño para: tpwgl tpwlo tpbgl tpblo? (teni┬éndolos siempre
//    multiplicados *2 o *4)
//    - a) en incializacion se debe multiplicar *2 o *4 la constante
//    (ptrbyte=&tablabyte)
//    - b) como cojones se hace ptrbyte=&loquesea ? (como paso de par┬ámetro,
//    asignaci┬ón, etc...)
//    - c) y que pasa con ┬¿ptrword=ptrbyte?
//    - d) es m┬ás, ┬¿y que pasa si se envia como par┬ámetro un ptrbyte y se
//    recibe un ptrint?

//    - Solo podr┬ía hacerse de forma que todos los accesos de lectura a ptrbyte
//    se dividiera por 4
//      y en los de escritura se multiplicara por cuatro, y aun as┬í:
//      ┬¡┬¡┬¡ fallar┬ía ptrbyte1=ptrbyte2 !!! (o pasando y recibiendo un
//      ptrbyte par┬ámetro) ┬¿[ptrbyte+n] ... y *ptrbyte?
...
//    - Deber┬ía haber alguna forma de tener todos los offset multiplicados por
//    4 (en bytes)

//    - Problemas: ["ptr"+n] (por cuanto multiplicar n), adem┬ás de como
//    resolver
//      [ptrint+ptrbyte+2*x...], acceso en el int┬érprete a todos los datos de
//      mem mem[id+_Graph], los propios id (multiplicarlos o no?- > mejor no, y
//      hacer que el p_punto los multiplique, ya que, adem┬ás deben ser
//      impares), programas antiguos que hicieran uso de punteros (p=&kk; *p...,
//      p++, *p... para ello se deber┬ía emitir un error de compilaci┬ón al
//      utilizar *p o [p], obligando a que redeclaren p como un ptrint), que
//      ptrint++ sea ptrint+=4 y ptrword++ sea ptrword+=2,

//    - Cambiarian tambien los sizeof(), ahora en bytes, y las operaciones con
//      ficheros, aumentando la incompatibilidad con DIV1 (ya que la gente
//      podr┬ía haber utilizado un save("pepe.dat",&tabla,10) ...
...
//      ... que es la forma correcta de hacerlo, y que si no se hace ahora
//      quiz┬á no se pueda hacer nunca.
...
//           acceso libre a mem[], para localizar el problema de la
//           optimizaci┬ón
...
//├äMnem┬ónico├äC┬ódg.├äOperandos (Generaci┬ón de c┬ódigo EML, "*" ├░ "a┬ún no
//usado")
...
#define lfrf                                                                   \
	55      //              Pseudo-Frame (frame a un porcentaje,
		//              frame(100)==frame)
...
#define lasichr                                                                \
	79 // Saca (valor, index, offset) y mete el valor en [offset+byte index]
...
#define lstradd                                                                \
	98 // Suma dos strings "en el aire" y deja en pila el puntero al aire
...
#define lasiwor                                                                \
	110 // Saca (valor, index, offset) y mete el valor en [offset+byte
	    // index]
...
void comp_exit(void) { longjmp(buf, 1); }
...
// Tabla de objetos
// ├ä├ä├ä├ä├ä├ä├ä├ä├ä├ä├ä├ä├ä├ä├ä├ä├ä├ä├ä├ä├ä├ä├ä├ä├ä├ä├ä├ä├ä├ä├ä├ä├ä *iobj
// primer objeto libre
...
struct objeto {
	...
	byte *ierror; // Puntero al c┬ódigo para determinar la columna si es
		      // necesario
	...
	struct objeto
	    *bloque; // Bloque de este proceso (0-global/local N-private)
    ...
    *iobj; // ├ä├ä├ä├ä├ä├ä├ä├ä├ä├ä├ä├ä├ä├ä├ä├ä├ä├ä├ä├ä├ä├ä├ä├ä├ä├ä├ä├ä├ä├ä├ä├ä├ä├ä├ä├ä├ä├ä├ä├ä├ä├ä├ä├ä├ä├ä├ä├ä├ä├ä├ä├ä

//├ì├ì├ì├ì├ì├ì├ì├ì├ì├ì├ì├ì├ì├ì├ì├ì├ì├ì├ì├ì├ì├ì├ì├ì├ì├ì├ì├ì├ì├ì├ì├ì├ì├ì├ì├ì├ì├ì├ì├ì├ì├ì├ì├ì├ì├ì├ì├ì├ì├ì├ì├ì├ì├ì├ì├ì├ì├ì├ì├ì├ì├ì├ì├ì├ì├ì├ì├ì├ì├ì├ì├ì├ì├ì├ì├ì├ì

byte *vnom =
    NULL; // Vector de nombres (cad_hash:int, pieza (o iobj):int, asciiz)
union {
	byte *b;
	byte **p;
} ivnom;
...
int struct_pointer; // p_pointer / p_pointerchar / p_string / p_pointerbyte /
		    // p_pointerword
...
int pila[long_pila + max_exp +
	 64]; // c┬álculo de expresiones (compilaci┬ón y ejecuci┬ón)
...
//├ä├ä├ä├ä├ä├ä├ä├ä├ä├ä├ä├ä├ä Inserci┬ón en la tabla de expresiones, acceso a
//variables
...
struct objeto *analiza_pointer(int tipo, int offset)
{
	...
	ob = o;
	if ((*ob).tipo != tnone) {
		if (parametros == -1 && (*ob).param == 1 &&
		    (*ob).bloque == bloque_actual) {
			if ((*ob).tipo == tipo) { // Se repite un pointer
						  // par┬ámetro como private
				save_error(0);
				lexico();
				len1 = -1;
				len2 = -1;
	...
	if (pieza == p_corab) {
		lexico();
		if (((*ob).pilo.len1 = constante()) < 0)
			c_error(4, 35);
		if (pieza == p_coma) {
			lexico();
			if (((*ob).pilo.len2 = constante()) < 0)
				c_error(4, 35);
			if (pieza == p_coma) {
				lexico();
				if (((*ob).pilo.len3 = constante()) < 0)
					c_error(4, 35);
			}
		}
		if (pieza != p_corce)
...
void factor(void)
{
	...
	case p_sizeof:
		...
		case tsloc:
			(*_exp++).valor =
			    (*o).sglo.len_item * (*o).sglo.totalitems;
			break;
		case tpsgl:
		case tpslo:
			(*_exp++).valor = (*((*o).psgl.ostruct)).sglo.len_item *
					  (*o).sglo.totalitems;
			break;
		case ttglo:
		case ttloc:
	...
	case p_struct: // Se comprueba la declaraci┬ón de un puntero a struct
		       // par┬ámetro
		if (parametros <= 0) {
			if (error_27 == 27)
				c_error(1, 27);
			...
			lexico();
			if (pieza != p_pointer)
				c_error(0, 151); // No se define el pointer as┬í
			lexico();
			if (pieza != p_id)
				c_error(1, 110);
			obs = o;
			if ((*obs).tipo == tnone)
				c_error(0, 150); // No se define el pointer as┬í
	...
	case p_int: // Se comprueba la declaraci┬ón de un int par┬ámetro
		if (parametros <= 0) {
			...
			case p_pointer: // Declaraci┬ón de un puntero a int
					// par┬ámetro
				if (parametros <= 0) {
					if (error_27 == 27)
						c_error(1, 27);
	...
	case p_id:
		...
		case tfunc:
			...
			while (pieza != p_cerrar) { // p├░num_par
				...
				if ((*ob).func.par[p] ==
				    1) { // Par┬ámetro del tipo 1 (struct,campo)

					// RECIBE: nombre_struct,nombre_campo
					// GENERA:
					// offset_struct,len_item,totalen,offset_campo,tipo_campo
	...
	if (pieza == p_inc || pieza == p_dec) {
		if ((*(_exp - 1)).tipo == eoper &&
		    (*(_exp - 1)).token == p_pointer) {
			_exp--;
			(*_exp).tipo = eoper;
			if (pieza == p_inc)
				(*_exp++).token = p_suma;
				...
				(*_exp++).token = p_resta;
		} else if ((*(_exp - 1)).tipo == eoper &&
			   (*(_exp - 1)).token == p_pointerchar) {
			_exp--;
			(*_exp).tipo = eoper;
			if (pieza == p_inc)
...
int analiza_pointer_struct(int tipo, int offset, struct objeto *estructura)
{
	...
	if ((*ob).tipo != tnone) {
		if (parametros == -1 && (*ob).param == 1 &&
			...
			if ((*ob).tipo == tipo) { // Se repite un pointer
				...
				if (pieza == p_corab) {
					lexico();
					if ((items1 = constante()) < 0)
						c_error(4, 35);
					if (pieza == p_coma) {
						lexico();
						if ((items2 = constante()) < 0)
							c_error(4, 35);
						if (pieza == p_coma) {
							lexico();
							if ((items3 =
				...
				if (items1 != (*ob).psgl.items1 ||
				    items2 != (*ob).psgl.items2 ||
	...
	if (pieza == p_corab) {
		lexico();
		if (((*ob).psgl.items1 = constante()) < 0)
			c_error(4, 35);
		if (pieza == p_coma) {
			lexico();
			if (((*ob).psgl.items2 = constante()) < 0)
				c_error(4, 35);
			if (pieza == p_coma) {
				lexico();
				if (((*ob).psgl.items3 = constante()) < 0)
					c_error(4, 35);
			}
		}
		if (pieza != p_corce)
...
{ // Tipo: 0-Int o variado (struct), 1-Word, 2-Byte
	if (tipo == 3) {
...
{ // tras " struct id [ <const> ] " // id├░member
	...
	while (pieza != p_end) {
		...
		if (pieza == p_struct) { // Miembro struct ├ä├ä├ä
			...
			if (pieza ==
			    p_pointer) { // Se define un puntero a struct
				...
				if ((*ob).tipo == tnone)
					c_error(0, 150); // No se define el
							 // pointer as┬í
				if ((*ob).tipo != tsglo && (*ob).tipo != tsloc)
					c_error(0, 150);
				lexico();
			puntero_a_struct:
				...
				if (((*ob).sloc.len_item =
					 analiza_struct_private(offstruct +
								len)) == 0)
					c_error(0, 57);
				i = offstruct + len;
				...
				len +=
				    (*ob).sloc.len_item * (*ob).sloc.totalitems;
				member = old_member;
				lexico();
				while (pieza == p_ptocoma)
					lexico();
				...
				if (pieza == p_asig) {
					...
					if (pieza != p_lit &&
					    !(pieza == p_id &&
					      ...
					      (*o).cons.literal))
						c_error(3, 128);
					if (strlen((char *)&mem[pieza_num]) >
					    (*ob).cloc.totalen + 1)
						c_error(4, 129);
					test_buffer(&mem, &imem_max,
						    offstruct + len);
					mem[offstruct + len] =
					    0xDAD00000 | (*ob).cloc.totalen;
					strcpy(
					    (char *)&mem[offstruct + len + 1],
					    (char *)&mem[pieza_num]);
					len += 1 + ((*ob).cloc.totalen + 5) / 4;
					itxt = _itxt; // Saca la cadena del
						      // segmento de textos
					lexico();
				} else {
					test_buffer(&mem, &imem_max,
			...
			puntero2:
				...
				if (pieza == p_corab) {
					...
					if (pieza == p_asig) {
						...
						mem[offstruct + len] =
						    constante();
						if (mem[offstruct + len] < 0 ||
						    mem[offstruct + len] > 255)
							c_error(4, 143);
					}
				}
				len += (*ob).bloc.totalen;
		...
		} else if (pieza == p_int || pieza == p_id ||
			   pieza == p_pointer) {
			...
			puntero4:
				...
				lexico();
				if (pieza == p_corab) {
					lexico(); // Miembro tabla ├ä├ä├ä
					(*ob).tipo = ttloc;
					(*ob).tloc.offset = len;
					if (((*ob).tloc.len1 = constante()) < 0)
						c_error(4, 35);
					if (pieza == p_coma) {
						lexico();
						if (((*ob).tloc.len2 =
	...
	while (pieza != p_end) {
		...
		if (pieza == p_struct) { // Miembro struct ├ä├ä├ä
			...
			puntero_a_struct:
				...
				if (((*ob).sglo.len_item =
					 analiza_struct(offstruct + len)) == 0)
					c_error(0, 57);
				i = offstruct + len;
				...
				len +=
				    (*ob).sglo.len_item * (*ob).sglo.totalitems;
				member = old_member;
				lexico();
				while (pieza == p_ptocoma)
					lexico();
	...
	while (pieza != p_end) {
		...
		if (pieza == p_struct) { // Miembro struct ├ä├ä├ä
			...
			puntero_a_struct:
				...
				if (((*ob).sloc.len_item = analiza_struct_local(
					 offstruct + len)) == 0)
					c_error(0, 57);
				i = offstruct + len;
				dup = (*ob).sloc.totalitems + 1;
			...
			puntero2:
				...
				if (pieza == p_corab) {
					...
					if (pieza == p_asig) {
						...
						loc[offstruct + len] =
						    constante();
						if (loc[offstruct + len] < 0 ||
						    loc[offstruct + len] > 255)
							c_error(4, 143);
					}
				}
				len += (*ob).bloc.totalen;
	...
	if ((p = pieza) == p_asig || (p >= p_add_asig && p <= p_shl_asig)) {
		if ((*(_exp - 1)).tipo == eoper &&
			...
			if (p == p_asig) { // cadena="..."; // Hacer el strcpy
				...
				(*_exp).tipo = eoper;
				(*_exp++).token =
				    p_string; // Sigue habiendo una cadena en la
					      // pila
			} else if (p == p_add_asig) { // cadena+="..."; // Hacer
						      // el strcat
				_exp--;
				lexico();
				exp00(2);
				...
				(*_exp++).token =
				    p_string; // Sigue habiendo una cadena en la
					      // pila
			} else if (p == p_sub_asig) { // cadena-=n; // Quita
						      // caracteres
				_exp--;
				lexico();
				exp00(-1);
...
{ // Operadores con asociatividad de izquierda a derecha ->
	...
	while ((p = pieza) >= p_or && p <= p_andofs) {
		if (p >= p_xorptr)
			p -= p_xorptr -
			     p_xor; // Convierte xorptr,andofs en xor,and
		lexico();
		div_exp2();
		(*_exp).tipo = eoper;
		(*_exp++).token = p;
...
void div_exp2()
{
	...
	while ((p = pieza) >= p_igu && p <= p_mai) {
		...
		if (tf == 2 || tipo_factor == 2) {
			// OJO, p==q no se har┬á el strcmp() si son dos punteros
			// a cadenas
			p += p_strigu - p_igu;
			(*_exp).tipo = eoper;
			(*_exp++).token = p;
...
void exp4()
{
	...
	while ((p = pieza) >= p_suma && p <= p_sub &&
		...
		exp5();
		if (p == p_add &&
		    (tf == 2 || tipo_factor == 2 || tipo_expresion == 2)) {
			(*_exp).tipo = eoper;
			(*_exp++).token = p_stradd;
			(*_exp).tipo = eoper;
...
void unario()
{
	...
	if ((p >= p_xorptr && p <= p_mul) || (p == p_inc || p == p_dec)) {
		...
		if (p >= p_sub && p <= p_mul)
			...
			p += p_pointer -
			     p_xorptr; // xorptr,andofs pointer,offset
...
{ // Operador de acceso a variables o tablas locales ajenas
	...
	while (pieza == p_punto) {
		...
		// (bueno, si, hacer aqu┬í una b┬úsqueda del p_punto entre e-1 y
		// _exp ...)
...
lex_scan:
	ierror = _source;
	switch (
	    (memptrsize)lex_case[*_source]) { // Puntero a un lex_ele o l_???
	...
	case l_id:
		...
		*ivnom.p++ = 0;
		...
		ptr = &vhash[h];
		while (*ptr &&
		       strcmp((char *)(ptr + 2), (char *)(_ivnom + ptr8)))
			ptr = (byte **)*ptr;
		if (!strcmp((char *)(ptr + 2),
			    (char *)_ivnom + ptr8)) { // id found
			ivnom.b = _ivnom;	     // lo saca de vnom
			pieza = (memptrsize) * (ptr + 1);
			if (pieza < 256 &&
			    pieza >= 0) { // palabra reservada (token)
				if (pieza == p_rem) {
					while (*_source != cr)
						_source++;
					goto lex_scan;
	...
	case l_lit:
		...
		pieza = p_lit;
		h = *_source;
		_ivnom = ivnom.b; // Literal entre dos h
		do {
			if (*++_source == cr)
				c_error(0, 11);
			if (*_source == h)
		...
		if (*_source == '0' && lower[*(_source + 1)] == 'x') {
			...
			while ((memptrsize)lex_case[*_source] == l_num ||
				...
				if ((memptrsize)lex_case[*_source] == l_num)
					pieza_num =
					    pieza_num * 16 + *_source++ - 0x30;
				else
					pieza_num = pieza_num * 16 +
						    lower[*_source++] - 'a' +
						    10;
			}
		} else
...
void tglo_init2(int tipo)
{
	...
	while (1) {
		...
		if (pieza == p_cerrar || pieza == p_ptocoma) {
			if (*(imemptr - (memptrsize)mem + (memptrsize)frm) ==
				1 ||
			    tipo == 1) {
				imemptr += 2;
				imem = ((memptrsize)imemptr - (memptrsize)mem +
					3) /
				       4;
				if (*(imemptr - (memptrsize)mem +
				      (memptrsize)frm) != 1 &&
				    tipo != 1)
					imemptr = (byte *)&mem[imem];
			} else if (*(imemptr - (memptrsize)mem +
				     (memptrsize)frm) == 2 ||
				   tipo == 2) {
				imemptr++;
				imem = ((memptrsize)imemptr - (memptrsize)mem +
		...
		// Una coma sola, como "3,,4", avanza una posici┬ón (y define un
		// 0)
		...
		if (pieza == p_abrir) { // Dup x 1
			...
			if ((((memptrsize)imemptr - (memptrsize)mem) % 4) ==
			    0) {
				imem = ((memptrsize)imemptr - (memptrsize)mem +
					3) /
				       4;
				if (frm[imem] == 0xdad00000) {
					if (strlen((char *)&mem_ory[valor]) >
					    (mem[imem] & 0xFFFFF) + 1)
						c_error(2, 129);
					_imem = imem;
					imem +=
					    1 + ((mem[imem] & 0xFFFFF) + 5) / 4;
					//          test_buffer(&mem,&imem_max,imem);
					imemptr = (byte *)&mem[imem];
					strcpy((char *)&mem[_imem + 1],
					       (char *)&mem_ory[valor]);
					if (pieza != p_coma)
						return;
					lexico();
			...
			// Mete un valor literal en otro tipo de dato (en lugar
			// de un string)

			if (*(imemptr - (memptrsize)mem + (memptrsize)frm) ==
				1 ||
			    tipo ==
				1) { // Un string en words (el puntero short)

				imemptr = (byte *)(((memptrsize)imemptr + 1) &
						   -2); // Lo hace par
				*((word *)imemptr) = valor;
				imemptr += 2;
				imem = ((memptrsize)imemptr - (memptrsize)mem +
				...
				continue;
				     ...
				     (memptrsize)frm) == 2 ||
				   tipo == 2) { // Un string en bytes

				oimemptr = imemptr;
				while (*(oimemptr - (memptrsize)mem +
					 (memptrsize)frm) == 2)
					oimemptr++;
				if (tipo == 0)
					if (strlen((char *)&mem_ory[valor]) >
					    (memptrsize)(oimemptr - imemptr))
						c_error(2, 33);
				strcpy((char *)imemptr,
				       (char *)&mem_ory[valor]);
			...
			} else { // Un string en un int (el puntero long, como
				 // en DIV 1)
			...
			if (pieza != p_abrir) {
				if (pieza != p_dup) {
					...
					if ((((memptrsize)imemptr -
					      (memptrsize)mem) %
					     4) == 0) {
						imem = ((memptrsize)imemptr -
							(memptrsize)mem + 3) /
						       4;
						if (frm[imem] == 0xdad00000)
							c_error(2, 128);
					...
					} else { // En un int (lo normal en DIV
						 // 1)
		...
		if (dup > 1) {
			...
			while (--dup) {
				for (n = 0; n < len; n++) {
					...
					// Comprueba que las cadenas caigan
					// sobre si mismas en los dup()...
					if ((((memptrsize)imemptr -
					      (memptrsize)mem) %
					     4) == 0) {
						imem = ((memptrsize)imemptr -
					...
					// Comprueba que los words caigan sobre
					// words
					if (*(byte *)((memptrsize)imemptr -
						      (memptrsize)mem +
						      (memptrsize)frm) == 1) {
						if (*(byte *)((memptrsize)
								  oimemptr +
					...
					// Comprueba que los bytes caigan sobre
					// bytes
					if (*(byte *)((memptrsize)imemptr -
						      (memptrsize)mem +
						      (memptrsize)frm) == 2) {
	...
	do {
		...
		case eoper:
			...
			case p_pointerbyte:
				...
				break;
			case p_string:
				break; // Es como un p_pointer nulo, que no se
				       // hace
...
void analiza_private(void)
{
	...
	if (pieza == p_private) {
		...
		while (pieza == p_id || pieza == p_int || pieza == p_pointer ||
		       pieza == p_struct || pieza == p_string ||
		       pieza == p_byte || pieza == p_word) {

			if (pieza == p_struct) {

				lexico();

				if (pieza == p_pointer) { // Se define un
							  // puntero a struct
					...
					lexico();
				puntero_a_struct:
					if (analiza_pointer_struct(tpslo, iloc,
								   ob) == 1) {
						iloc++;
						if (pieza == p_asig) {
							lexico();
				...
				if (pieza ==
				    p_pointer) { // Miembro puntero a int
				...
				puntero4:
					...
					// Si el objeto no es tnone, se repite
					// un par┬ámetro o bien es un error
...
struct _dlls {
	...
	int linea1,
	    columna1; // Posicion en la que comienza la sentencia en el fuente
	int linea2,
	    columna2; // Posicion en la que acaba la sentencia en el fuente
} dlls[64];
	...
	do
		...
		case eoper:
			...
			case p_neg:
				...
				// Creo que se quit┬ó los pointer (y los rangos,
				// por consiguiente) por los FROM, ya que
				// provocaba confusion el tomar como constante
				// una variable ...

				//      case p_pointer: pila[i]=mem[pila[i]];
				//      break;
	...
	while (pieza != p_ultima)
		...
		case p_local:
			...
			if (pieza == p_id) {
				...
				iloc +=
				    (*ob).sloc.totalitems * (*ob).sloc.len_item;
			...
			if (*buf == cr || *buf == ' ' || *buf == tab)
				...
				*ptr = _ivnom;
				...
				ivnom.b++;
				   ...
				   t <= 0x7b) { // Analiza un delimitador de
						// literal
...
void test_buffer(int **buffer, int *maximo, int n)
{
	...
	if (n > *maximo - security_distance) {
		...
		*maximo = n + buffer_grow;
		max = *maximo - max;
		if (*buffer == mem) {
			if ((*buffer = mem_ory = (int *)realloc(
				 *buffer, *maximo * sizeof(memptrsize))) ==
			    NULL)
				c_error(0, 0);
		} else {
			if ((*buffer = (int *)realloc(
				 *buffer, *maximo * sizeof(memptrsize))) ==
			    NULL)
				c_error(0, 0);
		}
		memset((byte *)*buffer + (*maximo - max) * sizeof(memptrsize),
		       0, max * sizeof(memptrsize));
	}
}
...
void sintactico(void)
{
	...
	if (pieza == p_setup_program) {
		...
		strcpy((char *)cWork, "system/exec.ins");
	...
	while (pieza == p_import) {
		...
		lexico();
		if (pieza != p_lit &&
		    !(pieza == p_id && (*o).tipo == tcons && (*o).cons.literal))
			c_error(1, 62);
		old_source = source;
		...
		if (nombre_dll == NULL)
			c_error(0, 63);
		if ((num_extern = ImportDll((char *)nombre_dll)) == 0)
			c_error(0, 63);
		if (num_extern > 0) {
			printf("num funcs: %d\n", num_extern);
	...
	if (pieza == p_const) {
		...
		while (pieza == p_id) {
			ob = o;
			if ((*ob).tipo != tnone && (*ob).tipo != tcons)
				c_error(0, 116);
			(*ob).tipo = tcons;
			lexico();
			if (pieza != p_asig)
	...
	if (pieza == p_import)
		c_error(0, 147); // Advertir a usuarios de DIV 1
	...
	// Procesos
	//├ä├ä├ä├ä├ä├ä├ä├ä├ä├ä├ä├ä├ä├ä├ä├ä├ä├ä├ä├ä├ä├ä├ä├ä├ä├ä├ä├ä├ä├ä├ä├ä├ä├ä├ä├ä├ä├ä├ä├ä├ä├ä├ä├ä├ä├ä├ä├ä├ä├ä├ä├ä├ä├ä├ä├ä├ä├ä├ä├ä├ä├ä├ä├ä├ä├ä├ä├ä├ä├ä├ä├ä├ä├ä├ä
	// p_process p_id p_abrir [ id { , id } ] p_cerrar { ; } p_begin ...
	// p_end
	//├ä├ä├ä├ä├ä├ä├ä├ä├ä├ä├ä├ä├ä├ä├ä├ä├ä├ä├ä├ä├ä├ä├ä├ä├ä├ä├ä├ä├ä├ä├ä├ä├ä├ä├ä├ä├ä├ä├ä├ä├ä├ä├ä├ä├ä├ä├ä├ä├ä├ä├ä├ä├ä├ä├ä├ä├ä├ä├ä├ä├ä├ä├ä├ä├ä├ä├ä├ä├ä├ä├ä├ä├ä├ä├ä
	...
	while (pieza == p_process || pieza == p_function) {
		...
		parametros = -1; // Para que los par┬ámetros se puedan repetir
				 // como PRIVATE
...
void l_ensamblador(void)
{
	...
	while (i < imem) {
		...
		case lcar2:
			fprintf(sta, "%5u\tcar2 %u %u", i, mem[i + 1],
				mem[i + 2]);
			i += 2;
			break;
		case lcar3:
			fprintf(sta, "%5u\tcar3 %u %u %u", i, mem[i + 1],
				mem[i + 2], mem[i + 3]);
			i += 3;
			break;
		case lcar4:
			fprintf(sta, "%5u\tcar4 %u %u %u %u", i, mem[i + 1],
				mem[i + 2], mem[i + 3], mem[i + 4]);
			i += 4;
			break;
		case lasiasp:
...
void save_dbg(void)
{
	...
	for (n = 0; n < num_obj; n++) {
		...
		if (obj[n].tipo == tpsgl || obj[n].tipo == tpslo)
			ob.v1 = (ob.v1 - (memptrsize)&obj[0]) /
				sizeof(struct objeto);
		// OJO ! que no se pueden a┬ñadir objetos aqu┬í (ver uso de
		// &obj[0] y sizeof(struct objeto))
...
lex_scan:
	...
	case l_cr:
		if (linea) {
			...
			if (convert) {
				fwrite(ultima_linea, 1, _source - ultima_linea,
				       lprg);
	...
	case l_id:
		...
		*ivnom.p++ = 0;
		...
		while (*ptr != NULL &&
			...
			ptr = (byte **)*ptr;
		if (!strcmp((char *)(ptr + 2),
			    (char *)_ivnom + ptr8)) { // id encontrado
			ivnom.b = _ivnom;	     // lo saca de vnom
			next_pieza = (memptrsize) * (ptr + 1);
			if (next_pieza < 256 &&
			    next_pieza >= 0) { // palabra reservada (token)
				if (next_pieza == p_rem) {
					while (*_source != cr)
						_source++;
					goto lex_scan;
...
void sentencia()
{
	...
	while (pieza >= p_return) {
		...
		case p_from:
			...
			if (dir >=
			    0) { // Comparaci┬ón de la condici┬ón de permanencia
				g2(lcar, dir);
			} else {
				g2(lcar, -dir);
				g1(laid);
			...
			if (dir >=
			    0) { // Incremento y vuelta al inicio del bucle
				g2(lcar, dir);
			} else {
				g2(lcar, -dir);
				g1(laid);
	...
	case ldiv:
		if (code[15].op == lcar2) {
			...
			if (mem[imem - 1] == 1)
				...
				code[15].op = mem[imem - 2] =
				    lcardiv; // Un cardiv nunca ser┬á "cardiv 0"
				optimizado = 1;
			}
		}
		break;
	case lneg:
		if (code[15].op == lcar || code[15].op == lcar2 ||
		    code[15].op == lcar3 || code[15].op == lcar4) {
...
lex_scan:
	...
	case l_lit:
		...
		if ((f = div_open_file((char *)ivnom.b)) != NULL) {
			...
			_splitpath(full, (char *)drive, (char *)dir,
				   (char *)fname, (char *)ext);
			strupr((char *)ext);

			if (!strcmp((char *)ext, ".MOD") ||
			    !strcmp((char *)ext, ".S3M") ||
			    !strcmp((char *)ext, ".XM")) {
				empaquetable = 1;
...
*/
	...
	imem = 0;
	if ((mem_ory = mem = (int *)malloc(imem_max * sizeof(memptrsize))) ==
	    NULL)
		c_error(0, 0);
	...
	*(source + source_len + 1) = cr;
	...
	mem[4] =
	    0; // Antes mem[1]-mem[3] (long datos globales), ahora no se utiliza
	mem[5] = iloc_len - iloc;
	mem[6] = iloc;
	...
	if (program_type == 1)
		if ((f = fopen("install/setup.ovl", "wb")) != NULL) {
			...
			if (p != NULL && q != NULL) {
				...
				if (true)
#else
				if (!compress(q, (uLongf *)&m, p,
					      (unsigned long)n))
#endif
				{
					fwrite(&n, 1, 4, f); // mem[0]..mem[8],longitud_datos_descomp,datos
							     // comp...
					fwrite(q, 1, m, f);
					free(q);
					free(p);
...
{
	inicializa_textos(
	    (uint8_t *)"system/lenguaje.div"); // OJO emitir un error si
					       // lenguaje.div no existe
	FILE *f;
	byte *prgbuf;
