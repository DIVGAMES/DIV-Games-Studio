...
unsigned int _dos_findnext(struct find_t *result)
{
	...
	while ((n = _findnext(hFile, &result2)) == 0) {
		...
		//result2.name,result2.attrib,type);
		...
		if ((type == _A_NORMAL && result2.attrib != _A_SUBDIR) ||
			...
			// [%s]\n",findmask,&findname[strlen(findname)-3],
			// &findmask[2]);

			if (strchr(findmask, '*') != NULL) {

				if (!strcmp(&findname[strlen(findname) - 3],
					    &findmask[2])) {
					// printf("found it %s
					// [%s]\n",findname,findmask);
...
void __mkdir(char *dir) { printf("mkdir %s\n", dir); }
