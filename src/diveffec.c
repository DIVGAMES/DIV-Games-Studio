...
void init_rnd(int,
	      char *); // Inicializa el generador aleatorio con un int y clave
...
void crear_puntos(void)
{
	...
	for (m = 0; m < n_exp; m++) {
		...
		for (n = 0; n < n_pun; n++) {
			...
			dist = (double)(rnd() % 10000) /
			       10000.0; // Distancia (0 .. 0.8)

			e[m].p[n].ix = cos(ang) * 10000.0;
			e[m].p[n].iy = sin(ang) * 10000.0;

			e[m].p[n].x = cx + (int)(cos(ang) * dist *
						 (float)(exp_ancho / 2));
			e[m].p[n].y =
			    cy + (int)(sin(ang) * dist * (float)(exp_alto / 2));
			...
			e[m].p[n].fuerza =
			    e[m].p[n].radio + rnd() % (e[m].p[n].radio) * 4;
		}
	}
}
		...
		for (x = 0; x < exp_ancho; x++) {
			...
			for (m = 0; m < n_exp; m++) {
				...
				for (n = 0; n < n_pun; n++) {
					dx = abs(x - e[m].p[n].x);
					dy = abs(y - e[m].p[n].y);
					if ((dx < e[m].p[n].radio / 10000) &&
					    (dy < e[m].p[n].radio / 10000)) {
...
void Explode1(void)
{
	...
	for (x = 0; x < 64; x++)
		wbox(v.ptr, an, al, exp_colores[x * 2], an - 69 + x, 18 + 1, 1,
		     9);
