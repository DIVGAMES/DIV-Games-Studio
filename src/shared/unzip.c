...
/* Slightly modified 19th Jan 2000 M.Green to remove some ambiguous compiler
 * errors */
...
int divz_open_file(char *full)
{
	...
	if (unzLocateFile(zip, full, 2) == UNZ_OK) {
		if (unzOpenCurrentFile(zip) == UNZ_OK) {
			printf("FOUND IT!\n");
			if (unzGetCurrentFileInfo(zip, &fileInfo, NULL, 0, NULL,
						  0, NULL, 0) == UNZ_OK) {
				//          char *filename = (char
				//          *)malloc(fileInfo.size_filename +
				//          1); unzGetCurrentFileInfo(zip,
				//          &fileInfo, filename,
				//          fileInfo.size_filename + 1, NULL, 0,
				//          NULL, 0);
				...
				if (zipptr)
					free(zipptr);
				// alloc the space
				zipptr = (unsigned char *)malloc(
				    fileInfo.uncompressed_size);

				// read the data
				readBytes = unzReadCurrentFile(
				    zip, zipptr, fileInfo.uncompressed_size);
				unzClose(zip);
				zip = NULL;
	...
	*psize_local_extrafield = 0;
	...
	if (fseek(s->file,
		  s->cur_file_info_internal.offset_curfile +
		      s->byte_before_the_zipfile + datastartpos,
		  SEEK_SET) != 0)
		return UNZ_ERRNO;
