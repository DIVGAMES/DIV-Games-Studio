...
void call(const voidReturnType func) { func(); }
...
void _splitpath(const char *Path, char *Drive, char *Directory, char *Filename,
		char *Extension)
{
	...
	char *ext = Extension;
	while (*Filename != '\0') {
		// the part behind the point is called extension in windows
		// systems at least that is what i thought apperantly the '.' is
		// used as part of the extension too .
		if (*Filename == '.') {
			*Filename = '\0';
			*Filename++;
			*Extension = '.';
...
#ifndef __llvm___
#ifdef __cplusplus
void mkdir(char *dir) { printf("mkdir %s\n", dir); }
#endif
#endif
...
void sdlkeyinit(void)
{
	...
	const _enter = 28
	    ...
	    const _num_lock = 69 const _c_backslash = 53 const _c_asterisk =
		55 const _c_minus = 74 const _c_home = 71 const _c_up =
		    72 const _c_pgup = 73 const _c_left = 75 const _c_center =
			76 const _c_right = 77 const _c_end = 79 const _c_down =
			    80 const _c_pgdn = 81 const _c_ins =
				82 const _c_del = 83 const _c_plus =
				    78 const _c_enter = 28
#endif
}
