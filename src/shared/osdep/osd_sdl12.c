...
void OSDEP_Quit(void) {
	...
	// Fix: according with official DOC of SDL 1.2: https://www.libsdl.org/release/SDL-1.2.15/docs/html/sdlsetvideomode.html
	// screen surface created by SDL_SetVideoMode is freed by SDL_Quit (last instruction) and should not be freed by the caller (causing possible crash)
...
//	vheight    = height;
	...
	OSDEP_screen = SDL_SetVideoMode(width,
	                                height,
	                                bpp,
	                                flags);
	if(OSDEP_screen == NULL) { // Fix: memory alloc check
		fprintf(stderr,
		        "SetVideoMode failed: %s\n",
		        SDL_GetError());
		exit(1);
	}
#ifndef __EMSCRIPTEN__
	OSDEP_surface = SDL_CreateRGBSurface(0,
	                                     width,
	                                     height,
	                                     8,
	                                     0, 0, 0, 0);
	if(OSDEP_surface == NULL) { // Fix: memory alloc check
		fprintf(stderr,
...
void OSDEP_Flip(OSDEP_Surface *s) {
	...
	if((vwidth == vga_an && vheight == vga_al) ||
	   (vwidth == 0      && vheight == 0)) {
		...
		OSDEP_zoomsurface = zoomSurface(s,
		                                (float)vwidth / vga_an,
		                                (float)vheight / vga_al,
		                                1);

		// Clear screen
		SDL_FillRect(OSDEP_screen,
		             NULL,
		             0);

		// Blit
		SDL_BlitSurface(OSDEP_zoomsurface,
		                NULL,
		                OSDEP_screen,
		                NULL);
...
fprintf(stdout,
        ...
        vheight);
			...
			OSDEP_screen = SDL_SetVideoMode(vwidth,
			                                vheight,
			                                8,
			                                flags);
			if(OSDEP_screen == NULL) { // Fix: memory alloc check
...
int OSDEP_SetPalette(OSDEP_Surface *surface, OSDEP_Color *colors, int firstcolor, int ncolors) {
	memcpy(OSDEP_pal,
	       colors,
	       256 * sizeof(OSDEP_Color));

	SDL_SetPalette(OSDEP_screen,
	               SDL_LOGPAL | SDL_PHYSPAL,
	               colors,
	               0,
	               256);

	return SDL_SetPalette(surface,
	                      SDL_LOGPAL | SDL_PHYSPAL,
	                      colors,
	                      0,
	                      256);
