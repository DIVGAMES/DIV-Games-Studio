...
// Timer
uint32_t OSDEP_GetTicks(void) { return SDL_GetTicks(); }
...
void OSDEP_WarpMouse(int x, int y) { SDL_WarpMouse(x, y); }
...
OSDEP_Surface *OSDEP_SetVideoMode(int width, int height, int bpp, char fs)
{
	...
	OSDEP_surface = SDL_CreateRGBSurface(0, width, height, 8, 0, 0, 0, 0);
#endif
	sw = width;
	sh = height;
...
void OSDEP_Flip(OSDEP_Surface *s)
{
	...
	if ((vwidth == vga_an && vheight == vga_al) ||
		...
		OSDEP_zoomsurface = zoomSurface(s, (float)vwidth / vga_an,
						(float)vheight / vga_al, 1);
		// Clear screen
		...
		if (sw != vwidth || sh != vheight) {
			...
			// OSDEP_screen = OSDEP_SetVideoMode(width, height, 8,
			// fullscreen);
			fprintf(stdout, "%s %d %d\n", __FUNCTION__, vwidth,
				vheight);
			if (OSDEP_screen != NULL) {
		     ...
		     int firstcolor, int ncolors)
{
	...
	return SDL_SetPalette(surface, SDL_LOGPAL | SDL_PHYSPAL, colors, 0,
			      256);
}

// Joysticks
int32_t OSDEP_NumJoysticks(void) { return SDL_NumJoysticks(); }

int OSDEP_JoystickNumButtons(int n) { return SDL_JoystickNumButtons(n); }

int OSDEP_JoystickNumHats(int n) { return SDL_JoystickNumHats(n); }

int OSDEP_JoystickNumAxes(int n) { return SDL_JoystickNumAxes(n); }
...
OSDEP_Joystick *OSDEP_JoystickOpen(int n) { return SDL_JoystickOpen(n); }
...
char *OSDEP_JoystickName(int n) { return SDL_JoystickName(n); }
