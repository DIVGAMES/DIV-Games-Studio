...
void path_find(void)
{
	...
	if (m(bx, by)) { // Caso en el que la casilla final est ocupada
		...
		x = 0; // Si la casilla final no est  libre, prueba con una
		       // adyacente
	...
	if (ax == bx && ay == by) {
		if (size < 2)
			return;
		mem[offset] = x;
		mem[offset + 1] = y; // Va directamente hasta (x,y)
		pila[sp] = 1;
		return;
	}
	...
	memset(distancias, 0,
		   ...
		   2); // Distancias si hace falta limpiarlo, para ver
	...
	if (!modo)
		do {
			...
			if (c == 65535)
				break;
			cy = c / max_map_size;
			cx = c % max_map_size; // Hace falta por comprobar los
					       // lกmites (y el mapa)
		} while (!fin);
	else
		do {
	...
	// Si (fin==1) se alcanzข la casilla (bx,by), y el camino se saca de
	// dis()
	...
	// Hay un camino, ahora obtiene los vrtices en la tabla pasada como
	// par metro

	pila[sp] = calcula_vertices(&mem[offset], size / 2, mem[id + _X],
				    mem[id + _Y], x, y);
...
{		   // Versiขn r pida
	...
	a = c;
	do { // Busca el lugar para esta nueva casilla
		i = a;
		a = sig(a);
		if (a == 65535)
			break;
...
int calcula_vertices(int *ptr, int max_ver, int x0, int y0, int x1, int y1)
{
	...
	int nextx[8],
	    nexty[8]; // Las prขximas ocho casillas (ix,iy desde bx,by)

	if (num < 1)
		return (0);
	*(--p) = y1;
	*(--p) = x1;
	num--; // Mete el ฃltimo vrtice
	...
	do {
		...
		do {
			...
			if (d > 14 * 8) {
				...
				puede_ir(x1, y1, x + nextx[7] * tile,
					 y + nexty[7] * tile);
				if (!choque_linea) {
					bx += nextx[7];
					by += nexty[7];
				} else {
					puede_ir(x1, y1, x + nextx[3] * tile,
						 y + nexty[3] * tile);
					if (!choque_linea) {
						bx += nextx[3];
						by += nexty[3];
				...
				xx = x;
				yy = y; // desde x1,y1 hasta xx,yy "SI" puede ir
					// SIEMPRE
				x = bx * tile + tile / 2;
				y = by * tile + tile / 2;
				cas = bx + by * max_map_size;
				...
				xx = x;
				yy = y;
				dir = 0; // desde x1,y1 hasta xx,yy "SI" puede
					 // ir SIEMPRE
	...
	if (x != x0 ||
	    y != y0) { // จes necesario ir al centro de la primera casilla?
		...
		if (choque_linea) {
			...
			*(--p) = y;
			*(--p) = x;
			num--; // Un caso poco probable, pero bueno ...
		}
	}
