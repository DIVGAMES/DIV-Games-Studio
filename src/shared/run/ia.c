...
void add(int x, int y, word bnew, word step) {
	...
	// Busca el lugar para esta nueva casilla
	a = c;
	do {
		i = a;
		a = sig(a);
...
int calcula_vertices(int *ptr, int max_ver, int x0, int y0, int x1, int y1) {
	...
	*(--p) = x1;
	...
	do {
		do {
			// Obtiene la siguiente (bx,by) y (x,y)
			d = dis(cas);
			...
			if(d > 14 * 8) {
				...
				// desde x1, y1 hasta xx, yy "SI" puede ir SIEMPRE
				xx  = x;
				yy  = y;
				x   = bx * tile + tile / 2;
				y   = by * tile + tile / 2;
		...
		// A¤ade un nuevo v‚rtice a la ruta
		if(choque_linea) {
			if(!num) {
				return (0);
			}
	...
	// ¨es necesario ir al centro de la primera casilla?
	if(x != x0 || y != y0) {
		puede_ir(x1,
		         y1,
		         x0,
		...
		if(choque_linea) {
			...
			// Un caso poco probable, pero bueno ...
			*(--p) = y;
			*(--p) = x;
			num--;
		}
...
/*
	// Tama¤o del tile ... si es muy grande lo divide entre 2
	if(tile > 3) {
		x0       = x0 / 2;
		y0       = y0 / 2;
		x1       = x1 / 2;
...
*/
	...
	if(x0 > x1) { x = x1; dx = x0 - x1; }
	else        { x = x0; dx = x1 - x0; }
	if(y0 > y1) { y = y1; dy = y0 - y1; }
	else        { y = y0; dy = y1 - y0; }
...
//══════════════════════════════════════════════════════════════════════════════════════════════════════
// DIV path_find(INT mode, INT file, INT graph, INT tilesize, INT x, INT y, INT * table, INT table_size)
//══════════════════════════════════════════════════════════════════════════════════════════════════════
void DIV_path_find(void) {
	...
	// Por defecto, y hasta que se demuestre lo contrario
	pila[sp] = 0;
	...
	if(an < 1            || al < 1 ||
	   an > max_map_size || al > max_map_size) {
		e(152);
		return;
	}
	...
	// Caso en el que la casilla final est‚ ocupada
	if(m(bx, by)) {
		// Si la casilla final no est  libre, prueba con una adyacente
		x = 0;
	...
	// Distancias si hace falta limpiarlo, para ver
	// que casillas est n ya visitadas en el "fill"
	memset(distancias,
	       0,
	       max_map_size * max_map_size * 2);
	...
	sig(c) = 65535;
	// No hay una distancia (1 para marcarla como visitada)
	dis(c) = 1;
	...
	if(!modo) {
		do {
			...
			// Hace falta por comprobar los l¡mites (y el mapa)
			cy = c / max_map_size;
			cx = c % max_map_size;
		}while(!fin);
	}
	...
	// ­­ No se encontr¢ (no hay) un camino hasta bx,by !!!
	if(!fin) {
		m(ax, ay) = cas_inicial;
		return;
	}
  ...
  pila[sp] = calcula_vertices(&mem[offset],
	                             size / 2,
	                             mem[id + _X],
	                             mem[id + _Y],
	                             x,
...
//════════════════════════════════════════════════════════════════════════════
// DIV path_line(INT file, INT graph, INT tilesize, INT x, INT y)
//════════════════════════════════════════════════════════════════════════════
void DIV_path_line(void) {
	int file, code, x ,y;
	int *ptr;
...
//════════════════════════════════════════════════════════════════════════════
// DIV path_free(INT file, INT graph, INT tilesize_graph, INT x, INT y)
//════════════════════════════════════════════════════════════════════════════
void DIV_path_free(void) {
	int file, code, x, y;
	int *ptr;
