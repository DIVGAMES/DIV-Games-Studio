...
void InitSound(void)
{
	...
	char Device[8] = {DEV_NOSOUND, DEV_SB,   DEV_SB,  DEV_SBPRO,
			  DEV_SB16,    DEV_SB16, DEV_GUS, DEV_GUS};
	...
	if (SoundActive) {
			...
			if (File_Cfg != NULL) {
				...
				if (fread(&mixer, 1, 4, File_Cfg) == 4) {
					...
					if (mixrate > 44100)
						mixrate = 44100;
					if (mixmode == 16)
						mixmode = SIXTEENBIT | STEREO;
					else
						mixmode = EIGHTBIT | STEREO;
				}
		...
		if (mixmode == (SIXTEENBIT | STEREO))
			...
			setup->mixmode = 8;
		setup->card = setup->port = setup->irq = setup->dma =
		    setup->dma2 = 0;
...
void ResetSound(void)
{
	...
	if (setup->mixrate < 11025)
		setup->mixrate = 11025;
	if (setup->mixrate > 44100)
		setup->mixrate = 44100;
	if (setup->mixmode == 16)
		setup->mixmode = 16;
	else
...
#endif
	// always play as loop, let the freqEffect manage stop_sound when loop
	// is zero this permits slow playing sound to run for the correct
	// length.
...
int PlaySong(int NumSong)
{
	...
	if (cancion[NumSong].music) {
		Mix_PlayMusic(cancion[NumSong].music,
			      cancion[NumSong].loop ? -1 : 0);
	}
#endif
	...
	case XM:
		...
		if (cancion[NumSong].loop)
			judas_playxm(1000000000);
		else
			judas_playxm(1);
...
void SetSongPos(int SongPat)
{
	...
	switch (SongType) {
	case XM:
		judas_set_xm_pos(SongPat);
		break;
	case S3M:
		judas_set_s3m_pos(SongPat);
		break;
	case MOD:
		judas_set_mod_pos(SongPat);
		break;
	}
#endif
}
...
int GetSongLine(void)
{
	...
	case XM:
		...
		break;
	case S3M:
		pos = judas_gets3mline();
		break;
	case MOD:
		pos = judas_getmodline();
		break;
	}
