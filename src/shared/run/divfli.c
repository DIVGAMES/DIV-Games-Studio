...
int StartFLI(char *flinombre, char *Buffer, int Buff_anc, int Buff_alt, int cx,
	     int cy)
{
	...
	if ((info.Width + cx > Buff_anc) || (info.Height + cy > Buff_alt))
		return (0);
	if ((TFframe = (TFUByte *)malloc(info.Height * info.Width + 256)) ==
	    NULL)
		return (0);
...
void SS2()
{
	...
	while (Lines--) {
		...
		while (Count & 0xc000) {
			 ...
			 */
			if ((Count & 0xc000) ==
			    0xc000) { // 0xc000h = 1100000000000000
				pDst += (0x10000 - Count) *
					vga_an; // flc.mainscreen->pitch;
			}

			if ((Count & 0xc000) ==
			    0x4000) { // 0x4000h = 0100000000000000
...
int Nextframe()
{
	...
	for (i = 0; i < info.Height; i++)
		memcpy(CBuffer + TFOffset + i * CBuff_anc,
		       TFframe + i * info.Width, info.Width);
	TFAnimation_GetInfo(animation, &info);
