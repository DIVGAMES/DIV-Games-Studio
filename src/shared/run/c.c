...
void test_collision(byte *buffer, int *ptr, int x, int y, int xg, int yg,
		    int angle, int size, int flags);
...
void test_scanc(byte *p, short n, short m, short o, byte *si, int an, int x0,
		int y0, int x1, int y1);
void test_scan(byte *p, short n, byte *si, int an, int x0, int y0, int x1,
	       int y1);
...
void out_region(void)
{
	...
	reg = pila[sp--];
	id = pila[sp];
	pila[sp] = 1; // Por defecto no est  en la region
	...
	if (angle) {
		sp_size(&x, &y, &an, &al, xg, yg, angle, mem[id + _Size],
			mem[id + _Flags]);
		an = an - x + 1;
		al = al - y + 1;
	} else if (mem[id + _Size] != 100) {
		sp_size_scaled(&x, &y, &an, &al, xg, yg, mem[id + _Size],
			       mem[id + _Flags]);
		an = an - x + 1;
		al = al - y + 1;
	} else {
	...
	if (mem[id + _Ctype] == 1) {
		...
		for (n = 0; n < 10; n++)
			if (iscroll[n].on &&
				...
				xg = x + iscroll[n].x -
				     iscroll[n].map1_x; // Posiciขn del sprite
							// en el scroll
				yg = y + iscroll[n].y - iscroll[n].map1_y;

				xg1 = xg + an;
				yg1 = yg + al; // 1ง Intersecciขn de xg,yg,an,al
					       // e iscroll[n]
				...
				if (xg >= xg1 || yg >= yg1)
					continue; // Si no hay intersecciขn,
						  // continua
		...
		if (x < region[reg].x1 && y < region[reg].y1 &&
		    x + an > region[reg].x0 && y + al > region[reg].y0)
			pila[sp] = 0; // Si est  en la regiขn
	}
}
...
void collision(void)
{
	...
	int i, bloque; // Recorre procesos de _IdScan en adelante, si _IdScan=0
		       // desde start
	...
	if (mem[id + _Ctype] == 1) {
		...
		pila[sp] = comprobar_colisiones(i, bloque, -1);
...
int comprobar_colisiones(int i, int bloque, int scroll)
{
	...
	if (bloque == 0) { // collision(type mouse)
		if (mouse->x >= clipx0 && mouse->x <= clipx1 &&
		    mouse->y >= clipy0 && mouse->y <= clipy1) {
			if (*(buffer + buffer_an * (mouse->y - clipy0) +
			      (mouse->x - clipx0)))
				return (id);
			else
				return (0);
	...
	for (; i <= id_end; i += iloc_len) {
		...
		if (i != id && mem[i + _Bloque] == bloque &&
			...
			if (mem[i + _Ctype] ==
			    1) { // Si es de scroll, determina que scroll (n)
				if (scroll < 0) { // Choque de pantalla con
						  // scroll - el primero activo
					for (n = 0; n < 10; n++)
						if (iscroll[n].on &&
						    (mem[i + _Cnumber] == 0 ||
						     (mem[i + _Cnumber] &
					...
					if (n == 10)
						continue;
				} else { // Choque de scroll con scroll - simpre
					 // que coincidan en el 1ง
					if (mem[i + _Cnumber] &&
					    !(mem[i + _Cnumber] &
					      (1 << scroll)))
						continue;
			...
			if (angle) {
				...
				if (dist == 0)
					dist =
					    (int)sqrt((buffer_an * buffer_an +
						       buffer_al * buffer_al) /
						      4);
				an = ((int)sqrt(ptr[13] * ptr[13] +
						ptr[14] * ptr[14]) *
				      mem[i + _Size]) /
					 100 +
				     dist;
				...
				an = ptr[13];
				al = ptr[14];
				sp_size_scaled(&xx, &yy, &an, &al, xg, yg,
					       mem[i + _Size], mem[i + _Flags]);
			...
			if (colisiona == 1) {
				colisiona = 0;
				test_collision(buffer, ptr, x, y, xg, yg, angle,
					       mem[i + _Size], mem[i + _Flags]);
				if (colisiona) {
...
void sp_size(int *x, int *y, int *xx, int *yy, int xg, int yg, int ang,
	     int size, int flags)
{
...
void sp_size_scaled(int *x, int *y, int *xx, int *yy, int xg, int yg, int size,
		    int flags)
{

	int x0, y0, x1,
	    y1; // Ventana ocupada por el sprite en el plano de copia
...
void put_collision(byte *buffer, int *ptr, int x, int y, int xg, int yg,
		   int angle, int size, int flags)
{
...
void sp_rotado_p(byte *si, int an, int al, int flags)
{
	...
	do {
		...
		if (h < clipy1 && h >= clipy0 && x0.w[1] < clipx1 &&
			...
			sp_scan(ptrcopia + x0.w[1], x1.w[1] - x0.w[1], si, an,
	...
	do {
		...
		if (h < clipy1 && h >= clipy0 && x0.w[1] < clipx1 &&
			...
			if (x0.w[1] < clipx0)
				...
				test_scanc(ptrcopia + x0.w[1],
					   x1.w[1] - x0.w[1],
					   clipx1 - 1 - x0.w[1], 0, si, an,
					   g0x.l, g0y.l, g1x.l, g1y.l);
			else
				test_scan(ptrcopia + x0.w[1], x1.w[1] - x0.w[1],
