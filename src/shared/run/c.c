...
//════════════════════════════════════════════════════════════════════════════
// DIV out_region(INT id, INT region)
// - solo para gr ficos de pantalla o scroll
//════════════════════════════════════════════════════════════════════════════
void DIV_out_region(void) {
		...
		for(n = 0; n < 10; n++) {
			if(iscroll[n].on &&
			  (mem[id + _Cnumber] == 0 || (mem[id + _Cnumber] & (1 << n)))) {
				...
				// 1§ Intersecci¢n de xg,yg,an,al e iscroll[n]
				xg1 = xg + an;
				yg1 = yg + al;
				...
				// Si no hay intersecci¢n, continua
				if(xg >= xg1 || yg >= yg1) {
					continue;
				}

				// Si est  en la regi¢n
				if(xg  < region[reg].x1 && yg  < region[reg].y1 &&
				   xg1 > region[reg].x0 && yg1 > region[reg].y0) {
					pila[sp] = 0;
					return;
				}
			}
...
//════════════════════════════════════════════════════════════════════════════
// DIV graphic_info(INT fpg_id, INT graph, INT info)
//════════════════════════════════════════════════════════════════════════════
void DIV_graphic_info(void) {
	int info, graph, file;
	int xg, yg;
	int *ptr;
...
int comprobar_colisiones(int i, int bloque, int scroll) {
	...
	for(; i <= id_end; i += iloc_len) {
		if(i != id                    &&
		   mem[i + _Bloque] == bloque &&
		  (mem[i + _Status] == 2      ||
		   mem[i + _Status] == 4)     &&
		   mem[i + _Ctype ] <  2) {
			// Si es de scroll, determina que scroll (n)
			if(mem[i + _Ctype] == 1) {
				// Choque de pantalla con scroll - el primero activo
				if(scroll < 0) {
					for(n = 0; n < 10; n++) {
				...
				// Choque de scroll con scroll - simpre que coincidan en el 1§
				else {
			...
			if(angle) {
				...
				an = ptr[13];
				al = ptr[14];
				if(mem[i + _Flags] & 1) { xxg = an - 1 - xg; } else { xxg = xg; }
				if(mem[i + _Flags] & 2) { yyg = al - 1 - yg; } else { yyg = yg; }
				xx = x - xxg;
				yy = y - yyg;
...
//════════════════════════════════════════════════════════════════════════════
// DIV collision(INT process_id/TYPE process)
// - Funcion principal del lenguaje collision(tipo de proceso)
//════════════════════════════════════════════════════════════════════════════
void DIV_collision(void) {
