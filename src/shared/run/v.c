...
struct {
	...
	{0xe3,0x0d06,0x3e07,0x4109,0xea10,0xac11,0xdf12,0x0014,0xe715, 0x0616,0xe317,0}, //320x240
...
// Tabla ghost
//════════════════════════════════════════════════════════════════════════════
typedef struct _t_tpuntos { // Para la creaci�n de la tabla ghost
	int r, g, b;
	struct _t_tpuntos *next;
} t_tpuntos;

t_tpuntos  tpuntos[256];
t_tpuntos *vcubos[512]; // Para la creaci�n de la tabla ghost
...
//		SDL_FreeSurface(vga);	
	...
	fsmode = 1;
//		if(!vga)
	vga = OSDEP_SetVideoMode(480, 272, 8, 0); //SDL_SWSURFACE | SDL_FULLSCREEN);
#else
	...
	for(n = 0; n < num_video_modes; n++) {
		if(vga_an == video_modes[n].ancho &&
		   vga_al == video_modes[n].alto) {
			if(video_modes[n].modo && video_modes[n].modo < 256000) {
...
#else
	SDL_UpdateTexture(divTexture,
	                  NULL,
	                  copia,
	                  vga_an * sizeof(Uint8));

	SDL_RenderClear(divRender);
	SDL_RenderCopy(divRender,
	               divTexture,
	               NULL,
	               NULL);
...
//		}
			if(!(framecount % (fps / 12)) && framecount > 5 && framecount < maxframes) { // && framecount % 3 == 1) {
				...
				sprintf(filename,
				        "/home/mike/Desktop/out/out%05d.bmp",
				        framecount);
...
void snapshot(byte *p) {
	...
	f = fopen(cwork, "wb");
	if(f) { // Fix: check file opening success to avoid crash saving PCX
		graba_PCX(p,
		          vga_an,
		          vga_al,
		          f);
...
int graba_PCX(byte *mapa, int an, int al, FILE *f) {
	...
	fwrite(&pcx.header, 1, sizeof(t_pcx_header), f);
...
int graba_MAP(byte *mapa, int an, int al, FILE *f) {
	int y;
	int code = 1, npoints = 0;
	char cwork[32] = "";
	char reglas[576];

	// +000 Cabecera y version
	fwrite("map\x1a\x0d\x0a\x00\x00", 8, 1, f);
	...
	for(y = 0; y < 16; y++) {
		...
		reglas[y * 36 + 2] = 0;
		reglas[y * 36 + 3] = 0;
		memset(&reglas[y * 36 + 4],
		        y * 16,
		        32);
	}
	// +816 Reglas de color
	fwrite(reglas, 1, sizeof(reglas), f);

	// +1392 Numero de puntos
	fwrite(&npoints, 2, 1, f);
		...
		while(y < vga_al) {
			...
			if(scan[n + 1]) {
				...
				if(point + scan[n + 1] > 65536) {
					...
					if(page != old_page) {
						SV_setBank((signed long)(old_page = page));
					}
...
void volcado_parcial(int x, int y, int an, int al) {
	...
	if(an > 0 && al > 0 && x < vga_an && y < vga_al) {
		...
		while(y <= ymax) {
			...
			// Caso 1, el scan estaba vac�o ...
			if(scan[n + 1] == 0) {
				scan[n    ] = x;
				scan[n + 1] = an;
			}
			// Caso 2, ya hay un scan definido ...
			else if(scan[n + 3] == 0) {
				// ... hueco entre medias
				...
				// ... no hay hueco, amplia el primer scan
				else {
					if(x < (x2 = scan[n])) {
						scan[n] = x;
					}
...
void crear_ghost(void) {
	...
	do {
		...
		do {
			...
			if(g3 > 0) {
				if(b3 > 0)      { crear_ghost_vc(vcubo - 8 - 1); }
...
void crear_ghost_vc(int m) {
	...
	if((p = vcubos[m]) != NULL) {
		do {
			...
			if(dif < find_min) {
				find_min = dif;
				find_col = ((byte *)p - (byte *)tpuntos) / sizeof(t_tpuntos);
			}
		} while((p = (*p).next) != NULL);
	}
}
