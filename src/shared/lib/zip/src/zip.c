...
struct zip_t *zip_open(const char *zipname, int level, int append)
{
	...
	if (zip) {
		...
		if (append && MZ_FILE_STAT(zipname, &fstat) == 0) {
			...
			if (!mz_zip_writer_init_from_reader(&(zip->archive),
							    zipname)) {
			...
			if (!mz_zip_writer_init_file(&(zip->archive), zipname,
						     0)) {
...
void zip_close(struct zip_t *zip)
{
	if (zip) {
		...
		// have a valid central directory.
...
int zip_entry_close(struct zip_t *zip)
{
	...
	level = zip->level & 0xF;
	if (level) {
		done = tdefl_compress_buffer(&(zip->entry.comp), "", 0,
					     TDEFL_FINISH);
	...
	if (!mz_zip_writer_create_local_dir_header(
		...
		zip->entry.comp_size, zip->entry.uncomp_crc32,
	...
	if (!mz_zip_writer_add_to_central_dir(
		...
		zip->entry.uncomp_crc32, zip->entry.method, 0, dos_time,
		dos_date, zip->entry.header_offset, 0)) {
...
int zip_entry_write(struct zip_t *zip, const void *buf, size_t bufsize)
{
	...
	if (buf && bufsize > 0) {
		...
		if (!level) {
			...
			status = tdefl_compress_buffer(&(zip->entry.comp), buf,
						       bufsize, TDEFL_NO_FLUSH);
...
int zip_entry_fwrite(struct zip_t *zip, const char *filename)
{
	...
	while ((n = fread(buf, sizeof(mz_uint8), MZ_ZIP_MAX_IO_BUF_SIZE,
			  stream)) > 0) {
...
int zip_create(const char *zipname, const char *filenames[], size_t len)
{
	...
	for (i = 0; i < len; ++i) {
		...
		if (!mz_zip_writer_add_file(&zip_archive, basename(name), name,
					    "", 0,
