...
void draw_object(SDL_Surface *bmp, SDL_Surface *obj, float angle, float cx,
		 float cy, MODE_7_PARAMS params)
{
	...
	//    ' calculate the screen coordinates that go with these space
	//    coordinates ' by dividing everything by space_x (the distance)

	screen_x = bmp->w / 2 + (int)(((params.scale_x / space_x) * space_y));
	screen_y = (int)(((params.space_z * params.scale_y) / space_x)) -
		   params.horizon;

	//    ' the size of the Object has To be scaled according To the
	//    distance
	...
	//    DrawImageRect(img, screen_x - width / 2, screen_y - height, width,
	//    height);
}

void mode_7(SDL_Surface *bmp, SDL_Surface *tile, float angle, float cx,
	    float cy, MODE_7_PARAMS params)
{
	...
	for (; screen_y < bmp->h; screen_y++) {
		...
		//       ' first calculate the distance of the line we are
		//       drawing
		distance = ((params.space_z * params.scale_y) /
			    (float)(screen_y + params.horizon));
		//       ' Then calculate the horizontal scale, Or the distance
		//       between ' space points on this horizontal line
		horizontal_scale = (distance / params.scale_x);

		//        ' calculate the dx And dy of points in space when we
		//        Step ' through all points on this line
		...
		for (screen_x = 0; screen_x < bmp->w; screen_x++) {
			...
			//            ' get a pixel from the tile And put it on
			//            the screen

			//			putpixel(bmp,screen_x,screen_y,
			//				getpixel(tile,(int)space_x
			//&mask_x,(int)space_y &mask_y));

			//
			SDL_put_pixel32(bmp, screen_x, screen_y,
					SDL_get_pixel32(tile,
							(int)space_x & mask_x,
							(int)space_y & mask_y));
...
void test_mode_7(void)
{
	...
	//    ' To afunction  flicker the program makes use of a
...
*/
		...
		//		draw_object (buffer, sprite, angle, x, y,
		//params);
