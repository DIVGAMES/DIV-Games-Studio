...
typedef struct _sector {
	...
	signed char
	    *neighbors; // Each edge may have a corresponding neighboring sector
...
typedef struct _m8camera {
	...
	float angle, anglesin, anglecos,
	    yaw;	 // Looking towards (and sin() and cos() thereof)
...
#define HeadMargin                                                             \
	1 // How much room there is above camera before the head hits the
	  // ceiling
#define KneeHeight                                                             \
	2 // How tall obstacles the player can simply walk over without jumping
...
#define Intersect(x1, y1, x2, y2, x3, y3, x4, y4)                              \
	((vertxy){                                                             \
	    vxs(vxs(x1, y1, x2, y2), (x1) - (x2), vxs(x3, y3, x4, y4),         \
		(x3) - (x4)) /                                                 \
