...
void map2port(struct ZF_Header *Header)
{
	...
	//	wallcount = (int
	//*)malloc(sizeof(int)*my_map->map.num_regions+1);
	...
	for (i = 0; i < Header->NumRegions; i++) {
		...
		//		sectors = realloc(sectors, ++NumSectors *
		//sizeof(sector));
		NumSectors++;
		...
		sect->floor =
		    0; //(float)FIX_INT(pRegion->FloorH);//Header->NumRegions;//my_map->regions[i]->floor_height;
		sect->ceil =
		    2048; //(float)FIX_INT(pRegion->CeilH);//20;//my_map->regions[i]->ceil_height;
		sect->npoints = wallcount[i];
		sect->vertex =
		    malloc((wallcount[i] + 1) * sizeof(*sect->vertex));

		sect->neighbors =
		    malloc((wallcount[i] + 1) * sizeof(*sect->neighbors));
		...
		for (n = 0; n < pRegion->NumWallPtrs; n++, ptr++) {
			...
			if(pWall->Back!=NULL)
				sect->vertex[n].front_region =
			pWall->Front->Idx; else sect->vertex[n].front_region =
			-1;
*/
			printf("sect->vertex[%d] x=%f y=%f\n", n,
			       sect->vertex[n].x, sect->vertex[n].y);
			...
			for(j=0;j<my_map->num_walls;j++) {
				if( (
(my_map->walls[j]->p1==my_map->walls[m]->p2 &&
my_map->walls[j]->p2==my_map->walls[m]->p1) ||
				 (my_map->walls[j]->p1==my_map->walls[m]->p1 &&
my_map->walls[j]->p2==my_map->walls[m]->p2) )
				 && my_map->walls[j]->front_region>=0 &&
my_map->walls[j]->front_region!=i)  {
...
neighbor: =%d\n", i,n,sect->neighbors[n]);
	...
	*/
		...
		sect->vertex[n].x =
		    sect->vertex[0]
			.x; /// 10.0f;//(float)(my_map->points[my_map->walls[m]->p1]->x*1.0f);
		sect->vertex[n].y =
		    sect->vertex[0]
			.y; // 10.0f;//(float)(my_map->points[my_map->walls[m]->p1]->y*1.0f);
		...
		//	n=0;
		//	printf("sect->vertex[%d] x=%f
		//y=%f\n",n,sect->vertex[n].x,sect->vertex[n].y);

		printf("done wals\n");
		//	printf("walls[%d]= %d <-> %d - fregion =%d bregion =
		//%d\n",i,
		//my_map->walls[i]->p1,my_map->walls[i]->p2,my_map->walls[i]->front_region,my_map->walls[i]->back_region);
		...
		//	printf("wallcount: %d
		//%d\n",wallcount[i],wallcount[i]/=2); 	printf("wallcount N:
		//%d\n",wallcount[i]);

		/*
		sscanf(ptr += n, "%f%f%n", &sect->floor,&sect->ceil, &n);
		for(m=0; sscanf(ptr += n, "%32s%n", _word, &n) == 1 && _word[0]
		!= '#'; ) { num = realloc(num, ++m * sizeof(*num)); num[m-1] =
		_word[0]=='x' ? -1 : atoi(_word); }

		sect->npoints   = m /= 2;
		sect->neighbors = malloc( (m  ) * sizeof(*sect->neighbors) );
		sect->vertex    = malloc( (m+1) * sizeof(*sect->vertex)    );
		for(n=0; n<m; ++n) sect->neighbors[n] = num[m + n];
		for(n=0; n<m; ++n) sect->vertex[n+1]  = vert[num[n]]; // TODO:
		Range checking sect->vertex[0] = sect->vertex[m]; // Ensure the
		vertexes form a loop free(num);
	*/
	}
...
static void LoadData()
{
	...
	{
	    case 'v': // vertex
		for(sscanf(ptr += n, "%f%n", &v.y, &n); sscanf(ptr += n, "%f%n",
&v.x, &n) == 1; ) { vert = realloc(vert, ++NumVertices * sizeof(*vert));
vert[NumVertices-1] = ver; } break; case 's': // sector sectors =
realloc(sectors, ++NumSectors * sizeof(*sectors)); sector* sect =
&sectors[NumSectors-1]; int* num = NULL; sscanf(ptr += n, "%f%f%n",
&sect->floor,&sect->ceil, &n); for(m=0; sscanf(ptr += n, "%32s%n", _word, &n) ==
1 && _word[0] != '#'; ) { num = realloc(num, ++m * sizeof(*num)); num[m-1] =
_word[0]=='x' ? -1 : atoi(_word); } sect->npoints   = m /= 2; sect->neighbors =
malloc( (m  ) * sizeof(*sect->neighbors) ); sect->vertex    = malloc( (m+1) *
sizeof(*sect->vertex)    ); for(n=0; n<m; ++n) sect->neighbors[n] = num[m + n];
		for(n=0; n<m; ++n) sect->vertex[n+1]  = vert[num[n]]; // TODO:
Range checking sect->vertex[0] = sect->vertex[m]; // Ensure the vertexes form a
loop free(num); break; case 'p':; // player float angle; sscanf(ptr += n, "%f %f
%f %d", &ver.x, &ver.y, &angle,&n); player = (m8camera *) {(float)ver.x, ver.y,
0}, {0,0,0}, angle,0,0,0, n }; // TODO: Range checking player.where.z =
sectors[player.sector].floor + EyeHeight;
	}
    fclose(fp);
    free(vert);
}
...
/* vline: Draw a vertical line on screen, with a different color pixel in top &
 * bottom */
...
void DrawScreen(void)
{
	...
	*head = (struct item){player.sector, 0, W - 1};
	...
	do {
		...
		//    player.where.y,player.angle, player.sector);
		...
		for (s = 0; s < sect->npoints; ++s) {
			...
			if (tz1 <= 0 && tz2 <= 0)
				continue;
			/* If it's partially behind the player, clip it against
			 * player's view frustrum */
			if (tz1 <= 0 || tz2 <= 0) {
				float nearz = 1e-4f, farz = 5, nearside = 1e-5f,
				      farside = 20.f;
				// Find an intersection between the wall and the
				// approximate edges of player's view
				vertxy i1 =
				    Intersect(tx1, tz1, tx2, tz2, -nearside,
					      nearz, -farside, farz);
				vertxy i2 =
				    Intersect(tx1, tz1, tx2, tz2, nearside,
					      nearz, farside, farz);
			...
			/* Acquire the floor and ceiling heights, relative to
			 * where the player's view is */
			...
			if (neighbor >=
			    0) // Is another sector showing through this portal?
			{
				nyceil =
				    sectors[neighbor].ceil - player.where.z;
				nyfloor =
				    sectors[neighbor].floor - player.where.z;
			}
...
#define Yaw(y, z) (y + z * player.yaw)
			...
			for (x = beginx; x <= endx; ++x) {
				...
				/* Render floor: everything below this sector's
				 * floor height. */
				...
				if (neighbor >= 0) {
					...
					int nya = (x - x1) * (ny2a - ny1a) /
						      (x2 - x1) +
						  ny1a,
					    cnya =
						clamp(nya, ytop[x], ybottom[x]);
					int nyb = (x - x1) * (ny2b - ny1b) /
						      (x2 - x1) +
						  ny1b,
					    cnyb =
						clamp(nyb, ytop[x], ybottom[x]);
					/* If our ceiling is higher than their
					 * ceiling, render upper wall */
					unsigned r1 = 0x01 * (255 - z),
						 r2 = 0x07 * (31 - z / 8);
					vline(
					    x, cya, cnya - 1, 0, r1,
					    0); // x==x1||x==x2 ? 0 : r1, 0); //
					...
					ybottom[x] = clamp(
					    min(cyb, cnyb), 0,
					    ybottom[x]); // Shrink the remaining
							 // window above these
							 // floors
				} else {
					/* There's no neighbor. Render wall from
					 * top (cya = ceiling level) to bottom
					 * (cyb = floor level). */
			...
			/* Schedule the neighboring sector for rendering within
			 * the window formed by this wall. */
...
void _rend_mainloop(void)
{
	...
	if (falling) {
		...
		if (player.velocity.z < 0 &&
		    nextz < sectors[player.sector].floor +
	...
	if (moving) {
		...
		/* Check if the player is about to cross one of the sector's
		 * edges */
		for (unsigned s = 0; s < sect->npoints; ++s)
			if (IntersectBox(px, py, px + dx, py + dy,
					 vert[s + 0].x, vert[s + 0].y,
					 vert[s + 1].x, vert[s + 1].y) &&
			    PointSide(px + dx, py + dy, vert[s + 0].x,
				      vert[s + 0].y, vert[s + 1].x,
				      vert[s + 1].y) < 0) {
				...
				if (hole_high < player.where.z + HeadMargin ||
				    hole_low > player.where.z - eyeheight +
						   KneeHeight) {
					/* Bumps into a wall! Slide along the
					 * wall. */
					/* This formula is from Wikipedia
					 * article "vector projection". */
					float xd =
						  vert[s + 1].x - vert[s + 0].x,
					      yd =
	...
	player.velocity.x =
	    player.velocity.x * (1 - acceleration) + move_vec[0] * acceleration;
	player.velocity.y =
	    player.velocity.y * (1 - acceleration) + move_vec[1] * acceleration;
