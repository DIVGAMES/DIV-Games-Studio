...
L21011:
	...
	for (i = 0; i < SrcLength / 8; i++) {
		...
		mm1 = _m_psrlwi(
		    mm1, 1); /* shift 4 WORDS of mm1 1 bit to the right */
		mm2 = _m_psrlwi(
		    mm2, 1); /* shift 4 WORDS of mm2 1 bit to the right */
		mm1 = _m_pand(mm1, *mMask); /* apply Mask to 8 BYTES of mm1 */
		mm2 = _m_pand(mm2, *mMask); /* apply Mask to 8 BYTES of mm2 */
		*mDest = _m_paddusb(
		    mm1, mm2); /* mm1+mm2 (add 8 bytes with saturation) */
		mSrc1++;
		mSrc2++;
		mDest++;
	}
...
L1013:
	...
	for (i = 0; i < SrcLength / 8; i++) {
		__m64 mm1 = _m_psubusb(
		    *mSrc2,
		    *mSrc1); /* Src1-Src2 (sub 8 bytes with saturation) */
		__m64 mm2 = _m_psubusb(
		    *mSrc1,
		    *mSrc2); /* Src2-Src1 (sub 8 bytes with saturation) */
		*mDest =
		    _m_por(mm1, mm2); /* combine both mm2 and mm1 results */
		mSrc1++;
		mSrc2++;
		mDest++;
	}
...
L1014:
	...
	/* asm volatile ( */
	/* 	"shr $3, %%ecx \n\t"	/\* counter/8 (MMX loads 8 bytes at a
	 * time) *\/ */
	/* 	"pxor      %%mm0, %%mm0 \n\t"	/\* zero mm0 register *\/ */
	/* 	".align 16       \n\t"	/\* 16 byte alignment of the loop
	 * entry *\/ */
	/* 	"1: movq (%%eax), %%mm1 \n\t"     /\* load 8 bytes from Src1
	 * into mm1 *\/ */
	/* 	"movq    (%%ebx), %%mm3 \n\t"	/\* load 8 bytes from Src2
	 * into mm3 *\/ */
	/* 	"movq      %%mm1, %%mm2 \n\t"	/\* copy mm1 into mm2 *\/ */
	/* 	"movq      %%mm3, %%mm4 \n\t"	/\* copy mm3 into mm4  *\/ */
	/* 	"punpcklbw %%mm0, %%mm1 \n\t"	/\* unpack low  bytes of Src1
	 * into words *\/ */
	/* 	"punpckhbw %%mm0, %%mm2 \n\t"	/\* unpack high bytes of Src1
	 * into words *\/ */
	/* 	"punpcklbw %%mm0, %%mm3 \n\t"	/\* unpack low  bytes of Src2
	 * into words *\/ */
	/* 	"punpckhbw %%mm0, %%mm4 \n\t"	/\* unpack high bytes of Src2
	 * into words *\/ */
	/* 	"pmullw    %%mm3, %%mm1 \n\t"	/\* mul low  bytes of Src1 and
	 * Src2  *\/ */
	/* 	"pmullw    %%mm4, %%mm2 \n\t"	/\* mul high bytes of Src1 and
	 * Src2 *\/ */
	/* 	/\* Take abs value of the results (signed words) *\/ */
	/* 	"movq      %%mm1, %%mm5 \n\t"	/\* copy mm1 into mm5 *\/ */
	/* 	"movq      %%mm2, %%mm6 \n\t"	/\* copy mm2 into mm6 *\/ */
	/* 	"psraw       $15, %%mm5 \n\t"	/\* fill mm5 words with word
	 * sign bit *\/ */
	/* 	"psraw       $15, %%mm6 \n\t"	/\* fill mm6 words with word
	 * sign bit *\/ */
	/* 	"pxor      %%mm5, %%mm1 \n\t"	/\* take 1's compliment of
	 * only neg. words *\/ */
	/* 	"pxor      %%mm6, %%mm2 \n\t"	/\* take 1's compliment of
	 * only neg. words *\/ */
	/* 	"psubsw    %%mm5, %%mm1 \n\t"	/\* add 1 to only neg. words,
	 * W-(-1) or W-0 *\/ */
	/* 	"psubsw    %%mm6, %%mm2 \n\t"	/\* add 1 to only neg. words,
	 * W-(-1) or W-0 *\/ */
	/* 	"packuswb  %%mm2, %%mm1 \n\t"	/\* pack words back into bytes
	 * with saturation *\/ */
	 ...
	 * required *\/ */
	/* 	"emms          \n\t"	/\* exit MMX state *\/ */
	/* 	: "+a" (Src1),		/\* load Src1 address into rax,
	 * modified by the loop *\/ */
	/* 	  "+b" (Src2),		/\* load Src2 address into rbx,
	 * modified by the loop *\/ */
	/* 	  "+c" (SrcLength),	/\* load loop counter (SIZE) into rcx,
	 * modified by the loop *\/ */
	/* 	  "+D" (Dest)		/\* load Dest address into rdi,
	 * modified by the loop *\/ */
	/* 	: */
	...
	for (i = 0; i < SrcLength / 8; i++) {
		...
		mm1 = _m_punpcklbw(
		    *mSrc1, mm0); /* unpack low  bytes of Src1 into words */
		mm2 = _m_punpckhbw(
		    *mSrc1, mm0); /* unpack high bytes of Src1 into words */
		mm3 = _m_punpcklbw(
		    *mSrc2, mm0); /* unpack low  bytes of Src2 into words */
		mm4 = _m_punpckhbw(
		    *mSrc2, mm0); /* unpack high bytes of Src2 into words */
		mm1 =
		    _m_pmullw(mm1, mm3); /* mul low  bytes of Src1 and Src2  */
		mm2 = _m_pmullw(mm2, mm4); /* mul high bytes of Src1 and Src2 */
		mm5 =
		    _m_psrawi(mm1, 15); /* fill mm5 words with word sign bit */
		mm6 =
		    _m_psrawi(mm2, 15); /* fill mm6 words with word sign bit */
		mm1 = _m_pxor(mm1,
			      mm5); /* take 1's compliment of only neg. words */
		mm2 = _m_pxor(mm2,
			      mm6); /* take 1's compliment of only neg. words */
		mm1 = _m_psubsw(
		    mm1, mm5); /* add 1 to only neg. words, W-(-1) or W-0 */
		mm2 = _m_psubsw(
		    mm2, mm6); /* add 1 to only neg. words, W-(-1) or W-0 */
		*mDest = _m_packuswb(
		    mm1, mm2); /* pack words back into bytes with saturation */
		mSrc1++;
		mSrc2++;
		mDest++;
	}
...
L10141:
		...
		mov [edi], al /* move a byte result to Dest */
			...
			inc edi
			dec ecx /* decrease loop counter */
			jnz L10141 /* check loop termination, proceed if
					      required */
			popa
	}
#else
...
L1015:
	...
	for (i = 0; i < SrcLength / 8; i++) {
		...
		mm4 = _m_punpckhbw(
		    *mSrc2, mm0); /* unpack high bytes of Src2 into words */
		mm1 = _m_psrlwi(mm1,
				1); /* divide mm1 words by 2, Src1 low bytes */
		mm2 = _m_psrlwi(mm2,
				1); /* divide mm2 words by 2, Src1 high bytes */
		mm1 =
		    _m_pmullw(mm1, mm3); /* mul low  bytes of Src1 and Src2  */
		mm2 = _m_pmullw(mm2, mm4); /* mul high bytes of Src1 and Src2 */
		*mDest = _m_packuswb(
...
L1016:
	...
	for (i = 0; i < SrcLength / 8; i++) {
		...
		mm2 = _m_psrlwi(mm2,
				1); /* divide mm2 words by 2, Src1 high bytes */
		mm3 = _m_psrlwi(mm3,
				1); /* divide mm3 words by 2, Src2 low bytes */
		mm4 = _m_psrlwi(mm4,
				1); /* divide mm4 words by 2, Src2 high bytes */
		mm1 =
		    _m_pmullw(mm1, mm3); /* mul low  bytes of Src1 and Src2  */
		mm2 = _m_pmullw(mm2, mm4); /* mul high bytes of Src1 and Src2 */
		*mDest = _m_packuswb(
...
L10193:
		inc edx /* increment Src1, Src2, Dest */
			...
			inc edi
			dec ecx /* decrease loop counter */
			jnz L10191 /* check loop termination, proceed if
					  required */
			popa
	}
#else
		       ...
		       unsigned char *Dest, unsigned int length)
{
	...
	for (i = istart; i < length; i++) {
		if (*cursrc2 == 0) {
			...
			*curdst = (int)*cursrc1 /
				  (int)*cursrc2; // (int) for efficiency
		}
		/* Advance pointers */
		cursrc1++;
		cursrc2++;
...
L91117:
	...
	for (i = 0; i < SrcLength / 8; i++) {
		*mDest =
		    _m_pxor(*mSrc1, mm1); /* negate mm0 by xoring with mm1 */
		mSrc1++;
		mDest++;
	}
	_m_empty(); /* clean MMX state */
...
L1021:
	...
	for (i = 0; i < SrcLength / 8; i++) {
		*mDest = _m_paddusb(
		    *mSrc1, mm1); /* Src1+C (add 8 bytes with saturation) */
		mSrc1++;
		mDest++;
	}
	_m_empty(); /* clean MMX state */
...
static int SDL_imageFilterAddByteToHalfMMX(unsigned char *Src1,
					   unsigned char *Dest,
					   unsigned int SrcLength,
					   unsigned char C, unsigned char *Mask)
{
#ifdef USE_MMX
#if !defined(GCC__)
	__asm {
...
L1022:
	...
	for (i = 0; i < SrcLength / 8; i++) {
		__m64 mm2 = _m_psrlwi(
		    *mSrc1, 1); /* shift 4 WORDS of MM2 1 bit to the right */
		mm2 = _m_pand(mm2, *mMask); /* apply Mask to 8 BYTES of MM2 */
					    /* byte     0x0f, 0xdb, 0xd0 */
		*mDest = _m_paddusb(
		    mm1, mm2); /* Src1+C (add 8 bytes with saturation) */
		mSrc1++;
		mDest++;
	}
	_m_empty(); /* clean MMX state */
...
L1023:
	...
	for (i = 0; i < SrcLength / 8; i++) {
		*mDest = _m_psubusb(
		    *mSrc1, mm1); /* Src1-C (sub 8 bytes with saturation) */
		mSrc1++;
		mDest++;
	}
	_m_empty(); /* clean MMX state */
...
L10240: /* ** Prepare proper bit-Mask in MM1 ** */
		psrlw mm1,  1 /* shift 4 WORDS of MM1 1 bit to the right */
			...
			jnz            L10240 /* check loop termination, proceed if
				      required */
...
L10241:
	...
	for (i = 0; i < SrcLength / 8; i++) {
		__m64 mm0 = _m_psrlwi(
		    *mSrc1, N); /* shift 4 WORDS of MM0 (N) bits to the right */
		*mDest = _m_pand(
		    mm0, mm1); /* apply proper bit-Mask to 8 BYTES of MM0 */
		mSrc1++;
		mDest++;
	}
	_m_empty(); /* clean MMX state */
...
L10252:
	...
	if (C <= 128) { /* if (C <= 128) execute more efficient code */
		for (i = 0; i < SrcLength / 8; i++) {
			...
			mm3 = _m_punpcklbw(
			    *mSrc1,
			    mm0); /* unpack low  bytes of Src1 into words */
			mm4 = _m_punpckhbw(
			    *mSrc1,
			    mm0); /* unpack high bytes of Src1 into words */
			mm3 = _m_pmullw(
			    mm3, mm1); /* mul low  bytes of Src1 and MM1 */
			mm4 = _m_pmullw(
			    mm4, mm1); /* mul high bytes of Src1 and MM1 */
			*mDest =
			    _m_packuswb(mm3, mm4); /* pack words back into bytes
						      with saturation */
			mSrc1++;
			mDest++;
		}
		...
		for (i = 0; i < SrcLength / 8; i++) {
			...
			/* Take abs value of the results (signed words) */
			mm5 = _m_psrawi(
			    mm3, 15); /* fill mm5 words with word sign bit */
			mm6 = _m_psrawi(
			    mm4, 15); /* fill mm6 words with word sign bit */
			mm3 = _m_pxor(
			    mm3,
			    mm5); /* take 1's compliment of only neg. words */
			mm4 = _m_pxor(
			...
			mm3 = _m_psubsw(
			    mm3,
			    mm5); /* add 1 to only neg. words, W-(-1) or W-0 */
			mm4 = _m_psubsw(
			    mm4,
			    mm6); /* add 1 to only neg. words, W-(-1) or W-0 */
			*mDest =
			    _m_packuswb(mm3, mm4); /* pack words back into bytes
						      with saturation */
			mSrc1++;
...
L1026:
	...
	for (i = 0; i < SrcLength / 8; i++) {
		...
		mm4 = _m_punpckhbw(
		    *mSrc1, mm0); /* unpack high bytes of Src1 into words */
		mm3 = _m_psrlwi(
		    mm3, N); /* shift 4 WORDS of MM3 (N) bits to the right */
		mm4 = _m_psrlwi(
		    mm4, N); /* shift 4 WORDS of MM4 (N) bits to the right */
					   ...
					   unsigned char C)
{
	...
	if ((SDL_imageFilterMMXdetect()) && (length > 7)) {
		...
		SDL_imageFilterShiftRightAndMultByByteMMX(Src1, Dest, length, N,
							  C);
...
L10270: /* ** Prepare proper bit-Mask in MM1 ** */
		psllw mm1, 1 /* shift 4 WORDS of MM1 1 bit to the left */
		    ...
		    /*  byte     0x0f, 0xdb, 0xc8 */
			dec cl /* decrease loop counter */
			jnz            L10270 /* check loop termination, proceed if
				      required */
				       /* ** Shift all bytes of the image ** */
			mov eax, Src1 /* load Src1 address into eax */
...
L10271:
	...
	for (i = 0; i < N; i++) {
		mm1 = _m_psllwi(mm1,
				1); /* shift 4 WORDS of MM1 1 bit to the left */
		mm1 = _m_pand(mm1, *mMask); /* apply Mask to 8 BYTES of MM1 */
	}
	/* ** Shift all bytes of the image ** */
	for (i = 0; i < SrcLength / 8; i++) {
		__m64 mm0 = _m_psllwi(
		    *mSrc1, N); /* shift 4 WORDS of MM0 (N) bits to the left */
		*mDest = _m_pand(
		    mm0, mm1); /* apply proper bit-Mask to 8 BYTES of MM0 */
		mSrc1++;
		mDest++;
...
L12023:
	...
	for (i = 0; i < SrcLength / 8; i++) {
		*mDest = _m_pslldi(
		    *mSrc1, N); /* Src1+C (add 8 bytes with saturation) */
		mSrc1++;
		mDest++;
	}
	_m_empty(); /* clean MMX state */
...
L10282:
	...
	if (N <= 7) { /* if (N <= 7) execute more efficient code */
		for (i = 0; i < SrcLength / 8; i++) {
			...
			mm3 = _m_psllwi(
			    mm3,
			    N); /* shift 4 WORDS of MM3 (N) bits to the left */
			mm4 = _m_psllwi(
			    mm4,
			    N); /* shift 4 WORDS of MM4 (N) bits to the left */
			*mDest =
			    _m_packuswb(mm3, mm4); /* pack words back into bytes
						      with saturation */
			mSrc1++;
...
L1029:
	...
	for (i = 0; i < SrcLength / 8; i++) {
		__m64 mm0 = _m_paddusb(
		    *mSrc1,
		    mm2); /* Src1+(0xFF-T) (add 8 bytes with saturation) */
		*mDest =
		    _m_pcmpeqb(mm0, mm1); /* binarize 255:0, comparing to 255 */
		mSrc1++;
		mDest++;
	}
	_m_empty(); /* clean MMX state */
					  ...
					  unsigned int length, unsigned char T)
{
	...
	for (i = istart; i < length; i++) {
		*curdest =
		    (unsigned char)(((unsigned char)*cursrc1 >= T) ? 255 : 0);
		/* Advance pointers */
		cursrc1++;
		curdest++;
	}
...
L1031:
	...
	for (i = 0; i < SrcLength / 8; i++) {
		__m64 mm3, mm4, mm5, mm6;
		mm3 = _m_punpcklbw(
		    *mSrc1, mm7); /* unpack low  bytes of Src1 into words */
		mm4 = _m_punpckhbw(
		    *mSrc1, mm7); /* unpack high bytes of Src1 into words */
		...
		/* Take abs value of the signed words */
		mm5 =
		    _m_psrawi(mm3, 15); /* fill mm5 words with word sign bit */
		mm6 =
		    _m_psrawi(mm4, 15); /* fill mm6 words with word sign bit */
		mm3 = _m_pxor(mm3,
			      mm5); /* take 1's compliment of only neg. words */
		mm4 = _m_pxor(mm4,
			      mm6); /* take 1's compliment of only neg. words */
		mm3 = _m_psubsw(
		    mm3, mm5); /* add 1 to only neg. words, W-(-1) or W-0 */
		mm4 = _m_psubsw(
		    mm4, mm6); /* add 1 to only neg. words, W-(-1) or W-0 */
		*mDest = _m_packuswb(
		    mm3, mm4); /* pack words back into bytes with saturation */
		mSrc1++;
		mDest++;
				   ...
				   int Nmin, int Nmax)
{
	...
	if ((SDL_imageFilterMMXdetect()) && (length > 7)) {
		...
		SDL_imageFilterNormalizeLinearMMX(Src, Dest, length, Cmin, Cmax,
						  Nmin, Nmax);
...
L10322:
			...
			punpcklbw mm1, mm0 /* unpack first 4 bytes into words */
				...
				jnz            L10320 /* check loop termination, proceed if
					  required */
					   /* ---, */
				emms /* exit MMX state */
				popa
		...
		asm volatile(
		    ...
		    "sub          $2, %%ecx \n\t" /* do not use first and last
						     column */
		    ".align 16              \n\t" /* 16 byte alignment of the
						     loop entry */
		    ".L10322:               \n\t"
		    /* --- */
		    "movq    (%%esi), %%mm1 \n\t" /* load 8 bytes of the image
						     first row */
		    "add       %%eax, %%esi \n\t" /* move one row below */
		    "movq    (%%esi), %%mm2 \n\t" /* load 8 bytes of the image
						     second row */
		    ...
		    "movq    (%%esi), %%mm3 \n\t" /* load 8 bytes of the image
						     third row */
		    "punpcklbw %%mm0, %%mm1 \n\t" /* unpack first 4 bytes into
						     words */
		    "punpcklbw %%mm0, %%mm2 \n\t" /* unpack first 4 bytes into
						     words */
		    "punpcklbw %%mm0, %%mm3 \n\t" /* unpack first 4 bytes into
						     words */
		    "pmullw    %%mm5, %%mm1 \n\t" /* multiply words first row
						     image*Kernel */
		    "pmullw    %%mm6, %%mm2 \n\t" /* multiply words second row
						     image*Kernel */
		    "pmullw    %%mm7, %%mm3 \n\t" /* multiply words third row
						     image*Kernel */
		    "paddsw    %%mm2, %%mm1 \n\t" /* add 4 words of the first
						     and second rows */
		    "paddsw    %%mm3, %%mm1 \n\t" /* add 4 words of the third
						     row and result */
		    "movq      %%mm1, %%mm2 \n\t" /* copy MM1 into MM2 */
		    "psrlq       $32, %%mm1 \n\t" /* shift 2 left words to the
						     right */
		    "paddsw    %%mm2, %%mm1 \n\t" /* add 2 left and 2 right
						     result words */
		    "movq      %%mm1, %%mm3 \n\t" /* copy MM1 into MM3 */
		    "psrlq       $16, %%mm1 \n\t" /* shift 1 left word to the
						     right */
		    "paddsw    %%mm3, %%mm1 \n\t" /* add 1 left and 1 right
						     result words */
		    /* -- */
		    "movd      %%eax, %%mm2 \n\t" /* save EAX in MM2 */
		    "movd      %%edx, %%mm3 \n\t" /* save EDX in MM3 */
		    "movd      %%mm1, %%eax \n\t" /* copy MM1 into EAX */
		    "psraw       $15, %%mm1 \n\t" /* spread sign bit of the
						     result */
		    "movd      %%mm1, %%edx \n\t" /* fill EDX with a sign bit */
		    "idivw             %%bx \n\t" /* IDIV - VERY EXPENSIVE */
		    "movd      %%eax, %%mm1 \n\t" /* move result of division
						     into MM1 */
		    "packuswb  %%mm0, %%mm1 \n\t" /* pack division result with
						     saturation */
		    "movd      %%mm1, %%eax \n\t" /* copy saturated result into
						     EAX */
		    "mov      %%al, (%%edi) \n\t" /* copy a byte result into
						     Dest */
		    "movd      %%mm3, %%edx \n\t" /* restore saved EDX */
		    "movd      %%mm2, %%eax \n\t" /* restore saved EAX */
		    /* -- */
		    "sub       %%eax, %%esi \n\t" /* move two rows up */
		    "sub       %%eax, %%esi \n\t" /* */
		    "inc              %%esi \n\t" /* move Src  pointer to the
						     next pixel */
		    "inc              %%edi \n\t" /* move Dest pointer to the
						     next pixel */
		    /* --- */
		    "dec              %%ecx \n\t" /* decrease loop counter
						     COLUMNS */
...
L10332:
		...
		asm volatile(
		    ...
		    "mov          %0, %%edi \n\t" /* load Dest address to EDI */
		    "add          $2, %%edi \n\t" /* 2 column offset from the
						     left edge */
		    "mov          %3, %%eax \n\t" /* load columns into EAX */
		    "shl          $1, %%eax \n\t" /* EAX = columns * 2 */
		    "add       %%eax, %%edi \n\t" /* 2 row offset from the top
						     edge */
		    "shr          $1, %%eax \n\t" /* EAX = columns */
		    "mov          %2, %%ebx \n\t" /* initialize ROWS counter */
		    "sub          $4, %%ebx \n\t" /* do not use first 2 and last
						     2 rows */
		    /* --- */
		    ".L10330:               \n\t"
		    ...
		    "movq      %%mm1, %%mm2 \n\t" /* copy MM1 into MM2 */
		    "add       %%eax, %%esi \n\t" /* move Src pointer 1 row
						     below */
		    "movq    (%%edx), %%mm3 \n\t" /* load 4 words of Kernel */
		    "add          $8, %%edx \n\t" /* move pointer to other 4
						     words */
		    "movq    (%%edx), %%mm4 \n\t" /* load 4 words of Kernel */
		    "add          $8, %%edx \n\t" /* move pointer to other 4
						     words */
		    "punpcklbw %%mm0, %%mm1 \n\t" /* unpack first  4 bytes into
						     words */
		    "punpckhbw %%mm0, %%mm2 \n\t" /* unpack second 4 bytes into
						     words */
		    "pmullw    %%mm3, %%mm1 \n\t" /* mult. 4 low  words of Src
						     and Kernel */
		    "pmullw    %%mm4, %%mm2 \n\t" /* mult. 4 high words of Src
						     and Kernel */
		    "paddsw    %%mm2, %%mm1 \n\t" /* add 4 words of the high and
						     low bytes */
...
L10342:
			...
			punpcklbw mm1, mm0 /* unpack first  4 bytes into words */
				...
				/* ---, */
				dec              ecx /* decrease loop counter COLUMNS */
				jnz            L10342 /* check loop termination,
						  proceed if required */
				add esi, 6 /* move to the next row in Src */
				add edi, 6 /* move to the next row in Dest */
				dec              ebx /* decrease loop counter ROWS */
				jnz            L10340 /* check loop termination, proceed if
					  required */
					   /* ---, */
				emms /* exit MMX state */
				popa
		...
		asm volatile(
		    ...
		    "mov          %1, %%esi \n\t" /* load Src  address to ESI */
		    "mov          %0, %%edi \n\t" /* load Dest address to EDI */
		    "add          $3, %%edi \n\t" /* 3 column offset from the
						     left edge */
		    "mov          %3, %%eax \n\t" /* load columns into EAX */
		    "add       %%eax, %%edi \n\t" /* 3 row offset from the top
						     edge */
		    "add       %%eax, %%edi \n\t"
		    "add       %%eax, %%edi \n\t"
		    "mov          %2, %%ebx \n\t" /* initialize ROWS counter */
...
L10352:
			...
			punpcklbw mm1, mm0 /* unpack first  4 bytes into words */
				...
				/* ---, */
				dec              ecx /* decrease loop counter COLUMNS */
				jnz            L10352 /* check loop termination,
						  proceed if required */
				add esi, 8 /* move to the next row in Src */
				add edi, 8 /* move to the next row in Dest */
				dec              ebx /* decrease loop counter ROWS */
				jnz            L10350 /* check loop termination, proceed if
					  required */
					   /* ---, */
				emms /* exit MMX state */
				popa
		...
		asm volatile(
		    ...
		    "mov          %1, %%esi \n\t" /* load Src  address to ESI */
		    "mov          %0, %%edi \n\t" /* load Dest address to EDI */
		    "add          $4, %%edi \n\t" /* 4 column offset from the
						     left edge */
		    "mov          %3, %%eax \n\t" /* load columns into EAX */
		    "add       %%eax, %%edi \n\t" /* 4 row offset from the top
						     edge */
		    "add       %%eax, %%edi \n\t"
		    "add       %%eax, %%edi \n\t"
		    "add       %%eax, %%edi \n\t"
		    "mov          %2, %%ebx \n\t" /* initialize ROWS counter */
		    "sub          $8, %%ebx \n\t" /* do not use first 4 and last
						     4 rows */
		    /* --- */
		    ".L10350:               \n\t"
		    "mov       %%eax, %%ecx \n\t" /* initialize COLUMNS counter
...
L10362:
			...
			punpcklbw mm1, mm0 /* unpack first 4 bytes into words */
				...
				/* ---, */
				dec              ecx /* decrease loop counter COLUMNS */
				jnz            L10362 /* check loop termination,
						  proceed if required */
				add esi, 2 /* move to the next row in Src */
				add edi, 2 /* move to the next row in Dest */
				dec              edx /* decrease loop counter ROWS */
				jnz            L10360 /* check loop termination, proceed if
					  required */
					   /* ---, */
				emms /* exit MMX state */
				popa
...
L10372:
			...
			punpcklbw mm1, mm0 /* unpack first  4 bytes into words */
				...
				/* ---, */
				dec              ecx /* decrease loop counter COLUMNS */
				jnz            L10372 /* check loop termination,
						  proceed if required */
				add esi, 4 /* move to the next row in Src */
				add edi, 4 /* move to the next row in Dest */
				dec              ebx /* decrease loop counter ROWS */
				jnz            L10370 /* check loop termination, proceed if
					  required */
					   /* ---, */
				emms /* exit MMX state */
				popa
...
L10382:
			...
			punpcklbw mm1, mm0 /* unpack first  4 bytes into words */
				...
				/* ---, */
				dec              ecx /* decrease loop counter COLUMNS */
				jnz            L10382 /* check loop termination,
						  proceed if required */
				add esi, 6 /* move to the next row in Src */
				add edi, 6 /* move to the next row in Dest */
				dec              ebx /* decrease loop counter ROWS */
				jnz            L10380 /* check loop termination, proceed if
					  required */
					   /* ---, */
				emms /* exit MMX state */
				popa
...
L10392:
			...
			punpcklbw mm1, mm0 /* unpack first  4 bytes into words */
				...
				/* ---, */
				dec              ecx /* decrease loop counter COLUMNS */
				jnz            L10392 /* check loop termination,
						  proceed if required */
				add esi, 8 /* move to the next row in Src */
				add edi, 8 /* move to the next row in Dest */
				dec              ebx /* decrease loop counter ROWS */
				jnz            L10390 /* check loop termination, proceed if
					  required */
					   /* ---, */
				emms /* exit MMX state */
				popa
...
L10402:
			...
			/* ---, */
				dec              ecx /* decrease loop counter COLUMNS */
				jnz            L10402     /* check loop termination, proceed if
					      required */
				...
				dec              edx /* decrease loop counter ROWS */
				jnz            L10400 /* check loop termination, proceed if
					      required */
					       /* ---, */
				emms /* exit MMX state */
				popa
		...
		asm volatile(
		    ...
		    "movq      %%mm4, %%mm5 \n\t" /* save MM4 in MM5 */
		    "add          $2, %%esi \n\t" /* move ESI pointer 2 bytes
						     right */
		    "punpcklbw %%mm0, %%mm4 \n\t" /* unpack 4 low  bytes into
						     words */
		    "punpckhbw %%mm0, %%mm5 \n\t" /* unpack 4 high bytes into
						     words */
		    "movq    (%%esi), %%mm6 \n\t" /* load 8 bytes from Src */
		    "movq      %%mm6, %%mm7 \n\t" /* save MM6 in MM7 */
		    "sub          $2, %%esi \n\t" /* move ESI pointer back 2
						     bytes left */
		    "punpcklbw %%mm0, %%mm6 \n\t" /* unpack 4 low  bytes into
						     words */
		    "punpckhbw %%mm0, %%mm7 \n\t" /* unpack 4 high bytes into
		    ...
		    "psraw       $15, %%mm6 \n\t" /* fill MM6 words with word
						     sign bit */
		    "psraw       $15, %%mm7 \n\t" /* fill MM7 words with word
						     sign bit */
		    "pxor      %%mm6, %%mm4 \n\t" /* take 1's compliment of only
						     neg. words */
		    "pxor      %%mm7, %%mm5 \n\t" /* take 1's compliment of only
						     neg. words */
		    "psubsw    %%mm6, %%mm4 \n\t" /* add 1 to only neg. words,
		    ...
		    "psubsw    %%mm7, %%mm5 \n\t" /* add 1 to only neg. words,
						     W-(-1) or W-0 */
		    "packuswb  %%mm5, %%mm4 \n\t" /* combine and pack/saturate
						     MM5 and MM4 */
		    "movq    %%mm4, (%%edi) \n\t" /* store result in Dest */
		    /* --- */
		    "sub       %%eax, %%esi \n\t" /* move to the current top row
						     in Src */
		    "sub       %%eax, %%esi \n\t"
		    "add $8,          %%esi \n\t" /* move Src  pointer to the
						     next 8 pixels */
...
L10412:
			...
			/* ---, */
				dec              ecx /* decrease loop counter COLUMNS */
				jnz            L10412     /* check loop termination, proceed if
					      required */
				mov esi, ebx /* restore most left current row Src  address */
				...
				jnz            L10410 /* check loop termination, proceed if
					      required */
					       /* ---, */
				emms /* exit MMX state */
				popa
...
void SDL_imageFilterAlignStack(void)
{
	...
	asm volatile(				   /* --- stack alignment --- */
		     ...
		     "and        $-32, %%ebx \n\t" /* align EBX along a 32 byte
						      boundary */
		     "mov     %%esp, (%%ebx) \n\t" /* save old value of ESP in
						      stack, behind the bndry */
