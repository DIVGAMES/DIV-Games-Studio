		     ...
		     int smooth)
{
	int x, y, sx, sy, ssx, ssy, *sax, *say, *csax, *csay, *salast, csx, csy,
	    ex, ey, cx, cy, sstep, sstepx, sstepy;
	...
	if (smooth) {
		...
		for (y = 0; y < dst->h; y++) {
			...
			for (x = 0; x < dst->w; x++) {
				 ...
				 */
				t1 = ((((c01->r - c00->r) * ex) >> 16) +
				      c00->r) &
				     0xff;
				t2 = ((((c11->r - c10->r) * ex) >> 16) +
				      c10->r) &
				     0xff;
				dp->r = (((t2 - t1) * ey) >> 16) + t1;
				t1 = ((((c01->g - c00->g) * ex) >> 16) +
				      c00->g) &
				     0xff;
				t2 = ((((c11->g - c10->g) * ex) >> 16) +
				      c10->g) &
				     0xff;
				dp->g = (((t2 - t1) * ey) >> 16) + t1;
				t1 = ((((c01->b - c00->b) * ex) >> 16) +
				      c00->b) &
				     0xff;
				t2 = ((((c11->b - c10->b) * ex) >> 16) +
				      c10->b) &
				     0xff;
				dp->b = (((t2 - t1) * ey) >> 16) + t1;
				t1 = ((((c01->a - c00->a) * ex) >> 16) +
				      c00->a) &
				     0xff;
				t2 = ((((c11->a - c10->a) * ex) >> 16) +
				      c10->a) &
				     0xff;
				dp->a = (((t2 - t1) * ey) >> 16) + t1;
				/*
...
SDL_Surface *rotateSurface90Degrees(SDL_Surface *src, int numClockwiseTurns)
{
	...
	{
		/* Unfortunately SDL_BlitSurface cannot be used to make a copy
		of the surface since it does not preserve alpha. */

		if (src->pitch == dst->pitch) {
			/* If the pitch is the same for both surfaces, the
			 * memory can be copied all at once. */
			      ...
			      double *canglezoom, double *sanglezoom)
{
	...
	/*
	 * Determine destination width and height by rotating a centered source
	 * box
	 */
			       ...
			       double zoomy, int smooth)
{
	...
	if (fabs(angle) > VALUE_LIMIT) {
		...
		_rotozoomSurfaceSizeTrig(rz_src->w, rz_src->h, angle, zoomx,
					 zoomy, &dstwidth, &dstheight,
		...
		if (is32bit) {
			/*
			 * Call the 32bit transformation routine to do the
			 * rotation (using alpha)
			 */
			_transformSurfaceRGBA(
			    rz_src, rz_dst, dstwidthhalf, dstheighthalf,
			 ...
			 * rotation
			 */
			transformSurfaceY(rz_src, rz_dst, dstwidthhalf,
...
SDL_Surface *shrinkSurface(SDL_Surface *src, int factorx, int factory)
{
	...
	if (is32bit == 1) {
		/*
		 * Call the 32bit transformation routine to do the shrinking
		 * (using alpha)
		 */
		 ...
		 */
		result = SDL_SetColorKey(rz_dst, SDL_SRCCOLORKEY | SDL_RLEACCEL,
					 _colorkey(rz_src));
		if (result != 0) {
			haveError = 1;
			goto exitShrinkSurface;
