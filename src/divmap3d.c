...
struct t_listbox lpared = {ANCHO_VENTANA + 6 + 50 + 1,
			   11,
			   DummyArrayForFakingPurposesUsedInMapper,
			   1,
			   6,
			   1,
			   6 * 8 + 2};
struct t_listbox lsector = {ANCHO_VENTANA + 6 + 50 + 1,
			    11 + 50 + 1,
			    DummyArrayForFakingPurposesUsedInMapper,
			    1,
			    6,
			    1,
			    6 * 8 + 2};
// struct t_listbox ltecho={ANCHO_VENTANA+6+50+1,
// 11+50+1,DummyArrayForFakingPurposesUsedInMapper,1,7,1,7*8+6}; struct t_listbox
// lsuelo={ANCHO_VENTANA+6+50+1,11+100+2+12,DummyArrayForFakingPurposesUsedInMapper,1,7,1,7*8+6};
...
struct t_listboxbr ltexturasbr = {
    3 - 2, 11 - 2, m3d_fpgcodesbr, an_textura, 4, 4, 32, 32};
...
void MapperCreator0(void)
{
	...
	_get(414, ANCHO_VENTANA + 7, 11 + 100 + 2 - 8 + 1, 48,
	     (byte *)cadenas[3], 5, 0, 4095);
	_get(414, ANCHO_VENTANA + 7, 11 + 150 + 3 + 11 - 8 + 2, 48,
	     (byte *)cadenas[4], 5, 0, 4095);
...
void MapperCreator1(void)
{
	...
	lpared.creada = 0;
	lpared.maximo = TOPE_FADE + 6;
	lsector.creada = 0;
	lsector.maximo = TOPE_FADE + 6;
...
*/
	// Zona de edicion
	wrectangulo(v.ptr, an, al, c0, 3, 11, ANCHO_VENTANA + 2,
		    ALTO_VENTANA + 2);
	//  wbox       (v.ptr, an, al, c1, 4, 12,   ANCHO_VENTANA,
	//  ALTO_VENTANA);
	...
	wrectangulo(v.ptr, an, al, c0, ANCHO_VENTANA + 6, 11 + 100 + 2 + 12, 50,
		    50);
	wbox(v.ptr, an, al, c1, ANCHO_VENTANA + 7, 12 + 100 + 2 + 12, 48, 48);
	PintaMapperThumbs();

	// Barra de alturas
	wbox(v.ptr, an, al, c_b_low, ANCHO_VENTANA + 7 + 50, 11 + 100 + 3, 9,
	     72);
	alto = (altura_techo * 72) / (TOPE_TECHO - 1) -
	       (altura_suelo * 72) / (TOPE_TECHO - 1);
	if (alto < 1) {
		alto = 1;
		if (((altura_techo * 72) / (TOPE_TECHO - 1)) < 1) {
			wbox(v.ptr, an, al, c4, ANCHO_VENTANA + 7 + 50,
			     11 + 100 + 3 + 71 -
				 (altura_techo * 72) / (TOPE_TECHO - 1),
			     9, alto);
		} else
			wbox(v.ptr, an, al, c4, ANCHO_VENTANA + 7 + 50,
			     11 + 100 + 3 + 72 -
		...
		wbox(v.ptr, an, al, c4, ANCHO_VENTANA + 7 + 50,
		     11 + 100 + 3 + 72 - (altura_techo * 72) / (TOPE_TECHO - 1),
		     9, alto);
	wrectangulo(v.ptr, an, al, c0, ANCHO_VENTANA + 7 + 50, 11 + 100 + 2, 9,
		    74);
...
*/
	...
	case 8: // Get de Altura de Techo
		...
		if (edit_region != -1) {
			my_map->regions[edit_region]->ceil_height =
			    altura_techo;
			my_map->regions[edit_region]->floor_height =
			    altura_suelo;
		}
		need_refresh = 1;
		break;
	...
	if (!(mouse_b & 1) && (old_mouse_b & 1)) {
		if (lpared.zona == 2 || lpared.zona == 3 || lpared.zona == 4) {
			if (modo_edicion == PINTA_SECTOR ||
				...
				if (edit_wall != -1)
					my_map->walls[edit_wall]->fade =
					    fade_pared;
				need_refresh = 1;
			}
		}
		if (lsector.zona == 2 || lsector.zona == 3 ||
		    lsector.zona == 4) {
			if (modo_edicion == PINTA_SECTOR ||
			    modo_edicion == EDITA_SECTOR) {
				...
				if (edit_region != -1)
					my_map->regions[edit_region]->fade =
					    fade_sector;
				need_refresh = 1;
			}
		}
...
*/
	...
	if (wmouse_in(4, 12, ANCHO_VENTANA, ALTO_VENTANA)) {
		...
		//-------------------------------------------------------------------------
		pos_x = pos_x / zoom_level + M_ANCHO_VENTANA * big2 -
			M_ANCHO_VENTANA * big2 / zoom_level;
		pos_y = pos_y / zoom_level + M_ALTO_VENTANA * big2 -
			M_ALTO_VENTANA * big2 / zoom_level;
		...
		if (snap) {
			...
			if (zoom_level > 2.0) {
				dif_x = 2 * dif_x / zoom_level +
					2 * M_ANCHO_VENTANA * big2 -
					2 * M_ANCHO_VENTANA * big2 / zoom_level;
				if (dif_x & 1)
					pos_x++;

				dif_y = 2 * dif_y / zoom_level +
					2 * M_ALTO_VENTANA * big2 -
					2 * M_ALTO_VENTANA * big2 / zoom_level;
				if (dif_y & 1)
					pos_y++;
			}
		}
		...
		if (mouse_b && edit_point == -1 && edit_wall == -1 &&
		    edit_region == -1)
	...
	if (mouse_b & 1) {
		if (modo_edicion == EDITA_LINEA ||
		    modo_edicion == PINTA_SECTOR) {
			if (wmouse_in(ANCHO_VENTANA + 6, 11, 50,
				      50)) // Textura de Pared
			{
				if (m3d_edit.fpg_path[0] == 0) {
					if (comprobar_fichero()) {
						strcpy(
						    (char *)m3d_edit.fpg_name,
						    input);
						strcpy(
						    (char *)m3d_edit.fpg_path,
						    full);
						M3D_crear_thumbs(&ltexturasbr,
								 1);
					}
				}
				if (m3d_edit.fpg_path[0] != 0) {
					...
					if (edit_wall != -1)
						my_map->walls[edit_wall]
						    ->texture = Tex[PARED].cod;
					need_refresh = 1;
				}
			}
		}
		if (modo_edicion == EDITA_SECTOR ||
		    modo_edicion == PINTA_SECTOR) {
			if (wmouse_in(ANCHO_VENTANA + 6, 11 + 50 + 1, 50,
				      50)) // Textura de Techo
			{
				if (m3d_edit.fpg_path[0] == 0) {
					if (comprobar_fichero()) {
				...
				if (m3d_edit.fpg_path[0] != 0) {
					...
					if (edit_region != -1)
						my_map->regions[edit_region]
						    ->ceil_tex = Tex[TECHO].cod;
					need_refresh = 1;
				}
			} else if (wmouse_in(ANCHO_VENTANA + 6,
					     11 + 100 + 2 + 12, 50,
					     50)) // Textura de Suelo
			{
				if (m3d_edit.fpg_path[0] == 0) {
	...
	case _C:
		...
		if (my_map->bbox_x_ini == 65536) {
			...
			scroll_x =
			    (my_map->bbox_x_ini + my_map->bbox_x_fin) / 2;
			scroll_y =
			    (my_map->bbox_y_ini + my_map->bbox_y_fin) / 2;
		}
		scroll_x &= -64;
		scroll_y &= -64;
		map_draw();
	...
	// Scroll del mapa
	if (!(v.item[5].estado & 2) && !(v.item[8].estado & 2) &&
	    !(v.item[9].estado & 2)) {
		if (kbdFLAGS[0x4B]) {
...
*/
	...
	if (MouseFocus)
		if ((mouse_x - v.x) / big2 > 3 &&
		    (mouse_x - v.x) / big2 < 3 + ANCHO_VENTANA + 2 &&
		    (mouse_y - v.y) / big2 > 11 &&
		    (mouse_y - v.y) / big2 < 11 + ALTO_VENTANA + 2) {
			if (mouse_b & 2) {
				map_draw();
				if (!(old_mouse_b & 2)) {
					/*
	...
	*/
				...
				if (scroll_y < 0)
					scroll_y = 0;
				if (scroll_x > FIN_GRID)
					scroll_x = FIN_GRID;
				if (scroll_y > FIN_GRID)
					scroll_y = FIN_GRID;
			} else
				leer_mouse = 1;
		} else
...
void MapperBrowseFPG0(void)
{
	...
	if (modo < 100) {
		if (TipoBrowser == MAPBR) {
			...
			memcpy(m3d_fpgcodesbr, fondo,
			       max_texturas * an_textura);
...
void MapperBrowseFPG2(void)
{
	...
	if ((mouse_b & 1) && !(old_mouse_b & 1)) {
		if (ltexturasbr.zona >= 10) {
			if (modo >= 100) {
				if (TipoTex == FONDO)
					CrearMapperThumb(-1, 42, 21);
				else
					CrearMapperThumb(-1, 48, 48);
				FPG_thumbpos = ltexturasbr.inicial;
			} else {
				if (TipoBrowser == BRUSH) {
					old_pincel = num_pincel;
					num_pincel = ltexturasbr.inicial +
						     ltexturasbr.zona - 10;
					M3D_muestra_thumb(&ltexturasbr,
							  old_pincel);
					M3D_muestra_thumb(&ltexturasbr,
							  num_pincel);
...
void M3D_crear_thumbs(struct t_listboxbr *l, int prog)
{
	...
	if (modo < 100) {
		for (n = 0; n < 256; n++) {
			switch ((FPG_pal[n * 3] + FPG_pal[n * 3 + 1] +
				 FPG_pal[n * 3 + 2]) /
				22) {
			case 1:
				FPG_xlat[n] = c0;
		...
		for (n = 0; n < 256; n++)
			FPG_xlat[n] =
			    fast_find_color(FPG_pal[n * 3], FPG_pal[n * 3 + 1],
					    FPG_pal[n * 3 + 2]);
	}
	...
	for (;;) {
		if (fread(&(FPG_D.info), 1, sizeof(FPG_info), FPG_F) !=
		    sizeof(FPG_info)) {
			if (feof(FPG_F)) {
				l->maximo = f_maximo = n;
				break;
		...
		FPG_progress.pos += (64 + 4 * FPG_D.info.puntos +
				     FPG_D.info.ancho * FPG_D.info.alto);
		if (prog)
			Progress((char *)texto[93], FPG_progress.pos,
				 FPG_progress.total);

		thumb_tex[n].Cuad = 0;
		for (con = 0; con < 11; con++) {
			if (FPG_D.info.ancho == tex_sop[con] &&
			    FPG_D.info.alto == tex_sop[con]) {
				thumb_tex[n].Cuad = 1;
			}
		}
		...
		if ((FPG_D.imagen = (byte *)malloc(FPG_D.info.ancho *
						   FPG_D.info.alto)) == NULL) {
			for (n = 0; n < max_texturas; n++) {
				if (thumb_tex[n].ptr != NULL) {
					free(thumb_tex[n].ptr);
					thumb_tex[n].ptr = NULL;
			...
			fclose(FPG_F);
			if (prog)
				Progress((char *)texto[93], FPG_progress.total,
					 FPG_progress.total);
		...
		if (fread(FPG_D.imagen, 1, FPG_D.info.ancho * FPG_D.info.alto,
			  FPG_F) != FPG_D.info.ancho * FPG_D.info.alto) {
			for (n = 0; n < max_texturas; n++) {
				if (thumb_tex[n].ptr != NULL) {
					free(thumb_tex[n].ptr);
					thumb_tex[n].ptr = NULL;
	...
	for (con = n; con < l->maximo; con++) {
		for (;;) {
			...
			{
				for (n = thumb_tex[con].an * thumb_tex[con].al -
					 1;
				     n >= 0; n--) {
			...
			} else // Crea el thumbnail
			{
				...
				if ((temp2 = (byte *)malloc(
					 thumb_tex[con].an *
					 thumb_tex[con].al)) == NULL) {
				...
				memset(temp2, 0,
				       thumb_tex[con].an * thumb_tex[con].al);
				a = (float)0.0;
				for (y = 0; y < thumb_tex[con].al; y++) {
					b = (float)0.0;
					for (x = 0; x < thumb_tex[con].an;
					     x++) {
						temp2[y * thumb_tex[con].an +
						      x] = temp[((int)a) * man +
								(int)b];
						b += coefredx;
					}
				...
				if ((thumb_tex[con].ptr =
					 (char *)malloc((thumb_tex[con].an *
							 thumb_tex[con].al) /
							4)) == NULL) {
					free(temp2);
					thumb_tex[con].ptr = NULL;
			...
			FPG_progress.pos +=
			    (FPG_progress.total - FPG_progress.pos) /
			    (l->maximo - con);
			if (prog)
				Progress((char *)texto[93], FPG_progress.pos,
					 FPG_progress.total);
...
void M3D_muestra_thumb(struct t_listboxbr *l, int num)
{
	...
	if (TipoBrowser == MAPBR)
		memcpy(&thumb_tmp, &thumb_map,
		       sizeof(struct _thumb_map) * max_windows);
	else
		memcpy(&thumb_tmp, &thumb_tex,
		       sizeof(struct _thumb_tex) * max_texturas);
	...
	if (num >= l->inicial && num < l->inicial + l->lineas * l->columnas) {
		if (TipoBrowser == MAPBR) {
			...
			num_tex =
			    atoi(m3d_fpgcodesbr +
				 num * an_textura); // Codigo de esa posicion
			if (num_tex) {
				for (x = 0; x < max_texturas; x++) {
		...
		px = (l->x + 1 +
		      (l->an + 1) * ((num - l->inicial) % l->columnas)) *
			 big2 +
		     (l->an * big2 - thumb_tmp[num_tex].an) / 2;
		if ((incy = ((l->al - 8) * big2 - thumb_tmp[num_tex].al) / 2) <
		    0)
			incy = 0;
		py = (l->y + 1 +
		      (l->al + 1) * ((num - l->inicial) / l->columnas)) *
			 big2 +
		     incy;

		ly = (l->y + (l->al + 1) * ((num - l->inicial) / l->columnas) +
		      l->al - 8) *
		     big2;

		wbox(ptr, an, al, c01,
		     (px - (l->an * big2 - thumb_tmp[num_tex].an) / 2) / big2,
		     (py - incy) / big2 + l->al - 8, l->an, 8);

		if (thumb_tmp[num_tex].ptr != NULL &&
		    thumb_tmp[num_tex].status) {
			if (TipoBrowser == BRUSH)
				for (y = 0; y < thumb_tmp[num_tex].al; y++) {
					for (x = 0; x < thumb_tmp[num_tex].an;
					     x++) {
						if (py + y > ly)
							c = *(
							    ghost + c0 * 256 +
							    thumb_tmp[num_tex].ptr
								[y * thumb_tmp
									 [num_tex]
									     .an +
								 x]);
						else
							c = thumb_tmp[num_tex].ptr
								[y * thumb_tmp
									 [num_tex]
									     .an +
								 x];
		...
		if (thumb_tmp[num_tex].ptr == NULL &&
		    !thumb_tmp[num_tex].status && num != 0) {
			wput(ptr, an, al, px + (l->an - 21) / 2, py + 1, 60);
		}

		if (TipoBrowser != MAPBR) {
			if (modo < 100 && num_tex == num_pincel) {
				wbox(ptr, an, al, c_b_low, px, py + l->al - 8,
				     l->an, 8);
			}
			...
			if (l->zona - 10 == num - l->inicial)
				...
				x = c3;
			wwrite(ptr, an, al, px + l->an / 2 + 1, py, 7,
			       (byte *)p, c0);
			wwrite(ptr, an, al, px + l->an / 2, py, 7, (byte *)p,
			       x);
		}
...
void M3D_crear_listboxbr(struct t_listboxbr *l)
{
	...
	wbox(ptr, an, al, c1, l->x, l->y, (l->an + 1) * l->columnas,
	     (l->al + 1) * l->lineas);

	for (y = 0; y < l->lineas; y++)
		for (x = 0; x < l->columnas; x++)
			wrectangulo(ptr, an, al, c0, l->x + (x * (l->an + 1)),
				    l->y + (y * (l->al + 1)), l->an + 2,
				    l->al + 2);

	wrectangulo(ptr, an, al, c0, l->x + (l->an + 1) * l->columnas, l->y, 9,
		    (l->al + 1) * l->lineas + 1);
	wrectangulo(ptr, an, al, c0, l->x + (l->an + 1) * l->columnas, l->y + 8,
		    9, (l->al + 1) * l->lineas - 15);
	wput(ptr, an, al, l->x + (l->an + 1) * l->columnas + 1, l->y + 1, -39);
	wput(ptr, an, al, l->x + (l->an + 1) * l->columnas + 1,
	     l->y + (l->al + 1) * l->lineas - 7, -40);
...
void M3D_actualiza_listboxbr(struct t_listboxbr *l)
{
	...
	if (wmouse_in(l->x, l->y, (l->an + 1) * l->columnas,
		      (l->al + 1) * l->lineas)) { // Calcula zona
		l->zona = (wmouse_x - l->x) / (l->an + 1) +
			  ((wmouse_y - l->y) / (l->al + 1)) * l->columnas;
		if (l->zona >= l->maximo - l->inicial ||
		    l->zona >= l->lineas * l->columnas)
			l->zona = 1;
		else
			l->zona += 10;
	} else if (wmouse_in(l->x + (l->an + 1) * l->columnas, l->y, 9, 9))
		l->zona = 2;
	else if (wmouse_in(l->x + (l->an + 1) * l->columnas,
			   l->y + (l->al + 1) * l->lineas - 8, 9, 9))
		l->zona = 3;
	else if (wmouse_in(l->x + (l->an + 1) * l->columnas, l->y + 9, 9,
			   (l->al + 1) * l->lineas - 17))
		l->zona = 4;
	...
	if (TipoBrowser != MAPBR) {
		if (old_zona != l->zona)
			if (old_zona >= 10) { // Desmarca zona
				x = l->x + 1 +
				    ((old_zona - 10) % l->columnas) *
					(l->an + 1);
				y = l->y + l->al +
				    ((old_zona - 10) / l->columnas) *
					(l->al + 1);
				p = (byte *)l->lista +
				    l->lista_an * (l->inicial + old_zona - 10);
	...
	if (l->zona == 4 && (mouse_b & 1)) {
		...
		if (l->maximo > l->lineas * l->columnas) {
			n = (l->maximo - l->lineas * l->columnas + l->columnas -
			     1) /
			    l->columnas;

			n = 0.5 +
			    (float)(n * (l->slide - l->s0)) / (l->s1 - l->s0);
		...
		if (l->maximo <= l->lineas * l->columnas)
			...
			n = (l->s0 * (n - l->inicial / l->columnas) +
			     l->s1 * (l->inicial / l->columnas)) /
			    n;
		}
		if (n != l->slide) {
			l->slide = n;
	...
	if (TipoBrowser != MAPBR) {
		if (old_zona != l->zona)
			if (l->zona >= 10) { // Marca zona
				x = l->x + 1 +
				    ((l->zona - 10) % l->columnas) *
					(l->an + 1);
				y = l->y + l->al +
				    ((l->zona - 10) / l->columnas) *
					(l->al + 1);
				p = (byte *)l->lista +
				    l->lista_an * (l->inicial + l->zona - 10);
...
void M3D_pinta_listboxbr(struct t_listboxbr *l)
{
	...
	for (y = 0; y < l->lineas; y++)
		for (x = 0; x < l->columnas; x++) {
			wbox(ptr, an, al, c1, l->x + (x * (l->an + 1)) + 1,
			     l->y + (y * (l->al + 1)) + 1, l->an, l->al - 8);
			wbox(ptr, an, al, c01, l->x + (x * (l->an + 1)) + 1,
			     l->y + (y * (l->al + 1)) + 1 + l->al - 8, l->an,
			     8);
		}

	if (wmouse_in(l->x, l->y, (l->an + 1) * l->columnas,
		      (l->al + 1) * l->lineas)) { // Calcula zona
		l->zona = ((mouse_x - v.x) / big2 - l->x) / (l->an + 1) +
			  (((mouse_y - v.y) / big2 - l->y) / (l->al + 1)) *
			      l->columnas;
		if (l->zona >= l->maximo - l->inicial ||
		    l->zona >= l->lineas * l->columnas)
			l->zona = 1;
	...
	while (n > 0)
		M3D_muestra_thumb(l, l->inicial + --n);
}
...
void PintaVisorThumb(void)
{
	...
	for (y = 0; y < mal; y += 2) {
		for (x = 0; x < man; x += 2) {
			n = *(ghost + temp[x + y * man] * 256 +
			      temp[x + 1 + y * man]);
			m = *(ghost + temp[x + (y + 1) * man] * 256 +
			      temp[x + 1 + (y + 1) * man]);
			temp2[x / 2 + (y / 2) * (man / 2) +
			      4 * big2 * (y / 2)] = *(ghost + n * 256 + m);
		}
	}
...
void CrearMapperThumb(int tex_cod, int an_thumb, int al_thumb)
{
	...
	if (Tex[TipoTex].cod == 0)
		return;
	if (!thumb_tex[num_tex].Cuad && TipoTex != FONDO && TipoTex < 4)
		return;
	...
	if ((temp2 = (byte *)malloc(Tex[TipoTex].an * Tex[TipoTex].al)) ==
	    NULL) {
		free(temp);
		return;
	}
	...
	for (y = 0; y < Tex[TipoTex].al; y++) {
		...
		for (x = 0; x < Tex[TipoTex].an; x++) {
			temp2[y * Tex[TipoTex].an + x] =
			    temp[((int)a) * man + (int)b];
			b += coefredx;
		}
		a += coefredy;
	}
	...
	if ((Tex[TipoTex].gra = (byte *)malloc(
		 (Tex[TipoTex].an * Tex[TipoTex].al) / 4)) == NULL) {
		free(temp2);
		return;
	}
	...
	for (y = 0; y < Tex[TipoTex].al; y += 2) {
		for (x = 0; x < Tex[TipoTex].an; x += 2) {
			n = *(ghost + temp2[x + y * Tex[TipoTex].an] * 256 +
			      temp2[x + 1 + y * Tex[TipoTex].an]);
			m = *(ghost +
			      temp2[x + (y + 1) * Tex[TipoTex].an] * 256 +
			      temp2[x + 1 + (y + 1) * Tex[TipoTex].an]);
			Tex[TipoTex]
			    .gra[x / 2 + (y / 2) * (Tex[TipoTex].an / 2)] =
			    *(ghost + n * 256 + m);
		}
	}
...
void PintaMapperThumbs(void)
{
	...
	for (con = 0; con < 3; con++) {
		if (Tex[con].gra != NULL) {
			...
			case PARED:
				py = (12) * big2;
				line_fx = TOPE_FADE - fade_pared + 1;
				wbox(v.ptr, an, al, c0, ANCHO_VENTANA + 7, 12,
				     48, 48);
				break;
			case TECHO:
				py = (12 + 50 + 1) * big2;
				line_fx = TOPE_FADE - fade_sector + 1;
				wbox(v.ptr, an, al, c0, ANCHO_VENTANA + 7,
				     12 + 50 + 1, 48, 48);
				break;
			case SUELO:
				py = (12 + 100 + 2 + 12) * big2;
			...
			for (y = 0; y < Tex[con].al; y++) {
				for (x = 0; x < Tex[con].an; x++) {
					// color=???; line_fx=1..16;
					// pixel_sin_mask(v.ptr[x+y*v.an])
					//        v.ptr[(py+y)*v.an+(px+x)]=Tex[con].gra[y*Tex[con].an+x];
					color =
					    Tex[con].gra[y * Tex[con].an + x];
					pixel_sin_mask(&(
...
void PintaFondoThumb(void)
{
	...
	if (Tex[FONDO].gra != NULL) {
		for (y = 0; y < 21 * big2; y++) {
			for (x = 0; x < 42 * big2; x++) {
				v.ptr[(175 * big2 + y) * v.an +
				      (202 * big2 + x)] =
				    Tex[FONDO].gra[y * 42 * big2 + x];
			}
		}
	}
}
...
void nuevo_mapa3d(void)
{
	...
	for (con = 0; con < max_windows - 1; con++) {
		if (ventana[con].tipo == 106) {
			if (((M3D_info *)ventana[con].aux)->numero ==
			    m3d->numero) {
				m3d->numero++;
				con = -1;
			}
...
void mostrar_coordenadas(void)
{
	...
	new_scroll_x = scroll_x + M_ANCHO_VENTANA * big2 -
		       M_ANCHO_VENTANA * big2 / zoom_level;
	new_scroll_y = scroll_y + M_ALTO_VENTANA * big2 -
		       M_ALTO_VENTANA * big2 / zoom_level;

	if (wmouse_in(4, 12, ANCHO_VENTANA, ALTO_VENTANA)) {
		if (region_status) { // Coordenadas relativas
			x0 = abs(my_map->points[last_point]->x - pos_x -
				 scroll_x);
			y0 = abs(my_map->points[last_point]->y - pos_y -
				 scroll_y);
			sprintf(cadenas[0], "%04d", x0);
			sprintf(cadenas[1], "%04d", y0);

			sprintf(distance, "D: %04d",
				(int)sqrt(x0 * x0 + y0 * y0));
			wwrite_in_box(v.ptr, an, an - 4, al, an - 152, al - 10,
				      0, (byte *)distance, c12);
			wwrite_in_box(v.ptr, an, an - 4, al, an - 153, al - 10,
				      0, (byte *)distance, c3);
			map_draw();
	...
	wwrite_in_box(v.ptr, an, an - 4, al, an - 62, al - 10, 0,
		      m3d_edit.fpg_name, c3);
	...
	if (edit_point != -1) {
		...
		wwrite_in_box(v.ptr, an, an - 4, al, an - 152, al - 10, 0,
			      (byte *)cadenas[0], c12);
		wwrite_in_box(v.ptr, an, an - 4, al, an - 153, al - 10, 0,
			      (byte *)cadenas[0], c3);
	} else if (edit_wall != -1) {
...
int cmpcode(const void *a, const void *b)
{
	return (((struct _thumb_tex *)a)->Code -
		((struct _thumb_tex *)b)->Code);
}
...
void draw_line(int x0, int y0, int x1, int y1, byte color)
{
	...
	test0 = 0;
	test1 = 0;
	if (x0 < clipping_min_x)
		test0 = LEFT_CLIP;
	else if (x0 > clipping_max_x)
		test0 = RIGHT_CLIP;
	if (y0 < clipping_min_y)
		test0 += TOP_CLIP;
	else if (y0 > clipping_max_y)
		test0 += BOTTOM_CLIP;

	if (x1 < clipping_min_x)
		test1 = LEFT_CLIP;
	else if (x1 > clipping_max_x)
		test1 = RIGHT_CLIP;
	if (y1 < clipping_min_y)
		test1 += TOP_CLIP;
	else if (y1 > clipping_max_y)
		test1 += BOTTOM_CLIP;
	...
	while (resultado) {
		...
		if (resultado & RIGHT_CLIP) {
			if (test0 & RIGHT_CLIP) {
				y0 = ((clipping_max_x - x1) * (y0 - y1)) /
					 (x0 - x1) +
				     y1;
				x0 = clipping_max_x;
			} else {
				y1 = ((clipping_max_x - x1) * (y0 - y1)) /
					 (x0 - x1) +
				     y1;
				x1 = clipping_max_x;
			}
		...
		// Clipping abajo
			...
			if (test0 & BOTTOM_CLIP) {
				x0 = ((clipping_max_y - y1) * (x0 - x1)) /
					 (y0 - y1) +
				     x1;
				y0 = clipping_max_y;
			} else {
				x1 = ((clipping_max_y - y1) * (x0 - x1)) /
					 (y0 - y1) +
				     x1;
				y1 = clipping_max_y;
			}
		...
		// Clipping izquierda
			...
			if (test0 & LEFT_CLIP) {
				y0 = ((clipping_min_x - x1) * (y0 - y1)) /
					 (x0 - x1) +
				     y1;
				x0 = clipping_min_x;
			} else {
				y1 = ((clipping_min_x - x1) * (y0 - y1)) /
					 (x0 - x1) +
				     y1;
				x1 = clipping_min_x;
			}
		...
		// Clipping arriba
			...
			if (test0 & TOP_CLIP) {
				x0 = ((clipping_min_y - y1) * (x0 - x1)) /
					 (y0 - y1) +
				     x1;
				y0 = clipping_min_y;
			} else {
				x1 = ((clipping_min_y - y1) * (x0 - x1)) /
					 (y0 - y1) +
				     x1;
				y1 = clipping_min_y;
			}
	...
	wline((char *)v.ptr + v.an * 12 * big2 + 4 * big2, v.an,
	      ANCHO_VENTANA * big2, ALTO_VENTANA * big2, x0 * big2, y0 * big2,
	      x1 * big2, y1 * big2, color);
}
...
void map_addpoint()
{
	...
	if (mouse_b & 1 && old_but1 == 0) {
		...
		if ((new_point = map_findpoint(pos_x, pos_y, -1)) < 0) {
			...
			my_map->points[new_point] =
			    (lptpoint)malloc(sizeof(tpoint));
		...
		if (region_status == 0) { // Si es el primer muro
			...
			if (first_point == last_point && region_status > 2) {
				...
				my_map->regions[new_region] =
				    (lptregion)malloc(sizeof(tregion));
				my_map->regions[new_region]->floor_height =
				    altura_suelo;
				my_map->regions[new_region]->ceil_height =
				    altura_techo;
				my_map->regions[new_region]->floor_tex =
				    Tex[SUELO].cod;
				my_map->regions[new_region]->ceil_tex =
				    Tex[TECHO].cod;
...
void map_editpoint()
{
	...
	if (mouse_b & 1) {
		if (old_but1 == 0) {
			...
			//-----------------------------------------------------------------------
			if ((edit_point =
				 map_selectpoint(pos_x, pos_y, &region)) == -1)
				return;
			...
			if ((pos_x + scroll_x) < 0 || (pos_y + scroll_y) < 0 ||
			    (pos_x + scroll_x) > FIN_GRID ||
			    (pos_y + scroll_y) > FIN_GRID)
				return;
		...
		if (abs(my_map->points[edit_point]->x - pos_x - scroll_x) >=
			grid_size ||
		    abs(my_map->points[edit_point]->y - pos_y - scroll_y) >=
			grid_size) {
			edit_point = -1;
...
void map_editwall()
{
	...
	if (mouse_b & 1) {
		if (old_but1 == 0) {
			...
			//-----------------------------------------------------------------------
			if ((edit_wall =
				 map_findwall(pos_x, pos_y, edit_wall)) == -1) {
				return;
			}

			TipoTex = PARED;
			CrearMapperThumb(my_map->walls[edit_wall]->texture, 48,
					 48);
			...
			//-----------------------------------------------------------------------
			for (i = 0; i < my_map->num_walls; i++) {
				if (my_map->walls[i]->front_region ==
				    my_map->walls[edit_wall]->front_region) {
					my_map->points[my_map->walls[i]->p1]
					    ->active = 1;
					my_map->points[my_map->walls[i]->p2]
					    ->active = 1;
				}
			}
...
void map_editregion()
{
	...
	if (mouse_b & 1) {
		if (old_but1 == 0) {
			...
			//  Busca el vertice y si no lo encuentra sale
			//-----------------------------------------------------------------------
			if ((edit_region = map_findregion(pos_x, pos_y, -1)) ==
			    -1) {
				return;
			}

			TipoTex = TECHO;
			CrearMapperThumb(my_map->regions[edit_region]->ceil_tex,
					 48, 48);
			TipoTex = SUELO;
			CrearMapperThumb(
			    my_map->regions[edit_region]->floor_tex, 48, 48);
...
void map_draw()
{
	...
	if (x0 < x1 && y0 < y1)
		wbox(v.ptr, v.an / big2, v.al / big2, c1, 4 + x0, 12 + y0,
		     x1 - x0, y1 - y0);

	if (x0 > 0)
		wbox(v.ptr, v.an / big2, v.al / big2, c01, 4, 12, x0,
		     ALTO_VENTANA);
	if (x1 < ANCHO_VENTANA)
		wbox(v.ptr, v.an / big2, v.al / big2, c01, 4 + x1, 12,
		     ANCHO_VENTANA - x1, ALTO_VENTANA);
	if (y0 > 0)
		wbox(v.ptr, v.an / big2, v.al / big2, c01, 4 + x0, 12, x1 - x0,
		     y0);
	if (y1 < ALTO_VENTANA)
		wbox(v.ptr, v.an / big2, v.al / big2, c01, 4 + x0, 12 + y1,
		     x1 - x0, ALTO_VENTANA - y1);
	...
	if (grid) {
		...
		do {
			...
			do {
				...
				x0 = zoom_level *
				     (x0 - (x0 + scroll_x) % inc - aux_x);
				y0 = zoom_level *
				     (y0 - (y0 + scroll_y) % inc - aux_y);

				i += inc_aux;

				if (x0 < ANCHO_VENTANA * big2 &&
				    y0 < ALTO_VENTANA * big2)
	...
	for (i = 0; i < my_map->num_flags; i++) {
		...
		wput_in_box((byte *)direccion, ancho, ANCHO_VENTANA,
			    ALTO_VENTANA, x0 - 2, y0 - 2, 244);
	}
	if (modo_edicion == PINTA_BANDERA && j)
		wput_in_box((byte *)direccion, ancho, ANCHO_VENTANA,
			    ALTO_VENTANA, x1 - 2, y1 - 2, 243);
	...
	for (i = 0; i < my_map->num_points; i++) {
		...
		if (my_map->points[i]->active)
			wput_in_box((byte *)direccion, ancho, ANCHO_VENTANA,
				    ALTO_VENTANA, x0 - 2, y0 - 2, 241);
		else
			wput_in_box((byte *)direccion, ancho, ANCHO_VENTANA,
				    ALTO_VENTANA, x0 - 2, y0 - 2, 242);
	}
	...
	for (i = 0; i < my_map->num_walls; i++) {
		if (!my_map->walls[i]->active) {
			...
			//  Coordenadas de pantalla con el zoom
			x0 = zoom_level *
			     (my_map->points[p1]->x - new_scroll_x) / big2;
			y0 = zoom_level *
			     (my_map->points[p1]->y - new_scroll_y) / big2;
			x1 = zoom_level *
			     (my_map->points[p2]->x - new_scroll_x) / big2;
			y1 = zoom_level *
			     (my_map->points[p2]->y - new_scroll_y) / big2;
	...
	if (region_status && wmouse_in(4, 12, ANCHO_VENTANA, ALTO_VENTANA)) {
		//  Coordenadas de pantalla con el zoom
		x0 = zoom_level *
		     (my_map->points[last_point]->x - new_scroll_x) / big2;
		y0 = zoom_level *
		     (my_map->points[last_point]->y - new_scroll_y) / big2;
	...
	if (mouse_b & 1 && old_but1 == 0) {
		...
		if ((edit_flag = map_findflag(pos_x, pos_y, -1)) < 0) {
			for (i = 0; i < my_map->num_flags; i++) {
				if (my_map->flags[i]->number == num_bandera) {
					edit_flag = i;
					my_map->flags[edit_flag]->x =
					    pos_x + scroll_x;
					my_map->flags[edit_flag]->y =
					    pos_y + scroll_y;
					return;
				}
			}
			edit_flag = my_map->num_flags;
			my_map->flags[edit_flag] =
			    (lptflag)malloc(sizeof(tflag));
...
int map_findwall(int x, int y, int discard_wall)
{
	...
	for (i = 0; i < my_map->num_walls; i++) {
		...
		//-------------------------------------------------------------------------
		// Compruebo si el punto esta dentro de la caja definida por p1
		// y p2
		//-------------------------------------------------------------------------
		if (((my_map->points[p1]->x - grid_size <= x &&
		      x <= my_map->points[p2]->x + grid_size) ||
		     (my_map->points[p2]->x - grid_size <= x &&
		      x <= my_map->points[p1]->x + grid_size)) &&
		    ((my_map->points[p1]->y - grid_size <= y &&
		      y <= my_map->points[p2]->y + grid_size) ||
		     (my_map->points[p2]->y - grid_size <= y &&
		      y <= my_map->points[p1]->y + grid_size))) {
			//-----------------------------------------------------------------------
			// Utilizo la ecuacion de la recta para saber si
			...
			distancia =
			    abs(x1 * y2 - x2 * y1) / sqrt(x1 * x1 + y1 * y1);
...
void map_deleteregion(int region)
{
	...
	if (region < 0 || region >= my_map->num_regions)
		return;
...
void map_deletewall(int wall)
{
	if (wall < 0 || wall >= my_map->num_walls)
		return;
...
void map_deletepoint(int point)
{
	...
	if (point < 0 || point >= my_map->num_points)
		return;
...
int map_selectpoint(int x, int y, int *region)
{
	...
	if (lowest_region != MAX_REGIONS - 1 &&
...
void map_joinpoints()
{
	...
	for (i = 0; i < my_map->num_points; i++) {
		while ((new_point = map_findpoint(
			    my_map->points[i]->x - scroll_x,
			    my_map->points[i]->y - scroll_y, i)) >= 0) {
			my_map->points[i]->links +=
			    my_map->points[new_point]->links; // OJO 2
			for (j = 0; j < my_map->num_walls; j++) {
			...
			my_map->points[new_point] =
			    my_map->points[my_map->num_points];
			for (j = 0; j < my_map->num_walls; j++) {
...
int map_joinpoint(int point)
{
	...
	if ((new_point = map_findpoint(my_map->points[point]->x - scroll_x,
				       my_map->points[point]->y - scroll_y,
				       point)) >= 0) {
		...
		my_map->points[new_point]->links +=
		    my_map->points[point]->links; // OJO 2
		my_map->points[new_point]->active = 1;
		for (i = 0; i < my_map->num_walls; i++) {
			if (my_map->walls[i]->p1 == point)
				my_map->walls[i]->p1 = new_point;
...
void map_save()
{
	...
	//  Asigno los puntos
	//---------------------------------------------------------------------------
	zp = (struct ZF_Point *)malloc(my_map->num_points *
				       sizeof(struct ZF_Point));

	for (i = 0; i < my_map->num_points; i++) {
		zp[i].Type = 0;			// Type               *  0 * OK
		zp[i].x = my_map->points[i]->x; // Point x coordinate
		zp[i].y = FIN_GRID -
			  my_map->points[i]->y; // Point y coordinate OJO
						//    zp[i].y=my_map->points[i]->y;
						//    // Point y coordinate OJO
		zp[i].path =
		    -1; // Path index                          * -1 * OK
		zp[i].link =
		    -1; // Index of the next point in the link * -1 * OK
	}
	...
	//  Asigno las regiones
	//---------------------------------------------------------------------------
	zr = (struct ZF_Region *)malloc(my_map->num_regions *
					sizeof(struct ZF_Region));

	for (i = 0; i < my_map->num_regions; i++) {
		zr[i].Type = 0; // Type
		zr[i].FloorH =
		    (M8SHORT)my_map->regions[i]->floor_height; // Floor height
		zr[i].CeilH =
		    (M8SHORT)my_map->regions[i]->ceil_height; // Ceiling height
		zr[i].Below = -1;			      // Region below
		zr[i].Above = -1;			      // Region above
		zr[i].FloorTex =
		    my_map->regions[i]->floor_tex; // Textura de suelo
		zr[i].CeilTex =
		    my_map->regions[i]->ceil_tex; // Textura de techo
		strcpy(zr[i].Eff, "NO_NAME");     // Eff program
		zr[i].Fade =
		    (M8SHORT)my_map->regions[i]->fade; // Effect's param
		zr[i].Tag = 0;			       // Tag ID
	}
	...
	//  Asigno los muros
	//---------------------------------------------------------------------------
	zw = (struct ZF_Wall *)malloc(my_map->num_walls *
				      sizeof(struct ZF_Wall));
	...
	for (i = 0; i < my_map->num_walls; i++) {
		...
		zw[i].TopTex =
		    my_map->walls[i]->texture_top;	// Top texture name
		zw[i].MidTex = my_map->walls[i]->texture; // Mid texture name
		zw[i].BotTex =
		    my_map->walls[i]->texture_bot; // Bot texture name
	...
	//  Asigno las banderas
	//---------------------------------------------------------------------------
	zf = (struct ZF_Flag *)malloc(my_map->num_flags *
				      sizeof(struct ZF_Flag));
...
void map_saveedit(FILE *fichero, lptmap map_aux)
{
	...
	total = 548 + 4 + map_aux->num_points * sizeof(tpoint) + 4 +
		map_aux->num_walls * sizeof(twall) + 4 +
		map_aux->num_regions * sizeof(tregion) + 4 +
		map_aux->num_flags * sizeof(tflag) + 4;
...
void map_read(M3D_info *m3d_aux)
{
	...
	for (i = 0; i < my_map->num_walls; i++) {
		...
		debugprintf("walls[%d]= %d <-> %d - fregion =%d bregion = %d\n",
			    i, my_map->walls[i]->p1, my_map->walls[i]->p2,
			    my_map->walls[i]->front_region,
			    my_map->walls[i]->back_region);
	}

	fread(&my_map->num_regions, 4, 1, fichero);
	//	printf("regions: %d region size:
	...
	for (i = 0; i < my_map->num_regions; i++) {
		...
		//my_map->regions[i]->floor_height,my_map->regions[i]->ceil_height);
	}

	fread(&my_map->num_flags, 4, 1, fichero);
	//  printf("nume flags: %d flag size:
	...
	for (i = 0; i < my_map->num_flags; i++) {
		...
		//   my_map->flags[i]->x,my_map->flags[i]->y);
	}
	fread(&my_map->fondo, 4, 1, fichero);
	...
	for (i = 0; i < my_map->num_regions; i++) {
		...
		for (j = 0; j < my_map->num_walls; j++) {
			if (my_map->walls[j]->front_region == i) {
				poligono[n_puntos * 2] =
				    (short)my_map->points[my_map->walls[j]->p1]
					->x;
				poligono[n_puntos * 2 + 1] =
				    (short)my_map->points[my_map->walls[j]->p1]
					->y;
				if (poligono[n_puntos * 2] < x0)
					x0 = poligono[n_puntos * 2];
				if (poligono[n_puntos * 2] > x1)
					x1 = poligono[n_puntos * 2];
				if (poligono[n_puntos * 2 + 1] < y0)
					y0 = poligono[n_puntos * 2 + 1];
				if (poligono[n_puntos * 2 + 1] > y1)
					y1 = poligono[n_puntos * 2 + 1];
				n_puntos++;
			}
		}
...
void map_sortregions(int free_polys)
{
	...
	if ((polys = (word *)malloc(my_map->num_regions * reg_polys * 2)) ==
	    NULL)
		return;
	...
	for (i = 0; i < my_map->num_regions; i++) {
		...
		for (npuntos = 0, j = 0; j < my_map->num_walls; j++) {
			if (my_map->walls[j]->front_region == i) {
				...
				if (pol[5 + npuntos] < pol[0])
					pol[0] = pol[5 + npuntos];
				if (pol[5 + npuntos] > pol[1])
					pol[1] = pol[5 + npuntos];
				if (pol[6 + npuntos] < pol[2])
					pol[2] = pol[6 + npuntos];
				if (pol[6 + npuntos] > pol[3])
					pol[3] = pol[6 + npuntos];
	...
	for (j = 0; j < my_map->num_walls; j++) {
		if (regiones[j])
			continue;
		xm = (my_map->points[my_map->walls[j]->p1]->x +
		      my_map->points[my_map->walls[j]->p2]->x) /
		     2;
		ym = (my_map->points[my_map->walls[j]->p1]->y +
		      my_map->points[my_map->walls[j]->p2]->y) /
		     2;
		map_findregion2(xm, ym, my_map->walls[j]->front_region);
		if (inside >
		    my_map->regions[my_map->walls[j]->front_region]->type)
			my_map->regions[my_map->walls[j]->front_region]->type =
			    inside;
	}
...
void map_asignregions()
{
	...
	if ((muros = (lptmuros)malloc(my_map->num_walls * sizeof(tmuros))) ==
	    NULL) {
		free(polys);
		return; // OJO
	}
	...
	for (i = 0; i < my_map->num_walls; i++) {
		...
		muros[i].inside =
		    my_map->regions[my_map->walls[i]->front_region]->type;
		muros[i].front = my_map->walls[i]->front_region;

		if (my_map->walls[i]->p1 > my_map->walls[i]->p2) {
			muros[i].id =
			    my_map->walls[i]->p1 * 32000 + my_map->walls[i]->p2;
			muros[i].tipo = 0;
		} else {
			muros[i].id =
			    my_map->walls[i]->p2 * 32000 + my_map->walls[i]->p1;
			muros[i].tipo = 1;
		}
	}
					...
					if (muros[i].id == muros[j].id) {
						if (muros[i].tipo ==
							...
							if (muros[j].inside >
								...
								muros[i].front =
								    muros[j]
									.front; // my_map->walls[j]->front_region;
...
*/
		...
		//-------------------------------------------------------------------------
			...
			xm = (my_map->points[my_map->walls[i]->p1]->x +
			      my_map->points[my_map->walls[i]->p2]->x) /
			     2;
			ym = (my_map->points[my_map->walls[i]->p1]->y +
			      my_map->points[my_map->walls[i]->p2]->y) /
			     2;

			region_back = map_findregion2(xm, ym, muros[i].front);

			//      region_back=map_findregion(xm-scroll_x,ym-scroll_y,muros[i].front);
			//      //my_map->walls[i]->front_region);
			...
			//-------------------------------------------------------------------------
			if (region_back != -1) {
				my_map->walls[i]->texture_top =
				    my_map->walls[i]->texture;
				my_map->walls[i]->texture_bot =
				    my_map->walls[i]->texture;
...
void map_reduce(int ancho, int alto, char *buffer)
{
	...
	aspect_ratio_x =
	    (float)ancho / (float)(my_map->bbox_x_fin - my_map->bbox_x_ini + 1);
	aspect_ratio_y =
	    (float)alto / (float)(my_map->bbox_y_fin - my_map->bbox_y_ini + 1);
	...
	incx = (ancho - (aspect_ratio_x *
			 (my_map->bbox_x_fin - my_map->bbox_x_ini + 1))) /
	       2;
	incy = (alto - (aspect_ratio_y *
			(my_map->bbox_y_fin - my_map->bbox_y_ini + 1))) /
	       2;
	...
	for (i = 0; i < my_map->num_walls; i++) {
		...
		x0 = (int)((float)(my_map->points[p1]->x - my_map->bbox_x_ini) *
			   aspect_ratio_x);
		y0 = (int)((float)(my_map->points[p1]->y - my_map->bbox_y_ini) *
			   aspect_ratio_y);
		x1 = (int)((float)(my_map->points[p2]->x - my_map->bbox_x_ini) *
			   aspect_ratio_x);
		y1 = (int)((float)(my_map->points[p2]->y - my_map->bbox_y_ini) *
			   aspect_ratio_y);

		wline(buffer, ancho, ancho, alto, x0 + incx, y0 + incy,
		      x1 + incx, y1 + incy, c4);
	}
}
