...
int NewSample(
    pcminfo *mypcminfo); // 1-OK, 0-ERROR
			 // Espacio (malloc+lock) en mypcminfo->SoundData para
			 // mypcminfo->SoundSize shorts
void wline(char *ptr, int realan, int an, int al, int x0, int y0, int x1,
	   int y1, char color);
void linea_pixel(char *ptr, int an, int realan, int al, int x, int y,
		 char color);
		...
		if (length < 3 * Ancho) {
			...
			for (x = 0; x < length; x++) {
				...
				if (First) {
					...
					wline((char *)ptr, v.an, Ancho, Alto,
					      lx, ly, (int)position, y,
					      c_g_low);
					lx = (int)position;
					ly = y;
				}
...
void MOD1(void)
{
	...
	if (Mix_PlayingMusic() && mymodinfo->SongCode == SongCode) {
		if (ModButton && ModWindow == v.orden)
	...
	if (mymodinfo->SongCode == SongCode) {
		if (Mix_PlayingMusic()) {
			...
			for (canal = 0; canal < SongChannels; canal++) {
				x = ancho_barra * canal;
				y = (int)rand() % 25 *
				    big2; //(judas_getvumeter(canal)*(float)25)*big2;
...
Mix_Chunk *DIVMIX_LoadPCM(char *path)
{
	...
	_splitpath(full, (char *)drive, (char *)dir, (char *)fname,
		   (char *)ext);
	strupr((char *)ext);
	...
	if (f) {
		...
		rw = SDL_RWFromMem((void *)dst,
				   (int)(Len + 24 + sizeof(HeadDC)));
...
void OpenSound(void)
{
	...
	for (num = 0; num < larchivosbr.maximo; num++) {
		if (thumb[num].tagged) {
			...
			if ((pcminfo_aux = (byte *)malloc(sizeof(pcminfo))) ==
			    NULL) {
			...
			if (judas_error != JUDAS_OK &&
				...
				if (SI)
					free(SI);
				SI =
				    judas_loadrawsample(SoundPathName, 0, 0, 0);
			}
			...
			if (mypcminfo->SoundData != NULL)
				memcpy(mypcminfo->SoundData, SI->abuf,
				       SI->alen);
...
void SaveSound(pcminfo *mypcminfo, char *dst)
{
	...
	if (!strcmp(ext, ".WAV")) {
		...
		MyHeadDC.dwAvgBytesPerSec =
		    mypcminfo->SoundFreq * (mypcminfo->SoundBits / 8);
		MyHeadDC.wBlockAlign = 1;
		MyHeadDC.wBits = mypcminfo->SoundBits;

		if (fwrite(&MyHeadDC, 1, sizeof(HeadDC), dstfile) !=
		    sizeof(HeadDC)) {
			fclose(dstfile);
			DaniDel(dst);
			v_texto = (char *)texto[47];
		...
		if (mypcminfo->SoundBits == 8) {
			...
			for (con = 0; con < length; con++)
				byte_ptr[con] =
				    mypcminfo->SoundData[con] / 0xFF + 0x80;
		}
	} else {
		...
		for (con = 0; con < length; con++) {
			byte_ptr[con] =
			    mypcminfo->SoundData[(int)pos] / 0xFF + 0x80;
			pos += paso;
		}
	}
...
void OpenSong(void)
{
	...
	for (num = 0; num < larchivosbr.maximo; num++) {
		if (thumb[num].tagged) {
			...
			if ((modinfo_aux = (byte *)malloc(sizeof(modinfo))) ==
			    NULL) {
				v_texto = (char *)texto[45];
				dialogo(err0);
				continue;
...
void PlaySong(char *pathname)
{
	...
	if (Mix_PlayMusic(music, 1) == -1) {
		...
		// well, there's no music, but most games don't break without
		// music...
	}
	...
	switch (SongType) {
	case XM:
		SongChannels = judas_getxmchannels();
		break;
	case S3M:
		SongChannels = judas_gets3mchannels();
		break;
	case MOD:
		SongChannels = judas_getmodchannels();
		break;
	}
...
void EditSound0(void)
{
	...
	pos += text_len(texto[501]) + 3;
	_button(502, pos, v.al - 34, 0);
	pos += text_len(texto[502]) + 3;
	_button(503, pos, v.al - 34, 0);
	pos += text_len(texto[503]) + 3;
	...
	pos += text_len(texto[553]) + 3;
	_button(554, pos, 15, 0);

	pos = 193;

	_button(555, pos, 30, 0);
	pos += text_len(texto[555]) + 3;
	_button(556, pos, 30, 0);
	pos += text_len(texto[556]) + 3;
	_button(557, pos, 30, 0);
	pos += text_len(texto[557]) + 3;
...
*/
	...
	if (sel_1 <= sel_2) {
		...
		//  wbox(v.ptr, an, al, c3, (sel_1/big2)+4, 12+PosY,
		//  (sel_2/big2)-(sel_1/big2)+1, al_v);
		...
		//  No pone cajas de 1 de ancho
		//  wbox(v.ptr, an, al, c3, (sel_2/big2)+4, 12+PosY,
		//  (sel_1/big2)-(sel_2/big2)+1, al_v);
...
void RecSound1(void)
{
	...
	wwrite_in_box(v.ptr, an, an - 12, al, an - 85, 20, 0, (byte *)cwork,
		      c4);
...
void ModifySound(int option)
{
	...
	case 5: // Copy
		...
		memcpy(Clipboard.SoundData, buffer + ini,
		       Clipboard.SoundSize * 2);
		return;
	case 6: // Cut
		if (Clipboard.SoundData != NULL) {
			free(Clipboard.SoundData);
		...
		if (mypcminfo->SoundSize - Clipboard.SoundSize <= 0) {
			...
			if ((pcminfo_bak.SoundData = (short *)malloc(
				 (mypcminfo->SoundSize - Clipboard.SoundSize) *
				 2)) == NULL) {
				free(Clipboard.SoundData);
				Clipboard.SoundData = NULL;
				v_texto = (char *)texto[45];
			...
			pcminfo_bak.SoundSize =
			    mypcminfo->SoundSize - Clipboard.SoundSize;

			memcpy(pcminfo_bak.SoundData, buffer, ini * 2);
			memcpy(pcminfo_bak.SoundData + ini, buffer + fin,
			       (mypcminfo->SoundSize - fin) * 2);
			...
			//		rw =
			//SDL_RWFromMem(pcminfo_bak.SoundBits,pcminfo_bak.SoundSize);
	...
	case 7: // Paste
		...
		if (Clipboard.SoundFreq != mypcminfo->SoundFreq) {
			paso = (float)Clipboard.SoundFreq /
			       (float)mypcminfo->SoundFreq;
			...
			for (pos = 0; pos < length; pos++) {
				short_ptr[pos] =
				    Clipboard.SoundData[(int)pos_f];
				pos_f += paso;
			}
		}

		if ((pcminfo_bak.SoundData = (short *)malloc(
			 (mypcminfo->SoundSize + length) * 2)) == NULL) {
			v_texto = (char *)texto[45];
			dialogo(err0);
			return;
		}
		...
		memcpy(pcminfo_bak.SoundData + ini + length, buffer + ini,
		       (mypcminfo->SoundSize - ini) * 2);
		if (Clipboard.SoundFreq != mypcminfo->SoundFreq)
			free(short_ptr);
	...
	case 8: // Play
		...
		judas_playsample(&sample, 0, mypcminfo->SoundFreq, 64 * 256,
				 MIDDLE);
#endif
...
int NewSample(pcminfo *mypcminfo)
{
	...
	mypcminfo->sample->end =
	    mypcminfo->sample->start + mypcminfo->SoundSize * 2;
		...
		if (dy <= dx) {
			...
			if (y0 <= y1)
				while (x < x1) {
					...
					linea_pixel(ptr, an, realan, al, x, y,
						    color);
				}
			else
				while (x < x1) {
