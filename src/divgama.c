...
void crea_gama(struct _gcolor *t_gama, byte *gama)
{
	...
	for (n = 0; n < 9; n++)
		if (t_gama[n].selec) {
			t_gama[n].color =
			    find_color(t_gama[n].r, t_gama[n].g, t_gama[n].b);
		}
	...
	while (1) {
		...
		if (t_gama[boton2].selec || boton2 == 8) {
			while (boton1 + 1 < boton2) {
				tmp_r += ((float)t_gama[boton2].r - tmp_r) /
					 (float)(boton2 - boton1);
				tmp_g += ((float)t_gama[boton2].g - tmp_g) /
					 (float)(boton2 - boton1);
				tmp_b += ((float)t_gama[boton2].b - tmp_b) /
					 (float)(boton2 - boton1);
				...
				t_gama[boton1].color =
				    find_color(tmp_r, tmp_g, tmp_b);
				t_gama[boton1].r =
				    dac[t_gama[boton1].color * 3];
				t_gama[boton1].g =
				    dac[t_gama[boton1].color * 3 + 1];
				t_gama[boton1].b =
				    dac[t_gama[boton1].color * 3 + 2];

				if (t_gama[boton1].color !=
				    t_gama[boton1 - 1].color) {
	...
	while (1) {
		...
		if (boton2 % 16 == 0 || boton2 == 127) {
			while (boton1 + 1 < boton2) {
				tmp_r +=
				    ((float)dac[gama[boton2] * 3] - tmp_r) /
				    (float)(boton2 - boton1);
				tmp_g +=
				    ((float)dac[gama[boton2] * 3 + 1] - tmp_g) /
				    (float)(boton2 - boton1);
				tmp_b +=
				    ((float)dac[gama[boton2] * 3 + 2] - tmp_b) /
				    (float)(boton2 - boton1);
...
void gama0(void)
{
	...
	// Aqui se inicializa el estado de la gama y los botones (r, g, b y
	// selec)
...
void gama1(void)
{
	...
	for (con = 0; con < 9; con++) {
		if (t_gama[con].selec)
			wput(v.ptr, an, al, button_coords[con].x,
			     button_coords[con].y, -44);
		else
			wput(v.ptr, an, al, button_coords[con].x,
			     button_coords[con].y, -34);

		wrectangulo(v.ptr, an, al, c0, button_coords[con].x - 4,
			    button_coords[con].y - 15, 14, 14);
		wbox(v.ptr, an, al, t_gama[con].color,
		     button_coords[con].x - 4 + 1,
	...
	for (con = 0; con < 128; con++) {
		wbox(v.ptr, an, al, gama[con], gama_x + con + 1, gama_y + 1, 1,
		     2);
	}
}
...
void gama2(void)
{
	...
	if (!(mouse_b & 1) && (old_mouse_b & 1)) {
		for (con = 0; con < 9; con++) {
			if (wmouse_in(button_coords[con].x,
				      button_coords[con].y, 7, 7)) {
				t_gama[con].selec = !t_gama[con].selec;
				call((voidReturnType)v.paint_handler);
				v.volcar = 1;
			} else if (wmouse_in(button_coords[con].x - 3,
					     button_coords[con].y - 15, 14,
					     14)) {
