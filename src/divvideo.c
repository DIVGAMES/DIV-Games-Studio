...
int IsFullScreen(SDL_Surface *surface) { return OSDEP_IsFullScreen(); }
...
int nothing(SDL_Surface *surface)
{
	...
	if (IsFullScreen(surface)) {
		if ((vga = OSDEP_SetVideoMode(vga_an, vga_al, CDEPTH, 0)) ==
		    NULL)
			return 0;
		...
		vga =
		    OSDEP_SetVideoMode(vga_an, vga_al, CDEPTH,
				       1); // | SDL_HWSURFACE | SDL_DOUBLEBUF);
...
static short scan[MAX_YRES * 4]; // Por scan [x,an,x,an] se definen hasta 2
				 // segmentos a volcar
...
SDL_Surface *copy_surface(SDL_Surface *source)
{
	 ...
	 * clear the SDL_SRCALPHA flag presumably set by IMG_Load. But why
	 * wouldn't blitting work otherwise?
	 */
...
void svmode(void)
{
	...
	vga = OSDEP_SetVideoMode(
	    GCW_W, GCW_H, 8,
	    SDL_HWSURFACE |
		SDL_DOUBLEBUF); // SDL_HWPALETTE|SDL_SRCCOLORKEY|SDL_HWSURFACE|SDL_DOUBLEBUF);
	...
	//			SDL_SetAlpha(copia_surface,SDL_SRCALPHA |
	//SDL_RLEACCEL,128);
	...
	//	if(SDL_SetColorKey(copia_surface , SDL_SRCCOLORKEY ,
	//colorkey)==-1) 		fprintf(stderr, "Warning: colorkey will not be used,
	//reason: %s\n", SDL_GetError());;
	...
	if (vga != NULL) {
		...
		for (vn = max_windows - 1; vn >= 0; vn--) {
			if (ventana[vn].surfaceptr != NULL) {
				tempsurface =
				    SDL_DisplayFormat(ventana[vn].surfaceptr);
				if (tempsurface != NULL) {
...
void volcadosdl(byte *p)
{
	...
	for (vn = max_windows - 1; vn >= 0; vn--) {
		if (ventana[vn].surfaceptr != NULL) {
			...
			// SDL_SetAlpha(ventana[vn].surfaceptr,SDL_SRCALPHA |
			// SDL_RLEACCEL, 230);

			if (ventana[vn].exploding == 1) {
				SDL_SetAlpha(ventana[vn].surfaceptr,
					     SDL_SRCALPHA | SDL_RLEACCEL,
					     explode_num * 25);
			}
			SDL_BlitSurface(ventana[vn].surfaceptr, NULL, vga,
					&trc);
		}
	}
