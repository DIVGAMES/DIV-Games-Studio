...
void find_colors(void)
{
	...
	if ((f = fopen("help/help.fig", "rb")) != NULL) {
		...
		for (x = 1; x < 256; x++) {
			help_xlat[x] = find_color_not0(
			    pal[x * 3], pal[x * 3 + 1], pal[x * 3 + 2]);
		}
	}
}
...
void ord_paleta0(void)
{
	...
	create_dac4();
	for (n = 0; n < 256; n++)
		paleta[n] = find_color(paleta_original[n * 3],
				       paleta_original[n * 3 + 1],
				       paleta_original[n * 3 + 2]);
}
...
byte find_ord(byte *dac)
{
	...
	do
		if (*pal != 255) {
			dif = *(int *)(cuad + r2 + *pal * 4);
			pal++;
			dif += *(int *)(cuad + g2 + *pal * 4);
			pal++;
			dif += *(int *)(cuad + b2 + *pal * 4);
			pal += 2;
			if (dif < dmin) {
				dmin = dif;
				color = pal - 4;
...
{
	create_dac4();
	return (find_color((dac[a * 3] + dac[b * 3]) / 2,
...
void calcula_regla(int n)
{
	...
	if (!reglas[n].fijo) { // Las reglas fijas no se pueden calcular
		...
		case 0: // Lineal, el cálculo es obvio, se toman los colores en
			// secuencia
			for (a = 1; a < 32; a++)
				reglas[n].col[a + 1] = reglas[n].col[a] + 1;
			break;
		case 1: // Adaptable cada 1 color, no hay nada que calcular
			break;
		case 2: // Adaptable cada 2 colores, se rellenan con las medias
			for (a = 0; a < 32; a += 2)
				reglas[n].col[a + 1] = media(
				    reglas[n].col[a], reglas[n].col[a + 2]);
			break;
		case 4: // Adaptable cada 4
			for (a = 0; a < 32; a += 4) {
				reglas[n].col[a + 2] = media(
				    reglas[n].col[a], reglas[n].col[a + 4]);
				reglas[n].col[a + 1] = media(
				    reglas[n].col[a], reglas[n].col[a + 2]);
				reglas[n].col[a + 3] = media(
				    reglas[n].col[a + 2], reglas[n].col[a + 4]);
			}
			break;
		case 8: // Adaptable cada 8
			for (a = 0; a < 32; a += 8) {
				reglas[n].col[a + 4] = media(
				    reglas[n].col[a], reglas[n].col[a + 8]);
				reglas[n].col[a + 2] = media(
				    reglas[n].col[a], reglas[n].col[a + 4]);
				reglas[n].col[a + 6] = media(
				    reglas[n].col[a + 4], reglas[n].col[a + 8]);
				reglas[n].col[a + 1] = media(
				    reglas[n].col[a], reglas[n].col[a + 2]);
				reglas[n].col[a + 3] = media(
				    reglas[n].col[a + 2], reglas[n].col[a + 4]);
				reglas[n].col[a + 5] = media(
				    reglas[n].col[a + 4], reglas[n].col[a + 6]);
				reglas[n].col[a + 7] = media(
				    reglas[n].col[a + 6], reglas[n].col[a + 8]);
			}
			break;
		}
	}
...
void LoadPal()
{
	...
	if (num_taggeds == 1)
		for (num = 0; num < larchivosbr.maximo; num++) {
			if (thumb[num].tagged) {
				strcpy(input, larchivosbr.lista +
...
void RefPalAndDlg(int no_tocar_mapas, int guardar_original)
{
	...
	for (x = 1; x < 256; x++)
		xlat[x] =
		    find_color_not0(pal[x * 3], pal[x * 3 + 1], pal[x * 3 + 2]);

	if (!no_tocar_mapas) {
		for (n = 1; n < max_windows; n++) {
			if (ventana[n].tipo == 100) {
				ptr = ventana[n].mapa->map;
				sum = ventana[n].mapa->map_an *
				      ventana[n].mapa->map_al;
				x = 0;
				do {
					*ptr = xlat[*ptr];
					ptr++;
	...
	for (n = 0; n < 256; n++)
		if (graf_help[n].ptr) {
			for (x = graf_help[n].ran * graf_help[n].ral - 1;
			     x >= 0; x--) {
	...
	for (n = 0; n < max_windows; n++) {
		if (ventana[n].tipo == 102 &&
		    !strcmp((char *)ventana[n].nombre, (char *)texto[83])) {
...
void ordena1(void)
{
	...
	for (y = 0; y < 16; y++)
		for (x = 0; x < 16; x++)
			wbox(v.ptr, an, al, paleta[x + y * 16], 4 + x * 4,
			     12 + y * 4, 3, 3);

	ord_paleta1();
	for (y = 0; y < 16; y++)
		for (x = 0; x < 16; x++)
			wbox(v.ptr, an, al, paleta[x + y * 16], 66 + 4 + x * 4,
			     12 + y * 4, 3, 3);
...
void ordena2(void)
{
	...
	if ((mouse_b & 1) && wmouse_in(2, 10, 132, 132)) {
		...
		if (ord != ordenacion) {
			v.volcar = 1;
			wrectangulo(v.ptr, an, al, c2, 2, 10, 65 * 2 + 3,
				    65 * 2 + 3);
			wbox(v.ptr, an, al, c2, 2 + 66, 10, 1, 65 * 2 + 3);
			wbox(v.ptr, an, al, c2, 2, 10 + 66, 65 * 2 + 3, 1);
			switch (ord) {
			case 0:
				...
				break;
			case 1:
				wrectangulo(v.ptr, an, al, c4, 2 + 66, 10, 67,
					    67);
				ord_paleta1();
				break;
			case 2:
				wrectangulo(v.ptr, an, al, c4, 2, 10 + 66, 67,
					    67);
				ord_paleta2();
				break;
			case 3:
				wrectangulo(v.ptr, an, al, c4, 2 + 66, 10 + 66,
					    67, 67);
				ord_paleta3();
				break;
			}
...
void fusionar_paletas(void)
{
	byte pal[2048]; // Dos paletas en formato
			// R,G,B,sum(RGB),R,G,B,sum(RGB)...
	word paleta[512];
	int dist[512];
	int n, c, min, cmin;
	...
	for (n = 1; n < 511; n++) {
		dist[n] = *(int *)(cuad + pal[paleta[n] * 4] * 256 +
				   pal[paleta[n + 1] * 4] * 4);
		dist[n] += *(int *)(cuad + pal[paleta[n] * 4 + 1] * 256 +
				    pal[paleta[n + 1] * 4 + 1] * 4);
		dist[n] += *(int *)(cuad + pal[paleta[n] * 4 + 2] * 256 +
				    pal[paleta[n + 1] * 4 + 2] * 4);
	}
	...
	while (c > 255) {
		...
		g = (pal[paleta[cmin] * 4 + 1] +
		     pal[paleta[cmin + 1] * 4 + 1]) /
		    2;
		b = (pal[paleta[cmin] * 4 + 2] +
		     pal[paleta[cmin + 1] * 4 + 2]) /
		    2;

		memmove(&paleta[cmin], &paleta[cmin + 1],
			sizeof(paleta[0]) * (c - cmin));
		memmove(&dist[cmin], &dist[cmin + 1],
			sizeof(dist[0]) * (c - cmin));
		...
		if (cmin < c - 1) {
			...
			dist[n] = *(int *)(cuad + pal[paleta[n] * 4] * 256 +
					   pal[paleta[n + 1] * 4] * 4);
			dist[n] +=
			    *(int *)(cuad + pal[paleta[n] * 4 + 1] * 256 +
				     pal[paleta[n + 1] * 4 + 1] * 4);
			dist[n] +=
			    *(int *)(cuad + pal[paleta[n] * 4 + 2] * 256 +
				     pal[paleta[n + 1] * 4 + 2] * 4);
		}
...
void crear_paleta(void)
{
	byte *pal; // Una lista de colores en formato
		   // R,G,B,sum(RGB),R,G,B,sum(RGB)..
	word *paleta;
	int *dist;
	int rr, gg, bb, n;
	...
	while (c > 255) {
		...
		if (!cargar_paleta)
			if ((c & 127) == 0)
				Progress((char *)texto[497],
					 num_colores * 2 + num_colores - c,
					 num_colores * 4 - 256);
		...
		if (c - cmin > 0) {
			memmove(&paleta[cmin + 1], &paleta[cmin + 2],
				sizeof(word) * (c - cmin));
			memmove(&dist[cmin + 1], &dist[cmin + 2],
				sizeof(int) * (c - cmin));
		}
	...
	for (n = 0; n < 768; n++)
		if (col = nueva_paleta[n])
			nueva_paleta[n] = col * 2 + 1;

	if (!cargar_paleta)
		Progress((char *)texto[497], num_colores * 4 - 256,
			 num_colores * 4 - 256);
}
...
void preparar_tapiz(void)
{
	...
	if (tempsurface != NULL) {
		...
		// default zoom to fit
		tempsurface =
		    zoomSurface(tapiz_surface, (float)vga_an / tapiz_surface->w,
				(float)vga_al / tapiz_surface->h, 1);
		...
		SDL_FillRect(tempsurface, NULL,
			     SDL_MapRGB(tempsurface->format, colors[c1].r,
					colors[c1].g, colors[c1].b));
	}
	...
	if (es_MAP(temp2))
		x = 1;
	else if (es_PCX(temp2))
		x = 2;
	else if (es_BMP(temp2))
		x = 3;
	else if (es_JPG(temp2, lon))
		x = 4;
	else
		x = 0;
	swap(map_an, tap_an);
	...
	if (!Setupfile.Desktop_Gama) { // Si el fichero se muestra a color
		...
		do
			*p = ctapiz[*p];
		while (++p < q); // Lo adapta a la paleta actual
	} else {
		for (x = 0, p = pal; x < 256; x++, p += 3)
			ctapiz[x] =
			    tapiz_gama[(*p + *(p + 1) + *(p + 2)) * 2 / 3];
		p = temp;
		q = p + tap_an * tap_al;
		do
			*p = ctapiz[*p];
	...
	if (Setupfile.Desktop_Tile) {
		...
		if ((p = (byte *)malloc(vga_an * vga_al)) == NULL) {
			free(temp);
			tapiz = NULL;
			return;
		}
...
void InterPal1(void)
{
	...
	wrectangulo(v.ptr, an, al, c4, (SelColor % 16) * 8 + 1,
		    (SelColor / 16) * 8 + 9, 9, 9);
	if (dac[SelColor * 3] * dac[SelColor * 3 + 1] * dac[SelColor * 3 + 2] <
	    (32 * 32 * 32))
		wbox(v.ptr, an, al, c4, (SelColor % 16) * 8 + 4,
		     (SelColor / 16) * 8 + 12, 3, 3);
	else
		wbox(v.ptr, an, al, c0, (SelColor % 16) * 8 + 4,
		     (SelColor / 16) * 8 + 12, 3, 3);
...
void InterPal2(void)
{
	...
	if ((wmouse_y > 10) && (wmouse_y < 138) && (wmouse_x > 2) &&
		...
		if (cColor != OldColor) {
			...
			wwrite(v.ptr, an, al, 140, 63 - 21, 2, (byte *)cWork,
			       c3);
			wwrite(v.ptr, an, al, 147, 63 - 14, 0, texto[144], c3);
			sprintf(cWork, "%02d", dac[cColor * 3 + 1]);
			wwrite(v.ptr, an, al, 140, 63 - 14, 2, (byte *)cWork,
			       c3);
			wwrite(v.ptr, an, al, 147, 63 - 7, 0, texto[145], c3);
			sprintf(cWork, "%02d", dac[cColor * 3 + 2]);
			wwrite(v.ptr, an, al, 140, 63 - 7, 2, (byte *)cWork,
			       c3);
			ax = ((wmouse_x - 2) / 8);
			ay = ((wmouse_y - 10) / 8);
		...
		if ((mouse_b) && (cColor != SelColor)) {
			...
			wbox(v.ptr, an, al, SelColor, (SelColor % 16) * 8 + 4,
			     (SelColor / 16) * 8 + 12, 3, 3);
			SelColor = cColor;
			if (dac[SelColor * 3] * dac[SelColor * 3 + 1] *
				dac[SelColor * 3 + 2] <
			    (32 * 32 * 32))
				wbox(v.ptr, an, al, c4, (SelColor % 16) * 8 + 4,
				     (SelColor / 16) * 8 + 12, 3, 3);
...
void InterPal0(void)
{
	...
	_flag(106, v.an / 2 - (8 * big2 + text_len(texto[106])) / 2, 143,
	      &Intercambiar);
	_flag(107, (v.an - 5) - (8 * big2 + text_len(texto[107])), 143,
	      &Copiar);
}
