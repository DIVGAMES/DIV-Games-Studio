...
void AplieX(tmapa *MiMap,int man,int mal);
...
void imp_fontmap(void) {
  ...
  if (*(int*)(buffer+1356+'�'*16)) gencode|=16;
...
// v_mapa->filename   Ŀ Estos dos primeros campos ser�n "Mapa x",NULL si
// v_mapa->path       �� el mapa no tiene una correspondencia a�n en disco
// v_mapa->map
// v_mapa->mab
// v_mapa->map_an/al
...
extern t_listboxbr ltexturasbr;
...
int  crear_mapbr_thumbs(t_listboxbr * l);
...
t_listbox lmapsizes={64,19,mapsizes,9,5,50};
...
int nuevo_mapa(byte * mapilla) {
  ...
  if ((v_mapa=(tmapa *)malloc(sizeof(tmapa)))!=NULL) {
