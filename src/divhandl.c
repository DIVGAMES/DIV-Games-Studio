...
void menu_programas2(void)
{
	...
	if (determina_prg())
		actualiza_menu(900, 1, 0);
	else
		actualiza_menu(900, 3, 99);
...
void menu_edicion2(void)
{
	...
	if (determina_prg())
		actualiza_menu(950, 1, 0);
	else
		actualiza_menu(950, 0, 99);
...
void save_prg_buffer(memptrsize n)
{ // Actualiza el programa, guarda la línea actual
	wup(n);
	write_line();
	read_line();
	wdown(n);
...
void menu_mapas2(void)
{
	...
	if ((old_mouse_b & 1) && !(mouse_b & 1)) {
		...
		case 4: // Cerrar todos los mapas (maximizados)
			v_titulo = (char *)texto[334];
			v_texto = (char *)texto[335];
			dialogo(aceptar0);
			if (v_aceptar) {
		...
		case 5: // Guardar mapa
			if (n = determina_mapa()) {
				if (strchr((const char *)ventana[n].titulo,
					   ' ') != NULL)
					goto no_tiene_nombre;
				if (ventana[n].mapa->path[0] == 0) {
					v_modo = 2;
					strcpy(input2,
					       (char *)ventana[n].titulo);
					goto casi_no_tiene_nombre;
				}
				mouse_graf = 3;
				volcado_copia();
				mouse_graf = 1;
				strcpy(tipo[v_tipo].path,
				       ventana[n].mapa->path);
				strcpy(input, ventana[n].mapa->filename);
				guardar_mapa();
			}
			break;
...
void menu_mapas3D2(void)
{
	...
	if (determina_mapa3d())
		actualiza_menu(1000, 1, 0);
	else
		actualiza_menu(1000, 3, 7);
...
int determina_pcm(void)
{
	...
	for (m = 1; m < max_windows; m++)
		if (ventana[m].tipo == 105 && ventana[m].estado) {
			n = m;
			break;
		}
	return (n);
...
int determina_mod(void)
{
	...
	for (m = 1; m < max_windows; m++)
		if (ventana[m].tipo == 107 && ventana[m].estado) {
			n = m;
			break;
		}
	return (n);
...
int determina_fnt(void)
{
	...
	for (m = 1; m < max_windows; m++)
		if (ventana[m].tipo == 104 && ventana[m].estado) {
			n = m;
			break;
		}
	return (n);
...
int determina_fpg(void)
{
	...
	for (m = 1; m < max_windows; m++)
		if (ventana[m].tipo == 101 && ventana[m].estado) {
			n = m;
			break;
		}
	return (n);
...
int determina_map(void)
{
	...
	for (m = 1; m < max_windows; m++)
		if (ventana[m].tipo == 100 && ventana[m].estado) {
			n = m;
			break;
		}
	return (n);
...
int determina_mapa(void)
{
	...
	if (n) {
		v_ventana = n;
		v_mapa = ventana[n].mapa;
		map = v_mapa->map;
		map_an = v_mapa->map_an;
		map_al = v_mapa->map_al;
	}
	return (n);
}
...
int determina_mapa3d(void)
{
	...
	for (m = 1; m < max_windows; m++)
		if (ventana[m].tipo == 106 && ventana[m].estado) {
			n = m;
			break;
		}
	if (n) {
...
void menu_graficos1(void) { pinta_menu(825); }
...
void menu_graficos2(void)
{
	...
	if (determina_fpg() && determina_map())
		actualiza_menu(825, 1, 0);
	else if (determina_fpg())
		actualiza_menu(825, 9, 9);
	else if (determina_map())
		actualiza_menu(825, 3, 8);
	else
		actualiza_menu(825, 3, 9);
	...
	if ((old_mouse_b & 1) && !(mouse_b & 1)) {
		...
		case 4: // fichero guardar como...
			if (n = determina_fpg()) {
				v_modo = 1;
				v_texto = (char *)texto[830];
				dialogo(browser0);
				if (v_terminado) {
					if (v_existe) {

						// Comprueba que el fichero no
						// sea el mismo
...
void menu_fuentes1(void) { pinta_menu(850); }
...
void menu_fuentes2(void)
{
	...
	if (determina_fnt()) {
		if (determina_map())
			actualiza_menu(850, 1, 0);
		else
			actualiza_menu(850, 6, 6);
	} else {
		if (determina_map())
			actualiza_menu(850, 3, 5);
		else
			actualiza_menu(850, 3, 6);
	}
	...
	if ((old_mouse_b & 1) && !(mouse_b & 1)) {
		...
		case 4:
			if (n = determina_fnt()) {
				FntAux =
				    (char *)(ventana[n].aux + RES_FOR_NAME);
				dialogo(GetText0);
				if (v_aceptar)
					CreateText();
			}
...
void imp_fontmap(void)
{
	...
	buffer_len = 1356 + 256 * 16 +
		     map_an * map_al; // Pide memoria suficiente para el FNT
	...
	for (x = 1; x < map_an - 1; x++) {
		for (y = 1; y < map_al - 1; y++) {
			if (mapa(x - 1, y - 1) == color &&
			    mapa(x - 1, y) == color &&
			    mapa(x, y - 1) == color && mapa(x, y) != color) {
				x0 = x;
				y0 = y;
			}
			if (mapa(x, y) != color &&
...
end_bucle:
	...
	for (x = -1, y = 1; y < max_windows; y++)
		if (ventana[y].tipo == 104 &&
		    !strcmp((char *)ventana[y].titulo, FontName))
			x = y;
...
void gen_fontmap(void)
{
	...
	for (n = 0; n < 256; n++) {
		if (*(p + n * 4) == 0 || *(p + n * 4 + 1) == 0)
			...
			map_an += *(p + n * 4) + 1;
			if (map_al < 2 + *(p + n * 4 + 1) + *(p + n * 4 + 2))
				map_al =
				    2 + *(p + n * 4 + 1) + *(p + n * 4 + 2);
			FntEnd = FntAux + *(p + n * 4 + 3) +
				 *(p + n * 4) * *(p + n * 4 + 1);
		}
	}
	...
	if (v_mapa != NULL && v_mapa->map != NULL) {
		...
		for (x = 1, n = 0; n < 256; n++) {
			gen_char(v_mapa->map + map_an + x, *(p + n * 4),
				 *(p + n * 4 + 1), *(p + n * 4 + 2),
				 FntAux + *(p + n * 4 + 3));
			if (*(p + n * 4) == 0 || *(p + n * 4 + 1) == 0)
				x += 2;
...
void menu_sonidos2(void)
{
	...
	if (determina_pcm())
		actualiza_menu(925, 1, 0);
	else
		actualiza_menu(925, 3, 6);
	...
	if ((old_mouse_b & 1) && !(mouse_b & 1)) {
		...
		case 1:
			...
			if (false /*judascfg_device == DEV_NOSOUND*/) {
				if (SoundError) {
					v_texto = (char *)texto[549];
					dialogo(errhlp0);
					if (v_aceptar)
						help(2008);
				} else {
					v_texto = (char *)texto[548];
					dialogo(errhlp0);
					if (v_aceptar)
						help(2009);
				}
...
{ // (Min,Max) Opciones prohibidas
	...
	if (n != v.estado) {
		...
		if (v.estado) {
			wbox(ptr, an, al, c2, 2, 1 + abs(v.estado) * 9, an - 4,
			     8);
			...
			if ((q = (byte *)strchr((const char *)p, '[')) !=
			    NULL) {
				*q = 0;
				wwrite(ptr, an, al, 3, 2 + abs(v.estado) * 9, 0,
				       p, c3);
				*q = '[';
				wwrite(ptr, an, al, an - 4,
				       2 + abs(v.estado) * 9, 2, q, c1);
				wwrite(ptr, an, al, an - 5,
				       2 + abs(v.estado) * 9, 2, q, c3);
			} else
				wwrite(ptr, an, al, 3, 2 + abs(v.estado) * 9, 0,
				       p, c3);
		}
		...
		if (n) {
			if (n < 0) {
				...
				if ((q = (byte *)strchr((const char *)p,
							'[')) != NULL) {
					*q = 0;
					wwrite(ptr, an, al, 4, 2 - n * 9, 0, p,
					       c1);
					wwrite(ptr, an, al, 3, 2 - n * 9, 0, p,
					       c4);
					*q = '[';
					wwrite(ptr, an, al, an - 4, 2 - n * 9,
					       2, q, c1);
					wwrite(ptr, an, al, an - 5, 2 - n * 9,
					       2, q, c3);
				} else {
					wwrite(ptr, an, al, 4, 2 - n * 9, 0, p,
					       c1);
...
void mapa0(void)
{
	...
	v.tipo =
	    100; // Mapa (ventanas de las que puede haber solo una activada)
	v.mapa = v_mapa;

	if ((v.an = v.mapa->map_an) < 48 * big2)
		v.an = (24 + 4) * big2;
	else {
		max = vga_an / 2 - big2;
		v.an = v.an / 2 + 4 * big2;
	...
	if (big) {
		...
		if (v.al & 1)
			v.al++;
		v.an = -v.an; // Para indicar que no se multiplique la ventana
			      // por 2
	}
...
void mapa1(void)
{
	...
	if (v.mapa->map_an < 48 * big2)
		di += (24 * big2 - v.mapa->map_an / 2) / 2;
	if (v.mapa->map_al < 16 * big2)
		di += ((8 * big2 - v.mapa->map_al / 2) / 2) * andi;
	...
	if ((an = v.mapa->map_an / 2) > maxan) {
		...
		if (x < 0)
			x = 0;
		else if (x + maxan * 2 > v.mapa->map_an)
			x = v.mapa->map_an - maxan * 2;
		si += x;
	}
	...
	if ((al = v.mapa->map_al / 2) > maxal) {
		...
		if (y < 0)
			y = 0;
		else if (y + maxal * 2 > v.mapa->map_al)
			y = v.mapa->map_al - maxal * 2;
		si += y * ansi;
	}
	...
	if (an && al)
		for (y = 0, y2 = 0; y < al; y++, y2 += 2)
			for (x = 0, x2 = 0; x < an; x++, x2 += 2)
				*(di + andi * y + x) = *(
				    ghost +
				    *(ghost + *(word *)(si + ansi * y2 + x2)) *
					256 +
				    *(ghost +
				      *(word *)(si + ansi * (y2 + 1) + x2)));
	else {
		if (!an)
			an++;
		if (!al)
...
void mapa2(void)
{
	...
	if ((mouse_b & 1)) {
		...
		// The following four 'if' are to avoid problems when you change
		// (video mode?)
		...
		if (text_len(v.titulo) + 3 > an - 20) {
			...
			wwrite(v.ptr, an, al, 2 + (an - 20) / 2, 3, 1, v.titulo,
			       c0);
		...
		fondo_resaltado = 0;
		actual_mouse = 21 + paint_cursor * 4;
		...
		TipoTex = 0;
		if ((FilePaintFPG = fopen(full, "rb")) !=
		    NULL) // NOTE !!! Could provide message here
		{
		...
		do {
			...
			case 90:       // Paste a graphic
				...
				if ((sp = (byte *)malloc(sp_an * sp_al)) !=
				    NULL) {
					memcpy(sp, ventana[1].mapa->map,
					       sp_an * sp_al);
...
void paleta1(void)
{
	...
	for (y = 0; y < 16; y++)
		for (x = 0; x < 16; x++)
			wbox(v.ptr, an, al, x + y * 16, 2 + x * pixelxcolor,
			     10 + y * pixelxcolor, pixelxcolor - 1,
			     pixelxcolor - 1);
}
...
void err0(void)
{
	...
	v.al = 38; // Ojo: error_window=malloc(640*38*2)
	if (v.an > 320)
		v.an = 320;
...
void TratarPaleta1(void)
{
	...
	if (OpcPal[0] || OpcPal[1]) {
		...
		wwrite(v.ptr, an, al, 4 + 33, 14, 1, texto[576], c0);
		c = c4;
	} else
		c = c3;
	...
	if (OpcPal[1] || OpcPal[2]) {
		...
		wwrite(v.ptr, an, al, xx + 33, 14, 1, texto[577], c0);
		c = c4;
	} else
		c = c3;
	...
	for (y = 0; y < 16; y++) {
		for (x = 0; x < 16; x++) {
			wbox(v.ptr, an, al, x + y * 16, 5 + x * 4, 23 + y * 4,
			     3, 3);
			c = fast_find_color(paltratar[(x + y * 16) * 3 + 0],
					    paltratar[(x + y * 16) * 3 + 1],
					    paltratar[(x + y * 16) * 3 + 2]);
			wbox(v.ptr, an, al, c, xx + 1 + x * 4, 23 + y * 4, 3,
			     3);
		}
	}
...
void TratarPaleta3(void)
{
	if (v_aceptar && OpcPal[1])
		v_aceptar = 2;
	if (v_aceptar && OpcPal[2])
		v_aceptar = 3;
}
...
void pinta_listbox(struct t_listbox *l)
{
	...
	if (n = l->maximo) { // Si hay elementos en la lista
		n -= l->inicial;
		p = l->lista + l->lista_an * l->inicial;
		y = l->y + 2;
		if (n > l->lista_al)
			n = l->lista_al;
		do {
			wwrite_in_box(ptr + (l->x + 2) * big2, an, l->an - 4,
				      al, 0, y, 0, (byte *)p, c3);
			p += l->lista_an;
			y += 8;
		} while (--n);
	...
	if (wmouse_in(l->x, l->y, l->an - 1, l->al)) { // Calcula zona
		...
		if (l->maximo <= l->zona || l->zona >= l->lista_al)
			l->zona = 1;
		else
			l->zona += 10;
	...
	if (l->zona >= 10) { // Marca zona
		wwrite_in_box(ptr + (l->x + 2) * big2, an, l->an - 4, al, 0,
			      l->y + 2 + (l->zona - 10) * 8, 0,
			      (byte *)l->lista +
...
void actualiza_listbox(struct t_listbox *l)
{
	...
	if (old_zona != l->zona)
		if (old_zona >= 10) { // Desmarca zona
			wwrite_in_box(ptr + (l->x + 2) * big2, an, l->an - 4,
				      al, 0, l->y + 2 + (old_zona - 10) * 8, 0,
				      (byte *)l->lista +
					  l->lista_an *
					      (l->inicial + old_zona - 10),
				      c3);
			v.volcar = 1;
		}
	// printf("zona: %d\n",l->zona);

	if ((mouse_b & 8 && l->zona > 0) ||
	    (l->zona == 2 && ((mouse_b & 1) || (v_pausa && !(mouse_b & 1) &&
						(old_mouse_b & 1))))) {
		if (!v_pausa ||
		    (v_pausa && !(mouse_b & 1) && (old_mouse_b & 1))) {
	...
	if ((mouse_b & 4 && l->zona > 0) ||
	    (l->zona == 3 && ((mouse_b & 1) || (v_pausa && !(mouse_b & 1) &&
						(old_mouse_b & 1))))) {
		if (!v_pausa ||
		    (v_pausa && !(mouse_b & 1) && (old_mouse_b & 1))) {
			if ((old_mouse_b & 1) && !v_pausa) {
	...
	if (l->zona == 4 && (mouse_b & 1)) {
		...
		if (l->maximo <= l->lista_al)
			...
			n = l->maximo - l->lista_al;
			n = (l->s0 * (n - l->inicial) + l->s1 * (l->inicial)) /
			    n;
		}
		if (n != l->slide) {
			l->slide = n;
...
void analizar_input(void)
{
	...
	// Remove blank name
	while (strchr(input, ' '))
		strcpy(strchr(input, ' '), strchr(input, ' ') + 1);
	...
	if (strchr(input, ':') != NULL)
		if (strchr(unidades, toupper(*(strchr(input, ':') - 1))) ==
		    NULL) {
			strcpy(input, mascara);
			return;
		}
	...
	if (_fullpath(full, input, _MAX_PATH) != NULL) {
		...
		if (true || (full[0] == drive[0])) {
			...
			if (!strlen(dir) || !chdir(dir)) {
...
#endif
				if (strlen(input)) {
					// LÓGICA DE TRATAMIENTO DEL NOMBRE DE
					// FICHERO

					// si tiene comodines
					//   si no se convierte en ningún
					//   fichero, lo toma como nueva máscara
					//   si se convierte en un fichero,
					//   expande su nombre a input
					//   (editando) si se convierte en más
					//   de uno, lo toma como nueva máscara
					// si no tiene comodines
					//   si el fichero no existe ... (segun
					//   modo abrir/guardar) si existe como
					//   un fichero ... (segun modo
					//   abrir/guardar) si existe como
					//   directorio, se mueve hacia ese
					//   directorio

					// (la otra diferencia entre
					// abrir/guardar es que al entrar en
					// guardar se está en status_abrir=1 e
					// input="")
					...
					if (strchr(input, '*') != NULL ||
					    ...
					    if (_dos_findfirst(input, _A_SUBDIR,
							       &fileinfo) == 0)
						...
						if (!_dos_findfirst(input,
							...
							v_terminado = 1;
							v_existe = 0;
						} // Fichero no encontrado (con
						  // extensión añadida)
					} else {
						v_terminado = 1;
						v_existe = 0;
					} // Fichero no encontrado (sin
					  // extensión añadida)
				} else {
					strcpy(input, mascara); // Mantiene la
								// máscara vieja
				}
				...
				_dos_setdrive(tipo[v_tipo].path[0] - 'A' + 1,
					      &n);
		}
	} else
		strcpy(input, mascara);
...
#define y_nm 67
char mapsizes[9 * 9] = "320x200\0 320x240\0 360x240\0 360x360\0 376x282\0 "
		       "640x400\0 640x480\0 800x600\0 1024x768";
	...
	if ((mouse_b & 1) && !(old_mouse_b & 1)) {
		if (lmapsizes.zona >= 10) {
			...
			*strchr(&mapsizes[n * 9], 'x') = 0;
			...
			map_al = atoi(
			    &mapsizes[n * 9 + strlen(&mapsizes[n * 9]) + 1]);
			mapsizes[n * 9 + strlen(&mapsizes[n * 9])] = 'x';
			itoa(map_an, mancho, 10);
			itoa(map_al, malto, 10);
			//      v.item[0].estado&=-3; v.item[1].estado&=-3; //
			//      Evitar "out of range"
			call((voidReturnType)v.paint_handler);
			actualiza_listbox(&lmapsizes);
			_select_new_item(2);
...
int nuevo_mapa(byte *mapilla)
{
	...
	if ((v_mapa = (struct tmapa *)malloc(sizeof(struct tmapa))) != NULL) {
		...
		if (mapilla != NULL)
			...
			v_mapa->map = (byte *)malloc(map_an * map_al);
...
void abrir_mapa(void)
{
	...
	if (sum && n == 1) {
		...
		sum = 0;
		do {
			sum += abs((memptrsize)paleta_original[x] -
				   (memptrsize)dac[x]);
		} while (++x < 768);
	...
	if (sum) {
		...
		case 3: // Activar la nueva paleta
			if (muestra == NULL)
				memcpy(pal, paleta_original, 768);
			memcpy(dac4, pal, 768);
			RefPalAndDlg(0, 1);
			break;
		}
			...
			if ((f = fopen(full, "rb")) !=
				...
				if ((buffer = (byte *)malloc(n)) != NULL) {
					...
					if (fread(buffer, 1, n, f) == n) {
						...
						if (es_MAP(buffer))
							tipomapa = 1;
						else if (es_PCX(buffer))
							tipomapa = 2;
						else if (es_BMP(buffer))
							tipomapa = 3;
						else if (es_JPG(buffer, n))
							tipomapa = 4;
						else
							tipomapa = 0;
...
void guardar_mapa(void)
{
	...
	if (strchr(input, ' ') == NULL) {
		if ((f = fopen(full, "wb")) != NULL) { // Se ha elegido uno
			...
			if (e == 2) {
				v_texto = (char *)texto[48];
				dialogo(err0);
			}

			// *** OJO, se debe borrar el fichero si no se pudo
			// grabar entero
	...
	if (!e) { // Debe cambiar la ruta y nombre del mapa/ventana guardado
		...
		if (text_len(ventana[v_ventana].titulo) + 3 > an - 20) {
			wwrite_in_box(ventana[v_ventana].ptr, an, an - 19, al,
				      4, 2, 0, ventana[v_ventana].titulo, c1);
			wwrite_in_box(ventana[v_ventana].ptr, an, an - 19, al,
				      3, 2, 0, ventana[v_ventana].titulo, c4);
		} else {
			wwrite(ventana[v_ventana].ptr, an, al,
			       3 + (an - 20) / 2, 2, 1,
			       ventana[v_ventana].titulo, c1);
			wwrite(ventana[v_ventana].ptr, an, al,
			       2 + (an - 20) / 2, 2, 1,
			       ventana[v_ventana].titulo, c4);
		}
		vuelca_ventana(v_ventana);
	}
}
...
void Reducex2()
{
	...
	if ((MiMap->map_an < 2) || (MiMap->map_al < 2))
		return;
	...
	for (fy = 0; fy < map_al; fy++) {
		...
		for (fx = 0; fx < map_an; fx++) {
			if (fx == (map_an - 1)) {
				p1 = MiMap->map[(memptrsize)y * man +
						(memptrsize)x - 1];
				p2 = MiMap->map[(memptrsize)y * man +
						(memptrsize)x];
			} else {
				p1 = MiMap->map[(memptrsize)y * man +
						(memptrsize)x];
			...
			if (fy == (map_al - 1)) {
				p3 = MiMap->map[(memptrsize)y * man +
						(memptrsize)x - man - 1];
				p4 = MiMap->map[(memptrsize)y * man +
						(memptrsize)x - man];
			} else {
				p3 = MiMap->map[(memptrsize)y * man +
						(memptrsize)x + man];
	...
	if (fx < 0)
		fx = 0;
	else if (fx + vga_an > v.mapa->map_an)
		fx = v.mapa->map_an - vga_an;
	fy = (v.mapa->zoom_cy - vga_al / 2);
	if (fy < 0)
		fy = 0;
	else if (fy + vga_al > v.mapa->map_al)
...
void mapa_busqueda()
{
	...
	man = map_an;
	mal = map_al;
	ma = map; // Mapa original ma(man,mal)
	...
	if (map_an > max_map_size)
		map_an = max_map_size;
	if (map_al > max_map_size)
		map_al = max_map_size;
...
void Progress1()
{
	...
	wbox(ptr, an, al, c_r_low, 4, 12,
	     ProgressCurrent * (an - 8) / ProgressTotal, al - 16);
	wrectangulo(ptr, an, al, c0, 4, 12, an - 8, al - 16);
	sprintf((char *)cwork, "%d%c", ProgressCurrent * 100 / ProgressTotal,
		'%');
	wwrite(ptr, an, al, 4 + (an - 8) / 2, (14 + (al - 16) / 2) - 2, 4,
	       (byte *)cwork, c3);
}
...
{
	if (!ProgressInit) { // Inicializa la ventan de progreso
		mouse_graf = 3;
		volcado_copia();
		ProgressInit = 1;
		...
		ProgressCurrent = current;
		if (ProgressCurrent >=
		    ProgressTotal) { // Finaliza la ventan de progreso
			mouse_graf = 1;
			ProgressInit = 0;
			cierra_ventana();
			volcado_copia();
			return 0;
		} else { // Bucle de la ventan de progreso
			v = ventana[0];
			Progress2();
			vuelca_ventana(0);
			volcado_copia();
	...
	for (y = 0; y < map_al; y++) {
		...
		for (x = 0; x < map_an; x++) {
			...
			if (fy > ry)
				...
				pp2 += (ry - fy);
			pp2 = (2.0 - pp2) / 2.0;
			p2 = MiMap->map[(memptrsize)ry * man +
					(memptrsize)(rx + 1.0)];
			r += dac[p2 * 3] * pp2;
			g += dac[p2 * 3 + 1] * pp2;
			b += dac[p2 * 3 + 2] * pp2;
			...
			if (fy > ry + 1.0)
				...
				pp3 += ((ry + 1.0) - fy);
			pp3 = (2.0 - pp3) / 2.0;
			p3 = MiMap->map[(memptrsize)(ry + 1.0) * man +
					(memptrsize)rx];
			r += dac[p3 * 3] * pp3;
			g += dac[p3 * 3 + 1] * pp3;
			b += dac[p3 * 3 + 2] * pp3;
			...
			if (fy > ry + 1.0)
				...
				pp4 += ((ry + 1.0) - fy);
			pp4 = (2.0 - pp4) / 2.0;
			p4 = MiMap->map[(memptrsize)(ry + 1.0) * man +
					(memptrsize)(rx + 1.0)];
			r += dac[p4 * 3] * pp4;
			g += dac[p4 * 3 + 1] * pp4;
			b += dac[p4 * 3 + 2] * pp4;
			...
			if (Gris) {
				if (r + g + b > 189)
					...
					bufferaux[y * map_an + x] =
					    Tabla[r + g + b];
			} else
				bufferaux[y * map_an + x] =
				    rgb_table[(r >> 1) * 1024 + (g >> 1) * 32 +
					      (b >> 1)];
