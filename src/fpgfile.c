...
int Abrir_FPG(FPG *Fpg, char *Name)
{
	...
	while (ReadHead(&kkhead, fpg)) {
		...
		sprintf((char *)Fpg->CodDes[Fpg->nIndex++], "%c %03d %s", 255,
...
int Anadir_FPG(FPG *Fpg, int COD, char *tDescrip, char *tFilename, int Ancho,
	       int Alto, int nPuntos, char *Puntos, char *Imagen,
	       int BorrarAntiguo, int get_info)
{
	...
	if (BorrarAntiguo)
		Borrar_FPG(Fpg, OLDCOD); // Borra el antiguo

	if (Fpg->OffsGrf[COD] != 0)
		Borrar_FPG(Fpg, COD); // Si se machaca uno
	...
	memmove(&(Fpg->thumb[n + 1]), &(Fpg->thumb[n]),
		sizeof(t_thumb) * (999 - n));
	...
	while (Fpg->lInfoFPG.inicial +
		   (Fpg->lInfoFPG.lineas - 1) * Fpg->lInfoFPG.columnas + 1 >
	       Fpg->lInfoFPG.maximo) {
		Fpg->lInfoFPG.inicial -= Fpg->lInfoFPG.columnas;
	}

	if (Fpg->lInfoFPG.inicial < 0)
		Fpg->lInfoFPG.inicial = 0;
...
int RemapAllFileToPal(FPG *Fpg)
{
	...
	for (x = 0; x < 256; x++)
		MiTabla[x] =
		    find_color(tmp[x * 3], tmp[x * 3 + 1], tmp[x * 3 + 2]);
	...
	while (ReadHead(&MiOtraHeadFPG, fpg)) {
		OtraImagen =
		    (char *)malloc(MiOtraHeadFPG.Ancho * MiOtraHeadFPG.Alto);
		if (OtraImagen == NULL) {
			fclose(fpg);
			fclose(Oldfpg);
			v_texto = (char *)texto[45];
		...
		if (MiOtraHeadFPG.nPuntos != 0) {
			OtrosPuntos =
			    (short *)malloc(MiOtraHeadFPG.nPuntos * 4);
			if (OtrosPuntos == NULL) {
				fclose(fpg);
				fclose(Oldfpg);
				free(OtraImagen);
		...
		fread(OtraImagen, MiOtraHeadFPG.Ancho * MiOtraHeadFPG.Alto, 1,
		      fpg);
...
void SaveFPG(int n)
{
	...
	if (text_len(ventana[n].titulo) + 3 > an - 20) {
		wwrite_in_box(ventana[n].ptr, an, an - 19, al, 4, 2, 0,
			      ventana[n].titulo, c1);
		wwrite_in_box(ventana[n].ptr, an, an - 19, al, 3, 2, 0,
			      ventana[n].titulo, c4);
	} else {
		wwrite(ventana[n].ptr, an, al, 3 + (an - 20) / 2, 2, 1,
		       ventana[n].titulo, c1);
		wwrite(ventana[n].ptr, an, al, 2 + (an - 20) / 2, 2, 1,
		       ventana[n].titulo, c4);
	}
	vuelca_ventana(n);
...
int Borrar_FPG(
    FPG *Fpg,
    int COD) // Papelera, machacar uno existente y al cambiar el código a uno
{
	struct tipo_regla CopiaReglas[16];
	HeadFPG MiOtraHeadFPG;
	char ActualPath[_MAX_PATH + 14];
	...
	memmove(&(Fpg->thumb[n]), &(Fpg->thumb[n + 1]),
		sizeof(t_thumb) * (999 - n));
	...
	while (ReadHead(&MiOtraHeadFPG, fpg)) {
		...
		if (MiOtraHeadFPG.COD != COD) {
			WriteHead(&MiOtraHeadFPG, OtrosPuntos, OtraImagen,
				  Oldfpg);
		}
...
int Borrar_muchos_FPG(FPG *Fpg, int taggeds, int *array_del)
{
	...
	strcpy(ActualPath,
	       (char *)Fpg->ActualFile); // Nombre del fichero temporal
	for (x = strlen(ActualPath); x >= 0; x--)
		if (ActualPath[x] == '/')
			x = -1;
		else
	...
	while (ReadHead(&MiOtraHeadFPG, fpg)) {
		...
		if (y == taggeds) {
			...
			n = 0;
			while (1) {
				if (Fpg->DesIndex[n] == MiOtraHeadFPG.COD ||
				    Fpg->DesIndex[n] == 0)
					break;
				n++;
			}
			...
			memmove(&(Fpg->DesIndex[n]), &(Fpg->DesIndex[n + 1]),
				sizeof(int) * (999 - n));
		}
