...
void Vid_Setup1(void)
{
	...
	wwrite_in_box(v.ptr, v.an / big2, v.an / big2 - 3, v.al / big2, 5, 12,
		      0, (byte *)marcavga, c1);
	wwrite_in_box(v.ptr, v.an / big2, v.an / big2 - 3, v.al / big2, 4, 12,
		      0, (byte *)marcavga, c3);

	wwrite(v.ptr, v.an / big2, v.al / big2, 5, 22, 0, texto[177], c1);
	wwrite(v.ptr, v.an / big2, v.al / big2, 4, 22, 0, texto[177], c3);
	wwrite(v.ptr, v.an / big2, v.al / big2, 5 + 30, 22, 0, (byte *)cWork,
	       c1);
	wwrite(v.ptr, v.an / big2, v.al / big2, 4 + 30, 22, 0, (byte *)cWork,
	       c4);
}
...
void Vid_Setup2(void)
{
	...
	if ((lvgasizes.zona >= 10) && (mouse_b & 1)) {
		stvga_an =
		    modos[(lvgasizes.zona - 10) + lvgasizes.inicial].ancho;
		stvga_al =
		    modos[(lvgasizes.zona - 10) + lvgasizes.inicial].alto;
		need_refresh = 1;
	}
...
{
	if (v_aceptar)
		if (VS_ANCHO != stvga_an || VS_ALTO != stvga_al ||
		    VS_BIG != stbig) {
			VS_BIG = stbig;
			VS_ANCHO = stvga_an;
			VS_ALTO = stvga_al;
...
void Vid_Setup0(void)
{
	...
	for (n = 0; n < num_modos; n++) {
		...
		sprintf(&vgasizes[n * 16], "%s%d x %d",
			(modos[n].modo) ? "SDL " : "VGA ", modos[n].ancho,
			modos[n].alto);
	}
...
void Tap_Setup1(void)
{
	...
	for (x = 0; x < 32; x++)
		wbox(v.ptr, an, al, tapiz_gama[x * 4], an - 37 + x, 24 + 9, 1,
		     5);
}
...
typedef struct _meminfo {
	...
	unsigned Numero_de_paginas_libres_disponibles; // Number of free pages
						       // available
	unsigned
	    Numero_de_paginas_fisicas_libres; // Number of free physical pages
	unsigned Total_de_paginas_fisicas;    // Total physical pages
	unsigned Espacio_de_direccionamiento_lineal_libre; // Free linear
							   // address space
...
int Mem_GetHeapFree()
{
	...
	// file: do something appropriate like return an error code, throw an
	// exception, etc.
	fclose(meminfo);
	return -1;
#endif
...
void MemInfo1(void)
{
	...
	wwrite_in_box(v.ptr + 2, an, an - 4, al, (an - 4) / 2 + 1, 24, 1,
		      (byte *)user1, c0);
	wwrite_in_box(v.ptr + 2, an, an - 4, al, (an - 4) / 2, 24, 1,
		      (byte *)user1, c3);
	wwrite_in_box(v.ptr + 2, an, an - 4, al, (an - 4) / 2 + 1, 32, 1,
		      (byte *)user2, c0);
	wwrite_in_box(v.ptr + 2, an, an - 4, al, (an - 4) / 2, 32, 1,
		      (byte *)user2, c3);
	...
	for (x = 0; x < max_windows; x++) {
		...
		if (ventana[x].tipo == 100) {
			meminmaps +=
			    ventana[x].mapa->map_an * ventana[x].mapa->map_al;
		}
	}
	if ((mem = Mem_GetHeapFree()) == -1)
		strcpy(cWork, (char *)texto[193]);
	else {
		...
		//	fmem=fmem*1000;
		//    if(fmem/1000) sprintf(cWork,(char
		//    *)texto[194],fmem,sizes[csize]);
		// else
	...
	mem = meminmaps / 1024;
	if (mem / 1000)
		sprintf(cWork, (char *)texto[196], mem / 1000, mem % 1000,
			"KB");
	else
		sprintf(cWork, (char *)texto[197], mem % 1000, "KB");
...
void cfg_show_cursor(void)
{
	...
	if (big) {
		switch (paint_cursor) {
		case 0:
			wput(v.ptr, v.an, v.al, (92 + 3 + 30) * 2,
			     (69 + 3 + 30) * 2, 19);
			break;
		case 1:
			wput(v.ptr, v.an, v.al, (92 + 2 + 30) * 2,
			     (69 + 2 + 30) * 2, 22);
			break;
		case 2:
			wput(v.ptr, v.an, v.al, (92 + 2 + 30) * 2,
...
void cfg_show_font(void)
{
	...
	switch (editor_font) {
	case 0:
		wwrite(v.ptr, an, al, 101 + 18 + 50, 56 + 30, 1, (byte *)"6x8",
		       c3);
		break;
	case 1:
		wwrite(v.ptr, an, al, 101 + 18 + 50, 56 + 30, 1, (byte *)"8x8",
		       c3);
		break;
	case 2:
		wwrite(v.ptr, an, al, 101 + 18 + 50, 56 + 30, 1, (byte *)"8x11",
		       c3);
		break;
	case 3:
		wwrite(v.ptr, an, al, 101 + 18 + 50, 56 + 30, 1, (byte *)"9x16",
		       c3);
		break;
	}
...
void Cfg_Setup_end(void)
{
	...
	for (i = 0, n = 0; n < max_windows; n++) {
		if (ventana[n].click_handler == (voidReturnType)programa2)
			i++;
	}
	...
	if (v_aceptar && (color_cfg[0] != c2 || color_cfg[1] != c4 ||
			  color_cfg[2] != c_b_low || color_cfg[3] != ce1 ||
			  color_cfg[4] != ce4 || color_cfg[5] != c_y ||
			  color_cfg[6] != c_com || color_cfg[7] != c_sim ||
			  color_cfg[8] != c_res || color_cfg[9] != c_pre ||
			  color_cfg[10] != c_num || color_cfg[11] != c_lit ||
			  old_editor_font != editor_font ||
			  coloreador != setup_switches[3])) {
		...
		if (old_editor_font != editor_font) {
			for (n = 0; n < max_windows; n++)
				if (ventana[n].click_handler ==
				    (voidReturnType)help2) {
					move(0, n);
					cierra_ventana();
					help_item++;
					break;
			...
			do {
				...
				for (n = max_windows - 1; n >= 0; n--)
					if (ventana[n].click_handler ==
					    (voidReturnType)programa2) {
						memcpy(&vp[i].tipo,
						       &ventana[n].tipo,
						       sizeof(struct tventana));
						i++;
						ventana[n].close_handler =
						    (voidReturnType)
							dummy_handler;
						move(0, n);
						cierra_ventana();
		...
		for (n = 0; n < max_windows; n++)
			if (ventana[n].tipo) {
				...
				if (v.tipo >= 100 && !v.estado) {
					wgra(v.ptr, an, al, c1, 2, 2, an - 20,
					     7);
					if (text_len(v.titulo) + 3 > an - 20) {
						wwrite_in_box(v.ptr, an,
							      an - 19, al, 4, 2,
							      0, v.titulo, c0);
						wwrite_in_box(v.ptr, an,
							      an - 19, al, 3, 2,
					...
					wgra(ptr, an, al, c_b_low, 2, 2,
					     an - 20, 7);
					if (text_len(v.titulo) + 3 > an - 20) {
						wwrite_in_box(ptr, an, an - 19,
							      al, 4, 2, 0,
							      v.titulo, c1);
						wwrite_in_box(ptr, an, an - 19,
							      al, 3, 2, 0,
		...
		if (old_editor_font != editor_font) {
			for (n = 0; n < i; n++) {
				memcpy(&ventana_aux.tipo, &vp[n].tipo,
				       sizeof(struct tventana));
				v_prg = ventana_aux.prg;
				VidModeChanged = 33;
				nueva_ventana_carga(
				    (voidReturnType)resize_program,
				    ventana_aux.x, ventana_aux.y);
			}
			if (help_item)
				help(help_item - 1);
		}

		no_volcar_ventanas = 0;
		cierra_rapido = 0;
...
void resize_program(void)
{
	...
	if (v.an > vga_an) {
		v.prg->an = (vga_an - 12 * big2) /
			    font_an; // Calcula tamaño (en chr) maximizada
...
void tapiz_thumb(void)
{
	...
	tempsurface = zoomSurface(tapiz_temp_surface, (float)an / vga_an,
				  (float)al / vga_al, 0);
	...
	for (y = 0; y < al; y++) {
		...
		for (x = 0; x < an; x++) {
			ptr[y * an + x] =
			    x_mapa_tapiz[((int)a) * x_tapiz_an + (int)b];
			b += coefredx;
		}
		a += coefredy;
	}
...
void preparar_tapiz_temp(void)
{
		...
		if (Tap_mosaico) {
			...
			while (x < vga_an) {
				...
				while (y < vga_al) {
					...
					SDL_BlitSurface(tapiz_temp_surface,
							NULL, tempsurface, &rc);
					y += rc.h;
				}
				x += rc.w;
			...
			tempsurface = zoomSurface(
			    tapiz_temp_surface,
			    (float)vga_an / tapiz_temp_surface->w,
			    (float)vga_al / tapiz_temp_surface->h, 1);
		}
		SDL_FreeSurface(tapiz_temp_surface);
	...
	if ((x_tapiz = (byte *)malloc(lon)) == NULL) {
		fclose(f);
		return;
	}
	...
	if (es_MAP(x_tapiz))
		x = 1;
	else if (es_PCX(x_tapiz))
		x = 2;
	else if (es_BMP(x_tapiz))
		x = 3;
	else if (es_JPG(x_tapiz, lon))
		x = 4;
	else
		x = 0;
	an = map_an;
	...
	switch (x) {
	case 1:
		descomprime_MAP(x_tapiz, temp, 0);
		break;
	case 2:
		descomprime_PCX(x_tapiz, temp, 0);
		break;
	case 3:
		descomprime_BMP(x_tapiz, temp, 0);
		break;
	case 4:
		n = descomprime_JPG(x_tapiz, temp, 0, lon);
		break;
	}
	swap(map_an, tap_an);
	swap(map_al, tap_al);
