...
extern int backto[64]; // Cola circular para almacenar los topicos consultados
		       // {n,línea}
extern int i_back, a_back,
    f_back; // Inicio y final de la cola circular (ambos 0,2,..62)
...
void New_DownLoad_Desktop() {
  ...
  if ((desktop=fopen("desk\\window.000","rb+"))==NULL) {
DaniDel("desk\\window.000"); return; } fseek(desktop,4+4,SEEK_SET);
...
void DownLoad_Desktop()
{
	...
	iWork = Setupfile.Vid_modeAlto + Setupfile.Vid_modeAncho * 10000 +
		(Setupfile.Vid_modeBig << 31);
	n = fwrite(&iWork, 1, 4, desktop);
#ifdef SLST
	...
	for (x = max_windows - 1; x >= 0; x--) {
		if (ventana[x].tipo != 0 && ventana[x].titulo) {
			numvent++;
			n = fwrite(&ventana[x], 1, sizeof(struct tventana),
				   desktop);
#ifdef SLST
			fprintf(lst, "v %d elementos escritos <<<\n", n);
			if (ventana[x].tipo < 107)
				fprintf(lst, "Ventana %d \"%s\" (tipo %d)\n", x,
					ventana[x].titulo, ventana[x].tipo);

			fprintf(lst, "  primer_plano=%d, (%d,%d,%d,%d)\n",
				ventana[x].primer_plano, ventana[x].x,
				ventana[x].y, ventana[x].an, ventana[x].al);
#endif
			...
			case 4: // timer
				...
				fprintf(
				    lst,
				    "cdivorclock %d elementos escritos <<<\n",
				    n);
			...
			case 100: // map
				...
				man = ventana[x].mapa->map_an;
				mal = ventana[x].mapa->map_al;
				n = fwrite(ventana[x].mapa, 1,
					   sizeof(struct tmapa), desktop);

				// fprintf(lst,"tmapa %d elementos escritos
				// <<<\n",n); Grafico
				n = fwrite((char *)ventana[x].mapa->map,
					   man * mal, 1, desktop);
			...
			case 101: // fpg
				// estructura fpg
				n = fwrite(ventana[x].aux, 1, sizeof(FPG),
					   desktop);
			...
			case 102: // prg
				if (ventana[x].prg != NULL) {
					iWork = 0;
					n = fwrite(&iWork, 1, 4, desktop);
					// fprintf(lst,"tipoprg %d elementos
					// escritos <<<\n",n);
					n = fwrite(ventana[x].prg, 1,
						   sizeof(struct tprg),
						   desktop);
					// fprintf(lst,"tprg %d elementos
					// escritos <<<\n",n);
					n = fwrite(ventana[x].prg->buffer, 1,
						   ventana[x].prg->buffer_lon,
						   desktop);
					// fprintf(lst,"buffer %d elementos
					// escritos <<<\n",n);
					iWork =
					    (memptrsize)ventana[x].prg->lptr -
					    (memptrsize)ventana[x].prg->buffer;
					n = fwrite(&iWork, 1, 4, desktop);
					// fprintf(lst,"offset lptr %d elementos
					// escritos <<<\n",n);
					iWork =
					    (memptrsize)ventana[x].prg->vptr -
					    (memptrsize)ventana[x].prg->buffer;
					n = fwrite(&iWork, 1, 4, desktop);
					// fprintf(lst,"offset vptr %d elementos
					// escritos <<<\n",n); fprintf(lst,"
					// programa\n");
				} else {
					// if(ventana[x].aux!=NULL)
					if (!strcmp((char *)ventana[x].nombre,
						    (char *)texto[83])) {
						iWork = 1;
						n = fwrite(&iWork, 1, 4,
							   desktop);
						// fprintf(lst,"tipo fontgen %d
						// elementos escritos <<<\n",n);
						iWork = Save_Font_session(
						    desktop, iWork);
						// fprintf(lst,"  font
						// generator\n");
					} else if (!strcmp(
						...
						n = fwrite(&iWork, 1, 4,
							   desktop);
						strcpy(
						    ((struct _calc *)(ventana[x]
									  .aux))
							->ctext,
						    ((struct _calc *)(ventana[x]
									  .aux))
							->cget);
						n = fwrite(ventana[x].aux, 1,
							   sizeof(struct _calc),
							   desktop);
					} else {
						iWork = 2;
						n = fwrite(&iWork, 1, 4,
							   desktop);
						// fprintf(lst,"tipo help %d
						// elementos escritos <<<\n",n);
						n = fwrite(&help_item, 1, 4,
							   desktop);
						// fprintf(lst,"help_item %d
						// elementos escritos <<<\n",n);
						n = fwrite(&help_al, 1, 4,
							   desktop);
						// fprintf(lst,"help_al %d
						// elementos escritos <<<\n",n);
						n = fwrite(&help_l, 1, 4,
							   desktop);
						// fprintf(lst,"help_l %d
						// elementos escritos <<<\n",n);
						// fprintf(lst,"  help\n");
					}
				}
			...
			case 104: // fnt
				...
				// <<<\n",n);
				n = fwrite(ventana[x].aux + 14, 1,
					   _MAX_PATH - 14, desktop);
...
*/
				/*
				n=fwrite(mypcminfo, 1, sizeof(pcminfo),
				desktop); n=fwrite(mypcminfo->SoundData, 2,
				mypcminfo->SoundSize, desktop);
*/
			...
			case 107: // mod
				...
				n = fwrite(mymodinfo->pathname, 1, 256,
					   desktop);
				break;
			}
		}
...
int Can_UpLoad_Desktop()
{
	...
	if (iWork != Setupfile.Vid_modeAlto + Setupfile.Vid_modeAncho * 10000 +
			 (Setupfile.Vid_modeBig << 31)) {
		modo_anterior = iWork;
		VidModeChanged = 1;
	}
...
int UpLoad_Desktop()
{
	...
	for (x = 0; x < numvent; x++) {
		...
		case 2: // menu
			...
			switch (iWork) {
			case 0:
				nueva_ventana_carga(menu_principal0,
						    ventana_aux.x,
						    ventana_aux.y);
				break;
			case 1:
				nueva_ventana_carga(menu_programas0,
						    ventana_aux.x,
						    ventana_aux.y);
				break;
			case 2:
				nueva_ventana_carga(menu_paletas0,
						    ventana_aux.x,
						    ventana_aux.y);
				break;
			case 3:
				nueva_ventana_carga(menu_mapas0, ventana_aux.x,
						    ventana_aux.y);
				break;
			case 4:
				nueva_ventana_carga(menu_graficos0,
						    ventana_aux.x,
						    ventana_aux.y);
				break;
			case 5:
				nueva_ventana_carga(menu_fuentes0,
						    ventana_aux.x,
						    ventana_aux.y);
				break;
			case 6:
				nueva_ventana_carga(menu_sonidos0,
						    ventana_aux.x,
						    ventana_aux.y);
				break;
			case 7:
				nueva_ventana_carga(menu_sistema0,
						    ventana_aux.x,
						    ventana_aux.y);
				break;
			case 8:
				nueva_ventana_carga(menu_edicion0,
						    ventana_aux.x,
						    ventana_aux.y);
				break;
			case 9:
				nueva_ventana_carga(menu_mapas3D0,
						    ventana_aux.x,
						    ventana_aux.y);
				break;
			default:
			...
			break;
		case 3:
			nueva_ventana_carga(paleta0, ventana_aux.x,
					    ventana_aux.y);
			if (!Interpretando)
				actualiza_caja(0, 0, vga_an, vga_al);
			break;
		case 4: // timer
			...
			case 0: // CD
				...
				if (get_cd_error() & 0x200)
					...
					CDPlaying = 0;
				nueva_ventana_carga(CDiv0, ventana_aux.x,
						    ventana_aux.y);
				break;
			case 1: // Reloj
				nueva_ventana_carga(Clock0, ventana_aux.x,
						    ventana_aux.y);
				break;
			}
			if (!Interpretando)
		...
		case 5: // papelera
			...
			break;
		case 8: // mixer
			nueva_ventana_carga(mixer0, ventana_aux.x,
					    ventana_aux.y);
			if (!Interpretando)
				actualiza_caja(0, 0, vga_an, vga_al);
			break;
		case 100: // map
			...
			if (baux == NULL) {
				fseek(desktop, maux.map_an * maux.map_al,
				      SEEK_CUR);
				if (!Interpretando)
					actualiza_caja(0, 0, vga_an, vga_al);
				break;
			...
			map_an = maux.map_an;
			map_al = maux.map_al;
			if (nuevo_mapa_carga(ventana_aux.x, ventana_aux.y,
					     maux.filename, (byte *)baux)) {
				free(baux);
				if (!Interpretando)
					actualiza_caja(0, 0, vga_an, vga_al);
				break;
			...
			v.mapa->Codigo = maux.Codigo;
			memcpy((char *)v.mapa->path, (char *)maux.path,
			       _MAX_PATH + 1);
			memcpy((char *)v.mapa->filename, (char *)maux.filename,
			       255);
			...
			memcpy((char *)v.mapa->descripcion,
			       (char *)maux.descripcion, 32);
			memcpy((char *)v.mapa->puntos, (char *)maux.puntos,
			       512 * 2);
			// Grafico
		...
		case 101: // fpg
			...
			if ((f = fopen(full, "rb")) != NULL) {
				...
				if (v_aux != NULL) {
					...
					nueva_ventana_carga(FPG0A,
							    ventana_aux.x,
							    ventana_aux.y);
					wvolcado(copia, vga_an, vga_al, v.ptr,
						 v.x, v.y, v.an, v.al, 0);
					if (!Interpretando)
						actualiza_caja(0, 0, vga_an,
							       vga_al);
				}
			}
			break;
		case 102: // prg
			...
			if (iWork == 0) {
				if ((v_prg = (struct tprg *)malloc(
					...
					fread(v_prg, 1, sizeof(struct tprg),
					      desktop);
					v_prg->buffer =
					    (byte *)malloc(v_prg->buffer_lon);
					fread(v_prg->buffer, 1,
					      v_prg->buffer_lon, desktop);
					...
					nueva_ventana_carga(carga_programa0,
							    ventana_aux.x,
							    ventana_aux.y);
				...
				if (iWork == 1)
					nueva_ventana_carga(carga_Fonts0,
							    ventana_aux.x,
							    ventana_aux.y);
				else {
					if (iWork == 2) {
						...
						carga_help(iWork, iWork2,
							   iWork3,
							   ventana_aux.x,
							   ventana_aux.y);
					} else {
						if (iWork == 3) {
							readcalc = (struct _calc
									*)
							    malloc(sizeof(
								struct _calc));
							pcalc = NULL;
		...
		case 104: // fnt
			...
			if ((f = fopen(Load_FontPathName, "rb")) != NULL) {
				fclose(f);
				nueva_ventana_carga(ShowFont0, ventana_aux.x,
						    ventana_aux.y);
...
*/
		...
		//───────────────────────────────────────────────────────────────────────────
		// Los siguientes valores los debe definir init_handler, valores
		// por defecto:
		//───────────────────────────────────────────────────────────────────────────
...
*/
		if ((ptr = (byte *)malloc(an * al)) !=
		    NULL) { // Ventana, free en cierra_ventana
...
*/
			...
			if (text_len(v.titulo) + 3 > an - 20) {
				wwrite_in_box(ptr, an, an - 19, al, 4, 2, 0,
					      v.titulo, c1);
				wwrite_in_box(ptr, an, an - 19, al, 3, 2, 0,
					      v.titulo, c4);
			} else {
				wwrite(ptr, an, al, 3 + (an - 20) / 2, 2, 1,
				       v.titulo, c1);
				wwrite(ptr, an, al, 2 + (an - 20) / 2, 2, 1,
				       v.titulo, c4);
			}
...
*/
			...
			if (v.tipo >= 100 && v.estado == 0) {
				...
				if (text_len(v.titulo) + 3 > an - 20) {
					wwrite_in_box(v.ptr, an, an - 19, al, 4,
						      2, 0, v.titulo, c0);
					wwrite_in_box(v.ptr, an, an - 19, al, 3,
						      2, 0, v.titulo, c2);
				} else {
					wwrite(v.ptr, an, al, 2 + (an - 20) / 2,
					       3, 1, v.titulo, c0);
					wwrite(v.ptr, an, al, 2 + (an - 20) / 2,
					       2, 1, v.titulo, c2);
				}
			...
			if (v.primer_plano != 2) {
				if (v.primer_plano == 1)
					wvolcado(copia, vga_an, vga_al, ptr, x,
						 y, an, al, 0);
				else
					wvolcado_oscuro(copia, vga_an, vga_al,
							ptr, x, y, an, al, 0);
...
void carga_Fonts0(void) { Load_Font_session(desktop); }
...
void carga_help(int n, int helpal, int helpline, int x1, int x2)
{
	...
	if (helpidx[n * 2] && helpidx[n * 2 + 1]) {
		if ((f = fopen("help/help.div", "rb")) != NULL) {
			...
			if ((h_buffer = (byte *)malloc(helpidx[n * 2 + 1])) !=
			    NULL) {
				if ((help_buffer = (byte *)malloc(
					 helpidx[n * 2 + 1] + 1024)) != NULL) {
					if (i_back != a_back) {
						m_back = a_back - 2;
						if (m_back < 0)
							m_back = 62;
					...
					if (i_back == f_back)
						i_back = (i_back + 2) % 64;
					fread(h_buffer, 1, helpidx[n * 2 + 1],
					      f);
					p = h_buffer;
					while (*p != '}')
						p++;
					*p = 0;
					strcpy((char *)help_title,
					       (char *)h_buffer);
					help_an =
					    (vga_an - 12 * big2 - 1) / font_an;
					if (help_an > 120)
						help_an = 120;
					help_al = (vga_al / 2 - 12 * big2 - 1) /
						  font_al;
					help_al = helpal;

					help_l = 0;
					tabula_help(p + 1, help_buffer,
						    helpidx[n * 2 + 1] -
							(p + 1 - h_buffer));
