...
void sp_normal_mask(byte *p, int x, int y, int an, int al, int xg, int yg,
		    int flags);

void sp_scanc(byte *p, short n, short m, short o, byte *si, int an, int x0,
	      int y0, int x1, int y1, int flags);
...
void sp_scanc_mask(byte *p, short n, short m, short o, byte *si, int an, int x0,
		   int y0, int x1, int y1, int flags);

void sp_scan_mask(byte *p, short n, byte *si, int an, int x0, int y0, int x1,
		  int y1, int flags);
	       ...
	       int flags, float a, float s)
{
	...
	do {
		...
		if (h < map_al && h >= 0 && x0.w[1] < map_an && x1.w[1] >= 0 &&
		    x1.w[1] > x0.w[1]) {
			...
			if (!mask_on) {
				if (x0.w[1] < 0)
					...
					sp_scanc(
					    ptrmap + x0.w[1], x1.w[1] - x0.w[1],
					    map_an - 1 - x0.w[1], 0, si, an,
					    g0x.l, g0y.l, g1x.l, g1y.l, flags);
				else
					sp_scan(ptrmap + x0.w[1],
						x1.w[1] - x0.w[1], si, an,
