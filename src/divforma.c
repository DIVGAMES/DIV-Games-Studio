...
//      MÃ³dulo que contiene el cÃ³digo de lectura/escritura de formatos
//      grÃ¡ficos
//âââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââ
...
int graba_MAP(byte *mapa, FILE *f)
{
	...
	x = map_an;
	fwrite(&x, 2, 1, f); // +008 Ancho
	x = map_al;
	fwrite(&x, 2, 1, f); // +010 Alto
	y = ventana[v_ventana].mapa->Codigo;
	fwrite(&y, 4, 1, f); // +012 CÃ³digo

	fwrite(ventana[v_ventana].mapa->descripcion, 32, 1,
	       f);			      // +016 Descripcion
...
void browser2(void);
byte *descomprime_rle(byte *buffer, unsigned int bytes_line,
		      unsigned int last_byte, byte *pDest);
...
void descomprime_PCX(byte *buffer, byte *mapa, int vent)
{
	...
	if (header.color_planes == 3) {
		if ((pDest = (byte *)malloc((map_an + 1) * (map_al + 1) * 3)) ==
		    NULL) {
	...
	if (header.bits_per_pixel != 8 ||
	    header.color_planes != 1) // Si no es 8bpp
	{
		do {
	...
	if (header.bits_per_pixel == 8 && header.color_planes == 1) {
		...
		for (y = 0; y < map_al; y++) // For each line...
		{
			...
			for (x = 0; x < header.bytes_per_line;
			     x++) // For each byte...
			{
				for (con16 = 7; con16 >= 0 && pixel;
				     con16--) // For each pixel...
				{
					color16 = 0;
					color16 |= (((*(pSrc) & (1 << con16)) >>
						     con16));
					color16 |=
					    (((*(pSrc + header.bytes_per_line) &
					       (1 << con16)) >>
					      con16)
					     << 1);
					color16 |=
	...
	} else // Si es 24bpp.
	{
		...
		if ((v.click_handler == (voidReturnType)browser2 &&
			...
			for (rgb_color = 0; rgb_color < 256; rgb_color++) {
				Pcxdac[rgb_color].rgbRed =
				    (rgb_color & 0xE0) >> 5;
				Pcxdac[rgb_color].rgbGreen =
				    (rgb_color & 0x1C) >> 2;
...
int es_BMP(byte *buffer)
{
	...
	// if(InfoHeader.biCompression) if(InfoHeader.biCompression!=BI_RLE8)
	// return(0);
...
void descomprime_BMP(byte *buffer, byte *mapa, int vent)
{
	...
	if (InfoHeader.biBitCount == 8) {
		if (InfoHeader.biCompression == BI_RLE8) {
			for (y = 0; y < map_al && !bEOF;
			     y++) { // For each line...
				pDest =
				    &mapa[map_an * map_al - map_an * (y + 1)];
				bEOL = 0;
			...
			for (y = 0; y < map_al; y++) {
				memcpy(mapa + y * map_an,
				       pSrc + ((map_al - 1) - y) * fixmap_an,
				       map_an);
			}
		}
		y = 0;
	...
	} else if (InfoHeader.biBitCount == 24) // BMP de 24 bpp
	{
		...
		if ((v.click_handler == (voidReturnType)browser2 &&
			...
			// Crea la paleta 3-3-2
			for (rgb_color = 0; rgb_color < 256; rgb_color++) {
				Bmpdac[rgb_color].rgbRed =
				    (rgb_color & 0xE0) >> 5;
				Bmpdac[rgb_color].rgbGreen =
				    (rgb_color & 0x1C) >> 2;
...
int cargadac_PCX(char *name)
{
	...
	if (fread((byte *)&header, 1, sizeof(pcx_header), file) !=
	    sizeof(pcx_header)) {
		fclose(file);
		return (0);
	}
	...
	if (header.bits_per_pixel == 8 && header.color_planes == 1 &&
		...
		for (x = 0; x < 768; x++)
		   ...
		   (header.color_planes == 3 || header.color_planes == 1)) {
