...
void cd_get_volume(t_volumeinfo *vol) {
	struct {
		unsigned char  length;
		unsigned char  subunit;
		unsigned char  comcode;
...
void cd_set_volume(t_volumeinfo *vol) {
	struct {
		unsigned char  length;
		unsigned char  subunit;
		unsigned char  comcode;
...
void cd_get_audio_info(void) {
	...
	memcpy(&cdrom_data.diskid,
	       &track_data.lowest,
	        6);
	cdrom_data.low_audio  = track_data.lowest;
	cdrom_data.high_audio = track_data.highest;

	red_book(track_data.address,
	        &cdrom_data.disk_length_min,
	        &cdrom_data.disk_length_sec,
	        &cdrom_data.disk_length_frames);
...
void cd_getpos(t_playinfo *info) {
	struct {
		unsigned char  length;
		unsigned char  subunit;
		unsigned char  comcode;
