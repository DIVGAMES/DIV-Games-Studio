...
void mover_scroll(int back)
{
	...
	if (i = (scroll + snum)
		    ->camera) { // Comprueba si el scroll es autómático
		if (i >= id_start && i <= id_end && mem[i] == i) {
			...
			if (r >= 0 && r < max_region) {
				...
				if (y >= region[r].y1)
					iy = y - (region[r].y1 - 1);
			} else { // No hay region1 de bloqueo de scroll, se
				 // tiende al centro
...
{		// Reprocesa un scan por completo
...
void process_scan_right(void)
{
	...
	si = iscroll[snum].scr1 + iscroll[snum].an -
	     1; // Final de la primera línea
	...
	do {
		...
		if (*si) { // Si viene primer plano
			if (fast[in].nt &
			    1) { // Si el tramo acaba en primer plano
				if (fast[in].nt < max_inc)
...
void process_scan_left(void)
{
	...
	do {
		...
		if (fast[in].nt < max_inc) {
			...
			if (fast[in].nt &
			    1) { // Si la línea acababa en primer plano ...
		...
		if (*(si - iscroll[snum].an + 1)) { // Si viene primer plano
			if (!fast[in].inc[0]) {     // Si el tramo comienza en
						    // primer plano
				fast[in].inc[1]++;
...
void move_scroll(int plano, int x, int y)
{
	...
	if (ix > 0)
		do {
			if (plano == 0) {
				iscroll[snum].scr1 = scroll_right(
				    iscroll[snum]._scr1, iscroll[snum].scr1,
				    &iscroll[snum].block1, iscroll[snum].map1,
				    iscroll[snum].map1_an,
				    iscroll[snum].map1_al, x++, y);
				if (iscroll[snum].on == 2) {
					process_scan_right(); // Entra un scan
							      // por la derecha
							      // (sale por la
							      // izda)
				}
			} else {
				iscroll[snum].scr2 = scroll_right(
				    iscroll[snum]._scr2, iscroll[snum].scr2,
	...
	if (ix < 0)
		do {
			if (plano == 0) {
				iscroll[snum].scr1 = scroll_left(
				    iscroll[snum]._scr1, iscroll[snum].scr1,
				    &iscroll[snum].block1, iscroll[snum].map1,
				    iscroll[snum].map1_an,
				    iscroll[snum].map1_al, x--, y);
				if (iscroll[snum].on == 2) {
					process_scan_left(); // Entra un scan
							     // por la izda
							     // (sale por la
							     // dcha)
				}
			} else {
				iscroll[snum].scr2 = scroll_left(
				    iscroll[snum]._scr2, iscroll[snum].scr2,
				    &iscroll[snum].block2, iscroll[snum].map2,
				    iscroll[snum].map2_an,
				    iscroll[snum].map2_al, x--, y);
...
{ // Pinta un sprite (si se ve), según mem[ide+ ... ]
...
{ // E: texto[]
	...
	do
		if (texto[n].font) {
			...
			texto[n].an =
			    0; // Inicializa la region a volcar para este texto
			...
			case 1:
				if (mem[texto[n].ptr] < 0) {
					...
					ltoa(mem[texto[n].ptr], (char *)numero,
					     10);
				ptr = numero;
				break;
			}
...
{ // Le pasamos la posición de la cámara
	...
	do {
		...
		if (ide) {
			if (max >= 32) { // No pinta el objeto si está en las
					 // coordenadas de la cámara (±2)
...
void pinta_sprite_m7(int n, int ide, int x, int y, int size, int ang)
{
	...
	if ((p = mem[ide + _XGraph]) > 0) {
		...
		if (m < 1 || m > 256)
			return;
		// ang es el ángulo de visión del gráfico (0..4096), m el número
		// de particiones
		mem[ide + _Flags] &= 254;
...
#pragma aux mul_24 = "imul edx"                                                \
		     "shrd eax,edx,24" parm caller[EDX][EAX] value[EAX]
#endif
...
#pragma aux mul_16 = "imul edx"                                                \
		     "shrd eax,edx,16" parm caller[EDX][EAX] value[EAX]
#endif
...
void init_sin_cos()
{
	...
	for (i = 0; i < grados_90;
	     i++) { // Generamos la tabla del seno y el coseno
