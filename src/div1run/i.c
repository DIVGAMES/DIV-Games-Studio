...
int last_key_check, key_check, last_joy_check,
    joy_check; // Llamar al screensaver
...
int main(int argc, char *argv[])
{
	...
	if (OSDEP_NumJoysticks() > 0) {
		...
		// printf("NUmhats: %d\nNumButtons:
		// %d",SDL_JoystickNumHats(divjoy),SDL_JoystickNumButtons(divjoy));
	...
	if (argc < 2 && exesize == 0) {
		printf("DIV32RUN Run time library - version 1.03b - Freeware "
...
#endif
		printf("exesize: %d datasize: %d exestart: %d datstart: %d "
		       "filepos:%d\n ",
...
#ifndef DEBUG
			printf(
			    "Error: %d %d Needs a DIV32RUN executable to load.",
			    mem[0], mem[1]);
#endif
			exit(0);
		}
...
void inicializacion(void)
{
...
#ifdef DOS
		DetectBlaster(&setup->card, &setup->port, &setup->irq,
			      &setup->dma, &setup->dma2);
		DetectGUS(&setup->card, &setup->port, &setup->irq, &setup->dma,
			  &setup->dma2);
#endif
		setup->master = 15;
		setup->sound_fx = 15;
	...
	iloc_len =
	    mem[6] + mem[5];   // Longitud de las locales (públicas y privadas)
	...
	imem = itxt +
	       mem[8]; // Final de código, locales y textos (longitud del cmp)
	...
	if ((g[0].grf = (int **)malloc(sizeof(int32_t *) * 2000)) == NULL)
		exer(1);
	memset(g[0].grf, 0, sizeof(int32_t *) * 2000);
	next_map_code = 1000;
...
#ifdef DIVDLL
	// DLL_3 Exportación de funciones y variables (para utilizarlas en las
	// DLL)
...
void exec_process(void)
{
	...
	if (ide)
...
#ifdef NETLIB
			net_receive(); // Recibe los paquetes justo antes de
				       // ejecutar el proceso
#endif
			id = ide;
			ip = mem[id + _IP];
		...
		continue_process:
			...
			//    printf("running bytecode: %d
			//    %d\n",ip+1,mem[ip+1]);
			// printf("process entered at %d \n",ip);
			do {
				//		if((byte)mem[ip+1]>0)
				//			printf("running bytecode: %d
				//%d\n",ip,(byte)mem[ip]); 	   if(ip>18819)
				//		printf("fucking hell\n");
				switch ((byte)mem[ip++]) {
				case lnop:
					break;
...
#ifdef DEBUG
					for (ibreakpoint = 0;
					     ibreakpoint < max_breakpoint;
					     ibreakpoint++)
						if (breakpoint[ibreakpoint]
							    .line > -1 &&
						    breakpoint[ibreakpoint]
							    .offset == ip - 1)
							break;
					if (ibreakpoint <
					    max_breakpoint) { // Se llegó a un
							      // breakpoint
						mem[--ip] =
						    breakpoint[ibreakpoint]
							.code;
						breakpoint[ibreakpoint].line =
						    -1;
						call_to_debug = 1;
						process_stoped = id;
						return;
	...
	if (ide)
		...
		continue_process:
			...
			case ltyp:
				...
				break;
			case lpri:
				memcpy(&mem[id + inicio_privadas], &mem[ip + 1],
				       (mem[ip] - ip - 1) << 2);
				ip = mem[ip];
				break;
			case lcse:
...
#ifdef DEBUG
			case lchk:
				if (pila[sp] < id_init || pila[sp] > id_end ||
				    pila[sp] != mem[pila[sp]]) {
					v_function = -2;
					e(e141);
					if (call_to_debug) {
			...
			case ldbg:
				for (ibreakpoint = 0;
				     ibreakpoint < max_breakpoint;
				     ibreakpoint++)
					if (breakpoint[ibreakpoint].line > -1 &&
					    breakpoint[ibreakpoint].offset ==
						ip - 1)
						break;
				if (ibreakpoint <
				    max_breakpoint) { // Se llegó a un
...
void frame_start(void)
{
	...
	if (get_reloj() > ultimo_reloj) {
		ffps = (ffps * 49.0 +
			100.0 / (float)(get_reloj() - ultimo_reloj)) /
		       50.0;
		fps = (int)ffps;
	}
	...
	if (get_reloj() >
		...
		do {
			 ...
			 (int)freloj); // Espera para no dar más de "n" fps
		volcados_saltados = 0;
		saltar_volcado = 0;
		freloj += ireloj;
	}
...
void frame_end(void)
{
	...
	if (!dll_loaded) {
		...
		putsprite =
		    (void (*)(unsigned char *, int, int, int, int, int, int,
			      int, int, int))DIV_import("put_sprite"); // ok
	...
	// Si el usuario modificó mouse.x o mouse.y, posiciona el ratón
	// debidamente
	if (_mouse_x != mouse->x || _mouse_y != mouse->y)
		set_mouse(mouse->x, mouse->y);

	if (!saltar_volcado) {
		// printf("Restore type: %d DUmp type:
		...
		if (!debugger_step)
			...
			if (buffer_to_video != NULL)
				...
				if (old_dump_type) {
					...
					// Añade los volcados de este frame a
...
void exer(int e)
{
	...
	if (e) {
		...
		case 4:
			...
			break;
		case 5:
			printf("System font file missed!");
			break;
		case 6:
			printf("System graphic file missed!");
			break;
		default:
	...
	// máximo de procesos = %u", procesos,(id_end-id_start)/iloc_len+1);
...
void e(char *texto)
{
	...
	if (v_function >= 0)
		printf("DIV Execution error: %s (%s)", texto,
		       fname[v_function]);
	else
