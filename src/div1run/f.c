...
void function(void)
{
	...
	case 87:
		...
		break;
#endif
		/* OJO al añadir una funcion, hacerlo también en la lista fname
		 * de i.cpp, ltobj, add.prg y help */
	}
}
...
void _signal(void)
{
	...
	if ((bp & 1) && bp >= id_init && bp <= id_end && bp == mem[bp]) {
		if (mem[bp + _Status])
			...
			pila[--sp] =
			    0; // Devuelve el proceso o 0 si estaba muerto
	} else {
...
void load_fpg(void)
{
	...
	while (ptr < (byte *)g[n].fpg + file_len && *(int *)ptr < 1000 &&
	       *(int *)ptr > 0) {
		lst[*(int *)ptr] = (int *)ptr;
		ptr += *(int *)(ptr + 4);
	}
...
void start_scroll(void)
{
	...
	if ((iscroll[snum]._scr1 = (byte *)malloc(
	...
	if (s == 2) {
		...
		if ((iscroll[snum]._scr2 = (byte *)malloc(
			 iscroll[snum].an * (iscroll[snum].al + 1))) == NULL) {
...
FILE *open_save_file(char *file)
{
	...
	if ((f = fopen(full, "wb")) == NULL) { // "paz\fixero.est"
		...
		if ((f = fopen(full, "wb")) == NULL) { // "est\paz\fixero.est"
			...
			strcat(full, ext);
			if ((f = fopen(full, "rb")) !=
			    NULL) { // Si está ya en el raiz de div ...
				fclose(f);
				if ((f = fopen(full, "wb")) ==
				    NULL) { // "fixero.est"
					strcpy(full, "");
...
void stop_sound(void) { StopSound(pila[sp]); }
...
void _system(void)
{
	...
	if (system(NULL)) {
		if (!strcmp(strupr((char *)&mem[itxt + pila[sp]]),
			    "COMMAND.COM")) {
#ifdef DOS
...
void _exit_dos(void)
{
	...
	if ((f = fopen("system/exec.err", "wb")) != NULL) {
		...
		fwrite(&mem[itxt + pila[sp - 1]], 1,
		       strlen((char *)(&mem[itxt + pila[sp - 1]])) + 1, f);
		fclose(f);
	}
#else
...
void EndSound2(void);
int InitSound2(int Tarj, int Port, int Irq, int Dma, int Dma2, int master,
	       int sound_fx, int cd_audio);

void reset_sound(void)
{
#ifdef DOS
	EndSound2();
	InitSound2(setup->card, setup->port, setup->irq, setup->dma,
		   setup->dma2, setup->master, setup->sound_fx,
		   setup->cd_audio);
#endif
	pila[++sp] = 0;
}
...
void _net_init_ipx(void)
{ // Ojo, emitir los errores, e(...)
	int s, j, t;
	int old_reloj = get_reloj();
...
void _net_asign(void)
{
	...
	if (off < long_header || off >= id_init) {
		e(e150);
		return;
	} // Sólo GLOBAL
