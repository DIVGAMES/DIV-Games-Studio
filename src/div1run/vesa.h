...
typedef struct RealPointer {
	...
	unsigned short
	    Selector; // In protected mode, you need to chuck this dude into a
		      // segment register and use an offset 0.
} RealPointer;
...
typedef struct VesaInfo {
	...
	unsigned short
	    SupportedModesSegment; // Pointer to list of supported VESA and OEM
				   // modes (terminated with 0xffff).
...
typedef struct VesaModeData {
	...
	// Remainder of this structure is optional for VESA modes in v1.0/1.1,
	// needed for OEM modes.
