...
void out_region(void) {
  ...
  if (mem[id+_Ctype]==2) { e(e137); return; }

  file=mem[id+_File]; graph=mem[id+_Graph]; angle=mem[id+_Angle];

  if (reg<0 || reg>=max_region) { e(e108); return; } // Ya tiene region[reg].(x0..y1)
  ...
  if ((ptr=g[file].grf[graph])==NULL) { e(e121); return; }
...
void collision(void) {
  ...
  if (mem[id+_Ctype]==2) { e(e139); return; }
  ...
  for (;i<=id_end;i+=iloc_len) {
    ...
    if (i!=id && mem[i+_Bloque]==bloque && (mem[i+_Status]==2 ||
      ...
      if (angle) {
        ...
        if (dist==0) dist=sqrt((buffer_an*buffer_an+buffer_al*buffer_al)/4);
        an=((int)sqrt(ptr[13]*ptr[13]+ptr[14]*ptr[14])*mem[i+_Size])/100+dist;

        // Distancia real

        xx=(clipx0+clipx1)/2-x; yy=(clipy0+clipy1)/2-y; al=sqrt(xx*xx+yy*yy);
...
void sp_size( int *x, int *y, int *xx, int *yy, int xg, int yg,
  ...
  a=(float)ang/radian; s=(float)size/100;
