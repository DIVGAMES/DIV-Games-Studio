...
*/
	...
	tundo=(tipo_undo *)malloc(sizeof(tipo_undo)*max_undos);
...
void copy(int a,int b) {
  ...
  if (a==-1) {
    memcpy(&vaux[ivaux].tipo,&ventana[b].tipo,sizeof(tventana));
  } else if (b==-1) {
    memcpy(&ventana[a].tipo,&vaux[ivaux].tipo,sizeof(tventana));
  } else {
    memcpy(&ventana[a].tipo,&ventana[b].tipo,sizeof(tventana));
  }
...
void move(int a,int b) {
  if (a<b) {
    copy(-1,b);
    memmove(&ventana[a+1].tipo,&ventana[a].tipo,sizeof(tventana)*(b-a));
    copy(a,-1);
  } else if (b<a) {
    copy(-1,b);
    memmove(&ventana[b].tipo,&ventana[b+1].tipo,sizeof(tventana)*(a-b));
    copy(a,-1);
  }
}

void divdelete(int a) {
  memmove(&ventana[a].tipo,&ventana[a+1].tipo,sizeof(tventana)*(max_windows-1-a));
