...
int __far critical_error(unsigned deverr, unsigned errcode,
			 unsigned far *devhdr)
{
	c_e_0 = deverr;
	c_e_1 = errcode;
...
void demo1(void)
{
	...
	if (exe_cola[1] - 0xF31725AB > 31) {
		...
		wbox(v.ptr, an, al, c_b_low, 4, 12 + 28,
		     ((exe_cola[1] - 0xF31725AB) * (an - 8)) / 31, 14);
	}
...
void betatest1(void) { _show_items(); }
...
#pragma aux DOSalloc4k = "mov bx,0100h", "mov ax,0100h", "int 31h",            \
	    "mov ecx,0", "jc short done", "mov bx,dx", "mov ax,0006h",         \
	    "int 31h", "shl ecx,16", "mov cx,dx",                              \
	    "done:", modify[ax bx dx] value[ecx];

#pragma aux DPMIalloc4k = "mov cx,1000h", "mov bx,0000h", "mov ax,0501h",      \
	    "int 31h", "mov edx,0", "jc short done", "shrd edx,ebx,16",        \
	    "mov dx,cx", "done:", modify[ax bx cx si di] value[edx];
...
int main(int argc, char *argv[])
{
	...
	if (argc > 2 &&
	    (!strcmp(argv[2], "/safe") || !strcmp(argv[2], "/SAFE"))) {
		safe = 33;
		DaniDel("sound.cfg");
	...
	tipo[2].ext = "*.* *.MAP *.PCX *.BMP *.JPG *.JPE *.PNG *.GIF *.TGA "
		      "*.TIF"; // Maps browser
	tipo[3].ext = "*.* *.PAL *.FPG *.FNT *.MAP *.BMP *.PCX *.JPG *.PNG "
		      "*.GIF *.TGA *.TIF"; // Palette Browser
	...
	if (compilemode == 1) {
		...
		if (argc < 3) {
			fprintf(stdout, "DIV Games Studio Compiler V2.02 - "
					"http://www.div-arena.co.uk\n");
			fprintf(stdout,
...
void crea_barratitulo(void)
{
	...
	cbs[0] = c_b_low;
	cbs[64] = media(c_b_low, c0);
	n = 64;
	while (n > 1) {
		m = 0;
		while (m < 64) {
			cbs[m + n / 2] = media(cbs[m], cbs[m + n]);
			m += n;
		}
		n >>= 1;
	}
	...
	crs[0] = media(c_r, c_r_low);
	crs[64] = media(c_r_low, c0);
	n = 64;
	while (n > 1) {
		m = 0;
		while (m < 64) {
			crs[m + n / 2] = media(crs[m], crs[m + n]);
			m += n;
		}
		n >>= 1;
	}
...
void inicializa_entorno()
{
	...
	//  if(!CopiaDesktop) nueva_ventana(menu_principal0); else
	//  UpLoad_Desktop();
	...
	if (!primera_vez) {
		for (n = 0; n < max_windows; n++)
			if (ventana[n].click_handler == menu_principal2)
				break;
		if (n == max_windows)
	...
	// OJO! no se informa si no se pudo inicializar el sonido (sólo en
	// Sonidos/Nuevo)
...
void interr0(void)
{
	...
	switch (error_code) {
	case 1:
		v_texto = (char *)texto[368];
		break;
	case 2:
		v_texto = (char *)texto[369];
		break;
	case 3:
		v_texto = (char *)texto[370];
		break;
	case 4:
		v_texto = (char *)texto[371];
		break;
	case 5:
		v_texto = (char *)texto[372];
		break;
	case 6:
		v_texto = (char *)texto[373];
		break;
	}
...
void mainloop(void)
{
	...
	while (n < max_windows && !(ventana[n].tipo > 0 &&
				    mouse_in(ventana[n].x, ventana[n].y,
					     ventana[n].x + ventana[n].an - 1,
					     ventana[n].y + ventana[n].al - 1)))
		n++;

	if (n < max_windows && arrastrar == 4 &&
	    ventana[n].orden == quien_arrastra)
		arrastrar = 5;
		...
		if (v_aceptar) {
			...
			if (!nuevo_mapa(NULL)) {
				...
				if (v_mapa->TengoNombre == 1) {
					...
					memcpy(v.mapa->filename,
					       v_mapa->filename, 13);
					memcpy(v.nombre, v_mapa->filename, 13);
					memcpy(v.titulo, v_mapa->filename, 13);
					v.mapa->Codigo = 0;
					memcpy(v.mapa->descripcion,
					       v_mapa->descripcion, 32);
				...
				wvolcado(copia, vga_an, vga_al, v.ptr, v.x, v.y,
					 v.an, v.al, 0);
	...
	if (n >= max_windows || n < 0) {
		...
		case 0:
			...
			break;
		case 1:
			if (mouse_in(ventana[n].x + 2 * big2,
				     ventana[n].y + 2 * big2,
				     ventana[n].x + ventana[n].an - 2 * big2,
				     ventana[n].y + 9 * big2))
				if (mouse_x <=
				    ventana[n].x + ventana[n].an - 18 * big2)
					mouse_graf = 2;
				else if (mouse_x <= ventana[n].x +
							ventana[n].an -
							10 * big2)
					mouse_graf = 4;
				else
	...
	if (n != 0 && n < max_windows) {
		...
		// Pasa la ventana a la ventana 0 si esta en primer plano o se
		// pulsa
		...
		if (n == 0 &&
		    v.primer_plano != 1) { // Se pulsó icono o 2º plano
			for (m = 1; m < max_windows; m++) {
				if (ventana[m].tipo &&
				    ventana[m].primer_plano == 1) {
					if (colisionan(0, m)) {
		...
		//─────────────────────────────────────────────────────────────────────────
		// Activa una ventana excluible (tipo>=100) cuando se haga algo
		// sobre ella
		//─────────────────────────────────────────────────────────────────────────

	} else if (n == 0 && (mouse_b & 1) && v.tipo >= 100 &&
		   v.primer_plano < 2 && !v.estado && mouse_graf != 4 &&
		   mouse_graf != 5) {
		activar();
		vuelca_ventana(0);
	}
	...
	if (n < max_windows && ventana[n].primer_plano == 0 && arrastrar == 4 &&
	    v.tipo >= 100 && ventana[n].tipo != 2) {
		move(0, n);
		n = 0;
	...
	if (n == 0 && v.primer_plano == 1)
		if (mouse_in(v.x + 2 * big2, v.y + 10 * big2,
			     v.x + v.an - 2 * big2, v.y + v.al - 2 * big2)) {
			...
			if (v.tipo == 100 && arrastrar != 4) {
				...
				if ((mouse_b & 1) && !(old_mouse_b & 1)) {
					if (*system_clock < doble_click + 10 &&
					    *system_clock > doble_click &&
					    abs(doble_click_x - mouse_x) < 8 &&
					    abs(doble_click_y - mouse_y) < 8) {
						llamar = 1;
					} else {
						doble_click = *system_clock;
						doble_click_x = mouse_x;
						doble_click_y = mouse_y;
					}
				}
			}
			...
			if (mouse_graf == 5) {
				if (mouse_b & 1) {
					if (big)
						wput(v.ptr, v.an / 2, v.al / 2,
						     v.an / 2 - 9, 2, -45);
					else
						wput(v.ptr, v.an, v.al,
						     v.an - 9, 2, -45);
	...
	if (n == 0 && v.primer_plano == 2) {
		if (mouse_graf == 2) {
			...
			if (mouse_b & 1) {
				if (big) {
					wput(copia, -vga_an, vga_al, v.x, v.y,
					     -48);
...
fin_bucle_entorno:
	...
	for (m = 0; m < max_windows; m++) {
		if (m == 0 &&
		    mouse_in(v.x + 2 * big2, v.y + 10 * big2,
			     v.x + v.an - 2 * big2, v.y + v.al - 2 * big2) &&
		    ventana[m].tipo != 107)
			continue;
		...
		if (ventana[m].tipo == 4 ||
		    ...
		    (ventana[m].tipo == 102 && ventana[m].estado &&
		     ventana[m].primer_plano != 2)) {
	...
	for (m = 0; m < max_windows; m++)
		if (ventana[m].tipo == 102 && ventana[m].estado &&
		    ventana[m].prg != NULL)
			break;
...
void dialog_loop(void)
{
	...
	if (n ==
	    0) // Si ahora estamos en la barra, también se repinta la ventana
		if (!mouse_in(v.x + 2 * big2, v.y + 10 * big2,
			      v.x + v.an - 2 * big2, v.y + v.al - 2 * big2))
			n--;
	...
	if (n == max_windows)
		mouse_graf = 1;
	else if (mouse_in(v.x + 2 * big2, v.y + 2 * big2, v.x + v.an - 2 * big2,
			  v.y + 9 * big2))
	...
	if ((key(_ESC) && !key(_L_CTRL)) || (modo < 100 && (mouse_b & 2))) {
		...
		for (n = 1; n < max_windows; n++)
			if (ventana[n].tipo && ventana[n].primer_plano == 0)
				...
				if (colisiona_con(n, x, y, an, al)) {
					...
					// Si esta tapada por otras ventanas (en
					// primer o segundo plano) ...

					for (m = 1; m < max_windows; m++)
						if (m != n && ventana[m].tipo &&
						    (ventana[m].primer_plano ==
							 1 ||
						     (ventana[m].primer_plano !=
							  1 &&
					...
					// Si destapamos la susodicha ventana ..
					// la pasa a primer plano ..
	...
	for (n = 1; n < max_windows; n++)
		if (ventana[n].tipo && ventana[n].primer_plano == 1)
			if (colisionan(0, n))
				m++;
	if (m)
		emplazar(v.lado * 2 + 1, &v.x, &v.y, v.an, v.al);
...
void minimiza_ventana(void)
{
	...
	v.an = (7 + 1 + text_len(v.nombre) + 1) * big2;
	v.al = 7 * big2;

	if (v.an != v._an ||
	    v.al != v._al) { // Si es la primera vez que se minimiza
			     // (o cambió de nombre ...)
...
void cierra_ventana(void)
{
	...
	if (v.click_handler == help2 && old_prg != NULL) {
		for (m = 1; m < max_windows; m++) {
			if (ventana[m].click_handler == programa2 ||
			    ventana[m].click_handler == calc2) {
				if ((ventana[m].prg == old_prg ||
				     ventana[m].aux == (byte *)old_prg) &&
				    ventana[m].primer_plano < 2) {
					ventana[m].estado = 1;
					wgra(ventana[m].ptr,
					     ventana[m].an / big2,
					     ventana[m].al / big2, c_b_low, 2,
					     2, ventana[m].an / big2 - 20, 7);
					if (text_len(ventana[m].titulo) + 3 >
					    ventana[m].an / big2 - 20) {
						wwrite_in_box(
						    ventana[m].ptr,
						    ventana[m].an / big2,
						    ventana[m].an / big2 - 19,
						    ventana[m].al / big2, 4, 2,
						    0, ventana[m].titulo, c1);
						wwrite_in_box(
						    ventana[m].ptr,
						    ventana[m].an / big2,
						    ventana[m].an / big2 - 19,
						    ventana[m].al / big2, 3, 2,
						    0, ventana[m].titulo, c4);
					} else {
						wwrite(
						    ventana[m].ptr,
						    ventana[m].an / big2,
	...
	if (v.tipo >= 100 &&
	    v.estado) { // Los mapas se auto-desactivan al cerrarse
		for (m = 1; m < max_windows; m++)
			if (ventana[m].tipo == v.tipo &&
			    ventana[m].primer_plano < 2) {
				ventana[m].estado = 1;
				wgra(ventana[m].ptr, ventana[m].an / big2,
				     ventana[m].al / big2, c_b_low, 2, 2,
				     ventana[m].an / big2 - 20, 7);
				if (text_len(ventana[m].titulo) + 3 >
				    ventana[m].an / big2 - 20) {
					wwrite_in_box(ventana[m].ptr,
						      ventana[m].an / big2,
						      ventana[m].an / big2 - 19,
						      ventana[m].al / big2, 4,
						      2, 0, ventana[m].titulo,
						      c1);
					wwrite_in_box(ventana[m].ptr,
						      ventana[m].an / big2,
						      ventana[m].an / big2 - 19,
					...
					wwrite(ventana[m].ptr,
					       ventana[m].an / big2,
					       ventana[m].al / big2,
					       3 + (ventana[m].an / big2 - 20) /
						       2,
					       2, 1, ventana[m].titulo, c1);
					wwrite(ventana[m].ptr,
					       ventana[m].an / big2,
					       ventana[m].al / big2,
					       2 + (ventana[m].an / big2 - 20) /
						       2,
					       2, 1, ventana[m].titulo, c4);
				}
				vuelca_ventana(m);
				break;
			}
	...
	if (n == 1 || n == 7) { // Los diálogos pasan a primer plano las hiden[]
				// al cerrarse
		if (v.tipo == 1 ||
		    v.tipo == 7) { // Diálogo sobre diálogo solo abre el último
			v.primer_plano = 1;
			vuelca_ventana(0);
		} else if (modo >= 100)
			for (n = 0; n < max_windows; n++)
...
void mueve_ventana(void)
{
	...
	//  if (v.primer_plano==1)
	//  wrectangulo(v.ptr,an/big2,al/big2,c2,0,0,an/big2,al/big2);
	...
	if (oldx == v.x && oldy == v.y) {
		if (*system_clock < doble_click + 10 &&
		    *system_clock > doble_click &&
		    ...
		    abs(doble_click_y - mouse_y) < 8) {
			if (v.primer_plano == 2)
				emplazar(v.lado * 2 + 0, &v.x, &v.y, v.an,
					 v.al);
			else
				emplazar(v.lado * 2 + 1, &v.x, &v.y, v.an,
					 v.al);
			if (v.x == oldx && v.y == oldy) {
				v.lado ^= 1;
				if (v.primer_plano == 2)
					emplazar(v.lado * 2 + 0, &v.x, &v.y,
						 v.an, v.al);
				else
					emplazar(v.lado * 2 + 1, &v.x, &v.y,
						 v.an, v.al);
				if (v.x == oldx && v.y == oldy)
					v.lado ^= 1;
			}
			if (v.x != oldx || v.y != oldy) {
				if (exploding_windows)
					extrude(oldx, oldy, v.an, v.al, v.x,
						v.y, v.an, v.al);
				se_ha_movido_desde(oldx, oldy, v.an, v.al);
				x++;
			}
		} else {
			doble_click = *system_clock;
...
void mueve_ventana_completa(void)
{
	...
	if (v.primer_plano == 1)
		wrectangulo(v.ptr, an / big2, al / big2, c4, 0, 0, an / big2,
			    al / big2);
	...
	if (v.primer_plano == 1)
		wrectangulo(v.ptr, an / big2, al / big2, c2, 0, 0, an / big2,
			    al / big2);
...
void actualiza_caja(int x, int y, int an, int al)
{
	...
	if (y < vga_al && y + al > vga_al - 8 * big2 && x < vga_an &&
	    x + an >= vga_an - (text_len(div_version) + 2) * big2) {
		wwrite_in_box(copia + y * vga_an + x, vga_an, an, al,
			      vga_an - 1 - x, vga_al - 1 - y, 18, div_version,
			      c0);
		wwrite_in_box(copia + y * vga_an + x, vga_an, an, al,
			      vga_an - 2 - x, vga_al - 1 - y, 18, div_version,
			      c2);
	}
				...
				if (ventana[n].primer_plano < 2) {
					...
					if (n == 0 && mover_ventana) {
						...
						wwrite_in_box(
						    copia + y * vga_an + x,
						    vga_an, an, al,
						    ventana[n].x + 9 * big2 - x,
						    ventana[n].y - y, 10,
						    ventana[n].nombre, c0);
						wwrite_in_box(
						    copia + y * vga_an + x,
						    vga_an, an, al,
						    ventana[n].x + 8 * big2 - x,
						    ventana[n].y - y, 10,
						    ventana[n].nombre, c4);
					}
...
void actualiza_caja2(int vent, int x, int y, int an, int al)
{
	...
	if (ventana[vent].primer_plano == 2) {
		actualiza_caja(x, y, an, al);
		return;
	}
...
void actualiza_background(void)
{
	...
	if (zy > 2) {
		y0 = zy - 2;
		actualiza_caja(0, 0, vga_an, zy - 2);
	}
	if (zy + zal < vga_al - 2) {
		y1 = zy + zal + 1;
		actualiza_caja(0, zy + zal + 2, vga_an, vga_al - 2 - zy - zal);
	}
	if (zx > 2) {
		actualiza_caja(0, y0, zx - 2, y1 - y0 + 1);
	}
	if (zx + zan < vga_an - 2) {
		actualiza_caja(zx + zan + 2, y0, vga_an - 2 - zx - zan,
			       y1 - y0 + 1);
	}
	...
	if (an > 0 && al > 0) {
		...
		if (tapiz != NULL) {
			...
			do {
				...
				p += vga_an - an;
				t += tapiz_an;
				if (t == mapa_tapiz + tapiz_an * tapiz_al)
					t = mapa_tapiz;
			} while (--al);
			/*
      t=tapiz+(y&127)*128;
...
//═════════════════════════════════════════════════════════════════════════════
//    Dumps a window, according this standard or obscured or not in the
//    foreground
//═════════════════════════════════════════════════════════════════════════════
...
void vuelca_ventana(int m)
{
	...
	if (no_volcar_ventanas)
		return;

	if (volcados_parciales) {
		if (ventana[m].tipo == 102 && ventana[m].volcar == 1 &&
		    ventana[m].prg != NULL) {
			ventana[m].volcar = 0;
			actualiza_caja2(m, ventana[m].x + 2 * big2,
					ventana[m].y + 10 * big2,
					ventana[m].an - 20, 7 * big2);
			actualiza_caja2(m, ventana[m].x + 2 * big2,
					ventana[m].y + 18 * big2 +
					    (ventana[m].prg->linea -
					     ventana[m].prg->primera_linea) *
						font_al,
					ventana[m].an - 12, font_al);
			if (ventana[m].prg->linea_vieja !=
			    ventana[m].prg->linea -
				ventana[m].prg->primera_linea)
				actualiza_caja2(
				    m, ventana[m].x + 2 * big2,
				    ventana[m].y + 18 * big2 +
					ventana[m].prg->linea_vieja * font_al,
				    ventana[m].an - 12, font_al);
			return;
		}
		if (ventana[m].click_handler == MapperCreator2 &&
		    ventana[m].volcar == 2) {
			ventana[m].volcar = 0;
			actualiza_caja2(m, ventana[m].x + 3 * big2,
					ventana[m].y + 182 * big2, 39 * big2,
					15 * big2);
			return;
		}
	}
		...
		if (ventana[n].tipo)
			if (colisiona_con(n, x, y, an, al))
				if (ventana[n].primer_plano < 2) {
					...
					wwrite_in_box(
					    copia + y * vga_an + x, vga_an, an,
					    al, ventana[n].x + 9 * big2 - x,
					    ventana[n].y - y, 10,
					    ventana[n].nombre, c0);
					wwrite_in_box(
					    copia + y * vga_an + x, vga_an, an,
					    al, ventana[n].x + 8 * big2 - x,
					    ventana[n].y - y, 10,
					    ventana[n].nombre, c4);
	...
	if (0) {
		if (ventana[n].surfaceptr != NULL)
			SDL_BlitSurface(ventana[n].surfaceptr, NULL,
					copia_surface, &trc);
	}
#endif
...
void emplazar(int flag, int *_x, int *_y, int an, int al)
{
	...
	// Primero crea en scan[] una lista de posibles alturas (0+fines de
	// ventana)
	...
	for (n = 1; n < max_windows; n++)
		if (ventana[n].tipo)
			if (flag & 1) {
				if ((y = ventana[n].y + ventana[n].al + 1) <
				    vga_al) {
					x = 0;
					do {
						x++;
	...
	if (flag & 2) {
		for (n = 0; n < scanes; n++) {
			...
			do {
				...
				for (m = 1; m < max_windows; m++)
					if (ventana[m].tipo &&
					    colisiona_con(m, x - 1, y - 1,
							  an + 2, al + 2))
						if (new_x >= ventana[m].x - an)
							new_x = ventana[m].x -
								an - 1;
	...
	if (flag & 1) {
		if (*_y + al > vga_al)
			posicion_menos_mala(_x, _y, an, al);
	} else {
		if (*_y < 0)
			*_y = old_y;
	}
...
void window_surface(int an, int al, byte type)
{
	...
	if (type == 1)
		v.surfaceptr =
		    IMG_Load("/home/mike/div2015/system/red_panel.png");
	else
		v.surfaceptr =
		    IMG_Load("/home/mike/div2015/system/blue_panel.png");
#endif

	//		SDL_SetAlpha(v.surfaceptr,SDL_SRCALPHA | SDL_RLEACCEL
	//,128);

	if (v.surfaceptr != NULL) {
		tempsurface = zoomSurface(
		    v.surfaceptr,
		    (float)((an * 1.0f) / (v.surfaceptr->w * 1.0f)),
		    (float)((al * 1.0f) / (v.surfaceptr->h * 1.0f)), 0);
		SDL_FreeSurface(v.surfaceptr);
	}

	// if couldnt be found
	//	if(tempsurface==NULL)
	//		tempsurface = SDL_CreateRGBSurface(SDL_SWSURFACE, an, al,
...
/* SDL interprets each pixel as a 32-bit number, so our masks must depend
	...
	if (!ventana[max_windows - 1].tipo) {
		...
		//───────────────────────────────────────────────────────────────────────────
		// The following values should define the init_handler, default
		// values:
		//───────────────────────────────────────────────────────────────────────────
		...
		if (primera_vez == 2) { // La ventana de ayuda (la primera vez)
			y = x = vga_an / 2 - an / 2;
			   ...
			   v.tipo ==
			       7) { // Los diálogos se colocan en el centro
			x = vga_an / 2 - an / 2;
			y = vga_al / 2 - al / 2;
		} else
			emplazar(v.lado * 2 + 1, &x, &y, an, al);
				...
				if (ventana[m].tipo == v.tipo &&
					...
					if (text_len(ventana[m].titulo) + 3 >
						...
						wwrite_in_box(
						    ...
						    ventana[m].an / big2 - 19,
						    ventana[m].al / big2, 4, 2,
						    0, ventana[m].titulo, c0);
						wwrite_in_box(
						    ventana[m].ptr,
						    ventana[m].an / big2,
						    ventana[m].an / big2 - 19,
						    ventana[m].al / big2, 3, 2,
						    0, ventana[m].titulo, c2);
					} else {
						wwrite(
						    ventana[m].ptr,
						    ventana[m].an / big2,
		...
		n = 0;
		if (v.tipo == 2 || v.tipo == 3 || v.tipo == 4 || v.tipo == 5 ||
		    v.tipo == 8) {
			for (m = 1; m < max_windows; m++)
				if (ventana[m].tipo == 2 ||
				    ventana[m].tipo == 3 ||
		...
		if (ptr != NULL) { // Ventana, free en cierra_ventana
			...
			if (v.tipo == 1 ||
			    v.tipo == 7) { // Los diálogos/progreso cierran
					   // todas las ventanas
				vtipo = v.tipo;
				v.tipo = 0;
				if (ventana[1].tipo == 1 ||
				    ventana[1].tipo ==
					7) { // Diálogo sobre diálogo
					ventana[1].primer_plano = 0;
					vuelca_ventana(1);
				} else
					for (n = 1; n < max_windows; n++)
						if (ventana[n].tipo &&
						    ventana[n].primer_plano ==
							1) {
							hiden[n - 1] = 1;
							ventana[n]
							    .primer_plano = 0;
			...
			//     SDL_FillRect(v.surfaceptr,NULL,SDL_MapRGB(
			//     v.surfaceptr->format, 0,0,0));
			...
			if (v.tipo == 1) { // Los diálogos no se minimizan
				...
				if (text_len(v.titulo) + 3 > an - 12) {
					wwrite_in_box(ptr, an, an - 11, al, 4,
						      2, 0, v.titulo, c1);
					wwrite_in_box(ptr, an, an - 11, al, 3,
						      2, 0, v.titulo, c4);
				} else {
					wwrite(ptr, an, al, 3 + (an - 12) / 2,
					       2, 1, v.titulo, c1);
					wwrite(ptr, an, al, 2 + (an - 12) / 2,
					       2, 1, v.titulo, c4);
				}
			} else if (v.tipo == 7) { // Barra de progreso
				wgra(ptr, an, al, c_b_low, 2, 2, an - 4, 7);
				if (text_len(v.titulo) + 3 > an - 4) {
					wwrite_in_box(ptr, an, an - 3, al, 4, 2,
						      0, v.titulo, c1);
					wwrite_in_box(ptr, an, an - 3, al, 3, 2,
						      0, v.titulo, c4);
				} else {
					wwrite(ptr, an, al, 3 + (an - 4) / 2, 2,
					       1, v.titulo, c1);
					wwrite(ptr, an, al, 2 + (an - 4) / 2, 2,
					       1, v.titulo, c4);
				}
			} else {
				...
				if (text_len(v.titulo) + 3 > an - 20) {
					wwrite_in_box(ptr, an, an - 19, al, 4,
						      2, 0, v.titulo, c1);
					wwrite_in_box(ptr, an, an - 19, al, 3,
						      2, 0, v.titulo, c4);
				} else {
					wwrite(ptr, an, al, 3 + (an - 20) / 2,
					       2, 1, v.titulo, c1);
					wwrite(ptr, an, al, 2 + (an - 20) / 2,
					       2, 1, v.titulo, c4);
				}
			}
			...
			if (primera_vez != 1) {
				...
				wvolcado(copia, vga_an, vga_al, ptr, x, y, an,
					 al, 0);
				volcado_parcial(x, y, an, al);
			}

			//───────────────────────────────────────────────────────────────────────────
			// No se pudo abrir la ventana, (no hay memoria o menú
			// duplicado)
			//───────────────────────────────────────────────────────────────────────────
			...
			if (n && ventana[n].primer_plano == 2) {
				...
				if (ventana[n].primer_plano == 0) {
					...
					v.primer_plano = 1;
					vuelca_ventana(0);
				} else { // Cuando el menu pedido ya está en
					 // primer plano lo resalta
					divdelete(0);
					move(0, n - 1);
					wrectangulo(v.ptr, v.an / big2,
						    v.al / big2, c4, 0, 0,
						    v.an / big2, v.al / big2);
					init_volcado();
					vuelca_ventana(0);
...
void dialogo(voidReturnType init_handler)
{
	...
	if (!ventana[max_windows - 1].tipo) {
		...
		if (ptr != NULL) { // Ventana, free en cierra_ventana
			...
			wput(ptr, an, al, an - 9, 2, 35);
			if (!strcmp((char *)v.titulo, (char *)texto[41]) ||
			    !strcmp((char *)v.titulo, (char *)texto[367]))
...
#ifdef TTF
	SDL_FillRect(v.surfaceptr, NULL,
		     SDL_MapRGB(v.surfaceptr->format, 0, 0, 0));
#endif
	wrectangulo(ptr, an, al, c2, 0, 0, an, al);
	wput(ptr, an, al, an - 9, 2, 35);
	if (!strcmp((char *)v.titulo, (char *)texto[41]))
...
*/
	...
	tundo =
	    (struct tipo_undo *)malloc(sizeof(struct tipo_undo) * max_undos);
	...
	if (fondo_raton == NULL || copia == NULL || dac == NULL ||
	    dac4 == NULL || cuad == NULL || ghost == NULL || barra == NULL ||
	    undo == NULL || tundo == NULL || fill_dac == NULL ||
	    error_window == NULL)
		error(0);
	...
	sysfont = loadfont("/home/mike/cool-retro-term/app/qml/fonts/"
			   "modern-fixedsys-excelsior/FSEX301-L2.ttf",
			   (big == 1) ? 18 : 9);
	editorfont = loadfont("/home/mike/cool-retro-term/app/qml/fonts/"
			      "modern-fixedsys-excelsior/FSEX301-L2.ttf",
			      (big == 1) ? 18 : 9);

	// sysfont = loadfont("system/KenPixel.ttf",(big==1)?12:6);
	// sysfont =
	// loadfont("/usr/share/fuze/assets/fuzebasic/zx.ttf",(big==1)?30:15);
	// sysfont = loadfont("/usr/share/wine/fonts/tahoma.ttf",(big==1)?14:7);
	// sysfont =
	// loadfont("/home/mike/cool-retro-term/app/qml/fonts/modern-fixedsys-excelsior/FSEX301-L2.ttf",20);
	// usr/share/wine/fonts/tahoma.ttf",(big==1)?14:7);

	// printf("Loaded TTF: %x\n",sysfont);

	//	font_an=(big==1)?16:8;
	tempsurface = drawtext(sysfont, colors[c].r, colors[c].g, colors[c].b,
			       0, 255, 255, 255, 0, "W", solid);
...
// alloc each graph
			...
			while (ftell(f) < file_len && len_ > 0 && num_ > 0) {
				...
				fread(&num_, 4, 1, f);
				fread(&len_, 4, 1, f);
				fseek(f, 44, SEEK_CUR) : fread(&an, 4, 1, f);
				fread(&al, 4, 1, f);
				fread(&pts, 4, 1, f);
...
// 	 printf("mem ptr is %x\n",iptr);
		 if (m!=palcrc) {
		 adaptar(ptr+64+iptr[15]*4, iptr[13]*iptr[14],
(byte*)(g[num].fpg)+8,&xlat[0]);
	 }
	 */
			}
#endif
			...
			while (ptr2 < ptr + n && *((int *)ptr2) < 384) {
				graf_help[*(int *)ptr2].an =
				    *(int *)(ptr2 + 52);
				graf_help[*(int *)ptr2].al =
				    *(int *)(ptr2 + 56);
				graf_help[*(int *)ptr2].offset =
				    (ptr2 - ptr) + 64 +
				    4 * (*(int *)(ptr2 + 60));
				ptr2 +=
				    *(int *)(ptr2 + 52) * *(int *)(ptr2 + 56) +
				    64 + 4 * (*(int *)(ptr2 + 60));
			}
			free(ptr);
		} else {
		...
		if (!Interpretando)
			printf("%s", (char *)texto[10]); // *** Carga los
							 // objetos editados ***

		if (auto_save_session || Interpretando)
			CopiaDesktop = Can_UpLoad_Desktop();

		if (!CopiaDesktop) { // Carga paleta comun
			if (!Interpretando)
				cprintf(
				    "%s",
				    (char *)texto[11]); // *** Cálculos sobre la
							// paleta ***
		...
		if (!Interpretando)
			printf("%s", (char *)texto[12]); // *** Miscelánea ***
		find_colors();
		...
		inicializa_compilador(); // *** Compilador *** espacios de lower
					 // a 00
		init_lexcolor();

		if (!Interpretando)
			printf("%s", (char *)texto[13]);
...
*/
	...
	void determina_unidades(void)
	{
		...
		// driveStrings is a double null terminated list of null
		// terminated strings)
		while (*driveStrings) {
			// Dump drive information
			driveType = GetDriveType(driveStrings);
			//        GetDiskFreeSpaceEx(driveStrings, &freeSpace,
			//        NULL, NULL);
		...
		do {
			...
			if (r.w.cflag & INTR_CF)
				...
				unidades[uni++] = 'A' + r.h.al - 1;
			if (uni > 1)
				for (m = 0; m < uni - 1; m++)
					if (unidades[m] == unidades[uni - 1])
						uni--;
		} while (++n <= 26);
		unidades[uni] = 0;
#endif
	...
	char *GetMemoryFree()
	{
		...
		if (MaxMemUsed < GetHeapFree(1))
			MaxMemUsed = GetHeapFree(1);
		sprintf(MemoriaLibre, "%u %u %u [%u]", MemInfo.data1,
			MemInfo.data1 + GetHeapFree(0), GetHeapFree(1),
			MaxMemUsed);
		return (MemoriaLibre);
#endif
	}
	...
	void _get(int t, int x, int y, int an, byte *buffer, int lon_buffer,
		  int r0, int r1)
	{
	...
	void _show_items(void)
	{
		int n = 0;
		show_items_called = 1;
		wbox(v.ptr, v.an / big2, v.al / big2, c2, 2, 10,
		     v.an / big2 - 4, v.al / big2 - 12);
		while (n < v.items) {
	...
	void show_button(struct t_item * i)
	{
		wwrite(v.ptr, v.an / big2, v.al / big2, i->button.x,
		       i->button.y, i->button.center, i->button.texto, c3);
	...
	void select_button(struct t_item * i, int activo)
	{
		...
		if (activo) {
			wrectangulo(v.ptr, v.an / big2, v.al / big2, c12, x - 4,
				    y - 4, an + 8, al + 8);
		} else {
			wrectangulo(v.ptr, v.an / big2, v.al / big2, c2, x - 4,
				    y - 4, an + 8, al + 8);
		}
	}
	...
	void show_get(struct t_item * i)
	{
		...
		wwrite_in_box(v.ptr, v.an / big2, i->get.an - 1 + i->get.x,
			      v.al / big2, i->get.x + 1, i->get.y + 9, 0,
			      i->get.buffer, c3);
		wwrite(v.ptr, v.an / big2, v.al / big2, i->get.x + 1, i->get.y,
		       0, i->get.texto, c12);
	...
	void select_get(struct t_item * i, int activo, int ocultar_error)
	{
		...
		int n;
		if (activo) {
			wrectangulo(v.ptr, v.an / big2, v.al / big2, c12,
			...
			if (!superget) {
				wbox(v.ptr, v.an / big2, v.al / big2, c1,
				     i->get.x, i->get.y + 8, i->get.an, 9);
				wwrite_in_box(v.ptr, v.an / big2,
					      i->get.an - 1 + i->get.x,
					      v.al / big2, i->get.x + 1,
					      i->get.y + 9, 0, i->get.buffer,
					      c3);
				wrectangulo(v.ptr, v.an / big2, v.al / big2, c2,
					    i->get.x - 1, i->get.y + 7,
					    i->get.an + 2, 11);
			}
	...
	{
		if (*i->flag.valor)
			wput(v.ptr, v.an / big2, v.al / big2, i->flag.x,
			     i->flag.y, -59);
		else
			wput(v.ptr, v.an / big2, v.al / big2, i->flag.x,
			     i->flag.y, 58);
		wwrite(v.ptr, v.an / big2, v.al / big2, i->flag.x + 9,
		       i->flag.y, 0, i->flag.texto, c12);
		wwrite(v.ptr, v.an / big2, v.al / big2, i->flag.x + 8,
		       i->flag.y, 0, i->flag.texto, c3);
	}
	...
	void _process_items(void)
	{
		...
		if (v.selected_item != -1) {
			if (!v.estado && v.tipo == 102) {
				...
				if (ascii == 0x1b) { // &&
						     // (v.item[v.selected_item].estado&2))
						     // {
					if (v.item[v.selected_item].tipo == 2) {
						asc = ascii;
						kesc = kbdFLAGS[28];
						est = v.item[v.selected_item]
							  .estado;
						ascii = 0;
						if (superget)
							strcpy(
						...
						get_pos = strlen(get);
						select_get(
						    &v.item[v.selected_item], 0,
						    1);
						select_get(
						    &v.item[v.selected_item], 1,
						    1);
						if (est ==
						    v.item[v.selected_item]
							.estado) {
							ascii = asc;
							kbdFLAGS[28] = kesc;
						} else {
	...
	void process_button(int n, int e)
	{
		...
		switch (e) {
		case 0:
			wwrite(v.ptr, v.an / big2, v.al / big2,
			       v.item[n].button.x, v.item[n].button.y,
			       v.item[n].button.center, v.item[n].button.texto,
			       c3);
			break;
		case 1:
	...
	int get_status(int n)
	{
		...
		if ((ascii && (ascii != 0x1b) &&
			...
			if (v.click_handler != MapperCreator2 ||
			    (ascii >= '0' && ascii <= '9')) {
				if (!(x & 2)) {
					if (ascii == 13)
						ascii = 0;
	...
	void process_flag(int n, int e)
	{
		...
		case 0:
			wwrite(v.ptr, v.an / big2, v.al / big2,
			       v.item[n].flag.x + 8, v.item[n].flag.y, 0,
			       v.item[n].flag.texto, c3);
			break;
		case 1:
			wwrite(v.ptr, v.an / big2, v.al / big2,
			       v.item[n].flag.x + 8, v.item[n].flag.y, 0,
		...
		case 2:
			...
			if (v.item[n].estado == 1) {
				v.active_item = n;
				if (*v.item[n].flag.valor =
					!*v.item[n].flag.valor)
					wput(v.ptr, v.an / big2, v.al / big2,
					     v.item[n].flag.x, v.item[n].flag.y,
					     -59);
				else
					wput(v.ptr, v.an / big2, v.al / big2,
					     v.item[n].flag.x, v.item[n].flag.y,
	...
	void get_input(int n)
	{
		...
		case 8:
			...
			if (!*get && superget)
				strcpy(
				    (char *)v.item[v.selected_item].get.buffer,
				    "");
			v.volcar = 1;
			break;
		case 13:
	...
	{ // Minimiza: se desactiva
		...
		if (v.estado) {
			...
			if (text_len(v.titulo) + 3 > an - 20) {
				wwrite_in_box(v.ptr, an, an - 19, al, 4, 2, 0,
					      v.titulo, c0);
				wwrite_in_box(v.ptr, an, an - 19, al, 3, 2, 0,
					      v.titulo, c2);
			} else {
				wwrite(v.ptr, an, al, 2 + (an - 20) / 2, 3, 1,
				       v.titulo, c0);
				wwrite(v.ptr, an, al, 2 + (an - 20) / 2, 2, 1,
				       v.titulo, c2);
			}
			for (m = 1; m < max_windows; m++)
				if (ventana[m].tipo == v.tipo &&
			...
			if (v.tipo == 102)
				call((voidReturnType)
					 v.paint_handler); // Borra cursor
		}
	}

	void activar(void)
	{	      // Maximiza: se activa *** OJO *** se llama en varias
		...
		if (text_len(v.titulo) + 3 > an - 20) {
			wwrite_in_box(v.ptr, an, an - 19, al, 4, 2, 0, v.titulo,
				      c1);
			wwrite_in_box(v.ptr, an, an - 19, al, 3, 2, 0, v.titulo,
				      c4);
		} else {
			wwrite(v.ptr, an, al, 3 + (an - 20) / 2, 2, 1, v.titulo,
			       c1);
			wwrite(v.ptr, an, al, 2 + (an - 20) / 2, 2, 1, v.titulo,
			       c4);
		}
		for (m = 1; m < max_windows; m++)
			if (ventana[m].tipo == v.tipo && ventana[m].estado) {
				ventana[m].estado = 0;
				wgra(ventana[m].ptr, ventana[m].an / big2,
				     ventana[m].al / big2, c1, 2, 2,
				     ventana[m].an / big2 - 20, 7);
				if (text_len(ventana[m].titulo) + 3 >
					...
					wwrite(ventana[m].ptr,
					       ...
					       2 + (ventana[m].an / big2 - 20) /
						       2,
					       3, 1, ventana[m].titulo, c0);
					wwrite(ventana[m].ptr,
					       ventana[m].an / big2,
					       ventana[m].al / big2,
					       2 + (ventana[m].an / big2 - 20) /
						       2,
					       2, 1, ventana[m].titulo, c2);
				}
				if (v.tipo == 102 &&
				    ventana[m].tipo == 102) { // Borra cursor
					wup(m);
					call((voidReturnType)v.paint_handler);
					wdown(m);
				}
	...
	void Save_Cfgbin()
	{
		...
		if (modo_de_retorno == 3) {
			Setupfile.Vid_modeBig =
			    (modo_anterior & 0x8000000) >> 31;
...
*/
	...
	void Load_Cfgbin()
	{
		...
		if (file == NULL) {
			if (primera_vez) {
				strcpy(Setupfile.Desktop_Image,
				       (char *)
					   texto[487]); // Informacion del tapiz
				Setupfile.Desktop_Gama = 1;
				Setupfile.Desktop_Tile = 0;
				for (n = 0; n < 8; n++)
					Setupfile.t_gama[n].selec = 0;
				...
				memcpy(&Setupfile.colors_rgb[0],
				       "\x1c\x1c\x1c\x3d\x3d\x3c\x0\x0\x20\x2"
				       "\x6\x7\x0\x27\x0\x3d\x3d\x3c\x14\x14"
				       "\x14\x3d\x3d\x3c\x0\x0\x2e\x34\x31\x31"
				       "\x22\x13\x13\x26\x26\x0",
				       12 * 3);
		...
		if (test_video && vga_an < 640 && editor_font > 1)
			editor_font = 0;
	...
	void check_free(void)
	{
		...
		totfree =
		    GetFreeUnid((toupper(tipo[1].path[0]) - 'A') + 1) / 1024;

		if (totfree < 4096) {
			v_titulo = t1;
			v_texto = t2;
			sprintf(t1, (char *)texto[377],
				toupper((memptrsize)tipo[1].path[0]));
	...
	int determina_calc(void)
	{
		...
		for (m = 0; m < max_windows; m++) {
			if (ventana[m].click_handler == calc2 &&
			    ventana[m].estado) {
				n = m;
				break;
			}
	...
	void copy(int a, int b)
	{
		...
		if (a == -1) {
			memcpy(&vaux[ivaux].tipo, &ventana[b].tipo,
			       sizeof(struct tventana));
		} else if (b == -1) {
			memcpy(&ventana[a].tipo, &vaux[ivaux].tipo,
			       sizeof(struct tventana));
		} else {
			memcpy(&ventana[a].tipo, &ventana[b].tipo,
			       sizeof(struct tventana));
		}
	}
	...
	void move(int a, int b)
	{
		if (a < b) {
			copy(-1, b);
			memmove(&ventana[a + 1].tipo, &ventana[a].tipo,
				sizeof(struct tventana) * (b - a));
			copy(a, -1);
		} else if (b < a) {
			copy(-1, b);
			memmove(&ventana[b].tipo, &ventana[b + 1].tipo,
				sizeof(struct tventana) * (a - b));
			copy(a, -1);
		}
	}
	...
	{
		memmove(&ventana[a].tipo, &ventana[a + 1].tipo,
			sizeof(struct tventana) * (max_windows - 1 - a));
	...
	void addwindow(void)
	{
		memmove(&ventana[1].tipo, &v.tipo,
			sizeof(struct tventana) * (max_windows - 1));
	}
	...
	void GetFree4kBlocks(void)
	{
		...
		if ((f = fopen("C:/DIV/FREEDIV.TXT", "a")) != NULL) {
			...
			fprintf(
			    ...
			    DOScount, DOScount * 4, DPMIcount, DPMIcount * 4,
