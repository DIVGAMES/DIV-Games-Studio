...
long kbloque =
    0; // 0-n/a 1-pivotante(kini,kcol1) 2-completo(kini,kcol1,kfin,kcol2)
...
void programa2(void)
{
	...
	if (wmouse_x != -1 && v.estado) {
		if ((mouse_b & 1) && mouse_y >= v.y + 18 * big2 &&
		    mouse_x >= v.x + 2 * big2) {
			n = v.prg->primera_linea +
			    (mouse_y - (v.y + 18 * big2)) / editor_font_al;
			m = v.prg->primera_columna +
			    (mouse_x - (v.x + 2 * big2)) / editor_font_an;
			if (n >= v.prg->primera_linea &&
			    n < v.prg->primera_linea + v.prg->al &&
			    m >= v.prg->primera_columna &&
			    m < v.prg->primera_columna + v.prg->an) {
				if (!(old_mouse_b & 1)) {
					bloque = 1;
					bloque_x = m;
					bloque_y = n;
...
*/
	...
	if (mouse_graf == 13 && (mouse_b & 1) && wmouse_x != -1) {
		...
		if (forced_slider < min)
			...
			forced_slider = max;
		n = 1 + ((v.prg->num_lineas - 1) * (forced_slider - min)) /
			    (max - min);
		if (n < 1)
			n = 1;
		else if (n > v.prg->num_lineas)
			n = v.prg->num_lineas;
		while (v.prg->linea > n) {
			write_line();
			retrocede_lptr();
	...
	if (mouse_graf == 14 && !(old_mouse_b & 1) && (mouse_b & 1) &&
	    wmouse_x != -1) {
		if (!(v.botones & 64)) {
...
void editor()
{
	...
	if (numero_error != -1 && eprg == v.prg) {
		if ((scan_code <= 1 || scan_code == 59) && ascii == 0 &&
		    mouse_b == 0) {
			_parcial();
			error_cursor2();
			v.volcar++;
	...
	if (edited != v.prg) { // Si se estaba editando otro ...
		for (n = 0; n < max_windows; n++)
			if (ventana[n].tipo == 102 && ventana[n].prg != NULL &&
			    ventana[n].prg == edited)
				break;
		if (n < max_windows) {
			wup(n);
			write_line();
	...
	if (bloque_edit == 1) // Bloque edit en una línea
		...
		if ((shift_status & 3) && !(shift_status & 12))
			...
			case 0:
				break;
			case 77:
				if (kcol2 + 1 == v.prg->columna)
					kcol2++;
				else if (kcol1 == v.prg->columna)
					kcol1++;
				if (kcol1 > kcol2)
					f_desmarcar();
				f_right();
			...
			case 0:
				break;
			case 45:
				f_cortar_bloque(1); // ctrl+x
				bloque_edit = 0;
				scan_code = 0;
				break;
			case 46:
				f_cortar_bloque(0); // ctrl+c
				scan_code = 0;
				break;
			case 47:
				f_cortar_bloque(2); // ctrl+v
				f_pegar_bloque();
				f_desmarcar();
				scan_code = 0;
				break;
	...
	if (bloque_edit == 0) // Solo si no hay un bloque de tipo edit
		...
		if ((shift_status & 3) && !(shift_status & 12))
			...
			case 15:
				...
				break; // shift+tab
			case 82:
				f_pegar_bloque(); // shift+insertar
				f_desmarcar();
				break;
			case 83:
				f_cortar_bloque(1);
				break; // shift+suprimir
			case 71:       // shift+inicio
				f_desmarcar();
				bloque_edit = 1;
				kbloque = 2;
...
void comprobar_memoria(int bloque_lon)
{
	...
	if (v.prg->buffer_lon <= v.prg->file_lon + bloque_lon + buffer_min) {
		write_line();
		p = (byte *)realloc(v.prg->buffer,
				    v.prg->file_lon + bloque_lon + buffer_grow);
		if (p != NULL) {
...
int linelen(byte *p)
{
	...
	if (p == v.prg->lptr)
		...
		while (*p != cr && p < v.prg->buffer + v.prg->file_lon) {
			p++;
			n++;
		}
		return (n);
	...
	if (kbloque == 2)
		if (kprg == v.prg) {
			if (in_block()) {
				kbloque = 1;
				kini = v.prg->lptr;
				if ((kcol1 = v.prg->columna) > strlen(v.prg->l))
					kcol1 = strlen(v.prg->l) + 1;
			} else if (v.prg->lptr < kini ||
				   (v.prg->lptr == kini &&
				    v.prg->columna < kcol1)) {
				kini = v.prg->lptr;
				kcol1 = v.prg->columna;
			...
			for (n = 0; n < max_windows; n++)
				if (ventana[n].tipo == 102 &&
				    ventana[n].prg == kprg && kprg != NULL)
					break;
			if (n < max_windows) {
				wup(n);
				f_desmarcar();
		...
		kbloque = 2;
		kfin = v.prg->lptr;
		if ((kcol2 = v.prg->columna) > strlen(v.prg->l))
			kcol2 = strlen(v.prg->l) + 1;
...
void f_desmarcar(void)
{
	...
	if (kbloque && kprg != v.prg) {
		for (n = 0; n < max_windows; n++)
			if (ventana[n].tipo == 102 && ventana[n].prg == kprg &&
			    kprg != NULL)
				break;
		if (n < max_windows) {
			wup(n);
...
{ // 0-Copiar, 1-Cortar, 2-Borrar
	...
	if (kcol1 == 1 && kcol2 == linelen(kfin) + 2)
		t_p = 1; // Tipo de papelera -> líneas

	k2 = kfin + kcol2 - 1; // Fija k2

	k1 = v.prg->buffer + v.prg->file_lon; // Si se marco el crlf inexistente
					      // del final, lo crea
	if (k2 > k1) {
		*k1++ = cr;
		*k1++ = lf;
	...
	if (borrar != 2) {
		...
		if ((papelera = (char *)malloc(lon_papelera = k2 - k1 + 1)) ==
		    NULL) {
	...
	if (borrar) {
		...
		if (in_block()) { // lptr dentro del bloque
			...
			v.prg->lptr -= k2 - k1 + 1;
		} else if (v.prg->lptr == kfin &&
			   v.prg->columna > kcol2) { // columna tras el bloque
			v.prg->linea -= num_lineas;
			v.prg->lptr = kini;
			v.prg->columna += kcol1 - (kcol2 + 1);
		}
		...
		if (k2 < v.prg->buffer + v.prg->file_lon)
			memmove(k1, k2 + 1,
				(byte *)v.prg->buffer + v.prg->file_lon - k2);
...
void test_cursor2(void)
{
	...
	while (v.prg->linea < v.prg->primera_linea)
		retrocede_vptr();
}
...
void f_pegar_bloque(void)
{
	...
	if (papelera != NULL) {
		...
		kprg = v.prg;
		kini = v.prg->lptr;
		kcol1 = v.prg->columna; // Fija kini

		// Rellena con espacios desde el final de la línea actual hasta
		// el cursor
		...
		ini = v.prg->lptr + v.prg->columna - 1 -
		      espacios; // Dode va a pegar el bloque

		if (ini < v.prg->buffer + v.prg->file_lon)
			memmove(ini + espacios + lon_papelera, ini,
				(byte *)v.prg->buffer + v.prg->file_lon - ini);

		memset(ini, ' ', espacios); // Pega los espacios

		memcpy(ini + espacios, papelera,
		       lon_papelera); // Pega la papelera
		...
		n = 0;
		p = v.prg->lptr; // Si la linea actual excede de 1023 chars, la
				 // corta
		while (*p != cr && p < v.prg->buffer + v.prg->file_lon) {
			p++;
			n++;
...
void f_avpag(void) { f_scroll(v.prg->al); }

void f_repag(void) { f_mscroll(v.prg->al); }

void f_insert(void) { modo_cursor ^= 1; }
...
void f_suprimir(void)
{
	...
	if (strlen(v.prg->l) < v.prg->columna) { // Junta dos líneas
		if (v.prg->lptr + v.prg->line_size + 2 <=
		    v.prg->buffer + v.prg->file_lon) {
			for (n = strlen(v.prg->l); n < v.prg->columna - 1; n++)
				v.prg->l[n] = ' ';
			p = v.prg->lptr + v.prg->line_size +
			    2; // Inicio de la siguiente línea
			n = v.prg->columna - 1;
			n_chars = 0;
			if (p == kini) {
				kini = v.prg->lptr;
			...
			while (p < v.prg->buffer + v.prg->file_lon &&
			       *p != cr) {
				n_chars++;
				if (n < long_line - 1)
					v.prg->l[n++] = *p++;
		...
		for (n = v.prg->columna - 1; n < strlen(v.prg->l); n++)
			v.prg->l[n] = v.prg->l[n + 1];
		if (kini == v.prg->lptr && kcol1 > v.prg->columna)
			kcol1--;
		if (kfin == v.prg->lptr)
			if (kcol2 > v.prg->columna)
				kcol2--;
...
void f_backspace(void)
{
	quitar_espacios();
	ascii = 0;
	if (strlen(v.prg->l) < v.prg->columna - 1)
		f_left();
...
void f_insertar(void)
{
	...
	if (v.prg->columna < long_line) {
		if (strlen(v.prg->l) < v.prg->columna) {
			...
			for (n = strlen(v.prg->l); n >= v.prg->columna; n--)
				v.prg->l[n] = v.prg->l[n - 1];
			v.prg->l[n] = ascii;
...
void f_enter(void)
{
	...
	if ((lon = strlen(v.prg->l)) < v.prg->columna) { // Enter normal
		...
		if (kini == v.prg->lptr && kcol1 >= v.prg->columna) {
			kini += n;
			kcol1 = kcol1 - v.prg->columna + t + 1;
		}
		if (kfin == v.prg->lptr && kcol2 >= v.prg->columna) {
			kfin += n;
			kcol2 = kcol2 - v.prg->columna + t + 1;
		}
...
void f_word_right(void)
{
	...
	do {
		...
		i = 1;
		if (!lower[v.prg->l[v.prg->columna - 1]])
			i = 0;
		else if (lower[v.prg->l[v.prg->columna - 2]])
			i = 0;
	} while (!i);
}
...
void f_eof(void)
{
	while (v.prg->lptr + v.prg->line_size <
	       v.prg->buffer + v.prg->file_lon) {
		write_line();
		avanza_lptr();
		read_line();
...
{ // Función para obtener los colores de la siguiente línea
...
void _completo(void)
{
	...
	al = v.al / big2;
#ifdef TTF
	SDL_FillRect(v.surfaceptr, &rc,
		     SDL_MapRGB(v.surfaceptr->format, colors[ce01].r,
				colors[ce01].g, colors[ce01].b));
#endif
	if (kbloque && kprg == v.prg) {
		if (kcol1 > linelen(kini))
	...
	*(v.prg->buffer + v.prg->file_lon) = cr;
	...
	do {
		old_di = di;
		n = editor_font_al;
		if (kbloque == 0 || kprg != v.prg || si < kini ||
		    si > kfin) { // Fuera del bloque
			...
			while (ancho--) {
				...
				if (x >= col0 && x <= col1) {
					while (n--) {
						memset(di, ce4, editor_font_an);
						di += v.an;
					}
				} else {
					while (n--) {
						memset(di, ce1, editor_font_an);
						di += v.an;
					}
				}
				di -= v.an * editor_font_al - editor_font_an;
				x++;
			}
		}
		di = old_di;
		...
		if (alto == 1 && big &&
		    (editor_font_al * v_prg->al & 1)) { // Fix ventana impar
			if (*di == ce01)
				memset(di + v.an * editor_font_al, ce01,
				       v.an - 24);
			else
				memset(di + v.an * editor_font_al, ce1,
				       v.an - 24);
		}
		...
		if (si != v.prg->lptr) {
			...
			while (--ancho && *si != cr &&
			       si < v.prg->buffer + v.prg->file_lon) {
				si++;
				i++;
			}
			ancho = v.prg->an;
		...
		while (*si != cr && si < v.prg->buffer + v.prg->file_lon)
			si++;
		if (si < v.prg->buffer + v.prg->file_lon)
			si += 2;
		else
...
void _parcial(void)
{
	...
	old_di = di =
	    v.ptr + (v.an * 18 + 2) * big2 + linea * v.an * editor_font_al;
	si = v.prg->lptr;
...
void barras_desplazamiento(void)
{
	...
	min = 18;
	max = al - 21;
	if (v.prg->num_lineas <= 1)
		slider = min;
	else
		slider = min + ((v.prg->primera_linea - 1) * (max - min)) /
				   (v.prg->num_lineas - 1);
	if (forced_slider) {
	...
	min = 10;
	max = an - 21;
	slider = min + (v.prg->primera_columna * (max - min)) /
			   (long_line - v.prg->an);
...
int get_slide_y(void)
{
	...
	if (v.prg->num_lineas <= 1)
		...
		return (min + ((v.prg->primera_linea - 1) * (max - min)) /
				  (v.prg->num_lineas - 1));
}
...
int get_slide_x(void)
{
	...
	min = 10;
	max = an - 21;
	return (min + (v.prg->primera_columna * (max - min)) /
			  (long_line - v.prg->an));
}
...
void barra_info(void)
{
	...
	itoa(v.prg->columna, num + 1, 10);
	num[0] = ',';
	wwrite_in_box(v.ptr, an, an - 19, al, 3 + ancho, 10, 0, (byte *)num,
		      c3);
	ancho += text_len((byte *)num) + 1;
	//  itoa(v.prg->num_lineas,num+1,10); num[0]='('; num[strlen(num)+1]=0;
	//  num[strlen(num)]=')';
	//  wwrite_in_box(v.ptr,an,an-19,al,3+ancho,10,0,num,c3);
	//  ancho+=text_len(num)+1;
}
...
void resize(void)
{
	...
	do {
		...
		if (v.prg->an < 4 * big2) {
			v.prg->an = 4 * big2;
			mx = _mx + (v.prg->an - _an) * editor_font_an;
		}
		if (v.prg->al < 2 * big2) {
			v.prg->al = 2 * big2;
			my = _my + (v.prg->al - _al) * editor_font_al;
		}
		if (v.prg->an > 80 * 2) {
			v.prg->an = 80 * 2;
			mx = _mx + (v.prg->an - _an) * editor_font_an;
		...
		if ((new_block = (byte *)realloc(v.ptr, v.an * v.al)) != NULL) {
			...
			test_cursor();
			repinta_ventana();
			wput(v.ptr, v.an / big2, v.al / big2, v.an / big2 - 9,
			     v.al / big2 - 9, -44);
			se_ha_movido_desde(v.x, v.y, an, al);
			actualiza_caja(v.x, v.y, v.an > an ? v.an : an,
				       v.al > al ? v.al : al);
		} else {
			v.prg->an = old_an;
			v.prg->al = old_al;
...
void maximizar(void)
{
	...
	_an = (vga_an - 12 * big2) /
	      editor_font_an; // Calcula tamaño (en chr) maximizada
	...
	if (v.prg->an != _an || v.prg->al != _al) { // *** Maximiza ***
		...
		if ((new_block = (byte *)realloc(v.ptr, v.an * v.al)) != NULL) {
			...
			if (exploding_windows)
				extrude(v.prg->old_x, v.prg->old_y, _an, _al,
					v.x, v.y, v.an, v.al);
			v.volcar = 2;
			se_ha_movido_desde(v.prg->old_x, v.prg->old_y, _an,
					   _al);
		...
		if ((new_block = (byte *)realloc(v.ptr, v.an * v.al)) != NULL) {
			...
			v.x = v.prg->old_x;
			v.y = v.prg->old_y;
			if (v.x > vga_an - 8 * big2)
				v.x = vga_an - 32 * big2;
			if (v.y > vga_al - 8 * big2)
				v.y = vga_al - 32 * big2;
			if (exploding_windows)
				extrude(_x, _y, _an2, _al2, v.x, v.y, v.an,
					v.al);
			v.volcar = 2;
...
void repinta_ventana(void)
{
	...
	if (v.tipo == 1) { // Los diálogos no se minimizan
		...
		if (text_len(v.titulo) + 3 > an - 12) {
			wwrite_in_box(v.ptr, an, an - 11, al, 4, 2, 0, v.titulo,
				      c1);
			wwrite_in_box(v.ptr, an, an - 11, al, 3, 2, 0, v.titulo,
				      c4);
		} else {
			wwrite(v.ptr, an, al, 3 + (an - 12) / 2, 2, 1, v.titulo,
			       c1);
			wwrite(v.ptr, an, al, 2 + (an - 12) / 2, 2, 1, v.titulo,
			       c4);
		}
	} else {
	...
	if (kbloque & 1)
		...
		if (v.prg->lptr > kini && v.prg->lptr < kfin)
			...
			if (v.prg->lptr == kfin) {
				...
				col1 = long_line;
			if (v.prg->columna >= col0 && v.prg->columna <= col1)
				return (1);
			else
				return (0);
		}
...
void text_cursor(void)
{
	...
	if (modo_cursor) {
		...
		alto = editor_font_al / 2;
		di = old_di + v.an * (editor_font_al - alto);
	}
...
void error_cursor2(void)
{
	...
	di = old_di;
	do {
		memset(di, c_r_low, editor_font_an);
		di += v.an;
	} while (--alto);
...
*/
	...
	if (tsurface != NULL) {
		SDL_Surface *surface =
		    drawtext(editorfont, colors[color].r, colors[color].g,
			     colors[color].b, 0, 0, 0, 0, 0, (char *)s, solid);
		rc.h = editor_font_al;
		rc.w = editor_font_an;
...
void write_line(void)
{
	...
	ini =
	    v.prg->lptr + v.prg->line_size; // Donde está el cr,lf de esta línea
...
void delete_line(void)
{
	...
	ini = v.prg->lptr +
	      (v.prg->line_size + 2); // Donde está el cr,lf de esta línea
	fin = v.prg->lptr + strlen(v.prg->l); // Donde debe ir
...
void programa0_nuevo(void)
{
	...
	if (v_terminado) {
		...
		if (f) {
			...
			fprintf(f, "/*\n * %s%s by %s\n * (c) %d %s \n */\n\n",
				fname, ext, user1, (timeinfo->tm_year) + 1900,
				user2);
			fprintf(f,
				"PROGRAM %s;\n\nBEGIN\n\n//Write your code "
				"here, make something amazing!\n\nEND\n\n",
				fname);
			fclose(f);
			abrir_programa();
...
void guardar_prg(void)
{
	...
	if ((f = fopen(full, "wb")) != NULL) {
		...
		fclose(f); // *** OJO, se debe borrar el fichero si no se pudo
			   // grabar entero
...
void buscar_texto1(void) { _show_items(); }
...
void buscar_texto(void)
{
	...
	while ((v.prg->linea < v.prg->num_lineas ||
		v.prg->columna <= strlen(v.prg->l)) &&
	       !encontrado) {
		if (v.prg->columna > strlen(v.prg->l)) {
			...
			if (cadena_encontrada(buscar,
					      &v.prg->l[v.prg->columna - 1],
					      may_min, completa))
				encontrado = 1;
			else
				f_right();
	...
	if (!encontrado) { // Restaura variables y emite dialogo informativo
		...
		v_titulo = (char *)texto[347];
		v_texto = (char *)texto[189];
		dialogo(info0);
	} else {
		n = strlen(buscar);
		while (n--)
...
void sustituir_texto1(void) { _show_items(); }
...
void sustituir_texto(void)
{
	...
	do {
		...
		while ((v.prg->linea < v.prg->num_lineas ||
			...
			if (v.prg->columna > strlen(v.prg->l)) {
				...
				if (cadena_encontrada(
					buscar2, &v.prg->l[v.prg->columna - 1],
					may_min2, completa2))
					encontrado = 1;
				else
					f_right();
		...
		if (encontrado) {
			...
			kcol1 = v.prg->columna;
			kfin = v.prg->lptr;
			kcol2 = v.prg->columna + strlen(buscar2) - 1;
			n = strlen(buscar2);
			while (n--)
				f_right();
			n = strlen(buscar2);
...
void sustituir0(void)
{
	...
	v.tipo = 1;
	v.titulo = texto[190];
	v.an = x4 + text_len(texto[101] + 1) + 7;
	v.al = 29;
...
{
	_show_items();
	wwrite(v.ptr, v.an / big2, v.al / big2, (v.an / big2) / 2, 12, 1,
	       (byte *)sus, c3);
}
...
void pintar_lista_procesos(void)
{
	...
	wbox(ptr, an, al, c1, 4, 20, 128 + 132 - 10,
	     121); // Límites listbox procesos

	end = ventana[1].prg->buffer + ventana[1].prg->file_lon;

	for (m = lp_ini; m < lp_ini + 15 && m < lp_num; m++) {
		if (m == lp_select) {
			wbox(ptr, an, al, c01, 4, 20 + (m - lp_ini) * 8,
			     150 + 100, 9); // Relleno listbox procesos
			x = c4;
		} else
			x = c3;
		p = lp2[m];
		...
		cwork[n] = 0;
		wwrite_in_box(ptr, an, 153 + 100, al, 5, 21 + (m - lp_ini) * 8,
			      0, (byte *)cwork, x);
	}
...
void pinta_segmento_procesos(void)
{
	...
	wbox(ptr, an, al, c2, 123 + 132, 28, 7,
	     max - min + 3); // Borra la barra del slider
...
void lista_procesos1(void)
{
	...
	wrectangulo(ptr, an, al, c0, 3, 19, 128 + 132,
		    123); // Límites listbox procesos
	wrectangulo(ptr, an, al, c0, 122 + 132, 19, 9, 123);
	wrectangulo(ptr, an, al, c0, 122 + 132, 27, 9, 123 - 16);

	wput(ptr, an, al, 123 + 132, 20,
	     -39); // Boton arriba / abajo (pulsados 41,42)
	wput(ptr, an, al, 123 + 132, 174 - 40, -40);

	crear_lista_procesos((char *)ventana[1].prg->buffer,
			     ventana[1].prg->file_lon);
...
void goto_error(void)
{
	...
	if (n)
		move(0, n); // Si no es la ventana[0] la sube a primer_plano
	...
	if (linea_error > v.prg->num_lineas) {
		linea_error = v.prg->num_lineas;
		m = 1;
	} else
		m = 0;
	if (linea_error < 1)
	...
	if (v.prg->linea <= v.prg->primera_linea)
		retrocede_vptr();
	if (v.prg->linea - v.prg->primera_linea >= v.prg->al - 1)
		avanza_vptr();
...
void printprogram1(void) { _show_items(); }
