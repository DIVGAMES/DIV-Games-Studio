...
double evaluar(void)
{
	...
	do {
		...
		case p_num:
			...
			break;
		case p_or:
			pila[sp - 1] = (double)((memptrsize)pila[sp - 1] |
						(memptrsize)pila[sp]);
			sp--;
			break;
		case p_xor:
...
void get_token(void)
{
	...
	reget_token:
		...
		case '!':
			...
			break;
		default:
			if (lower[*(expresion - 1)] >= 'a' &&
			    lower[*(expresion - 1)] <= 'z') {
				n = 1;
				cwork[0] = lower[*(expresion - 1)];
				while (n < 31 && lower[*expresion] >= 'a' &&
				       lower[*expresion] <= 'z') {
...
double get_num(void)
{ // Lee el número que hay en *expresion (double en hex o dec)
	double x = 0;
	double dec = 10;

	if (*expresion == '0' &&
	    lower[*(expresion + 1)] == 'x') { // Numeros en hex

		expresion += 2;
		if ((*expresion >= '0' && *expresion <= '9') ||
		    (lower[*expresion] >= 'a' && lower[*expresion] <= 'f')) {
			do {
				if (*expresion >= '0' && *expresion <= '9')
					x = x * 16 + *expresion++ - 0x30;
				else
					x = x * 16 + lower[*expresion++] - 'a' +
					    10;
...
void calc2(void)
{
	...
	if (v.active_item >= 0 &&
	    strlen(pcalc->ctext)) { // Se evalúa la expresión
		expresion = pcalc->ctext;
		calcular();

		if (token == p_num) {
			if (pcalc->chex)
				sprintf(pcalc->cresult, "0x%x",
					(memptrsize)tnumero);
			else if (pcalc->cint)
				sprintf(pcalc->cresult, "%d",
					(memptrsize)tnumero);
			else
				sprintf(pcalc->cresult, "%g", tnumero);
		} else
			strcpy(pcalc->cresult, (char *)texto[417]);
