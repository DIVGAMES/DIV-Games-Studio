	...
	if (mouse_x != (int)m_x || mouse_y != (int)m_y || mouse_b != m_b) {
		...
		if (vga_an != vwidth || vga_al != vheight) {
			...
			mouse_x =
			    (int)(m_x *
				  (float)((float)vga_an /
					  (float)vwidth)); // / (float)vga_an);
			mouse_y =
			    (int)(m_y *
				  (float)((float)vga_al /
					  (float)vheight)); // / (float)vga_al);
		}
#endif

	} else if (modo < 100 && hotkey &&
		   !help_paint_active) { // Las teclas están solo activas en
					 // edición
		...
		if (!(shift_status & 4)) {
			...
			if ((shift_status & 3) || key(_L_SHIFT) ||
			    key(_R_SHIFT))
				s = 8;
			else
				s = 1;
...
*/
	...
	if (free_drag) {
		...
		case 2:
			if (mouse_b & 1) {
				if (abs(mouse_x - arrastrar_x) > 1 ||
				    abs(mouse_y - arrastrar_y) > 1) {
					arrastrar = 3;
					wmouse_x = -1;
					wmouse_y = -1;
					mouse_b &= ~1;
...
void libera_drag(void)
{
	...
	if (n < max_windows && ventana[n].tipo == 101 &&
	    ventana[n].mapa != NULL) {
...
void read_mouse2(void)
{
	...
	//	while(0)
	{
		...
		if (event.type ==
		    SDL_JOYAXISMOTION) { // Analog joystick movement
		...
		if (event.type == SDL_KEYDOWN) {
			...
			if (event.key.keysym.sym == SDLK_LCTRL ||
				...
				scan_code =
				    OSDEP_key[event.key.keysym.sym < 2048
			...
			if (event.key.keysym.unicode < 0x80 &&
			    scan_code != 83 && event.key.keysym.unicode >= 0)
...
#endif
		// SDL_SetVideoMode(event.resize.w, event.resize.h, 8,
		// SDL_HWSURFACE | SDL_RESIZABLE); 				bW = buffer->w; bH =
		//buffer->h;
		...
		if (strcmp(v.titulo, (char *)texto[35])) {
			...
			for (n = max_windows; n >= 0; n--) {
				if (ventana[n].tipo) {
					...
					if (ventana[n].x + ventana[n].an >
					    vga_an)
						ventana[n].x =
						    vga_an - ventana[n].an;

					if (ventana[n].y + ventana[n].al >
					    vga_al)
						ventana[n].y =
						    vga_al - ventana[n].al;
					...
					if (colisiona_con(
						n, ventana[n].x, ventana[n].y,
						ventana[n].an, ventana[n].al))
						emplazar(1, &ventana[n].x,
							 &ventana[n].y,
							 ventana[n].an,
							 ventana[n].al);
				}
