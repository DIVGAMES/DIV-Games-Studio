...
void clexico(void)
{
	...
	switch (
	    (memptrsize)clex_case[*_source]) { // Puntero a un clex_ele o l_???
	...
	case l_cr:
		if ((*++_source) == lf)
			_source++;
		cpieza = p_ultima;
		break; // eol/eof OJO!!! esto es DIVPRO
	...
	case l_id:
		...
		*icvnom.p++ = (byte *)0x100;
		...
		if (!strcmp((char *)(ptr + 2),
			...
			if (cpieza < 256 &&
			    cpieza >= 0) { // palabra reservada (token)
				if (cpieza == p_rem) {
	...
	case l_lit:
		...
		cpieza = p_lit;
		h = *_source;
		_ivnom = icvnom.b; // Literal entre dos h
		do {
			_source++;
