...
int opc_img[8] = {0, 0, 1, 1,
		  0, 1, 1, 1}; // imagenes on/off en ventanas (v_tipo)
...
void imprime_rutabr(void)
{
	...
	if (v_thumb == 7)
		wbox_ancho = an - 4 - 12 - text_len(texto[419]) - 12 -
			     text_len(texto[567]);
	else if (v_tipo == 16)
	...
	wwrite_in_box(v.ptr, an, wbox_ancho + 2, al, 5, 12, 0, (byte *)full,
		      c1);
	wwrite_in_box(v.ptr, an, wbox_ancho + 2, al, 4, 12, 0, (byte *)full,
		      c3);
}
...
void crear_thumbs(void)
{
	if (opc_img[v_thumb]) {
		do {
			...
			{
			case 2:
				crear_un_thumb_MAP(&larchivosbr);
				break;
			case 3:
				crear_un_thumb_PAL(&larchivosbr);
				break;
			case 5:
				crear_un_thumb_FNT(&larchivosbr);
				break;
			case 6:
				crear_un_thumb_IFS(&larchivosbr);
				break;
			case 7:
				crear_un_thumb_PCM(&larchivosbr);
				break;
			}
			if (num > -1) {
				if (thumb[num].ptr != NULL &&
				    thumb[num].status == 0) {
					muestra_thumb(&larchivosbr, num);
					break;
				} else if (thumb[num].ptr == NULL &&
					   thumb[num].status == -1) {
					muestra_thumb(&larchivosbr, num);
					break; // OJO *** Este break no estaba
					       // (elimina parpadeos, pero
					       // enlentece)
				} else
					break;
			} else
...
void crear_un_thumb_MAP(struct t_listboxbr *l)
{
	...
	if (l->maximo) {
		...
		if (estado == 1) { // Read a new thumbnail
			...
			if (strchr(l->lista + (l->lista_an * num), '.') > 0 &&
			...
			} else if ((f = fopen(l->lista + (l->lista_an * num),
					      "rb")) != NULL) {
				fseek(f, 0, SEEK_END);
				...
				if ((thumb[num].ptr = (char *)malloc(
					 thumb[num].filesize)) != NULL) {
					if (thumb[num].filesize > incremento) {
						if (fread(thumb[num].ptr, 1,
							  incremento,
							  f) == incremento) {
							thumb[num].status =
							    incremento;
			...
			return;
			   ...
			   thumb[num].status !=
			       thumb[num].filesize) { // Se continúa leyendo un
						      // thumbnail

			if ((f = fopen(l->lista + (l->lista_an * num), "rb")) !=
			    NULL) {
				fseek(f, thumb[num].status, SEEK_SET);
				if (thumb[num].filesize - thumb[num].status >
				    incremento) {
					if (fread(thumb[num].ptr +
						      thumb[num].status,
						  1, incremento,
						  f) == incremento) {
		...
		if (estado == 2 && thumb[num].status == thumb[num].filesize &&
		    abs(_omx - mouse_x) + abs(_omy - mouse_y) + mouse_b +
			    ascii ==
			0) {
			...
			man = map_an;
			mal = map_al;
			if (es_MAP((byte *)thumb[num].ptr))
				tipomapa = 1;
			else if (es_PCX((byte *)thumb[num].ptr))
				tipomapa = 2;
			else if (es_BMP((byte *)thumb[num].ptr))
				tipomapa = 3;
			else if (es_JPG((byte *)thumb[num].ptr,
					thumb[num].filesize))
				tipomapa = 4;
			else
				tipomapa = 0;
			...
			if (tipomapa) {
				if ((temp = (byte *)malloc(man * mal + man)) !=
				    NULL) {
					...
					create_dac4();
					for (n = 0; n < 256; n++)
						xlat[n] = fast_find_color(
						    pal[n * 3], pal[n * 3 + 1],
						    pal[n * 3 + 2]);

					if (man <= 51 * big2 &&
					    mal <= 31 * big2) { // El grafico se
								// deja tal cual
						...
						if (coefredx > coefredy)
							...
							coefredx = coefredy;
						thumb[num].an =
						    (float)man / coefredx + 0.5;
						thumb[num].al =
						    (float)mal / coefredy + 0.5;
						thumb[num].an &= -2;
						thumb[num].al &= -2;
						if (thumb[num].an < 2)
							thumb[num].an = 2;
...
void crear_un_thumb_PAL(struct t_listboxbr *l)
{
	...
	if (l->maximo && incremento >= FILE_CHUNK) {
		...
		switch (tipo) {
		case 1:
			tipo = cargadac_FPG(l->lista + l->lista_an * num);
			break;
		case 2:
			tipo = cargadac_FNT(l->lista + l->lista_an * num);
			break;
		case 3:
			tipo = cargadac_PCX(l->lista + l->lista_an * num);
			break;
		case 4:
			tipo = cargadac_BMP(l->lista + l->lista_an * num);
			break;
		case 5:
			tipo = cargadac_MAP(l->lista + l->lista_an * num);
			break;
		case 6:
			tipo = cargadac_PAL(l->lista + l->lista_an * num);
			break;
		case 7:
			tipo = cargadac_JPG(l->lista + l->lista_an * num);
			break;
		}
		if (!tipo) {
			estado = 0;
		...
		// Copia paleta a pal, restaura dac4 y crea paleta en xlat a
		// partir de pal
		memcpy(pal, dac4, 768);
		create_dac4();
		for (n = 0; n < 256; n++)
			xlat[n] = fast_find_color(pal[n * 3], pal[n * 3 + 1],
						  pal[n * 3 + 2]);
		...
		if ((thumb[num].ptr = (char *)malloc(
			 (thumb[num].an * thumb[num].al))) == NULL) {
			estado = 0;
			thumb[num].status = -1;
			return;
		}
		...
		for (y = 0; y < thumb[num].al; y += big2) {
			for (x = 0; x < thumb[num].an; x += big2 * 2) {
				if (big) {
					thumb[num].ptr[y * thumb[num].an + x] =
					    xlat[y * 8 + x / 4];
					thumb[num]
					    .ptr[y * thumb[num].an + x + 1] =
					    xlat[y * 8 + x / 4];
					thumb[num]
					    .ptr[(y + 1) * thumb[num].an + x] =
					    xlat[y * 8 + x / 4];
					thumb[num].ptr[(y + 1) * thumb[num].an +
						       x + 1] =
					    xlat[y * 8 + x / 4];
					thumb[num]
					    .ptr[y * thumb[num].an + x + 2] =
					    xlat[y * 8 + x / 4];
					thumb[num]
					    .ptr[y * thumb[num].an + x + 3] =
					    xlat[y * 8 + x / 4];
					thumb[num].ptr[(y + 1) * thumb[num].an +
						       x + 2] =
					    xlat[y * 8 + x / 4];
	...
	if (l->maximo) {
		...
		if (estado == 1) {
			...
			if (thumb[num].filesize <= FILE_CHUNK)
				incremento = FILE_CHUNK;
			if ((thumb[num].ptr =
				 (char *)malloc(thumb[num].filesize)) == NULL) {
				fclose(f);
				estado = 0;
				thumb[num].status = -1;
				return;
			...
			if (thumb[num].filesize > incremento) {
				if (fread(thumb[num].ptr, 1, incremento, f) !=
				    incremento) {
					free(thumb[num].ptr);
					thumb[num].ptr = NULL;
					fclose(f);
		...
		// Se continúa leyendo un thumbnail
			...
			if (thumb[num].filesize - thumb[num].status >
			    incremento) {
				if (fread(thumb[num].ptr + thumb[num].status, 1,
				...
				if (fread(thumb[num].ptr + thumb[num].status, 1,
					  thumb[num].filesize -
					      thumb[num].status,
					  f) !=
				    thumb[num].filesize - thumb[num].status) {
					free(thumb[num].ptr);
					thumb[num].ptr = NULL;
					fclose(f);
					estado = 0;
		...
		if (estado == 2 && thumb[num].status == thumb[num].filesize &&
			...
			init = 0;
			for (x = 0; x < strlen(TestString2); x++) {
				len = ShowCharBuffer(TestString2[x], init, 0,
						     (char *)temp, TamaX,
						     thumb[num].ptr);
				if (len <= 1)
					len = 0;
			...
			if (TamaX > 101 * big2 || TamaY > 22 * big2) {
				// Crea el thumbnail
				coefredx =
				    TamaX / ((float)101 * 2 * (float)big2);
				coefredy =
				    TamaY / ((float)22 * 2 * (float)big2);
				if (coefredx > coefredy)
					coefredy = coefredx;
				else
					coefredx = coefredy;
				...
				if (coefredx * (float)(thumb[num].an - 1) >=
				    (float)TamaX)
					coefredx = (float)(TamaX - 1) /
						   (float)(thumb[num].an - 1);
				if (coefredy * (float)(thumb[num].al - 1) >=
				    (float)TamaY)
					coefredy = (float)(TamaY - 1) /
						   (float)(thumb[num].al - 1);

				if ((temp2 = (byte *)malloc(thumb[num].an *
							    thumb[num].al)) !=
				    NULL) {
					memset(temp2, 0,
					       thumb[num].an * thumb[num].al);

					a = (float)0.0;
					for (y = 0; y < thumb[num].al; y++) {
						b = (float)0.0;
						for (x = 0; x < thumb[num].an;
						     x++) {
							temp2[y * thumb[num]
								      .an +
							      x] =
					...
					if ((thumb[num].ptr = (char *)malloc(
						 (thumb[num].an *
						  thumb[num].al) /
						 4)) != NULL) {
						memset(thumb[num].ptr, 0,
						       (thumb[num].an *
							thumb[num].al) /
							   4);

						for (y = 0; y < thumb[num].al;
						     y += 2) {
							for (x = 0;
							     x < thumb[num].an;
							     x += 2) {
...
void crear_un_thumb_IFS(struct t_listboxbr *l)
{
	...
	if (l->maximo) {
		...
		// Se comienza a leer un nuevo thumbnail
		if ((fifs = fopen(l->lista + (l->lista_an * num), "rb")) ==
		    NULL) {
			thumb[num].status = -1;
			return;
		}
	...
	if (l->maximo) {
...
#endif
			mem = (Mi_meminfo.Bloque_mas_grande_disponible + mem -
			       1000000) /
			      2;
			if ((f = fopen(l->lista + (l->lista_an * num), "rb")) ==
			    NULL) {
		...
		if (estado == 2 && thumb[num].status == thumb[num].filesize &&
			...
			if (IsWAV(l->lista + (l->lista_an * num))) {
				...
				length = *(int *)(WAV + pos);
				pos += 4;
				if ((BufferIn = (char *)malloc(length)) ==
				    NULL) {
					free(thumb[num].ptr);
					thumb[num].ptr = NULL;
					thumb[num].status = -1;
				...
				// Paso 1 de 16 a 8
				if ((MyHeadDC.dwAvgBytesPerSec /
				     MyHeadDC.dwSamplePerSec) /
					MyHeadDC.wChannels ==
				    2) {
					length /= 2;
					BuffAux = (short *)BufferIn;
					if ((BufferOut = (char *)malloc(
						 length)) == NULL) {
						free(BufferIn);
						thumb[num].status = -1;
						return;
			...
			if ((temp = (char *)malloc(thumb[num].filesize)) ==
			    NULL) {
				free(thumb[num].ptr);
				thumb[num].ptr = NULL;
				thumb[num].status = -1;
			...
			memset(thumb[num].ptr, c1,
			       thumb[num].an * thumb[num].al);

			if (thumb[num].filesize > 1)
				if (thumb[num].filesize < 3 * thumb[num].an) {
					step = (float)thumb[num].an /
					       (float)(thumb[num].filesize - 1);
					for (x = 0; x < thumb[num].filesize;
					     x++) {
						y = (temp[x] * thumb[num].al /
						     256);
						if (First) {
							First = 0;
							lx = (memptrsize)
...
void carga_letra(uint8_t letra)
{
	...
	if (fseek(fifs, offset, SEEK_SET))
		error = 1;
	if (fread(&map_al, 2, 1, fifs) < 1)
		error = 1;
	if (fread(&despY, 2, 1, fifs) < 1)
		error = 1;
	if (fread(&map_an, 2, 1, fifs) < 1)
		error = 1;
...
void muestra_thumb(struct t_listboxbr *l, int num)
{
	...
	if (num >= l->inicial && num < l->inicial + l->lineas * l->columnas) {
		...
		if (text_len((byte *)p) < l->an - 2 && opc_img[v_thumb]) {
			...
			wwrite_in_box(ptr, an, px + l->an - 1, al, px + 2, py,
				      6, (byte *)p, c0);
			wwrite_in_box(ptr, an, px + l->an - 1, al, px + 1, py,
				      6, (byte *)p, x);
		}
...
void browser0(void)
{
	...
	lextbr.maximo = 0;
	n = 0; // Crea la lista de extensiones

	if (v_tipo == 2 && v_modo > 0)
		v_tipo = 14;
	if (v_tipo == 7 && v_modo > 0)
		v_tipo = 11;
	...
	_get(126, 4, v.al - 21,
	     v.an - (24 + text_len(texto[100]) + text_len(texto[101])),
	     (byte *)input, 512, 0, 0);
	...
	if (v_thumb)
		_flag(419, v.an - 12 - text_len(texto[419]), 12,
		      &opc_img[v_thumb]);
	...
	lunidadesbr.creada = 0;
	lextbr.creada = 0;

	n = 0;
	while (unidades[n]) { // Crea la lista de unidades <X:>
		*(unidad + an_unidad * n) = '<';
		*(unidad + an_unidad * n + 1) = unidades[n];
	...
	if (v_thumb == 7 && false /*judascfg_device == DEV_NOSOUND*/)
		opc_pru = 0;
}
...
void browser2(void)
{
	...
	if (v.active_item == 3) {
		if (opc_img[v_thumb]) {
			...
			larchivosbr.s1 = larchivosbr.y +
					 (larchivosbr.al * larchivosbr.lineas +
					  larchivosbr.lineas + 1) -
					 12;
			browser1();
			v.volcar = 1;
		} else {
	...
	if ((mouse_b & 1) && !(old_mouse_b & 1)) {
		if (larchivosbr.zona >= 10) {
			if (v_tipo != 6 && !v_modo) {
				...
				{
					for (num = 0; num < larchivosbr.maximo;
					     num++)
						thumb[num].tagged = 0;
					num_taggeds = 0;
					selected = larchivosbr.zona - 10 +
						   larchivosbr.inicial;
				...
				for (num = 0; num < larchivosbr.maximo; num++)
					thumb[num].tagged = 0;
				selected =
				    larchivosbr.zona - 10 + larchivosbr.inicial;
				thumb[selected].tagged = 1;
				num_taggeds = 1;
				ini_tagged = selected;
			}
			...
			browser1();
			v.volcar = 1;
			strcpy(full, archivo + (larchivosbr.zona - 10 +
						larchivosbr.inicial) *
						   an_archivo);

			// OJO !!! Tener en cuenta CTRL y SHIFT
			if (strcmp(input, full) ||
			    ((v_thumb == 7 || v_tipo == 16) && opc_pru)) {
				strcpy(input, full);
				browser1();
				v.volcar = 1;
#ifdef MIXER
				...
				if (v_thumb == 7 && opc_pru) {
...
#endif
					{
						...
						if (smp == NULL) {
							debugprintf("failed to "
								    "load %s\n",
								    full);

						} else {
							//              smp=SI->sample;
							Mix_PlayChannel(0, smp,
									0);
							//              judas_playsample(smp,
							//              0,
							//              SI->SoundFreq,
							//              64*256,
							//              MIDDLE);
				...
				if (num_taggeds == 1)
					v_existe = 1, v_terminado = 1;
#endif
			}

		} else if (ldirectoriosbr.zona >= 10) {
			v.volcar = 1;
			if (tipo[v_tipo].path[strlen(tipo[v_tipo].path) - 1] !=
			    '/')
				strcat(tipo[v_tipo].path, "/");
			strcat(tipo[v_tipo].path,
			       directorio + (ldirectoriosbr.zona - 10 +
					     ldirectoriosbr.inicial) *
						an_directorio);
			chdir(tipo[v_tipo].path);
			...
			_dos_setdrive(unidades[lunidadesbr.zona - 10 +
					       lunidadesbr.inicial] -
					  'A' + 1,
				      &n);
			getcwd(tipo[v_tipo].path, PATH_MAX + 1);
			if (tipo[v_tipo].path[0] ==
			    unidades[lunidadesbr.zona - 10 +
				     lunidadesbr.inicial]) {
				imprime_rutabr();
				v.volcar = 1;
				larchivosbr.creada = 0;
				...
				_dos_setdrive(tipo[v_tipo].path[0] - 'A' + 1,
					      &n);
				v_texto = (char *)texto[42];
				dialogo(err0);
				return;
			}
		} else if (lextbr.zona >= 10) {
			v.volcar = 1;
			tipo[v_tipo].defecto =
			    lextbr.zona - 10 + lextbr.inicial;
			strcpy(input,
			       ext + (lextbr.zona - 10 + lextbr.inicial) *
					 an_ext);
			strcpy(mascara, input);
			tipo[v_tipo].inicial = 0;
			imprime_rutabr();
...
void dir_abrirbr(void)
{
	...
	qsort(archivo, larchivosbr.maximo, (size_t)an_archivo,
	...
	qsort(directorio, ldirectoriosbr.maximo, an_directorio,
	      (int (*)(const void *, const void *))strcmp);
	...
	for (y = 0; y < l->lineas; y++)
		for (x = 0; x < l->columnas; x++) {
			...
			if (thumb[l->inicial + y * l->columnas + x].tagged)
				wbox(ptr, an, al, color_tag,
				     l->x + (x * (l->an + 1)) + 1,
				     l->y + (y * (l->al + 1)) + 1 + l->al - 8,
				     l->an, 8);
			else
				wbox(ptr, an, al, c01,
				     l->x + (x * (l->an + 1)) + 1,
...
void pinta_sliderbr(struct t_listboxbr *l)
{
	...
	wbox(ptr, an, al, c2, l->x + (l->an + 1) * l->columnas + 1, l->y + 9, 7,
	     (l->al + 1) * l->lineas - 17);
	if (l->slide > l->s0)
		wbox(ptr, an, al, c0, l->x + (l->an + 1) * l->columnas + 1,
		     l->slide - 1, 7, 1);
	if (l->slide < l->s1)
		wbox(ptr, an, al, c0, l->x + (l->an + 1) * l->columnas + 1,
		     l->slide + 3, 7, 1);
...
void crear_listboxbr(struct t_listboxbr *l)
{
	...
	if (!l->creada) {
		...
		if (l == &larchivosbr) {
			...
			if ((l->inicial + (l->lineas - 1) * l->columnas) >=
			    l->maximo) {
				l->inicial = 0;
			}
		} else
...
void actualiza_listboxbr(struct t_listboxbr *l)
{
	...
	if (old_zona != l->zona)
		if (l->zona >= 10) { // Marca zona
			x = l->x + 1 +
			    ((l->zona - 10) % l->columnas) * (l->an + 1);
			y = l->y + l->al +
			    ((l->zona - 10) / l->columnas) * (l->al + 1);
			p = (byte *)l->lista +
