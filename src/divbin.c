...
void Bin2(void)
{
	...
	if (arrastrar == 4) {
		...
		case 100: // Map dragged to bin from desktop (alias for close
			  // map)
			MiVentana = v;
			v = ventana[1];
			ventana[1] = MiVentana;
		...
		case 101: // Item dragged from FPG to bin. Remove from FPG
			...
			if (v_aceptar) {
				...
				// if
				// (MiMemoFPG->lInfoFPG.inicial+MiMemoFPG->lInfoFPG.lineas*MiMemoFPG->lInfoFPG.columnas>MiMemoFPG->lInfoFPG.maximo
				//    && MiMemoFPG->lInfoFPG.inicial)
				//    MiMemoFPG->lInfoFPG.inicial--;

				while (MiMemoFPG->lInfoFPG.inicial +
					   (MiMemoFPG->lInfoFPG.lineas - 1) *
					       MiMemoFPG->lInfoFPG.columnas +
					   1 >
				       MiMemoFPG->lInfoFPG.maximo) {
					MiMemoFPG->lInfoFPG.inicial -=
					    MiMemoFPG->lInfoFPG.columnas;
				}

				if (MiMemoFPG->lInfoFPG.inicial < 0)
					MiMemoFPG->lInfoFPG.inicial = 0;
...
void muestra_papelera() { nueva_ventana(Bin0); }
