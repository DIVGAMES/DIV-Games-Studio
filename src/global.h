...
void call(const voidReturnType
	      func); // void funcion(void); int n=(int)funcion; call(n);
...
void _get(int texto, int x, int y, int an, byte *buf, int lon_buf, int r0,
	  int r1);
...
GLOBAL_DATA int auto_save_session, exploding_windows, move_full_window,
    tab_size, coloreador;
...
void wgra(byte *copia, int an_copia, int al_copia, byte c, int x, int y, int an,
	  int al);
void wbox(byte *copia, int an_copia, int al_copia, byte c, int x, int y, int an,
	  int al);
void wresalta_box(byte *copia, int an_copia, int al_copia, int x, int y, int an,
		  int al);
void wbox_in_box(byte *copia, int an_real_copia, int an_copia, int al_copia,
		 byte c, int x, int y, int an, int al);
void wrectangulo(byte *copia, int an_copia, int al_copia, byte c, int x, int y,
		 int an, int al);
void wput(byte *copia, int an_copia, int al_copia, int x, int y, int n);
void wput_in_box(byte *copia, int an_real_copia, int an_copia, int al_copia,
		 int x, int y, int n);
void wvolcado(byte *copia, int an_copia, int al_copia, byte *p, int x, int y,
	      int an, int al, int salta);
void wvolcado_oscuro(byte *copia, int an_copia, int al_copia, byte *p, int x,
		     int y, int an, int al, int salta);
void wwrite(byte *copia, int an_copia, int al_copia, int x, int y, int centro,
	    byte *ptr, byte c);
...
void wline(char *ptr, int realan, int an, int al, int x0, int y0, int x1,
	   int y1, char color);
...
GLOBAL_DATA int vga_an,
    vga_al; // Modo de video (id vga_an*1000+vga_al, ej.320200)
...
GLOBAL_DATA byte
    *fill_dac; // Valores a 1 para los colores que puedan estar en la zona
GLOBAL_DATA byte *map; // Mapa editado (el dibujo, no el entorno)
GLOBAL_DATA int
    *mab; // Mapa editado (en mapa de bits para la selección de zonas)
GLOBAL_DATA byte
    *ghost;	    // Tabla ghost-layering para la paleta del objeto editado
GLOBAL_DATA byte *dac; // Paleta del objeto editado
GLOBAL_DATA byte *dac4; // Paleta con rangos 0..255

GLOBAL_DATA word mab_x0, mab_x1, mab_y0,
    mab_y1; // Límites de la zona seleccionada en mab
...
GLOBAL_DATA byte
    *undo; // Copias del mapa editado (para deshacer, NULL si falta memoria)
...
GLOBAL_DATA int
    zoom_background; // Si se ha pintado el fondo de la ventana de zoom

GLOBAL_DATA int
    hacer_zoom; // Indica si es necesario volver a volcar el zoom de map
...
GLOBAL_DATA int barra_x, barra_y,
    barra_an; // Posición de la barra de edición (paint)

GLOBAL_DATA int
    modo; // Modo de dibujo (0-lapiz,1-líneas,2-bezier, ...) (+100 en el menú)
...
GLOBAL_DATA struct tgraf_help
    graf_help[384];		 // Gráficos del hipertexto (inicio,an,al,ptr)
GLOBAL_DATA char help_xlat[256]; // Para convertir los gráficos de la ayuda

GLOBAL_DATA byte
    *text_font; // Font estándar, 7 puntos de alto, ancho proporcional
...
GLOBAL_DATA int editor_font_an,
    editor_font_al;	// Ancho y alto de los carácteres
...
GLOBAL_DATA int
    zoom_move; // Indica (con c3, c4) si se autodesplaza el zoom (paint)
...
GLOBAL_DATA int
    selected_icon; // (barra) -1 si no hay selec. ninguno, 0-zoom, ...

GLOBAL_DATA int
    volcado_completo; // Indica si se ha modificado toda la copia de vga
...
GLOBAL_DATA struct ttipo tipo[24]; // Paths 0-(current working directory),
				   // 1-(path d.exe), 2-MAP, 3-PAL, 4-FPG,
				   // 5-FNT, 6-IFS, 7-PCM, 8-PRG,
				   // 9-MAP(tapices), 10-PAL(?), 11-PCM(save
				   // as,?), 12-PRJ, 13-(genérico *.*),
				   // 14-MAP(grabar), 15-WLD 16-Modulo de musica
...
GLOBAL_DATA int
    mouse_shift; // Para en big, adaptar las coordenadas dentro de barra
GLOBAL_DATA int mouse_shift_x, mouse_shift_y;

GLOBAL_DATA int fin_dialogo;

GLOBAL_DATA int doble_click, doble_click_x,
    doble_click_y; // Para detectar doble click
