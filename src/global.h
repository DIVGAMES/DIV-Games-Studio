...
GLOBAL_DATA tipo_regla reglas[16];
...
GLOBAL_DATA tgraf_help graf_help[384];  // Gr�ficos del hipertexto (inicio,an,al,ptr)
...
//GLOBAL_DATA 
typedef struct _tipo_undo {               // Tabla c�clica con los undo
  int start, end, x, y, an, al, modo;   // start se refiere a *(undo+start)
  int codigo;                           // identificador del mapa editado
} tipo_undo;                                      // modo=-1 si entrada no usada

GLOBAL_DATA tipo_undo *tundo;
...
#endif
  tmapa * mapa;                  // Puntero a otro struct tipo mapa
  tprg * prg;                    // Puntero a otro struct tipo prg
...
GLOBAL_DATA tventana ventana[max_windows];
#define v ventana[0]
...
GLOBAL_DATA tmapa * v_mapa;           // Puntero a un mapa
GLOBAL_DATA tprg * v_prg;		// Buffer de la ventana
GLOBAL_DATA tprg * eprg;		// Buffer de la ventana
...
//GLOBAL_DATA 
typedef struct _ttipo {                   // Informaci�n de los diferentes tipos
...
GLOBAL_DATA ttipo tipo[24]; // Paths 0-(current working directory),
...
GLOBAL_DATA tmapa * ventanas_a_crear[max_windows];
...
void crear_listbox(t_listbox *l);
void actualiza_listbox(t_listbox *l);
void pinta_listbox(t_listbox *l);
