...
char OEM2ANSI[256] = {
    ...
    164, 0,   0,   0,   0,   182, 167, 0,   0,   0,   0,   0,   0,   0,   0,
    ...
    251, 249, 255, 214, 220, 248, 163, 216, 215, 131, 225, 237, 243, 250, 241,
    209, 170, 186, 191, 174, 172, 189, 188, 161, 171, 187, 95,  95,  95,  166,
    166, 193, 194, 192, 169, 166, 166, 43,  43,  162, 165, 43,  43,  45,  45,
    43,  45,  43,  227, 195, 43,  43,  45,  45,  166, 45,  43,  164, 240, 208,
    202, 203, 200, 105, 205, 206, 207, 43,  43,  95,  95,  166, 204, 95,  211,
    223, 212, 210, 245, 213, 181, 254, 222, 218, 219, 217, 253, 221, 175, 180,
    173, 177, 95,  190, 182, 167, 247, 184, 176, 168, 183, 185, 179, 178, 95,
    32};
...
unsigned short anchoreal, altoreal, anchoBody, altoBody, anchoOutline,
    altoOutline;
...
short escalar()
{
	...
	if (factX >= 2.0 || factY >= 2.0) // escalar por medias
	{
		...
		Buffer2 =
		    (char *)realloc(Buffer2, (altoreal + 3) * (anchoreal + 3));
		if (Buffer2 == NULL)
			return (IFS_MEM_ERROR);
		...
		for (iy = 0; iy < alto; iy += factY) {
			for (ix = 0; ix < ancho; ix += factX) {
				...
				for (iyy = iy; iyy < iy + (int)factY; iyy++)
					for (ixx = ix; ixx < ix + (int)factX;
					     ixx++)
						pixel +=
						    Buffer[iyy * Ancho + ixx];
				pixel /= (factX * factY);
	...
	for (y = 0.5; y <= alto + 0.9; y += factY) {
		for (x = 0.5; x <= ancho + 0.9; x += factX) {
			...
			porcien =
			    2 - ((x - x1) * (x - x1) + (y - y1) * (y - y1));
			acum = porcien;
			pixel = *p * porcien;
			p += (int)ancho;

			porcien =
			    2 - ((x - x1) * (x - x1) + (y - y2) * (y - y2));
			acum += porcien;
			pixel += *p * porcien;
			p -= (int)ancho - 1;

			porcien =
			    2 - ((x - x2) * (x - x2) + (y - y1) * (y - y1));
			acum += porcien;
			pixel += *p * porcien;
			p += (int)ancho;

			porcien =
			    2 - ((x - x2) * (x - x2) + (y - y2) * (y - y2));
			acum += porcien;
			pixel += *p * porcien;
...
short PintaOutline()
{
	...
	ptr = ptrBody;
	ptrOutline = ptr2 =
	    tmpBuffer + ifs.outline + 2 + (ifs.outline + 2) * anchoTotal;
...
short PintaSombra()
{
	...
	shadowBuffer =
	    (char *)realloc(shadowBuffer, (anchoreal + absSombraX + 1) *
					      (altoreal + absSombraY + 1));
	if (shadowBuffer == NULL)
		return (IFS_MEM_ERROR);
	memset(shadowBuffer, 0,
	       (anchoreal + absSombraX + 1) * (altoreal + absSombraY + 1));
	...
	ptr2=shadowBuffer;
	    ...
	    ptr2=shadowBuffer + absSombraX + absSombraY *
    (anchoreal+absSombraX); else if (ifs.sombraX > 0) ptr2=shadowBuffer +
    absSombraX; else ptr2=shadowBuffer + absSombraY*(anchoreal+absSombraX);
*/
...
void texturarOutline()
{
	...
	if (ifs.outTexModo == 1) // escalar textura
	{
		...
		for (y = 0, iy = 0; y < altoOutline - incY;
		     y++) // textura en tile
		{
			for (x = 0, ix = 0; x < anchoOutline; x++, ptr++) {
				if (*ptr) {
					color =
					    outTexBuffer[iy * ifs.outTexAncho +
							 ix];
					if (*ptr < 128)
						*ptr = ifs.oscuros[color];
					else if (*ptr > 128)
...
void unirOutlineConBody(void)
{ // Le añade el body al outline
		...
		if (ifs.tabla[x]) {
			...
			if (x != 0) {
				...
				if (anchoreal <= 0 || incY >= altoreal) {
					...
					if (fwrite(ptrLetra + incY * anchoreal,
						   tablaFNT[x].ancho *
						       tablaFNT[x].alto,
						   1, fichFNT) < 1)
						error = 1;
...
void GetCharSizeBuffer(int WhatChar, int *ancho, int *alto, char *buffer)
{
	...
	*alto = 1;
	...
	memcpy(tablaFNT, buffer + 8 + 768 + sizeof(reglas) + 4,
	       sizeof(tablaFNT));
	*alto = tablaFNT[WhatChar].incY + tablaFNT[WhatChar].alto;
	*ancho = tablaFNT[WhatChar].ancho + 1;
	return;
...
int ShowChar(int WhatChar, int cx, int cy, char *ptr, int an)
{
	...
	if (tablaFNT[WhatChar].ancho * tablaFNT[WhatChar].alto) {
		rawBuffer = (char *)malloc(tablaFNT[WhatChar].ancho *
					   tablaFNT[WhatChar].alto);
		if (rawBuffer == NULL) {
			fclose(fichFnt);
			if (WhatChar == 32)
		...
		if (fread(rawBuffer,
			  tablaFNT[WhatChar].ancho * tablaFNT[WhatChar].alto, 1,
			  fichFnt) < 1) {
			free(rawBuffer);
			fclose(fichFnt);
			if (WhatChar == 32)
		...
		for (y = 0; y < tablaFNT[WhatChar].alto; y++)
			memcpy(ptr + ((cy + iy) * an + cx) + y * an,
			       rawBuffer + y * tablaFNT[WhatChar].ancho,
	...
	if (tablaFNT[WhatChar].ancho * tablaFNT[WhatChar].alto) {
		...
		for (y = 0; y < tablaFNT[WhatChar].alto; y++) {
			for (x = 0; x < tablaFNT[WhatChar].ancho; x++) {
				if ((c = rawBuffer
					 [y * tablaFNT[WhatChar].ancho + x])) {
					ptr[(cy + iy) * an + cx + x + y * an] =
					    c;
				}
			}
...
void ConvertFntToPal(char *buffer)
{
	...
	xlat[0] = 0;
	for (x = 1; x < 256; x++)
		xlat[x] = find_color_not0(DacFnt[x * 3], DacFnt[x * 3 + 1],
					  DacFnt[x * 3 + 2]);
		...
		if (tablaFNT[x].ancho * tablaFNT[x].alto)
			for (a = 0; a < tablaFNT[x].alto; a++)
				for (b = 0; b < tablaFNT[x].ancho; b++) {
					acum = *(buffer + tablaFNT[x].offset +
						 a * tablaFNT[x].ancho + b);
					*(buffer + tablaFNT[x].offset +
					  a * tablaFNT[x].ancho + b) =
					    xlat[acum];
				}
	}
