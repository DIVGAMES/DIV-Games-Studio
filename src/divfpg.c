...
void GetGrafMAP(struct tmapa *mapa, byte *imagen, int x, int y, int ancho,
		int alto, int cod);
...
void FPG2(void)
{
	...
	/*
	x=0; y=0; do { y+=abs((memptrsize)dac[x]-(memptrsize)newdac[x]); } while
	(++x<768); if(y) { v_titulo=texto[53]; v_texto=texto[66];
		dialogo(aceptar0);
...
*/
	if ((arrastrar == 4)) { // Suelto
		...
		Anadir_FPG(MiFPG, COD, (char *)tDescrip,
			   (char *)ventana[1].mapa->filename,
			   ventana[1].mapa->map_an, ventana[1].mapa->map_al,
			   nPuntos, (char *)ventana[1].mapa->puntos,
			   (char *)ventana[1].mapa->map, 0, 1);
	...
	if ((MiFPG->FPGInfo) && (MiFPG->lInfoFPG.zona >= 10) && (mouse_b & 1)) {
		mouse_b = 0;
		Elemento = MiFPG->DesIndex[(MiFPG->lInfoFPG.zona - 10) +
					   MiFPG->lInfoFPG.inicial];
		...
		if (FPGimagen) {
			...
			Anadir_FPG(MiFPG, MiFPG->MiHeadFPG.COD,
				   ...
				   MiFPG->MiHeadFPG.nPuntos, (char *)FPGpuntos,
				   FPGimagen, 1, 1);
		}
	...
	if ((MiFPG->lInfoFPG.zona >= 10) && (mouse_b & 1 || old_mouse_b & 1) &&
	    (mouse_b != old_mouse_b) && (arrastrar < 3)) {
		Elemento =
		    (MiFPG->lInfoFPG.zona - 10) + MiFPG->lInfoFPG.inicial;
...
void FPG0N(void)
{
	...
	_flag(108, (v.an - 5) - (8 * big2 + text_len(texto[108])), v.al - 10,
	      &MiFPG->FPGInfo);
}
...
void GetCode3(void) { free(GetCodeImagenRed); }
...
void InitGetCode(void)
{
	...
	if (GetCodeAncho > 70 * big2 || GetCodeAlto > 44 * big2) {
		...
		if (coefredx * (float)(GetCodeAnchoRed - 1) >= (float)man)
			coefredx =
			    (float)(man - 1) / (float)(GetCodeAnchoRed - 1);

		if (coefredy * (float)(GetCodeAltoRed - 1) >= (float)mal)
			coefredy =
			    (float)(mal - 1) / (float)(GetCodeAltoRed - 1);

		if ((temp2 = (byte *)malloc(GetCodeAnchoRed *
					    GetCodeAltoRed)) != NULL) {
			...
			if ((GetCodeImagenRed = (char *)malloc(
				 (GetCodeAnchoRed * GetCodeAltoRed) / 4)) !=
			    NULL) {

				for (y = 0; y < GetCodeAltoRed; y += 2) {

					for (x = 0; x < GetCodeAnchoRed;
					     x += 2) {
						n = *(
						    ghost +
						    temp2[x +
		...
		if ((GetCodeImagenRed = (char *)malloc(
			 GetCodeAnchoRed * GetCodeAltoRed)) != NULL) {
			memcpy(GetCodeImagenRed, GetCodeImagen,
			       GetCodeAnchoRed * GetCodeAltoRed);
		}
	}
...
void Show_Taggeds()
{
	...
	for (a = 0; a < MiFPG->nIndex; a++)
		...
		if (MiFPG->CodDes[a][0] == 175) {
			...
			if ((fpg = fopen((char *)MiFPG->ActualFile, "rb")) ==
			    NULL) {
				// Error: no es un encontro el fichero.(grfxch
				// ?????)
				return;
			...
			v.mapa->Codigo = MiFPG->MiHeadFPG.COD;
			memcpy(v.mapa->descripcion, MiFPG->MiHeadFPG.Descrip,
			       32);
			memset(v.mapa->filename, 0, 13);
			memcpy(v.mapa->filename, MiFPG->MiHeadFPG.Filename, 12);
			...
			if (MiFPG->MiHeadFPG.nPuntos)
				fread(v.mapa->puntos, MiFPG->MiHeadFPG.nPuntos,
				      4, fpg);
			...
			activar();
			call((voidReturnType)v.paint_handler);
			wvolcado(copia, vga_an, vga_al, v.ptr, v.x, v.y, v.an,
				 v.al, 0);
		}
}
...
void Delete_Taggeds()
{
	...
	while (MiFPG->lInfoFPG.inicial +
		   (MiFPG->lInfoFPG.lineas - 1) * MiFPG->lInfoFPG.columnas + 1 >
	       MiFPG->lInfoFPG.maximo) {
		MiFPG->lInfoFPG.inicial -= MiFPG->lInfoFPG.columnas;
	}

	if (MiFPG->lInfoFPG.inicial < 0)
		MiFPG->lInfoFPG.inicial = 0;
...
void printlist1(void) { _show_items(); }
...
void printlist0(void)
{
	...
	_get(414, 16 + text_len(texto[449]), 12,
	     v.an - 20 - text_len(texto[449]), (byte *)v_texto, 15, 0, 0);
	v_aceptar = 0;
}
...
void Print_List(void)
{
	...
	if (v_aceptar) {
		...
		if (num > 0) {
			if (f_ar) {
				_dos_setdrive(toupper(*tipo[1].path) - 'A' + 1,
					      &u);
				chdir(tipo[1].path);
				f = fopen(n_ar, "rb");
			...
			for (n = 0; n < 1000; n++) {
				if (MiFPG->OffsGrf[n]) {
					...
					sprintf(cwork, "[%03d] %s (%s, %dx%d)",
						cab.COD, cab.Descrip, cwork2,
						cab.Ancho, cab.Alto);
					if (f_ar) {
						fwrite(cwork, 1, strlen(cwork),
						       f);
						fwrite("\xd\xa", 1, 2, f);
					} else {
						fwrite(cwork, 1, strlen(cwork),
...
void FPG_crear_thumbs(void)
{
	...
	if (MiFPG->thumb_on) {
		do {
			...
			if (num > -1) {
				if (MiFPG->thumb[num].ptr != NULL &&
				    MiFPG->thumb[num].status == 0) {
					FPG_muestra_thumb(&(MiFPG->lInfoFPG),
							  num);
					break;
				} else if (MiFPG->thumb[num].ptr == NULL &&
...
void FPG_muestra_thumb(struct t_listboxbr *l, int num)
{
	...
	if (num >= l->inicial && num < l->inicial + l->lineas * l->columnas) {
		...
		if ((incy = ((l->al - 8) * big2 - MiFPG->thumb[num].al) / 2) <
		    0)
			incy = 0;
		...
		if (MiFPG->thumb_on && MiFPG->thumb[num].ptr != NULL &&
		    MiFPG->thumb[num].status == 0) {
...
void FPG_pinta_listboxbr(struct t_listboxbr *l)
{
	...
	for (y = 0; y < l->lineas; y++)
		for (x = 0; x < l->columnas; x++) {
			...
			if (MiFPG->thumb[l->inicial + y * l->columnas + x]
				.tagged)
...
void FPG_actualiza_listboxbr(struct t_listboxbr *l)
{
	...
	if (old_zona != l->zona)
		if (old_zona >= 10) { // Desmarca zona
			...
			strcpy(p, l->lista + l->lista_an *
						 (l->inicial + old_zona - 10));

			if (MiFPG->thumb_on)
				p[5] = 0;

			if (text_len((byte *)p) < l->an - 2 &&
			    MiFPG->thumb_on) {
				wwrite(ptr, an, al, x + l->an / 2, y, 7,
				       (byte *)p, c3);
			} else {
...
void crear_un_thumb_FPG(struct t_listboxbr *l)
{
	...
	if (l->maximo) {
		...
		if (estado == 1) { // Se comienza a leer un nuevo thumbnail
			...
			if ((f = fopen((char *)MiFPG->ActualFile, "rb")) !=
			    NULL) {
				fseek(f, MiFPG->OffsGrf[MiFPG->DesIndex[num]],
				      SEEK_SET);
				ReadHead(&(MiFPG->MiHeadFPG), f);
				fseek(f, MiFPG->MiHeadFPG.nPuntos * 4,
				      SEEK_CUR);
				...
				if ((MiFPG->thumb[num].ptr = (char *)malloc(
					 MiFPG->thumb[num].filesize)) != NULL) {

					if (MiFPG->thumb[num].filesize >
					    incremento) {

						if (fread(MiFPG->thumb[num].ptr,
							  1, incremento,
							  f) == incremento) {
							MiFPG->thumb[num]
							    .status =
			...
			return;
			   ...
			   MiFPG->thumb[num].status !=
			       MiFPG->thumb[num]
				   .filesize) { // Se continúa leyendo un
						// thumbnail
		...
		if (estado == 2 &&
			...
			if (man > 47 * big2 || mal > 26 * big2) {
				...
				if ((temp2 = (byte *)malloc(
					 MiFPG->thumb[num].an *
					 MiFPG->thumb[num].al)) != NULL) {
					memset(temp2, 0,
					       MiFPG->thumb[num].an *
						   MiFPG->thumb[num].al);
					a = (float)0.0;

					for (y = 0; y < MiFPG->thumb[num].al;
					     y++) {
						b = (float)0.0;

						for (x = 0;
						     x < MiFPG->thumb[num].an;
						     x++) {
							temp2[y * MiFPG
								      ->thumb
									  [num]
					...
					if ((MiFPG->thumb[num].ptr = (char *)
						 malloc((MiFPG->thumb[num].an *
							 MiFPG->thumb[num].al) /
							4)) != NULL) {

						for (y = 0;
						     y < MiFPG->thumb[num].al;
						     y += 2) {
...
void MAPtoFPG(struct tmapa *mapa)
{
	...
	for (y = 0; y < mapa->map_al; y++) {
		for (x = 0; x < mapa->map_an; x++) {
			...
			if (imagen[pos]) {
				for (ancho = 0; ancho < mapa->map_an - x;
				     ancho++) {
					if (!imagen[pos + ancho])
						break;
					imagen[pos + ancho] = 0;
				...
				GetGrafMAP(mapa, imagen, x + 1, y + 1,
					   ancho - 2, alto - 2, cod++);
			}
		}
	}
		...
		int alto, int cod)
{
	...
	sprintf(str_desc, "%s%d%s%s", texto[499], cod, texto[500],
		mapa->filename);

	Anadir_FPG(MiFPG, cod, str_desc, str_file, ancho, alto, 0, NULL,
		   (char *)buffer, 0, 0);
...
void FPGtoMAP(FPG *MiFPG)
{
	...
	while (MiFPG->DesIndex[num]) {
		...
		if ((FPGimagen = (byte *)malloc((lgraf[num].an - 2) *
						(lgraf[num].al - 2))) == NULL) {
			fclose(fpg);
			free(FPGmap);
			v_texto = (char *)texto[45];
		...
		if (fread(FPGimagen, 1,
			  (lgraf[num].an - 2) * (lgraf[num].al - 2),
			  fpg) != (lgraf[num].an - 2) * (lgraf[num].al - 2)) {
			fclose(fpg);
			free(FPGmap);
			free(FPGimagen);
			v_texto = (char *)texto[44];
...
void PutGrafMAP(byte *imagen, byte *mapa, int num)
{
	...
	memset(
	    &mapa[(lgraf[num].y + lgraf[num].al - 1) * lmapan + lgraf[num].x],
	    c4, lgraf[num].an);

	for (y = 0; y < lgraf[num].al - 2; y++) {
		mapa[(y + lgraf[num].y + 1) * lmapan + lgraf[num].x] =
		    c4; // Columna izquierda

		for (x = 0; x < lgraf[num].an - 2; x++) {
			pos_im = y * (lgraf[num].an - 2) + x;
			pos_ma = (y + lgraf[num].y + 1) * lmapan + x +
				 lgraf[num].x + 1;
			mapa[pos_ma] = imagen[pos_im]; // Grafico
		}

		mapa[(y + lgraf[num].y + 1) * lmapan + x + lgraf[num].x + 1] =
		    c4; // Columna derecha
	}
}
...
{ // Emplaza grafico lnum
	...
	for (n = 0; n < scans; n++) {
		...
		do {
			x = new_x;
			for (m = 0; m < lnum; m++) {
				if (colisiona_con_map(m, x - 1, y - 1,
						      lgraf[lnum].an + 2,
						      lgraf[lnum].al + 2)) {
	...
	// if(lgraf[lnum].y+lgraf[lnum].al>lmapal) printf("Falla el grafico
	// %d\n", lnum);
}
...
void sustituir_FPG_0(void)
{
	...
	v.tipo = 1;
	v.an = text_len(texto[510]) + text_len(texto[511]) +
	       text_len(texto[101]) + 16;
