...
int ODig1 = -1, ODig2 = -1, ODig3 = -1, ODig4 = -1, ODig5 = -1, ODig6 = -1,
    OCDPlaying = -1;
...
void CDiv2(void)
{
	...
	for (x = 0; x < 6; x++) {
		if ((wmouse_y > 31) && (wmouse_y < 42) &&
		    (wmouse_x > x * 24 + 4) && (wmouse_x < x * 24 + 23)) {
			in = x;
			if (mouse_b & 1) {
				lp = in;
				wput(ptr, an, al, x * 24 + 4, 32,
				     (212 + x) * -1);
			}
		}
	}
...
void Show_Time()
{
	...
	sprintf(cTimeForIcon, "%s [%02d%c%02d]", texto[151], timeinfo->tm_hour,
...
void muestra_reloj() { nueva_ventana(Clock0); }
