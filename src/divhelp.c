...
int backto[64];     // Cola circular para almacenar los topicos consultados
		    // {n,línea}
...
{
	return (18 +
		(help_l * (v.al / big2 - 21 - 18)) / (help_lines - help_al));
}
...
void help1(void)
{
	...
	wbox(ptr, an, al, c0, an - 10, 10, 9,
	     al - 12); // Fondo negro del slider
...
void help2(void)
{
	...
	if (v.estado) {
		...
		if (!help_paint_active) {
			if (scan_code ==
			    74) { // Anterior elemento de la ayuda (-)
				n = help_item;
				while (--n >= 0) {
					if (helpidx[n * 2] &&
					    helpidx[n * 2 + 1]) {
			...
			if (scan_code ==
			    78) { // Siguiente elemento de la ayuda (+)
				n = help_item;
				while (++n < 2048) {
					if (helpidx[n * 2] &&
					    helpidx[n * 2 + 1]) {
		...
		if (scan_code == 72 ||
		    (mouse_graf == 7 && (mouse_b & 1) && wmouse_x != -1)) {
			if (scan_code != 72)
				if (!(v.botones & 2)) {
		...
		if (scan_code == 80 ||
		    (mouse_graf == 9 && (mouse_b & 1) && wmouse_x != -1)) {
			if (scan_code != 80)
				if (!(v.botones & 4)) {
					wput(v.ptr, an, al, an - 9, al - 17,
					     -42);
					v.botones |= 4;
				} else {
					retrazo();
...
*/
		...
		if (wmouse_x != -1 && wmouse_y >= 10 + 16) {
			...
			if (p + 1 <
			    help_end) { // We are in a line of hypertext, in mx
				n = -1;
				while (*p && mx >= 0) {
					switch (*p) {
					case 1:
				...
				// Estamos sobre una referencia cruzada en el
				// hipertexto

				if (n != -1) {
					mouse_graf = 2;
					if ((mouse_b & 1) &&
					    !(old_mouse_b & 1) && old_estado) {
						if (n ==
						    9999) { // *** Extrae un
							    // ejemplo ***
							if ((di = p =
		...
		if (((scan_code == 14 && !(shift_status & 3)) ||
		     v.active_item == 0) &&
		    a_back != i_back) {
			a_back -= 2;
			if (a_back < 0)
...
void help0(void)
{ // En help_item se indica sobre que término se pide ayuda
	int x;
...
void resize_help(void)
{
	...
	do {
		...
		if ((new_block = (byte *)realloc(v.ptr, v.an * v.al)) != NULL) {
			...
			if (modo < 100) {
				fondo_edicion(v.x, v.y, v.an > an ? v.an : an,
					      v.al > al ? v.al : al);
				volcar_barras(1);
			}
...
int determina_help(void)
{
	...
	for (m = 0; m < max_windows; m++) {
		if (ventana[m].tipo == 102 &&
		    ventana[m].click_handler == help2) {
			n = m;
			break;
		}
	}
...
void get_error(int n)
{
	FILE *f;
	byte *p;
	sprintf(cerror, "%s", (char *)texto[381]);
		...
		if (c != 0 || !ultimo_cr) {
			...
			if (c) { // Si viene un carácter
				...
				if (!ejemplo && chars == help_an + 1) {
					...
					chars2 = chars;
					help_an2 = help_an; // Arreglar la línea
							    // que se truncó
					...
					*di = c;
					...
					di2 = di; // Donde termina la línea
						  // anterior

					if (estado ==
					    0) { // Se corta el texto normal
						if (imagen) {
							memmove(di + 10, di + 1,
								nchars);
							*di++ = 0;
...
void arregla_linea(byte *end, int chars, int help_an)
{
	...
	if (espacios) {
		...
		if (error) {
			nuevo_esp =
			    (float)font_an + (float)error / (float)espacios;
			if (nuevo_esp > font_an + 25)
				nuevo_esp = font_an + 25;
			espacio_acum = 0;
...
void vuelca_help(void)
{
	...
	wbox(v.ptr, v.an / big2, v.al / big2, c12, 2, 10 + 16, v.an / big2 - 12,
	     v.al / big2 - 12 - 16);

	di = v.ptr +
	     (v.an * (10 + 16) + 2) * big2; // Volcado de la ventana de texto
	si = help_line;
	al = help_al;
...
void put_image_line(int n, int linea, byte *di, int v_an)
{
	...
	_an = graf_help[n].ran;
	al = graf_help[n].ral - linea * font_al;
	if (al > font_al)
		al = font_al;
...
void put_chr(byte *ptr, int an, byte c, byte color)
{
	...
	if (tsurface != NULL) {
		tempsurface =
		    drawtext(sysfont, colors[color].r, colors[color].g,
...
void Print_Help(void)
{
	...
	if (v_aceptar) {
		...
		if ((f = fopen("help/help.div", "rb")) != NULL) {
			...
			if ((h_buffer = (byte *)malloc(
				 helpidx[help_item * 2 + 1] + 2048)) != NULL) {
				if ((print_buffer = (byte *)malloc(
					 helpidx[help_item * 2 + 1] + 2048)) !=
				    NULL) {
					fread(h_buffer, 1,
					      helpidx[help_item * 2 + 1], f);
					p = h_buffer;
					while (*p != '}')
						p++;
					*p = 0;
					...
					tabula_help2(
					    p + 1, print_buffer,
					    helpidx[help_item * 2 + 1] -
						(p + 1 - h_buffer));

					// Imprime el título de la página de
					// ayuda
					...
					for (n = 0;
					     n <
					     (ancho_ayuda -
					      strlen((char *)help_title) - 4) /
						 2;
					     n++)
						fwrite(" ", 1, 1, g);
					fwrite("[ ", 1, 2, g);
					fwrite(help_title, 1,
					       strlen((char *)help_title), g);
					fwrite(" ]"
					       "\xd\xa\xd\xa───────────────────"
					       "───────────────────────────────"
					       "────────────────────",
					...
					while (p < help_end) {
						Progress((char *)texto[437],
							 p - print_buffer,
							 help_end -
							     print_buffer);
						fwrite(p, 1, strlen((char *)p),
						       g);
						fwrite("\xd\xa", 1, 2, g);
						p += strlen((char *)p) + 1;
					}
