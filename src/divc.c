...
// operaciones de cadenas (strcpy,strcat,stradd) 18 - Añadir y quitar caracteres
// con string++/--, string-=n, string-n (strdec,strsub,strlen) 19 - Poder
// definir literales en varias lineas del fuente (varios consecutivos se
...
// 26 - Añadidas la opciones _no_check (no comprobar rangos ni ides) y
...
// de recibir un parámetro en un string private, incluso indicando su longitud
// (ej, proc(string s[2])) 32 - Permitir la redeclaracion del parámetro string
...
// siguiente sentencia) 36 - Añadir bytes globales y un nuevo, nuevamente,
...
// 38 - Ultimos cambios en el Debugger (en Ver datos), y conversión al
// Castellano. 39 - Añadir bytes locales y privados (bloc) 40 - Bytes miembros
// de estructuras (globales/locales y privados) 41 - Inicialización de
// estructuras contemplando los bytes miembros (frm,ifrm_max) 42 - Añadidas al
// intérprete instrucciones para el manejo de words (???wor) 43 - Words al igual
// que bytes (wglo wloc *word ???wor) 44 - Poder recibir un byte o word como
// parámetro (y redeclararlo como private) 45 - Fallo del coloreador (debajo de
// process ... poner ?????????) 46 - Que en las listas de procesos (F5)
// aparezcan también las funciones (editor y debugger) 47 - Fallo de que
...
// hecho FRAME (sistema multipila) 51 - Añadido el tipo INT opcional para los
// datos enteros convencionales en DIV 1 52 - Declaración de punteros a INT
// globales/locales/privados y miembros de estructuras (tpigl,tpilo) de hasta 3
// dimensiones 53 - Utilización de punteros INT, generación de código para los
// mismos (y pruebas) 54 - Soporte en el debugger para punteros INT globales y
// locales (p, p[0], p[1], ...) 55 - Punteros a int como parámetros y
// redeclaración en sección PRIVATE 56 - Prohibir la redeclaración de variables
// private (solo si es 1 vez como parametro y otra como private) 57 - Punteros a
// byte/word, tanto global como local y private, member o no, declaración y
// código 58 - Soporte en el debugger de punteros de tipo byte y word (globales
// y locales) 59 - Eliminada la posibilidad de utilizar * para definir un
// puntero (y en el debugger se usa POINTER)
...
// (sólo varia la visualización en el debug) 62 - Punteros a estructuras STRUCT
// POINTER NOMBRESTRUCT tal; (por defecto a 0) 63 - Resuelto el problema del
// acceso a los miembros de la estructura original 64 - Soporte en el debugger
// para punteros a estructuras (!!! mas chungo de lo que yo pensaba) 65 -
// Funcion de "capar()" direcciones en DEBUG para evitar en lo posible los page
// fault 66 - Resueltos "n-mil" problemas con los putos punteros, el debugger y
// la madre que lo parió ... 67 - Hacer que int/byte/word/string pointer p1,p2;
// defina dos punteros 68 - Funcion screen_copy que puede pasar una region de
// pantalla a una region de un gráfico 69 - Argc/argv al igual que en C, con los
// parámetros del programa 70 - Nuevo sistema de carga de funciones, definiendo
// el retorno y el tipo de los parámetros
...
//    - También se podrían permitir las operaciones, aunque los resultados
//    fueran erróneos

//    - Aun así, ¿como explicar a la gente que ptrint++ funciona, pero que
//    ptrbyte++ no?
...
//    - El problema es: ¿Se prohibe operar (sumas y demás) con ptrbyte/word? ->
//    NO
...
//    - ¿Nuevo diseño para: tpwgl tpwlo tpbgl tpblo? (teniéndolos siempre
//    multiplicados *2 o *4)
//    - a) en incializacion se debe multiplicar *2 o *4 la constante
//    (ptrbyte=&tablabyte)
//    - b) como cojones se hace ptrbyte=&loquesea ? (como paso de parámetro,
//    asignación, etc...)
//    - c) y que pasa con ¿ptrword=ptrbyte?
//    - d) es más, ¿y que pasa si se envia como parámetro un ptrbyte y se recibe
//    un ptrint?

//    - Solo podría hacerse de forma que todos los accesos de lectura a ptrbyte
//    se dividiera por 4
//      y en los de escritura se multiplicara por cuatro, y aun así:
//      ¡¡¡ fallaría ptrbyte1=ptrbyte2 !!! (o pasando y recibiendo un ptrbyte
//      parámetro) ¿[ptrbyte+n] ... y *ptrbyte?
...
//    - Debería haber alguna forma de tener todos los offset multiplicados por 4
//    (en bytes)

//    - Problemas: ["ptr"+n] (por cuanto multiplicar n), además de como resolver
//      [ptrint+ptrbyte+2*x...], acceso en el intérprete a todos los datos de
//      mem mem[id+_Graph], los propios id (multiplicarlos o no?- > mejor no, y
//      hacer que el p_punto los multiplique, ya que, además deben ser impares),
//      programas antiguos que hicieran uso de punteros (p=&kk; *p..., p++,
//      *p... para ello se debería emitir un error de compilación al utilizar *p
//      o [p], obligando a que redeclaren p como un ptrint), que ptrint++ sea
//      ptrint+=4 y ptrword++ sea ptrword+=2,
...
//      podría haber utilizado un save("pepe.dat",&tabla,10) ...
...
struct objeto {
	...
	byte *ierror; // Puntero al código para determinar la columna si es
		      // necesario
...
int pila[long_pila + max_exp +
	 64]; // cálculo de expresiones (compilación y ejecución)
...
void mensaje_compilacion(byte *p)
{
	if (compilemode) {
		...
		wbox(v.ptr, v.an / big2, v.al / big2, c2, 2, 20,
		     v.an / big2 - 4, 7);
...
lex_scan:
	...
	case l_lit:
		...
		//	      fprintf(stdout,"LOOKING FOR FILE: %s [%s]
		//[%s]\n",(char *)ivnom.b,full,(char *)&tipo[8]);

		if ((!ivnom.b[0] != '.' && ivnom.b[1] != 0) &&
			...
			if (!stricmp((char *)ext, ".MOD") ||
			    !stricmp((char *)ext, ".S3M") ||
			    !stricmp((char *)ext, ".XM")) {
				empaquetable = 1;
		...
		if (pieza == p_struct) { // Miembro struct ───
			...
			if (pieza ==
				...
				if ((*ob).tipo == tnone)
					c_error(
					    0,
					    150); // No se define el pointer así
...
void sintactico(void)
{
		...
		do {
			...
			if (pieza == p_id && (*o).tipo == tcons) {
				...
				case 3: // _case_sensitive
					lexico();
					memcpy(lower + 129,
					       "üéâäàåçêëèïîìäåéææôöòûùÿöüø£Ø×ƒ"
					       "áíóú",
					       35);
					memcpy(lower + 'A',
	...
	while (pieza == p_process || pieza == p_function) {
		...
		parametros = -1; // Para que los parámetros se puedan repetir
				 // como PRIVATE
		...
		while (pieza == p_id || pieza == p_int || pieza == p_pointer ||
			...
			if (pieza == p_struct) {
				...
				puntero4:
					...
					// un parámetro o bien es un error
...
void tglo_init2(int tipo)
{
	...
	while (1) {
		...
		// Una coma sola, como "3,,4", avanza una posición (y define un
		// 0)
...
void sentencia()
{
	...
	while (pieza >= p_return) {
		...
		case p_from:
			...
			if (dir >=
			    0) { // Comparación de la condición de permanencia
				g2(lcar, dir);
			} else {
				g2(lcar, -dir);
				g1(laid);
	...
	do
		...
		case eoper:
			...
			case p_neg:
				...
				// Creo que se quitó los pointer (y los rangos,
...
void div_exp2()
{
	...
	while ((p = pieza) >= p_igu && p <= p_mai) {
		...
		exp3();
		if (tf == 2 || tipo_factor == 2) {
			// OJO, p==q no se hará el strcmp() si son dos punteros
			// a cadenas
			p += p_strigu - p_igu;
			(*_exp).tipo = eoper;
			(*_exp++).token = p;
...
{ // Operador de acceso a variables o tablas locales ajenas
	...
	while (pieza == p_punto) {
		...
		// (bueno, si, hacer aquí una búsqueda del p_punto entre e-1 y
		// _exp ...)
...
void factor(void)
{
	...
	case p_struct: // Se comprueba la declaración de un puntero a struct
		       // parámetro
		if (parametros <= 0) {
			if (error_27 == 27)
				c_error(1, 27);
			...
			lexico();
			if (pieza != p_pointer)
				c_error(0, 151); // No se define el pointer así
			lexico();
			if (pieza != p_id)
				c_error(1, 110);
			obs = o;
			if ((*obs).tipo == tnone)
				c_error(0, 150); // No se define el pointer así
	...
	case p_int: // Se comprueba la declaración de un int parámetro
		if (parametros <= 0) {
			...
			case p_pointer: // Declaración de un puntero a int
					// parámetro
				if (parametros <= 0) {
					if (error_27 == 27)
						c_error(1, 27);
	...
	case p_id:
		...
		case tfunc:
			...
			while (pieza != p_cerrar) { // p­num_par
				...
				if ((*ob).func.par[p] ==
				    1) { // Parámetro del tipo 1 (struct,campo)
...
void save_dbg(void)
{
	...
	for (n = 0; n < num_obj; n++) {
		...
		// OJO ! que no se pueden añadir objetos aquí (ver uso de
...
FILE *fpopen(byte *file) { return __fpopen(file, "rb"); }
...
void compilar1(void)
{
	...
	wwrite(v.ptr, v.an / big2, v.al / big2, 6 + text_len(texto[206]), 12, 0,
	       ventana[v_ventana + 1].titulo, c4);
}
...
//  int op;     // Opcode
//} code[16];   // En code[15] debe quedar siempre la última instrucción
//generada
		...
		if (code[15].op == lcar2) {
			...
			if (mem[imem - 1] == 1)
				...
				code[15].op = mem[imem - 2] =
				    lcardiv; // Un cardiv nunca será "cardiv 0"
				optimizado = 1;
			}
		}
		break;
