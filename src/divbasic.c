...
void put_bw(int x, int y, int n)
{ // Puts a contrasting graphic (mouse edition )
	int al, an;
	byte *p, *q;
	int salta_x, long_x, resto_x;
	int salta_y, long_y, resto_y;
...
void fondo_edicion(int x, int y, int an, int al)
{
	...
	// Repone el fondo del sistema de ventanas, OJO *** mejorar
	// actualiza_caja();

	if (zx || zy)
		if (x < zx || y < zy || x + an > zx + zan || y + al > zy + zal)
	...
	if (zy) {
		wbox_in_box(copia + y * vga_an + x, vga_an, an, al, c2,
			    zx - 2 - x, zy - 2 - y, zan + 4, 1);
		wbox_in_box(copia + y * vga_an + x, vga_an, an, al, c0,
			    zx - 1 - x, zy - 1 - y, zan + 2, 1);
	}
	if (zy + zal < vga_al) {
		wbox_in_box(copia + y * vga_an + x, vga_an, an, al, c2,
			    zx - 2 - x, zy + zal + 1 - y, zan + 4, 1);
		wbox_in_box(copia + y * vga_an + x, vga_an, an, al, c0,
			    zx - 1 - x, zy + zal - y, zan + 2, 1);
	}

	if (zx) {
		wbox_in_box(copia + y * vga_an + x, vga_an, an, al, c2,
			    zx - 2 - x, zy - 1 - y, 1, zal + 2);
		wbox_in_box(copia + y * vga_an + x, vga_an, an, al, c0,
			    zx - 1 - x, zy - y, 1, zal);
	}
	if (zx + zan < vga_an) {
		wbox_in_box(copia + y * vga_an + x, vga_an, an, al, c2,
			    zx + zan + 1 - x, zy - 1 - y, 1, zal + 2);
		wbox_in_box(copia + y * vga_an + x, vga_an, an, al, c0,
			    zx + zan - x, zy - y, 1, zal);
	}
	...
	zoom_q = copia + (zy + ((_y0 - zoom_y) << zoom)) * vga_an + zx +
		 ((_x0 - zoom_x) << zoom);
	zoom_an = _x1 - _x0 + 1;
	zoom_al = _y1 - _y0 + 1;
	...
	_zan = zan;
	_zal = zal;
	zx = zx + ((_x0 - zoom_x) << zoom);
	zy = zy + ((_y0 - zoom_y) << zoom);
	zoom_x = _x0;
	zoom_y = _y0;
	zan = (_x1 - _x0 + 1) << zoom;
	zal = (_y1 - _y0 + 1) << zoom;
...
void zoom_map(void)
{
	...
	if (!zoom_porcion) {
		...
		if ((map_an << zoom) <
		    vga_an) { // Copia de este trozo en select_zoom()
			zan = map_an << zoom;
			zx = (vga_an - zan) / 2;
			q += zx;
			an = map_an;
		...
		if (!zoom_background) {
			if (zx || zy) {
				...
				if (zy + zal < vga_al) {
					wbox(copia, vga_an, vga_al, c2, zx - 2,
					     zy + zal + 1, zan + 4, 1);
					wbox(copia, vga_an, vga_al, c0, zx - 1,
					     zy + zal, zan + 2, 1);
	...
	if (sel_status)
		...
		case 3:
			draw_selection_mab();
			if (sel_x1 >= 0)
				draw_selection_box(sel_x0, sel_y0, sel_x1,
						   sel_y1);
			break;
		case 4:
			draw_selection_box(sel_x0, sel_y0, sel_x1, sel_y1);
...
void modo_inter(void)
{
	...
	an = mab_x1 - mab_x0 + 1 + 4;
	al = mab_y1 - mab_y0 + 1 + 4;
...
{ // Rellena con medias una zona
	...
	for (y = mab_y0 - 1; y <= mab_y1 + 1; y++)
		if (y >= 0 && y < map_al)
			for (x = mab_x0 - 1; x <= mab_x1 + 1; x++)
				if (x >= 0 && x < map_an)
					if (is_near_mab(x, y)) {
						n = *(original +
						      (y - mab_y0 + 1) *
	...
	for (y = 0; y < al - 1; y++) {
		for (x = 0; x < an - 1; x++) {
			...
			if (n0 = *si) // Pone en *(di) la media de los cuatro
				      // *(si+?)
				if (n1 = *(si + 1))
					if (n2 = *(si + an))
						if (n3 = *(si + an + 1))
...
void fill(word x, word y)
{
	...
	if ((fss = (word *)malloc(60000)) != NULL) {
		...
		if (modo_fill == 2) {
			memset(fill_dac, 1,
			       256); // Rellena hasta el color seleccionado
			fill_dac[color] = 0;
		} else {
			memset(fill_dac, 0,
			       256); // Rellena el color de map(x,y)
		...
		if (modo_fill == 3) {
			...
			d = d - b + 1;
			an_original = c;
			if ((original = (byte *)save_undo(a, b, c, d)) !=
			    NULL) {
				if (!mab_x0)
					original--;
				if (!mab_y0)
...
nuevo_scan:
	...
	do
		x0--;
	while (x0 >= 0 && fill_dac[*(map + y * map_an + x0)]);
	x0++;
	x1 = x;
	do
		x1++;
	while (x1 < map_an && fill_dac[*(map + y * map_an + x1)]);
	x1--;
	...
	if (y > 0)
		while (x <= x1) {
			if (!is_mab(x, y - 1)) // Si no está pintado
				if (fill_dac[*(map + (y - 1) * map_an +
					       x)]) // Si se tiene que pintar
					if (fsp <
					    fsp_max) { // Si se puede pintar
						*fsp = x0;
						fsp++;
						*fsp = x1;
						fsp++;
...
void volcado_inter(int an)
{ // Vuelca a pantalla la zona interpolada
...
void draw_selection_mab(void)
{
	...
	int x0, y0, x1,
	    y1; // Intersección de la zona seleccionada y lo visto en zoom
	int x, y, c;
	byte *p, g0 = c0, g4 = c4;
	word g04, g40;
	int inc32, *_mab;
	...
	if (mab_y0 > y0)
		y0 = mab_y0;
	x1 = zoom_x + (zan >> zoom) - 1;
	if (mab_x1 < x1)
		x1 = mab_x1;
	y1 = zoom_y + (zal >> zoom) - 1;
	if (mab_y1 < y1)
		y1 = mab_y1;
...
void set_mab(int x, int y)
{ // Pone un pixel en el mapa de bits
...
int is_mab(int x, int y)
{ // Consulta un pixel en el mapa de bits
...
byte *save_undo(int x, int y, int an, int al)
{
	...
	if ((end = start + an * al) <= undo_memory) {
		...
		tundo[iundo].modo = -1; // Se prohibe undo_next()
...
void undo_next(void)
{
	...
	if (tundo[iundo].modo != -1 && tundo[iundo].codigo == codigo_mapa) {
		...
		xchg_block(undo + tundo[iundo].start, map + x + y * map_an, an,
			   al);
